...
void CRC_SOFT_GenTable32(uint32_t table[CRC_SOFT_TABLE_LEN], uint32_t poly)
{
    for (uint32_t i = 0; i < CRC_SOFT_TABLE_LEN; i++)
    {
        uint32_t crc = CRC_SOFT_Reflect8(i) << 24;
        for (uint8_t bit = 0; bit < 8; bit++)
        {
            crc = (crc & 0x80000000) ? ((crc << 1) ^ poly) : (crc << 1);
        }
        table[i] = CRC_SOFT_Reflect32(crc);
    }
}
...
uint16_t CRC_SOFT_CalcPartTable16(const void *data, size_t size, uint16_t initCrc, const uint16_t table[CRC_SOFT_TABLE_LEN])
{
    ...
    while (size--)
    {
        const uint8_t tableInd = (crc >> 8) ^ *dataBuf++;
        crc = (crc << 8) ^ table[tableInd];
    }
    return crc;
}
...
uint32_t CRC_SOFT_CalcPartTable32(const void *data, size_t size, uint32_t initCrc, const uint32_t table[CRC_SOFT_TABLE_LEN])
{
    ...
    while (size--)
    {
        const uint8_t tableInd = (crc ^ *dataBuf++) & 0xFF;
        crc = (crc >> 8) ^ table[tableInd];
    }
    return crc;
}
...
uint32_t CRC_SOFT_CalcPart32(const void *data, size_t size, uint32_t initCrc, uint32_t poly)
{
    uint32_t crc = initCrc;
    uint8_t *dataBuf = (uint8_t*)data;
    const uint32_t reflectPoly = CRC_SOFT_Reflect32(poly);
    while (size--)
    {
        crc ^= *dataBuf++;
        for (uint8_t j = 0; j < 8; j++)
        {
            crc = (crc & 1) ? ((crc >> 1) ^ reflectPoly) : (crc >> 1);
        }
    }
    return crc;
}
