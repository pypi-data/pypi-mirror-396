...
class BinAPI:
    ...
    async def import_clip(self, url: str, folder_id: str | None = None) -> str:
        """Import single media file. Returns clip ID.

        Note: Import is deferred - the server returns immediately.
        If clipId is not in the response, returns empty string.
        Caller should wait and use list_clips() to find the imported clip.
        """
        params: dict[str, str] = {"url": url}
        if folder_id:
            params["folderId"] = folder_id
        result = await self._client.call("bin.importClip", params)
        # Server may return {"importing": true} for deferred imports
        return result.get("clipId", "")

    async def import_clips(self, urls: list[str], folder_id: str | None = None) -> list[str]:
        """Import multiple media files. Returns clip IDs.

        Note: Import is deferred - the server returns immediately.
        Caller should wait and use list_clips() to find imported clips.
        """
        params: dict[str, str | list[str]] = {"urls": urls}
        if folder_id:
            params["folderId"] = folder_id
