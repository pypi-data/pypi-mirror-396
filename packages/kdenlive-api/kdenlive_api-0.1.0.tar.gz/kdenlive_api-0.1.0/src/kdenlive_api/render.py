...
class RenderAPI:
    ...
    async def get_presets(self) -> list[RenderPreset]:
        ...
        return [RenderPreset.model_validate(p) for p in result.get("presets", [])]
    ...
    async def start(self, preset_name: str, output_path: str) -> RenderJob:
        ...
        result = await self._client.call(
            "render.start", {"presetName": preset_name, "outputPath": output_path}
        )
    ...
    async def start_with_guides(self, preset_name: str, output_dir: str) -> list[GuideRenderJob]:
        ...
        result = await self._client.call(
            ...
            {"presetName": preset_name, "outputDir": output_dir},
        )
        return [GuideRenderJob.model_validate(j) for j in result.get("jobs", [])]
    ...
    async def get_jobs(self) -> list[RenderJob]:
        ...
        return [RenderJob.model_validate(j) for j in result.get("jobs", [])]
