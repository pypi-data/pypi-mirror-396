...
class TimelineAPI:
    ...
    async def get_tracks(self) -> list[TrackInfo]:
        ...
        return [TrackInfo.model_validate(t) for t in result.get("tracks", [])]
    ...
    async def get_clips(self, track_id: int | None = None) -> list[TimelineClip]:
        ...
        return [TimelineClip.model_validate(c) for c in result.get("clips", [])]
    ...
    async def resize_clip(self, clip_id: int, in_point: int, out_point: int) -> bool:
        ...
        result = await self._client.call(
            "timeline.resizeClip",
            {"clipId": clip_id, "inPoint": in_point, "outPoint": out_point},
        )
    ...
    async def set_selection(self, clip_ids: list[int]) -> bool:
        ...
        result = await self._client.call("timeline.setSelection", {"clipIds": clip_ids})
