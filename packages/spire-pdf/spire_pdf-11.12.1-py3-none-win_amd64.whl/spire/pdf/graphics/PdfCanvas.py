...
class PdfCanvas (SpireObject) :
    ...
    @dispatch
    def DrawPolygon(self ,pen:PdfPen,points:List[PointF]):
        """
        Draws a polygon.

        Args:
            pen (PdfPen): The pen.
			points (PointF): The PointF.
        """
        intPtrpen:c_void_p = pen.Ptr
        ...
        GetDllLibPdf().PdfCanvas_DrawPolygon.argtypes=[c_void_p ,c_void_p,ArrayTypepoints,c_int]
        CallCFunction(GetDllLibPdf().PdfCanvas_DrawPolygon,self.Ptr, intPtrpen,arraypoints,len(points))


    @dispatch
    def DrawPolygon(self ,brush:PdfBrush,points:List[PointF]):
        """
        Draws a polygon.

        Args:
            brush (PdfBrush): The PdfBrush.
			points (PointF): The PointF.
        """
        intPtrbrush:c_void_p = brush.Ptr
        countpoints = len(points)
        ...
        GetDllLibPdf().PdfCanvas_DrawPolygonBP.argtypes=[c_void_p ,c_void_p,ArrayTypepoints,c_int]
        CallCFunction(GetDllLibPdf().PdfCanvas_DrawPolygonBP,self.Ptr, intPtrbrush,arraypoints,len(points))

    @dispatch
    def DrawPolygon(self ,pen:PdfPen,brush:PdfBrush,points:List[PointF]):
        """
        Draws a polygon.
        ...
        GetDllLibPdf().PdfCanvas_DrawPolygonPBP.argtypes=[c_void_p ,c_void_p,c_void_p,ArrayTypepoints,c_int]
        CallCFunction(GetDllLibPdf().PdfCanvas_DrawPolygonPBP,self.Ptr, intPtrpen,intPtrbrush,arraypoints,len(points))
