...
def string_byte_dance(content: _Any, /, speed: int|float = 30, end: _Any = "\n", ms_mode: bool = True) -> None:
    """
...
def str_byte_dance(content: _Any, /, speed: int|float = 30, end: _Any = "\n", ms_mode: bool = True) -> None:
    """
...
def bd(c: _Any, /, s: int|float = 30, e: _Any = "\n", m: bool = True) -> None:
    """
...
def byte_dance(content: _Any, /, speed: int|float = 30, end: _Any = "\n", ms_mode: bool = True) -> None:
    """
    :note: 这是一个融合的sbd和cbd的函数 / This is a function that combines sbd and cbd
...
def sbd(c: _Any, /, s: int|float = 30, e: _Any = "\n", m: bool = True) -> None:
    """
...
def container_byte_dance(content: _Container[_Any], /, speed: int|float = 30, end: _Any = "\n", ms_mode: bool = True) -> None:
    """
...
def cbd(c: _Container[_Any], /, s: int|float = 30, e: _Any = "\n" , m: bool = True) -> None:
    """
...
def helps(func: _Function|None = None) -> None:
    ...
    if isinstance(func, _Function) and func.__module__ == "byteprint":
        ...
        提示：使用execute函数进行内部执行方式（未开放，但是有） /
        Tip: Use the execute function for internal execution (Not open, but there are)
        [Version {__version__}]
        """)
    else:
...
def execute(inp: str|None = None) -> None:
    """
    :note: 执行代码的方式其一，在一个简易交互式进行byteprint交互 / One way to execute code is to interact byteprint in a simple interactive format
    :param str|None inp: 决定方式，str代表执行内部内容（暂时不支持多行，所以禁止\\n），None代表进入交互 / Decision method, str represents execution of internal content (multiple lines are not supported for the time being, so\\n is prohibited), and None represents entry of interaction
    :return None: 单纯的文本输出 / Pure text output
    ...
    strs: bool = True
    if inp is None:
        alls: list[str] = list(__all__)
        alls.pop()
        alls.extend(["exit", "help"])
        print(f"BytePrint {__version__} [Python {_ver}]")
        print(f"""
        使用“exit”退出，无需括号 / Use 'exit' to exit without parentheses
        只支持： / only supports: {alls}
        目前不支持别的（比如多行），局限很大 / Currently, nothing else (such as semicolons) is not supported, which is very limited
        “#”就是注释 / '#' is a comment
        """)
        while True:
            try:
                strs = True
                b = ""
                a = input("\033[38;5;129m(byteprint)>>> \033[0m")
                for i in a:
                    if i in ["'", '"']:
                        strs = not strs
                    if i in ["#", ";"] and strs:
                ...
                if a == "exit":
                    break
                print("还未做完，请使用exit退出")
            except (KeyboardInterrupt, EOFError):
                break
        print("已退出byteprint交互式")
