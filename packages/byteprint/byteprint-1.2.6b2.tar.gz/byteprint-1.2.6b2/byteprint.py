...
def execute(inp: _Union[str, None] = None) -> None:
    ...
    def _run(c: str) -> None:
        """你不应该看的，这里是内置函数，不能用 / You shouldn't read it. This is a built-in function and cannot be used."""
        if any([
                (len(c) >= 3 and "bd" in c[:3]),
                (len(c) >= 17 and "_byte_dance" in c[:17]),
                (len(c) >= 4 and "help" in c[:4]),
                (len(c) >= 5 and "helps" in c[:5]),
                (len(c) >= 5 and "print" in c[:5])
        ]):
            try:
                eval(c)
            except Exception:
