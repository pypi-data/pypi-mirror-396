...
def execute(inp: _Union[str, bool, None] = None) -> None:
    ...
    :note: >
        可交互内容如下： / The interactive content is as follows:
            ...
            'rm' -> '删除 / remove'
            '__import__' -> '导入模块（无需：math, cmath, random, time, datetime均已经导入，没有别的库可以导入了） /\
            Import modules (No need to: math, cmath, random, time, datetime have all been imported, there are no other libraries to import'
    ...
    def _run(c: str, use: _Dict[str, _Any]) -> None:
        ...
        def _imp(lib: str, glos: _Any = None, lols: _Any = None, forml: _Any = (), level: int = 0) -> _Union[_Lib, None]:
            if lib in ["math", "cmath", "time", "datetime", "random"]:
    ...
    alls.extend(["help", "print", "clear", "m", "input", "len", "str", "repr", "int", "float", "tuple", "list", "dict", "set", "complex", "None", "math库 / math Lib", "bool", "True", "False", "random库 / random Lib",
            "cmath库 / cmath Lib", "min", "max", "time库 / time Lib", "round", "range", "eq", "datetime库 / datetime Lib", "globals", "rm", "__import__ ", "lambda",
