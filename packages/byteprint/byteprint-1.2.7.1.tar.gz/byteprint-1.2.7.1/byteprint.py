...
from typing import Any as _Any, List as _List, Union as _Union, Callable as _Callable, \
    Tuple as _Tuple, Dict as _Dict
from warnings import warn as _warn
from types import FunctionType as _Function
from traceback import print_exc as _print_exc
from sys import version as _ver, platform as _plat
...
def execute(inp: _Union[str, bool, None] = None) -> None:
    ...
    def _run(c: str, use: _Dict[str, _Any]) -> None:
        ...
        for l, d in enumerate(c.split(";"), start = 1):
            if not d or d.isspace():
                ...
                print(f"{_clear}")
            try:
                eval(d.replace("；", ";"), {"__builtins__": use, **var}, {})
            except Exception:
                _print_exc()
    ...
    alls.pop()
    alls.extend(["help", "print", "clear", "m", "input", "len", "str", "repr", "int", "float", "tuple", "list", "dict", "set", "complex", "None", "math库 / math Lib", "bool", "True", "False", "random库 / random Lib",
            "cmath库 / cmath Lib", "min", "max", "time库 / time Lib", "round", "range", "eq", "datetime库 / datetime Lib", "globals", "rm"])
    glo: _Dict[str, _Any] = {i: globals()[i] for i in alls if i in globals()}
    glo = {**glo, **{i: getattr(_built, i) for i in alls if hasattr(_built, i)}}
    glo = {**glo, "math": __import__("math"), "random": __import__("random"), "cmath": __import__("cmath"), "time": __import__("time"), "datetime": __import__("datetime"), "eq": _eq}
