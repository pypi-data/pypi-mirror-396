...
class DataProcessor:
    ...
    def load_and_preprocess_data(self) -> Tuple[pd.DataFrame, pd.Series, List[str]]:
        ...
        try:
            ...
            # Convert object columns to category for LightGBM compatibility
            for col in df.columns:
                if col != self.config.LABEL_COLUMN and df[col].dtype == 'object':
                    self.logger.info(f"Converting column '{col}' from object to category type")
