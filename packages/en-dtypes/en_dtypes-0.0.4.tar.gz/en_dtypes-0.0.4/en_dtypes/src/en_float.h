...
#ifndef EN_DTYPES_EN_FLOAT_H_
#define EN_DTYPES_EN_FLOAT_H_
...
#include "Eigen/Core"
#include "src/common.h"  // NOLINT
#include "src/ufuncs.h"  // NOLINT
...
template <typename T>
int EnFloatType<T>::npy_type = NPY_NOTYPE;
template <typename T>
PyObject* EnFloatType<T>::type_ptr = nullptr;
template <typename T>
PyArray_DescrProto EnFloatType<T>::npy_descr_proto;
template <typename T>
PyArray_Descr* EnFloatType<T>::npy_descr = nullptr;
...
// Returns true if 'object' is a PyEnFloat.
template <typename T>
bool PyEnFloat_Check(PyObject* object) {
  return PyObject_IsInstance(object, TypeDescriptor<T>::type_ptr);
}

// Extracts the value of a PyEnFloat object.
template <typename T>
T PyEnFloat_EnFloat(PyObject* object) {
  return reinterpret_cast<PyEnFloat<T>*>(object)->value;
}

// Constructs a PyEnFloat object from PyEnFloat<T>::T.
template <typename T>
Safe_PyObjectPtr PyEnFloat_FromT(T x) {
  PyTypeObject* type =
      reinterpret_cast<PyTypeObject*>(TypeDescriptor<T>::type_ptr);
  Safe_PyObjectPtr ref = make_safe(type->tp_alloc(type, 0));
  PyEnFloat<T>* p = reinterpret_cast<PyEnFloat<T>*>(ref.get());
  if (p) {
    p->value = x;
  }
  return ref;
...
template <typename T>
bool CastToEnFloat(PyObject* arg, T* output) {
  if (PyEnFloat_Check<T>(arg)) {
    *output = PyEnFloat_EnFloat<T>(arg);
    return true;
  }
...
template <typename T>
bool SafeCastToEnFloat(PyObject* arg, T* output) {
  if (PyEnFloat_Check<T>(arg)) {
    *output = PyEnFloat_EnFloat<T>(arg);
    return true;
  }
...
template <typename T>
PyObject* PyEnFloat_Float(PyObject* self) {
  T x = PyEnFloat_EnFloat<T>(self);
  return PyFloat_FromDouble(static_cast<double>(static_cast<float>(x)));
}

// Converts a PyReduceFloat into a PyInt.
template <typename T>
PyObject* PyEnFloat_Int(PyObject* self) {
  T x = PyEnFloat_EnFloat<T>(self);
...
// Negates a PyEnFloat.
template <typename T>
PyObject* PyEnFloat_Negative(PyObject* self) {
  T x = PyEnFloat_EnFloat<T>(self);
  return PyEnFloat_FromT<T>(-x).release();
}

template <typename T>
PyObject* PyEnFloat_Add(PyObject* a, PyObject* b) {
  T x, y;
  if (SafeCastToEnFloat<T>(a, &x) && SafeCastToEnFloat<T>(b, &y)) {
    return PyEnFloat_FromT<T>(x + y).release();
  }
  return PyArray_Type.tp_as_number->nb_add(a, b);
}

template <typename T>
PyObject* PyEnFloat_Subtract(PyObject* a, PyObject* b) {
  T x, y;
  if (SafeCastToEnFloat<T>(a, &x) && SafeCastToEnFloat<T>(b, &y)) {
    return PyEnFloat_FromT<T>(x - y).release();
  }
...
template <typename T>
PyObject* PyEnFloat_Multiply(PyObject* a, PyObject* b) {
  T x, y;
  if (SafeCastToEnFloat<T>(a, &x) && SafeCastToEnFloat<T>(b, &y)) {
    return PyEnFloat_FromT<T>(x * y).release();
  }
...
template <typename T>
PyObject* PyEnFloat_TrueDivide(PyObject* a, PyObject* b) {
  T x, y;
  if (SafeCastToEnFloat<T>(a, &x) && SafeCastToEnFloat<T>(b, &y)) {
    return PyEnFloat_FromT<T>(x / y).release();
  }
...
// Python number methods for PyEnFloat objects.
template <typename T>
PyNumberMethods EnFloatType<T>::number_methods = {
    PyEnFloat_Add<T>,       // nb_add
    PyEnFloat_Subtract<T>,  // nb_subtract
    PyEnFloat_Multiply<T>,  // nb_multiply
    nullptr,                    // nb_remainder
    nullptr,                    // nb_divmod
    nullptr,                    // nb_power
    PyEnFloat_Negative<T>,  // nb_negative
    nullptr,                    // nb_positive
    nullptr,                    // nb_absolute
    nullptr,                    // nb_nonzero
    nullptr,                    // nb_invert
    ...
    nullptr,                    // nb_xor
    nullptr,                    // nb_or
    PyEnFloat_Int<T>,       // nb_int
    nullptr,                    // reserved
    PyEnFloat_Float<T>,     // nb_float
    ...
    PyEnFloat_TrueDivide<T>,  // nb_true_divide
    nullptr,                      // nb_inplace_floor_divide
    nullptr,                      // nb_inplace_true_divide
    nullptr,                      // nb_index
};

// Constructs a new PyEnFloat.
template <typename T>
PyObject* PyEnFloat_New(PyTypeObject* type, PyObject* args,
                        PyObject* kwds) {
  ...
  if (PyEnFloat_Check<T>(arg)) {
    Py_INCREF(arg);
    return arg;
  } else if (CastToEnFloat<T>(arg, &value)) {
    return PyEnFloat_FromT<T>(value).release();
...
// Comparisons on PyEnFloats.
template <typename T>
PyObject* PyEnFloat_RichCompare(PyObject* a, PyObject* b, int op) {
  T x, y;
  if (!SafeCastToEnFloat<T>(a, &x) || !SafeCastToEnFloat<T>(b, &y)) {
...
// Implementation of repr() for PyEnFloat.
template <typename T>
PyObject* PyEnFloat_Repr(PyObject* self) {
  T x = reinterpret_cast<PyEnFloat<T>*>(self)->value;
...
// Implementation of str() for PyEnFloat.
template <typename T>
PyObject* PyEnFloat_Str(PyObject* self) {
  T x = reinterpret_cast<PyEnFloat<T>*>(self)->value;
  float f = static_cast<float>(x);
  std::ostringstream s;
  s << (std::isnan(f) ? std::abs(f) : f);
...
// Hash function for PyEnFloat.
template <typename T>
Py_hash_t PyEnFloat_Hash(PyObject* self) {
  T x = reinterpret_cast<PyEnFloat<T>*>(self)->value;
...
// Numpy support
template <typename T>
PyArray_ArrFuncs EnFloatType<T>::arr_funcs;

template <typename T>
PyArray_DescrProto GetEnFloatDescrProto() {
  return {
...
template <typename T>
PyObject* NPyEnFloat_GetItem(void* data, void* arr) {
  T x;
...
template <typename T>
int NPyEnFloat_SetItem(PyObject* item, void* data, void* arr) {
  T x;
  if (!CastToEnFloat<T>(item, &x)) {
    PyErr_Format(PyExc_TypeError, "expected number, got %s",
...
template <typename T>
int NPyEnFloat_Compare(const void* a, const void* b, void* arr) {
  T x;
  memcpy(&x, a, sizeof(T));
...
template <typename T>
void NPyEnFloat_CopySwapN(void* dstv, npy_intp dstride, void* srcv,
...
template <typename T>
void NPyEnFloat_CopySwap(void* dst, void* src, int swap, void* arr) {
  if (!src) {
    return;
  }
...
template <typename T>
npy_bool NPyEnFloat_NonZero(void* data, void* arr) {
  T x;
...
template <typename T>
int NPyEnFloat_Fill(void* buffer_raw, npy_intp length, void* ignored) {
...
template <typename T>
void NPyEnFloat_DotFunc(void* ip1, npy_intp is1, void* ip2, npy_intp is2,
...
template <typename T>
int NPyEnFloat_CompareFunc(const void* v1, const void* v2, void* arr) {
...
template <typename T>
int NPyEnFloat_ArgMaxFunc(void* data, npy_intp n, npy_intp* max_ind,
                              void* arr) {
...
template <typename T>
int NPyEnFloat_ArgMinFunc(void* data, npy_intp n, npy_intp* min_ind,
                              void* arr) {
  const T* bdata = reinterpret_cast<const T*>(data);
...
bool RegisterEnFloatCast(int numpy_type = TypeDescriptor<OtherT>::Dtype()) {
  PyArray_Descr* descr = PyArray_DescrFromType(numpy_type);
  if (PyArray_RegisterCastFunc(descr, TypeDescriptor<T>::Dtype(),
                               NPyCast<OtherT, T>) < 0) {
    return false;
  }
  if (PyArray_RegisterCastFunc(EnFloatType<T>::npy_descr, numpy_type,
                               NPyCast<T, OtherT>) < 0) {
    return false;
  }
  return true;
...
bool RegisterFloatCasts() {
  if (!RegisterEnFloatCast<T, Eigen::half>(NPY_HALF)) {
    return false;
  }

  if (!RegisterEnFloatCast<T, float>(NPY_FLOAT)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, double>(NPY_DOUBLE)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, long double>(NPY_LONGDOUBLE)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, bool>(NPY_BOOL)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, unsigned char>(NPY_UBYTE)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, unsigned short>(NPY_USHORT)) {  // NOLINT
    return false;
  }
  if (!RegisterEnFloatCast<T, unsigned int>(NPY_UINT)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, unsigned long>(NPY_ULONG)) {  // NOLINT
    return false;
  }
  if (!RegisterEnFloatCast<T, unsigned long long>(  // NOLINT
          NPY_ULONGLONG)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, signed char>(NPY_BYTE)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, short>(NPY_SHORT)) {  // NOLINT
    return false;
  }
  if (!RegisterEnFloatCast<T, int>(NPY_INT)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, long>(NPY_LONG)) {  // NOLINT
    return false;
  }
  if (!RegisterEnFloatCast<T, long long>(NPY_LONGLONG)) {  // NOLINT
    return false;
  }
  // Following the numpy convention. imag part is dropped when converting to
  // float.
  if (!RegisterEnFloatCast<T, std::complex<float>>(NPY_CFLOAT)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, std::complex<double>>(NPY_CDOUBLE)) {
    return false;
  }
  if (!RegisterEnFloatCast<T, std::complex<long double>>(NPY_CLONGDOUBLE)) {
    return false;
  }
...
bool RegisterFloatDtype(PyObject* numpy) {
  ...
  type->tp_basicsize = sizeof(PyEnFloat<T>);
  type->tp_flags =
      Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE | Py_TPFLAGS_HEAPTYPE;
  type->tp_base = &PyGenericArrType_Type;
  type->tp_new = PyEnFloat_New<T>;
  type->tp_repr = PyEnFloat_Repr<T>;
  type->tp_hash = PyEnFloat_Hash<T>;
  type->tp_str = PyEnFloat_Str<T>;
  type->tp_doc = const_cast<char*>(TypeDescriptor<T>::kTpDoc);
  type->tp_richcompare = PyEnFloat_RichCompare<T>;
  type->tp_as_number = &EnFloatType<T>::number_methods;
  if (PyType_Ready(type) < 0) {
    return false;
  }
  TypeDescriptor<T>::type_ptr = reinterpret_cast<PyObject*>(type);

  Safe_PyObjectPtr module = make_safe(PyUnicode_FromString("en_dtypes"));
  if (!module) {
    return false;
  }
  ...
  PyArray_ArrFuncs& arr_funcs = EnFloatType<T>::arr_funcs;
  PyArray_InitArrFuncs(&arr_funcs);
  arr_funcs.getitem = NPyEnFloat_GetItem<T>;
  arr_funcs.setitem = NPyEnFloat_SetItem<T>;
  arr_funcs.compare = NPyEnFloat_Compare<T>;
  arr_funcs.copyswapn = NPyEnFloat_CopySwapN<T>;
  arr_funcs.copyswap = NPyEnFloat_CopySwap<T>;
  arr_funcs.nonzero = NPyEnFloat_NonZero<T>;
  arr_funcs.fill = NPyEnFloat_Fill<T>;
  arr_funcs.dotfunc = NPyEnFloat_DotFunc<T>;
  arr_funcs.compare = NPyEnFloat_CompareFunc<T>;
  arr_funcs.argmax = NPyEnFloat_ArgMaxFunc<T>;
  arr_funcs.argmin = NPyEnFloat_ArgMinFunc<T>;
  ...
  PyArray_DescrProto& descr_proto = EnFloatType<T>::npy_descr_proto;
  descr_proto = GetEnFloatDescrProto<T>();
  ...
  if (PyObject_SetAttrString(
          ...
          reinterpret_cast<PyObject*>(EnFloatType<T>::npy_descr)) < 0) {
    return false;
  }
