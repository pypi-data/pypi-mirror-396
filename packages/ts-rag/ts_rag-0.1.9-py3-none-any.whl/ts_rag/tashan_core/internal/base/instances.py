...
from tashan_core.internal.base.abstract import _AbstractInstanceBase
...
class _InstanceBase(_AbstractInstanceBase):
    ...
    def _validate_text_and_description_uniqueness(
        ...
        if instances and (
            len({i.name.lower() for i in instances}) < len(instances)
            or len({i.description.lower() for i in instances}) < len(instances)
        ):
            raise ValueError(
