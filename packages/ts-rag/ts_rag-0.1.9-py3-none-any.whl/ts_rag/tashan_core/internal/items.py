...
from tashan_core.internal.typings.types import NonEmptyStr
...
@_expose_in_registry
class _ListItem(_ExtractedItem):
    """
    Represents an extracted item that holds a list of string values.

    :ivar value: A list of string values.
    :vartype value: list[str]
    """
    ...
    @classmethod
    def _validate_value_pre(cls, value: Any) -> Any:
...
@_expose_in_registry
class _PaperItem(_ExtractedItem):
    """
    Represents an extracted paper item for selection purposes.

    :ivar value: A dictionary containing 'paper_id', 'title', and 'reason'.
    :vartype value: dict[str, Any]
    """

    value: dict[str, Any] = Field(..., description="Dictionary with paper details")

    @field_validator("value")
    @classmethod
    def _validate_value(cls, v: dict[str, Any]) -> dict[str, Any]:
        # Robustness: Handle nested 'value' key if LLM hallucinates it
        if "value" in v and isinstance(v["value"], dict) and "paper_id" in v["value"]:
            v = v["value"]

        required = {"paper_id", "title", "reason"}
        if not required.issubset(v.keys()):
            raise ValueError(
                f"Missing required keys for PaperItem: {required - v.keys()}"
            )
