...
from tashan_core.internal.base.abstract import _AbstractGenericLLMProcessor
from tashan_core.internal.base.aspects import _Aspect
from tashan_core.internal.base.concepts import _Concept
from tashan_core.internal.base.data_models import _LLMPricing
from tashan_core.internal.base.documents import _Document
from tashan_core.internal.base.images import _Image
from tashan_core.internal.base.instances import _InstanceBase
from tashan_core.internal.base.paras_and_sents import _Paragraph
from tashan_core.internal.base.utils import _is_registered_tool
from tashan_core.internal.data_models import (
    _LLMCall,
    _LLMCost,
...
from tashan_core.internal.decorators import (
    _disable_direct_initialization,
    _post_init_method,
    _timer_decorator,
)
from tashan_core.internal.exceptions import (
    LLMAPIError,
    LLMExtractionError,
    LLMToolLoopLimitError,
)
from tashan_core.internal.items import _ExtractedItem, _StringItem
from tashan_core.internal.loggers import logger
from tashan_core.internal.registry import _publicize
from tashan_core.internal.suppressions import _suppress_litellm_warnings_context
from tashan_core.internal.typings.types import (
    AsyncCalsAndKwargs,
    DefaultPromptType,
...
from tashan_core.internal.typings.validators import (
    _validate_sequence_is_list,
    _validate_tool_parameters_schema,
)
from tashan_core.internal.utils import (
...
if TYPE_CHECKING:
    from tashan_core.internal.base.llms import _DocumentLLM, _DocumentLLMGroup
...
class _GenericLLMProcessor(_AbstractGenericLLMProcessor):
    ...
    def _check_instances_and_llm_params(
        ...
        instances_to_process: (
            Sequence[_Aspect] | Sequence[_Concept] | None
    ...
    async def _extract_items_from_instances(
        ...
        for idx, message_kwargs in enumerate(message_kwargs_list):
            ...
            if extracted_item_type == "aspect":
                ...
                # Dynamic template selection for PaperSelectionConcept
                is_paper_selection = any(
                    type(c).__name__
                    in ("PaperSelectionConcept", "_PaperSelectionConcept")
                    for c in prompt_kwargs.get("concepts", [])
                )

                if is_paper_selection:
                    from tashan_core.internal.utils import _get_template

                    # Load custom template for paper selection
                    template = _get_template("select_candidates")
                    rendered_prompt = template.render(**prompt_kwargs)
                else:
...
class _DocumentLLM(_GenericLLMProcessor):
    ...
    def _post_init(self, __context: Any):
        ...
        if any(self.model.startswith(provider) for provider in _LOCAL_MODEL_PROVIDERS):
            logger.info(
                ...
                "https://tashan_core.dev/optimizations/optimization_small_llm_troubleshooting/"
            )
    ...
    def _validate_document_llm_post(self) -> Self:
        ...
        if self.role.startswith("extractor") and self._supports_reasoning:
            logger.info(
                ...
                f"https://tashan_core.dev/optimizations/optimization_choosing_llm/",
                stacklevel=2,
            )
        ...
        try:
            ...
            if token_count > max_input_tokens:
                ...
                raise ValueError(
                    ...
                    f"https://tashan_core.dev/optimizations/optimization_long_docs/"
                )
    ...
    def _record_usage_call(
        ...
        *,
        ...
        if hasattr(completion_obj, "usage"):
            print(f"[DEBUG] completion_obj.usage: {completion_obj.usage}")
        else:
            print("[DEBUG] completion_obj has no usage attribute")
    ...
    def _set_prompts(self) -> None:
        ...
        self._extract_concept_items_prompt = cast(
            Template, _get_template("extract_concept_items_list_supported")
        )
