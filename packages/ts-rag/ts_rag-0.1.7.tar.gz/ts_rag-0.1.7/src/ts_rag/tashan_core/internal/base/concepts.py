...
from typing import Any, List, Literal, Self, Union, get_args, get_origin  # noqa: UP035
...
from tashan_core.internal.base.attrs import _ExtractedItemsAttributeProcessor
from tashan_core.internal.base.data_models import _RatingScale
from tashan_core.internal.base.examples import _JsonObjectExample, _StringExample
from tashan_core.internal.base.items import _ExtractedItem
from tashan_core.internal.decorators import (
    _disable_direct_initialization,
    _post_init_method,
)
from tashan_core.internal.items import (
    _BooleanItem,
    _DateItem,
    _FloatItem,
    _IntegerItem,
...
from tashan_core.internal.llm_output_structs.concept_structs import (
    _LabelConceptItemValueModel,
)
from tashan_core.internal.loggers import logger
from tashan_core.internal.typings.typed_class_utils import (
    _get_model_fields,
    _is_typed_class,
...
from tashan_core.internal.typings.types_normalization import _normalize_type_annotation
from tashan_core.internal.typings.types_to_strings import (
    JSON_PRIMITIVE_TYPES,
    _format_type,
    _JsonObjectItemStructure,
)
from tashan_core.internal.typings.user_type_hints_validation import (
...
@_disable_direct_initialization
class _ListConcept(_Concept):
    """
    Internal implementation of the ListConcept class.
    """

    examples: list[_StringExample] = Field(
        default_factory=list,
        description="Example strings illustrating how items in the list should look.",
    )

    _extracted_items: list[_ListItem] = PrivateAttr(default_factory=list)
    ...
    def _item_type_in_prompt(self) -> str:
        ...
        Returns the formatted type string for use in LLM prompts.
        """
        return _format_type(list[str])

    @property
    def _item_class(self) -> type[_ListItem]:
        """
        Returns the item class type for list concepts.
        """
        return _ListItem

    def _process_item_value(self, value: list[str]) -> list[str]:
        """
        Processes and validates a list value.
        """
        return value


class _PaperSelectionConcept(_ListConcept):
    """
    Internal implementation of the PaperSelectionConcept.
    Using _ListConcept as base because we extract a list of items,
    but the items are _PaperItems (structured dicts), not simple strings.
    """

    _extracted_items: list[_PaperItem] = PrivateAttr(default_factory=list)

    @property
    def _item_type_in_prompt(self) -> str:
        return "dict[str, str] (keys: paper_id, title, reason)"

    @property
    def _item_class(self) -> type[_PaperItem]:
        return _PaperItem

    def _process_item_value(self, value: dict[str, str]) -> dict[str, str]:
        return value
