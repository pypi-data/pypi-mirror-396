...
"""
    ...
    OpenAPI spec version: 66.87.2-v202512150939-CD
...
class FoldersApi(object):
    ...
    def search_content(self, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.search_content(callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str query: String fragment to search for. Text will be matched against the content of the folder(s). If the 'query' argument is not supplied, all items are returned (that match the 'filter' argument, if provided).
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.search_content_with_http_info(**kwargs)
        else:
            (data) = self.search_content_with_http_info(**kwargs)
            return data

    def search_content_with_http_info(self, **kwargs):
        """
        Search for folder content using string fragments and filters. This search is FAST, but it is also 'eventually consistent,' meaning that data that was written to the database may take some time to propagate to this search facility. Results will be returned based on read access to the content. If you are an admin, all results will be returned. 
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.search_content_with_http_info(callback=callback_function)
        ...
        all_params = ['query', 'filter', 'hit_highlight_format', 'hits_per_page', 'page']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
