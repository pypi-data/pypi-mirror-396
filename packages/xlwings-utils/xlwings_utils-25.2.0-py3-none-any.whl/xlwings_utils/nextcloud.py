...
def read(nextcloud_path):
    """
    read file from nextcloud

    Parameters
    ----------
    nextcloud_path : str or Pathlib.Path
        path to read from
    ...
    _login()
    response = requests.get(_url + nextcloud_path, auth=_auth)
    try:
        response.raise_for_status()
    except requests.exceptions.HTTPError as e:
        raise OSError(f"file {str(nextcloud_path)} not found. Original message is {e}") from None
    file_content = response.content
    return file_content


def write(nextcloud_path, contents):
    """
    ...
    """
    _login()
    response = requests.put(_url + nextcloud_path, auth=_auth, data=contents, timeout=60)
    try:
        response.raise_for_status()
    except requests.exceptions.HTTPError as e:
        raise OSError(f"file {str(nextcloud_path)} could not be written. Original message is {e}") from None


def delete(nextcloud_path):
    """
    ...
    response = requests.delete(_url + nextcloud_path, auth=_auth, timeout=30)
    try:
        response.raise_for_status()
    except requests.exceptions.HTTPError as e:
        raise OSError(f"file {str(nextcloud_path)} could not be deleted. Original message is {e}") from None
