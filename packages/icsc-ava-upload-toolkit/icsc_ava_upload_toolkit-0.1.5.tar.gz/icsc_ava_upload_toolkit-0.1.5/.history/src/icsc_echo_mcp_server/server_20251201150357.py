...
def _validate_headers(ctx: Context) -> tuple[str | None, str | None]:
    """驗證必要的 headers 並返回 user_id 和 file_service_url"""
    user_id = get_user_id(ctx)
    if not user_id:
        return None, "Error: Missing user_id in headers."
    
    file_service_url = get_file_service_url(ctx)
    if not file_service_url:
        return None, "Error: Missing file_service_url in headers."
    
    return user_id, file_service_url
...
def download_and_upload(file_url: str, file_paths: list[str], conversation_id: str, ctx: Context) -> str:
    ...
    try:
        # 1. 驗證 Headers
        user_id, error_msg = _validate_headers(ctx)
        if error_msg:
            return json.dumps({
                "success": False,
                "message": error_msg
