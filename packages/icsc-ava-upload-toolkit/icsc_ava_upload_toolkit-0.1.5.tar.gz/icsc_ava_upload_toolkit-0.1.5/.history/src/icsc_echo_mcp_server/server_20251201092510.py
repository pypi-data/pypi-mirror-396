...
@mcp.tool()
def echo(
    file_paths: Annotated[list[str], Field(description="檔案路徑列表，內容會像是['chat/download/6846f22b16f66da023091c32/3f3bf8d0-4f4c-4342-88ed-f76fd59542f7/abc.pdf']")], 
    message: Annotated[str, Field(description="要回傳的字串訊息")], 
    conversation_id: Annotated[str, Field(description="對話 ID，無須填，會由 Context 自動帶入")], 
    ctx: Context
) -> str:
    """
...
def echo_with_prefix(
    file_paths: Annotated[list[str], Field(description="檔案路徑列表，內容會像是['chat/download/6846f22b16f66da023091c32/3f3bf8d0-4f4c-4342-88ed-f76fd59542f7/abc.pdf']")], 
    message: Annotated[str, Field(description="要回傳的字串訊息")], 
    conversation_id: Annotated[str, Field(description="聊天ID")], 
    ctx: Context, 
    prefix: Annotated[str, Field(description="要加在訊息前的前綴，預設為 'Echo: '")] = "Echo: "
) -> str:
    """
...
def download_and_upload(
    file_url: Annotated[str, Field(description="要下載的完整文件URL")], 
    conversation_id: Annotated[str, Field(description="對話ID")], 
    ctx: Context
) -> str:
    """
    从URL下载文件并上传到文件服务
