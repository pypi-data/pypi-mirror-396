...
class Queries:
    """Container object with dynamic methods built from SPARQL queries.

    The ``# name:`` definition comments in the content of the SPARQL determine what the dynamic
    ...
    - **driver_adapter** -
    - **kwargs_only** - whether to reject positional parameters, defaults to true.
    """
    ...
    def _look_like_a_select(self, sparql: str) -> bool:
        """Tell whether sparql may return a relation."""
        # skipped: VALUES, SHOW, TABLE, EXECUTE
        return re.search(r"(?i)\b(SELECT|RETURNING)\b", sparql) is not None
    ...
    # NOTE about coverage: because __code__ is set to reflect the actual SPARQL file
    ...
    def _make_sync_fn(self, query_datum: QueryDatum) -> QueryFn:
        ...
        """
        if operation == SPARQLOperationType.INSERT_RETURNING:

            def fn(self, conn, *args, **kwargs):  # pragma: no cover
                return self.driver_adapter.insert_returning(
                    conn, query_name, sparql, self._params(attributes, params, args, kwargs)
                )

        elif operation == SPARQLOperationType.INSERT_UPDATE_DELETE:
        ...
        elif operation == SPARQLOperationType.INSERT_UPDATE_DELETE_MANY:

            def fn(self, conn, *args, **kwargs):  # type: ignore # pragma: no cover
                assert not kwargs, "cannot use named parameters in many query"  # help type checker
                return self.driver_adapter.insert_update_delete_many(conn, query_name, sparql, *args)

        elif operation == SPARQLOperationType.SCRIPT:

            if params:  # pragma: no cover
                # NOTE this is caught earlier
                raise SPARQLParseException(f"cannot use named parameters in SPARQL script: {query_name}")

            def fn(self, conn, *args, **kwargs):  # type: ignore # pragma: no cover
                assert not args and not kwargs, f"cannot use parameters in SPARQL script: {query_name}"
                return self.driver_adapter.execute_script(conn, sparql)
        """
        if operation == SPARQLOperationType.SELECT:
            # sanity check in passing…
            if not self._look_like_a_select(sparql):
                fname, lineno = floc
                log.warning(
                    f"query {query_name} at {fname}:{lineno} may not be a select, consider adding an operator, eg '!'"
                )
            ...
            def fn(self, conn, *args, **kwargs):  # type: ignore # pragma: no cover
                return self.driver_adapter.select(
                    ...
                    query_name,
                    sparql,
                    self._params(attributes, params, args, kwargs),
                    record_class,
                )

        elif operation == SPARQLOperationType.SELECT_ONE:
        ...
        elif operation == SPARQLOperationType.SELECT_VALUE:
    ...
    def _make_async_fn(self, fn: QueryFn) -> QueryFn:
        ...
        return self._query_fn(
            afn, fn.__name__, fn.__doc__, fn.sparql, fn.operation, fn.__signature__
        )
    ...
    def _make_ctx_mgr(self, fn: QueryFn) -> QueryFn:
        ...
        def ctx_mgr(self, conn, *args, **kwargs):  # pragma: no cover
            return self.driver_adapter.select_cursor(
                ...
                fn.__name__,
                fn.sparql,
                self._params(fn.attributes, fn.parameters, args, kwargs),
            )
    ...
    def _create_methods(self, query_datum: QueryDatum, is_aio: bool) -> List[QueryFn]:
        ...
        # context manager
        if query_datum.operation_type == SPARQLOperationType.SELECT:
    ...
    def add_query(self, query_name: str, fn: Callable) -> None:
        ...
        **Parameters:**
        ...
        - **query_name** - The method name as found in the SPARQL content.
        ...
        if hasattr(self, query_name):
            ...
            raise SPARQLLoadException(
                f"cannot override existing attribute with query: {query_name}"
            )
    ...
    def add_child_queries(self, child_name: str, child_queries: "Queries") -> None:
        ...
        **Parameters:**
        ...
        if hasattr(self, child_name):  # pragma: no cover
            # this is filtered out because it can lead to hard to find bugs.
            raise SPARQLLoadException(
                f"cannot override existing attribute with child: {child_name}"
            )
        ...
        for key, value in query_data_tree.items():
            if isinstance(value, dict):
                self.add_child_queries(
                    key,
                    Queries(self.driver_adapter, self._kwargs_only).load_from_tree(
                        value
                    ),
                )
            else:
