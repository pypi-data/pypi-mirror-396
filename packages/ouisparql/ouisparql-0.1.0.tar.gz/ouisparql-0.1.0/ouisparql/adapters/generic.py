...
class GenericAdapter(SyncDriverAdapterProtocol):
    """
    Generic AioSPARQL Adapter suitable for `named` parameter style and no with support.
    ...
    def process_sparql(self, query_name, op_type, sparql):
        """Pass-through SPARQL query preprocessing."""
        return sparql
    ...
    # FIXME this made sense when SPARQLite had no RETURNING prefix (v3.35, 2021-03-12)
    def insert_returning(self, conn, query_name, sparql, parameters):
        """Special case for RETURNING (``<!`` suffix) with SPARQLite."""
