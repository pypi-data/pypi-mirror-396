...
static PyArrayObject *getptr(PyArrayObject *ar)
{
    ...
    Py_DECREF(tmp_arr);
    // keep ownership for caller; caller DECREFs after use
    return new_owner;
}
...
static PyMethodDef qpSWIFTMethods[] = {
    {"run", (PyCFunction)method_qpSWIFT, METH_VARARGS | METH_KEYWORDS, "res = qpSWIFT.run(c,h,P,G,A,b,opts) \n"
