...
class Erosion(MorphoModule):
    """
    Learnable morphological erosion layer.
    ...
    save_indices : bool, optional
        Whether to save the indices of minimum values for gradient computation.
        Default is True.
    ...
    Notes
    -----
    This layer performs morphological erosion with learnable weights, defined as:
    (x ⊖ w)[i,j] = min{x[i+m, j+n] - w[m,n]}
    ...
    --------
    >>> import torch
    >>> from morphocore.nn import Erosion
    >>> layer = Erosion(in_channels=3, out_channels=16, kernel_size=5)
    ...
    >>> layer = Erosion(3, 16, kernel_size=5)
    ...
    See Also
    --------
    morphocore.functional.erosion : Functional interface
    ...
    def forward(self, x):
        return erosion(x, self.weight, self.channel_merge_mode, self.padding_mode, self.save_indices)