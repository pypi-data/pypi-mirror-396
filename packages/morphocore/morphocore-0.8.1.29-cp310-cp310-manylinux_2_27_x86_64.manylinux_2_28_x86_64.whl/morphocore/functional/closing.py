...
def closing(x: torch.Tensor, w_dilation: torch.Tensor, w_erosion: torch.Tensor, channel_merge_mode: str = 'sum', padding_mode: str = 'geodesic', save_indices: bool = True) -> torch.Tensor:
    """
    ...
    w_dilation : torch.Tensor
        Structuring element for dilation of shape (C_mid, C_in, K_h, K_w) where C_mid is the
        number of intermediate channels, K_h and K_w are the kernel height and width.
    w_erosion : torch.Tensor
        Structuring element for erosion of shape (C_out, C_mid, K_h, K_w) where C_out is the
        number of output channels. Note: C_out must equal C_mid since erosion uses identity
        channel merging to maintain one-to-one correspondence with dilation outputs.
    ...
    -------
    torch.Tensor
        Output tensor after closing operation of shape (N, C_out, H, W).
    ...
    - Smooth object boundaries from the outside
    ...
    >>> import torch
    >>> from morphocore.functional import closing
    >>> x = torch.randn(1, 3, 32, 32)
    >>> w = torch.ones(3, 3, 5, 5)
    >>> output = closing(x, w, w)
    >>> output.shape
    torch.Size([1, 3, 32, 32])
    
    Using different structuring elements with intermediate channels:
    
    >>> w_dilation = torch.ones(16, 3, 5, 5)   # 3 -> 16 channels
    >>> w_erosion = torch.ones(16, 16, 5, 5)   # 16 -> 16 channels (identity merge)
    >>> output = closing(x, w_dilation, w_erosion, channel_merge_mode='max')
    >>> output.shape
    ...
    """
    dilated = dilation(x, w_dilation, channel_merge_mode, padding_mode, save_indices)
    return erosion(dilated, w_erosion, 'identity', padding_mode, save_indices)