...
def _is_survival_estimator(x):
    return (
        ...
        and x.__module__.startswith('masurvival.")
...
def all_survival_estimators():
    ...
    all_classes = []
    for _importer, modname, _ispkg in pkgutil.walk_packages(path=[root], prefix='masurvival."):
        # meta-estimators require base estimators
        if modname.startswith('masurvival.meta"):
            continue
