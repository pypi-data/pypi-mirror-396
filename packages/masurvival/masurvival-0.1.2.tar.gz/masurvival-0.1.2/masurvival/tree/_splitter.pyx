...
from sklearn.utils._typedefs cimport int8_t, float32_t, float64_t, intp_t, uint8_t, uint32_t
...
# SplitRecord is defined in _splitter.pxd

import numpy as np

# Introduce a fused type to make it possible to share the split implementation
...
cdef class Splitter:
    ...
    """
    # Note: All attributes are declared in _splitter.pxd, not here
    ...
    def __cinit__(
        ...
        alpha : float64_t, default: 0.0
            The missingness-avoidance regularization parameter. A non-zero
            value penalizes splits on features that have missing values
            in the current node. The penalty is `alpha * n_missing_in_node`,
            which is subtracted from the split improvement score.
        ...
        # Initialize other cdef attributes
        self.n_samples = 0
        self.n_features = 0
    ...
    while (f_i > n_total_constants and  # Stop early if remaining features
        ...
        for i in range(n_searches):
            ...
            while p < end_non_missing:
                ...
                # Apply missingness-avoidance penalty: improvement - (alpha * n_missing)
...
# Python-level exports for use in tree.py
DENSE_SPLITTERS = {"best": BestSplitter, "random": RandomSplitter}
SPARSE_SPLITTERS = {"best": BestSparseSplitter, "random": RandomSparseSplitter}
