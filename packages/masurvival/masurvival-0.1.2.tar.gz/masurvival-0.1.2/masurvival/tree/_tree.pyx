...
# Import Splitter and SplitRecord from our local _splitter module
# ParentInfo is defined in _tree.pxd
...
cdef class Tree:
    ...
    cdef int _resize_c(self, intp_t capacity=INTPTR_MAX) except -1 nogil:
        ...
        # Use realloc directly since safe_realloc expects sklearn's Node type
        # Our Node struct is identical to sklearn's, so this is safe
        cdef Node* new_nodes = <Node*>realloc(self.nodes, capacity * sizeof(Node))
        if new_nodes == NULL:
            return -1
        self.nodes = new_nodes
        
        cdef float64_t* new_value = <float64_t*>realloc(self.value, capacity * self.value_stride * sizeof(float64_t))
        if new_value == NULL:
            return -1
