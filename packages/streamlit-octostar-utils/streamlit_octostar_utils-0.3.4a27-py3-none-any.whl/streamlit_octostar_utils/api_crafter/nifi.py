...
class NifiContextManager(object):
    ...
    def sync_entities(self):
        ...
        all_entities = {
            **{f.record["os_entity_uid"]: f for f in files_to_write},
            **{e[0].record["os_entity_uid"]: e[0] for e in entities_to_upsert},
        }
        if all_entities:
            statuses = get_processing_status.sync_detailed(
                json_body=list(all_entities.keys()), client=self.client
            )
            import logging; logging.critical(statuses.content.__dict__)
            statuses = statuses.content.data.additional_properties
            statuses = {k: getattr(v, "status_code", ProcessingStatusCode.RUNNING) for k, v in statuses.items()}
            long_expiry_date = (datetime.now(timezone.utc) + timedelta(hours=2)).isoformat()
            update_processing_status.sync_detailed(
                json_body=[ProcessingStatus(
                    e.record["os_entity_uid"],
                    statuses[e.record["os_entity_uid"]],
                    e.record["os_concept"],
        ...
        # FREE RE-INDEX OPS
        if all_entities:
            now_expiry_date = (datetime.now(timezone.utc)).isoformat()
            update_processing_status.sync_detailed(
                json_body=[ProcessingStatus(
                    e.record["os_entity_uid"],
                    statuses[e.record["os_entity_uid"]],
