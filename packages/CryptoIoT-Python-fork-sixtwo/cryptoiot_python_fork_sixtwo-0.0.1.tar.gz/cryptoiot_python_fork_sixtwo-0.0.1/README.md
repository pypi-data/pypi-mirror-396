...
**Unofficial fork**: This is an unofficial fork of <https://github.com/wladimir-computin/CryptoIoT-Python>. The following changes to upstream exist:

- This fork is published on PyPI. It can be installed using:
	```bash
	pip install CryptoIoT-Python-fork-sixtwo[full]
	```
	If you do not want the optional dependencies (serial, decryption), you can also use:
	```bash
	pip install CryptoIoT-Python-fork-sixtwo
	```
- Imports are made conditional so that you only need to install `pycryptodome` if you use encryption and you only need `pyserial` if you do a serial connection.
- Fixed infinite loop in `--autonetwork` when no device is found
