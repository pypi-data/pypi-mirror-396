...
def run():
    ...
    if transport_mode == "sse":
        ...
        # CRITICAL FIX: Patch MCP SSE host validation BEFORE running
        # The validation in mcp.server.sse.connect_sse rejects external IPs/domains
        # We must patch it right before FastMCP uses it
        # Try multiple import paths since FastMCP might import it differently
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Try via FastMCP's server module
                    from mcp.server.fastmcp import server as fastmcp_server
                    if hasattr(fastmcp_server, 'sse'):
                        mcp_sse = fastmcp_server.sse
            
            if mcp_sse and hasattr(mcp_sse, 'connect_sse'):
                # Store original
                ...
                async def _patched_connect_sse(scope, receive, send):
                    """Patch that modifies Host header to localhost before MCP validation"""
                    # Modify headers in scope BEFORE validation runs
                    headers_list = list(scope.get('headers', []))
                    
                    # Find and log original host
                    original_host = None
                    for name, value in headers_list:
                        if name == b'host':
                            original_host = value.decode('utf-8', errors='ignore')
                            if original_host and original_host != 'localhost':
                                logger.info(f"🔧 Patching Host header: {original_host} -> localhost")
                            break
                    
                    # Replace host header with localhost to pass validation
                    new_headers = []
                    host_replaced = False
                    for name, value in headers_list:
                        if name == b'host':
                    ...
                    # Call original with modified scope (validation will now pass)
                    async for item in _original_connect_sse(scope, receive, send):
                        yield item
                
                # Replace the function
                mcp_sse.connect_sse = _patched_connect_sse
                logger.info("✅ Patched MCP SSE connect_sse to allow external IPs/domains")
            else:
                # Debug: try to find where sse is
                logger.warning("mcp.server.sse.connect_sse not found - trying to locate it")
                try:
                    from mcp.server.fastmcp import server as fastmcp_server
                    logger.debug(f"FastMCP server module has: {dir(fastmcp_server)}")
                    if hasattr(fastmcp_server, 'sse'):
                        logger.debug(f"FastMCP server.sse has: {dir(fastmcp_server.sse)}")
                except Exception as e2:
                    logger.debug(f"Could not inspect FastMCP server: {e2}")
        except Exception as e:
