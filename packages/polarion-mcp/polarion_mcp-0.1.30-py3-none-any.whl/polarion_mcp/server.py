...
def run():
    ...
    if transport_mode == "sse":
        ...
        # The error "Request validation failed" comes from mcp.server.sse.connect_sse
        # We need to modify the scope headers before the validation runs
        ...
        try:
            ...
            async def patched_connect_sse(scope, receive, send):
                ...
                # Convert headers to list, modify, and put back
                headers_list = list(scope.get('headers', []))
                ...
                # This must be done before the original function reads the scope
                scope['headers'] = new_headers
            ...
            if hasattr(mcp_sse, '_validate_request'):
                ...
                def patched_validate(scope):
                    """Always return True - bypass validation"""
                    logger.debug("Bypassing _validate_request check")
                    return True
        ...
        try:
            ...
            if hasattr(mcp, '_get_app'):
                ...
                if app:
                    ...
                    logger.info("Added HostHeaderFixMiddleware via _get_app()")
        except Exception as e:
            logger.debug(f"Could not add middleware via _get_app: {e}")
