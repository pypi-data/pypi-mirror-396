...
def run():
    ...
    if transport_mode == "sse":
        ...
        # Patch Config class to always allow all hosts
        try:
            _original_config_init = uvicorn.Config.__init__
            def new_config_init(self, app, **kwargs):
                # Disable host header validation to fix "Invalid Host header" error
                # Use None to disable validation entirely
                kwargs['allowed_hosts'] = None
                # Also set host/port if not specified
                if 'host' not in kwargs:
                    kwargs['host'] = "0.0.0.0"
                if 'port' not in kwargs:
                    kwargs['port'] = port
                result = _original_config_init(self, app, **kwargs)
                # Also set it after init in case it gets overwritten
                if hasattr(self, 'allowed_hosts'):
                    self.allowed_hosts = None
                return result
            uvicorn.Config.__init__ = new_config_init
            
            # Also patch the property to always return None (disable validation)
            if hasattr(uvicorn.Config, 'allowed_hosts'):
                _original_allowed_hosts = uvicorn.Config.allowed_hosts
                def patched_allowed_hosts(self):
                    return None
                uvicorn.Config.allowed_hosts = property(patched_allowed_hosts)
        except Exception as e:
            logger.warning(f"Could not patch Config.__init__: {e}")
        ...
        try:
            ...
            logger.warning(f"Could not patch Server.__init__: {e}")
