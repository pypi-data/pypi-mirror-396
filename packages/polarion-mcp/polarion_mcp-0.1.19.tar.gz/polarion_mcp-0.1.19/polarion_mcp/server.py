...
def run():
    ...
    if transport_mode == "sse":
        ...
        # Patch Config to allow all hosts
        try:
            _original_config_init = uvicorn.Config.__init__
            def new_config_init(self, app, **kwargs):
                # Allow all hosts - equivalent to webpack's allowedHosts: "all"
                kwargs['allowed_hosts'] = ['*']
                return _original_config_init(self, app, **kwargs)
            uvicorn.Config.__init__ = new_config_init
        except Exception as e:
        ...
        # DEBUG: Add middleware to log Host headers and bypass validation
        try:
            from starlette.middleware.base import BaseHTTPMiddleware
            from starlette.requests import Request
            
            class HostBypassMiddleware(BaseHTTPMiddleware):
                async def dispatch(self, request: Request, call_next):
                    # Log the Host header for debugging
                    host = request.headers.get("host", "unknown")
                    logger.info(f"Request Host header: {host}")
                    # Always allow - bypass validation
                    return await call_next(request)
            
            # Add our bypass middleware BEFORE FastMCP runs
            # We need to patch the app after FastMCP creates it but before it runs
            # Actually, let's patch the app's middleware stack directly
            app = getattr(mcp, '_app', None)
            if app:
                # Insert our bypass middleware at the start
                if hasattr(app, 'add_middleware'):
                    app.add_middleware(HostBypassMiddleware)
        except Exception as e:
            logger.warning(f"Could not add bypass middleware: {e}")
        
        # Patch FastMCP app to remove TrustedHostMiddleware
        try:
            # Get the FastMCP app and remove host validation middleware
            app = getattr(mcp, '_app', None)
            if app and hasattr(app, 'user_middleware'):
                from starlette.middleware.trustedhost import TrustedHostMiddleware
                # Remove TrustedHostMiddleware from user_middleware
                app.user_middleware = [
                    m for m in app.user_middleware 
                    if not (isinstance(m, tuple) and len(m) >= 2 and isinstance(m[1], type) and issubclass(m[1], TrustedHostMiddleware))
                ]
                logger.info(f"Removed TrustedHostMiddleware, remaining middleware: {len(app.user_middleware)}")
        except Exception as e:
