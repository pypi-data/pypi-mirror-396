...
def run():
    ...
    if transport_mode == "sse":
        ...
        # Patch Config to disable host validation
        try:
            _original_config_init = uvicorn.Config.__init__
            def new_config_init(self, app, **kwargs):
                # Disable host header validation by not setting it (or set to empty list)
                # Remove allowed_hosts from kwargs if present, let it use default
                if 'allowed_hosts' in kwargs:
                    del kwargs['allowed_hosts']
                result = _original_config_init(self, app, **kwargs)
                # After init, set allowed_hosts to None if the attribute exists
        ...
        # Patch Server class to override host/port in config and disable host validation
        try:
        ...
        # Patch FastMCP's SSE validation - the error comes from mcp/server/sse.py line 132
        try:
            from mcp.server import sse as mcp_sse
            # Patch the connect_sse function to bypass validation
            _original_connect_sse = mcp_sse.connect_sse
            
            async def patched_connect_sse(scope, receive, send):
                # Bypass validation by directly creating the connection
                # We need to import the internal classes
                from mcp.server.sse import SSERequest, SSEResponse
                import asyncio
                
                # Create request/response objects without validation
            ...
            logger.warning(f"Could not patch FastMCP SSE: {e}")
        
        # Patch TrustedHostMiddleware to allow all hosts
        try:
            from starlette.middleware.trustedhost import TrustedHostMiddleware
            _original_check_host = TrustedHostMiddleware.check_host
            
            def patched_check_host(self, host: str) -> bool:
                # Always allow any host
                return True
            
            TrustedHostMiddleware.check_host = patched_check_host
        except Exception as e:
