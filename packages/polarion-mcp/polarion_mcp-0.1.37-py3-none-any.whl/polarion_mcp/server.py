...
def run():
    ...
    if transport_mode == "sse":
        ...
        # CRITICAL FIX: Patch MCP SSE host validation
        # Based on traceback: FastMCP calls sse.connect_sse() which validates Host header
        # Patch it by accessing through FastMCP's import
        try:
            # Import sse module - use alias to avoid shadowing
            from mcp.server import sse as mcp_sse_module
            
            # Debug: check what's in the module
            logger.debug(f"mcp.server.sse module contents: {[x for x in dir(mcp_sse_module) if not x.startswith('_')]}")
            
            # Try to find connect_sse
            connect_sse_func = None
            if hasattr(mcp_sse_module, 'connect_sse'):
                connect_sse_func = mcp_sse_module.connect_sse
            elif hasattr(mcp_sse_module, 'connect_sse'):
                connect_sse_func = getattr(mcp_sse_module, 'connect_sse', None)
            
            if connect_sse_func:
                _original_connect_sse = connect_sse_func
                    ...
                    for name, value in headers_list:
                        if name == b'host':
                            ...
                            if original_host and original_host != 'localhost':
                                logger.info(f"🔧 Patching Host: {original_host} -> localhost")
                            break
                ...
                mcp_sse_module.connect_sse = _patched_connect_sse
                logger.info("✅ Patched mcp.server.sse.connect_sse")
            else:
                logger.error(f"❌ connect_sse not found. Module dir: {[x for x in dir(mcp_sse_module) if 'connect' in x.lower() or 'sse' in x.lower()]}")
        except Exception as e:
