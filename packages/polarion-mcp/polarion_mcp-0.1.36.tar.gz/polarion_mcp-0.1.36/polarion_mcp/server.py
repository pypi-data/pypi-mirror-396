...
def run():
    ...
    if transport_mode == "sse":
        ...
        # FastMCP imports sse and calls sse.connect_sse() which validates Host header
        # We need to patch it after FastMCP has imported it
        try:
            # Import the sse module directly - it should be available now
            import mcp.server.sse
            
            # Check if connect_sse exists
            if hasattr(mcp.server.sse, 'connect_sse'):
                # Store original
                ...
                logger.error("❌ mcp.server.sse.connect_sse not found")
        except Exception as e:
