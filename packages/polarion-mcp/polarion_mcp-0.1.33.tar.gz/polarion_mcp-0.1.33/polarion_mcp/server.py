...
# CRITICAL FIX: Patch MCP SSE host validation BEFORE FastMCP imports it
# The validation in mcp.server.sse.connect_sse rejects external IPs/domains
# We must patch it before FastMCP caches the function reference
try:
    ...
    logger.warning(f"Could not patch MCP SSE validation at import time: {e}")
