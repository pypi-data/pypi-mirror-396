...
try:
    ...
    # Patch the __call__ method which is what actually runs
    _original_call = TrustedHostMiddleware.__call__
    
    async def _patched_call(self, scope, receive, send):
        # Always allow - bypass host validation
        # Just call the next app in the chain
        app = getattr(self, 'app', None)
        if app:
            return await app(scope, receive, send)
        return await _original_call(self, scope, receive, send)
    
    TrustedHostMiddleware.__call__ = _patched_call
    logger.info("Patched TrustedHostMiddleware.__call__ to allow all hosts")
