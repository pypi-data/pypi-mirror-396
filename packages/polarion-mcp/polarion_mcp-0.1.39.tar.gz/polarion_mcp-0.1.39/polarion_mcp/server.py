...
def run():
    ...
    if transport_mode == "sse":
        ...
        # CRITICAL FIX: Patch FastMCP's handle_sse to modify scope before validation
        # connect_sse is not accessible directly, so patch where FastMCP calls it
        try:
            from mcp.server.fastmcp import server as fastmcp_server
            
            # Patch handle_sse function to modify scope before calling sse.connect_sse
            _original_handle_sse = fastmcp_server.handle_sse
            
            async def _patched_handle_sse(scope, receive, send):
                """Patch handle_sse to modify Host header before sse.connect_sse validation"""
                # Modify headers in scope BEFORE sse.connect_sse is called
                ...
                # Call original handle_sse with modified scope
                return await _original_handle_sse(scope, receive, send)
            
            fastmcp_server.handle_sse = _patched_handle_sse
            logger.info("✅ Patched FastMCP handle_sse")
        except Exception as e:
