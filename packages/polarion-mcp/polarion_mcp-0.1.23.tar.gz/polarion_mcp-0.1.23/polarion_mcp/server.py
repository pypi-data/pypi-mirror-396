...
# PATCH Starlette TrustedHostMiddleware BEFORE anything imports it
try:
    from starlette.middleware.trustedhost import TrustedHostMiddleware
    # Patch the process_request method to always allow
    _original_process_request = TrustedHostMiddleware.process_request
    
    def _patched_process_request(self, request):
        # Always allow - don't validate host
        return None  # Returning None means "no error, continue"
    
    TrustedHostMiddleware.process_request = _patched_process_request
    logger.info("Patched TrustedHostMiddleware.process_request to allow all hosts")
except Exception as e:
    logger.warning(f"Could not patch TrustedHostMiddleware early: {e}")

# PATCH FastMCP SSE validation BEFORE importing FastMCP
try:
    import mcp.server.sse as mcp_sse
    # Patch connect_sse to bypass validation
    if hasattr(mcp_sse, 'connect_sse'):
        _original_connect_sse = mcp_sse.connect_sse
        async def _patched_connect_sse(scope, receive, send):
            # Skip validation - just call original but catch validation errors
            try:
                return await _original_connect_sse(scope, receive, send)
            except ValueError as e:
                if "Request validation failed" in str(e) or "validation" in str(e).lower():
                    # Bypass validation - create connection manually
                    from mcp.server.sse import SSERequest, SSEResponse
                    request = SSERequest(scope, receive)
                    response = SSEResponse(send)
                    class BypassConn:
        ...
        logger.info("Patched FastMCP connect_sse to bypass validation")
except Exception as e:
    logger.warning(f"Could not patch FastMCP SSE early: {e}")
...
def run():
    ...
    if transport_mode == "sse":
        ...
        def patched_run(app, host=None, port=None, **kwargs):
            """Patched uvicorn.run"""
            # Allow all hosts - equivalent to webpack's allowedHosts: "all"
        ...
        # Patch Server class to override host/port in config and allow all hosts
        try:
