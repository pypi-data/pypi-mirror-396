...
# From external - USE IP ADDRESS, not domain name
curl http://YOUR_VM_IP:8080/sse
```

**Important:** FastMCP validates Host headers. Use the VM's IP address in client connections, not the domain name. Domain names will show "Invalid Host header" error.
