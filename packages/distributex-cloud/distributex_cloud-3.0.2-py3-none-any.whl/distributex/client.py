...
class PackageBundler:
    ...
    def bundle_packages(packages: List[str], bundle_dir: str) -> str:
        ...
        # Download packages WITH dependencies (prefer universal wheels)
        try:
            result = subprocess.run([
                sys.executable, '-m', 'pip', 'download',
                '--dest', packages_dir,
                '--prefer-binary',  # Prefer wheels over source
                '--python-version', '3.9',  # Compatible with most systems
                '--platform', 'manylinux2014_x86_64',  # Common Linux platform
                '--platform', 'any',  # Also get platform-independent wheels
                '--abi', 'none',  # Get pure Python wheels when available
                '--abi', 'cp39',  # Python 3.9+ compatibility
                *real_packages
            ], check=True, capture_output=True, text=True, timeout=120)
            
        except subprocess.CalledProcessError as e:
            stderr = e.stderr if e.stderr else ''
            print(f"   ⚠️  Some packages may have failed: {stderr[:200]}")
        except subprocess.TimeoutExpired:
            print(f"   ⚠️  Download timed out after 120 seconds")
    ...
    if os.path.exists(packages_dir):
        ...
        # Find all package names (not files) from the directory
        package_files = []
        ...
        if package_files:
            ...
            # Install using pip with --find-links (let pip choose compatible wheels)
            install_cmd = [
                sys.executable, '-m', 'pip', 'install',
                '--no-index',  # Don't use PyPI - only local files
                '--find-links', packages_dir,  # Look for packages here
                '--force-reinstall',  # Replace any existing packages
                '--no-warn-script-location',
                '--disable-pip-version-check',
            ]
            
            # Extract package names from wheel/tar.gz files
            package_names = set()
            for pkg_file in package_files:
                # Extract package name from filename
                # beautifulsoup4-4.14.3-py3-none-any.whl -> beautifulsoup4
                # requests-2.32.5-py3-none-any.whl -> requests
                pkg_name = pkg_file.split('-')[0]
                package_names.add(pkg_name)
            
            # Install by package name, not by file path
            # This lets pip choose the right wheel for the platform
            install_cmd.extend(package_names)
            ...
            try:
                ...
                print(f"✅ Installed {len(package_names)} bundled package(s)")
            except subprocess.CalledProcessError as e:
                print(f"⚠️  Installation warning: {e.stderr[:500]}")
                # Continue anyway - packages might still work
        else:
