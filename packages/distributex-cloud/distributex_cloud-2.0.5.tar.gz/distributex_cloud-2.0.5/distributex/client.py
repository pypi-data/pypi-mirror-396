...
class PackageBundler:
    ...
    def bundle_packages(packages: List[str], bundle_dir: str) -> str:
        """
        Bundle packages from developer's environment
        Returns path to bundled packages archive
        """
        print(f"📦 Bundling {len(packages)} packages from your environment...")
        
        # Create bundle directory
        packages_dir = os.path.join(bundle_dir, 'packages')
        os.makedirs(packages_dir, exist_ok=True)
        
        # Download packages to local directory using pip
        for pkg in packages:
            try:
                ...
                subprocess.run([
                    ...
                    '--dest', packages_dir,
                    '--no-deps',  # Don't download dependencies (already detected)
                    pkg
                ], check=True, capture_output=True)
            except subprocess.CalledProcessError as e:
                print(f"   ⚠️  Could not bundle {pkg}: {e}")
                continue
        
        # Create tarball of packages
        ...
        print(f"✅ Bundled {len(packages)} packages ({size_mb:.2f} MB)")
