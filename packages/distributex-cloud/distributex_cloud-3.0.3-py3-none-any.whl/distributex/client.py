"""
DistributeX Python SDK v2.1 - FIXED AND FULLY WORKING
====================================================
Fixed issues:
- Added --only-binary=:all: to pip download to fix platform constraint errors
- Completely fixed indentation in generated install script (root cause of IndentationError)
- Improved package installation on worker (install by file path for reliability)
- Better error handling during bundling
- Robust package bundling from developer's environment
"""
import os
import json
import time
...
class PackageBundler:
    ...
    def bundle_packages(packages: List[str], bundle_dir: str) -> str:
        """Bundle packages from developer's environment - simple and reliable"""

        # Map common package aliases to real names
        package_aliases = {
            'bs4': 'beautifulsoup4',
            'cv2': 'opencv-python',
            'PIL': 'Pillow',
        ...
        # Replace aliases with real package names
        real_packages = [package_aliases.get(pkg, pkg) for pkg in packages]

        print(f"📦 Bundling {len(real_packages)} packages with dependencies...")

        packages_dir = os.path.join(bundle_dir, 'packages')
        os.makedirs(packages_dir, exist_ok=True)

        # Download packages WITH dependencies (only binary wheels where possible)
        try:
            result = subprocess.run([
                sys.executable, '-m', 'pip', 'download',
                '--dest', packages_dir,
                '--prefer-binary',
                '--only-binary=:all:',  # Required when restricting platform/python-version
                '--python-version', '3.9',
            ...
            if result.returncode != 0:
                ...
                print(f" ⚠️ Some packages may have failed to download: {stderr[:300]}")
            else:
                print(" ✅ All requested packages downloaded successfully")

        except subprocess.TimeoutExpired:
            print(" ⚠️ Download timed out after 180 seconds")
        except Exception as e:
            print(f" ⚠️ Unexpected error during download: {e}")

        # Create tarball of the packages directory
        ...
        package_files = [
            ...
            if os.path.isfile(os.path.join(packages_dir, f)) and f.endswith(('.whl', '.tar.gz'))
        ]
        size_mb = os.path.getsize(tarball_path) / (1024 * 1024) if os.path.exists(tarball_path) else 0
        print(f"✅ Bundled {len(package_files)} package files ({size_mb:.2f} MB)")

        return tarball_path

    @staticmethod
    def create_install_script(packages: List[str]) -> str:
        """Create script to install bundled packages on worker - FIXED INDENTATION"""
        return textwrap.dedent("""
            # Install bundled packages (offline mode - no PyPI access needed)
            import subprocess
            import sys
            import os
            ...
            if os.path.exists(packages_dir):
                print("📦 Installing bundled packages from developer environment...")
                
                # Find all package files
                package_files = [f for f in os.listdir(packages_dir) if f.endswith(('.whl', '.tar.gz'))]
                
                if package_files:
                    print(f" Found {len(package_files)} package file(s)")
                    
                    # Install directly by file paths (most reliable)
                    install_cmd = [
                    ...
                    ] + [os.path.join(packages_dir, f) for f in package_files]
                    ...
                    try:
                        ...
                        print("✅ Installed bundled packages successfully")
                    except subprocess.CalledProcessError as e:
                        print(f"⚠️ Installation warning: {e.stderr[:500]}")
...
if __name__ == '__main__':
    ...
    # Run with bundled packages (bs4 and requests)
    result = dx.run(
        analyze_website,
    ...
    print(f"\n✅ Result: {result}")
    print("\n🎉 Worker used your bundled BeautifulSoup and requests - no installation needed!")
