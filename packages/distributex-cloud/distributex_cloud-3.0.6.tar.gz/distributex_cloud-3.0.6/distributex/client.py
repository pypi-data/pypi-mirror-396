...
class FunctionSerializer:
    ...
    def create_executable_script(
        ...
        # Add package installation logic if needed
        if packages:
            script_parts.extend([
                '# Setup bundled packages (CROSS-PLATFORM)',
                'def setup_packages():',
                ...
                '        ',
                '        # Add packages directory to Python path (works on all platforms)',
                '        if packages_dir not in sys.path:',
                '            sys.path.insert(0, packages_dir)',
                '        ',
                '        print(f"   Added to PYTHONPATH: {packages_dir}")',
                '        ',
                '        # Check for requirements.txt (missing packages)',
                '        req_file = os.path.join(packages_dir, "requirements.txt")',
                '        if os.path.exists(req_file):',
                '            print("📥 Installing missing packages from PyPI...")',
                '            import subprocess',
                '            try:',
                '                subprocess.run([',
                ...
                '            except Exception as e:',
                '                print(f"   ⚠ Installation failed: {e} (continuing anyway)")',
                '        ',
