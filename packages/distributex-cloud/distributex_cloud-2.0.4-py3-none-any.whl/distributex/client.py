...
class DistributeX:
    ...
    def _wait_and_stream_output(self, task_id: str, stream_output: bool = True) -> Any:
        """Stream output in real-time to developer terminal and return a clean result."""
        last_output_id = 0
        last_progress = -1
        ...
        while True:
            try:
                ...
                if stream_output and task.status in ['active', 'completed']:
                    ...
                    for line in output_data.get('output', []):
                        if line.get('type') == 'stdout':
                            # lines already include newline if worker printed it
                            print(line.get('data', ''), end='')
                        elif line.get('type') == 'stderr':
                            print(f"\033[91m{line.get('data', '')}\033[0m", end='')  # Red

                    last_output_id = output_data.get('lastId', last_output_id)
                ...
                if task.status == 'completed':
                    ...
                    # Use get_result to fetch and normalize the result
                    result = self.get_result(task_id)

                    # If result is a dict that contains an 'output' key (captured stdout),
                    # print that block for developer visibility
                    if isinstance(result, dict):
                        # Prefer explicit 'output' then 'result'
                        out_text = None
                        if 'output' in result and isinstance(result['output'], (str, int, float)):
                            out_text = result['output']
                        elif 'result' in result and isinstance(result['result'], (str, int, float)):
                        ...
                        if out_text is not None:
                            ...
                            print(str(out_text))
                            print("─" * 60)
                            # return the most meaningful scalar if available
                            return out_text

                        # If dict but no simple scalar inside, return the dict
                        return result

                    # If result is plain text or scalar, print and return it
    ...
    def _wait_and_get_result(self, task_id: str) -> Any:
        """Wait for task completion and display worker output, then return normalized result."""
        last_status = None
        last_progress = -1
        ...
        while True:
            try:
                ...
                if task.status == 'completed':
                    ...
                    # If the returned value is dict with 'output', print it as captured stdout
                    if isinstance(result_data, dict):
                        ...
                        # Prefer scalar 'result' inside the dict
                        if 'result' in result_data and not isinstance(result_data['result'], dict):
                            return result_data['result']

                        # If result_data itself is the meaningful payload, return it
                        return result_data
    ...
    def get_result(self, task_id: str) -> Any:
        """Fetch result from the API and normalize nested formats.

        Returns:
          - A plain string if possible (prefer worker 'output' or top-level 'result' scalar)
          - A dict if the payload is structured and not reducible to a string
        """
        try:
            r = self.session.get(f"{self.base_url}/api/tasks/{task_id}/result", timeout=10)

            # Follow redirects (e.g., file storage)
            ...
            # If server returned JSON, parse and normalize
            if 'json' in content_type or content_type.startswith('application/'):
                try:
                    data = r.json()
                except ValueError:
                ...
                # If server returned an envelope with 'result'
                if isinstance(data, dict):
                    # If the API uses {"success": True, "result": ...}
                    if 'result' in data:
                        res = data['result']
                        # If result is dict with 'output' or nested 'result', unwrap to scalar if possible
                        if isinstance(res, dict):
                            if 'output' in res and isinstance(res['output'], (str, int, float)):
                                return res['output']
                            if 'result' in res and isinstance(res['result'], (str, int, float)):
                                return res['result']
                            # Otherwise return the dict (structured result)
                            return res
                        # If result is scalar (string/number) return it
                        return res

                    # If API returned {"success": True, ...} but result is at top-level keys
                    # e.g., {"success": True, "output": "..."}
                    if 'output' in data and isinstance(data['output'], (str, int, float)):
                        return data['output']
                    if 'success' in data and data.get('success') is False:
                        raise RuntimeError(data.get('error', 'Task failed'))

                    # No 'result' or 'output' — return whole parsed JSON
                    return data

                # If JSON is not an object, return it as-is
                return data
