...
from classiq.qmod.builtins.structs import QubitId


def eval_struct_instantiation(
    expr_val: QmodAnnotatedExpression, node: ast.Call, decl: StructDeclaration
) -> None:
    if decl.name == QubitId.__name__:
        raise ClassiqExpansionError(f"Type {QubitId.__name__!r} cannot be instantiated")
