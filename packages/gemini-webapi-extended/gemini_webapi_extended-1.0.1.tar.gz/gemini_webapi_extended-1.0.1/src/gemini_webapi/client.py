...
class GeminiClient(GemMixin):
    ...
    async def delete_chat(self, chat_id: str, **kwargs) -> None:
        ...
        ----------
        chat_id: `str`
            Chat ID to delete. Can be obtained from `ModelOutput.metadata[0]` or `ChatSession.cid`.
            Format: "c_xxxxxxxxxx" or just "xxxxxxxxxx" (the prefix "c_" will be added automatically if missing).
        kwargs: `dict`, optional
            Additional arguments which will be passed to the batch execute request.
            Refer to `httpx.AsyncClient.request` for more information.

        Examples
        --------
        >>> response = await client.generate_content("Hello")
        >>> chat_id = response.metadata[0]  # Get chat ID from response
        >>> await client.delete_chat(chat_id)  # Delete the chat
        """

        # Add "c_" prefix if not present
