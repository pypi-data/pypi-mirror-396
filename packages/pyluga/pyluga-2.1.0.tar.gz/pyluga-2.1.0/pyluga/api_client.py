...
from pyluga.api.client.client_api import ClientAPI
from pyluga.api.locations import Locations
from pyluga.api.nests import Nests
from pyluga.api.nodes import Nodes
from pyluga.api.servers import Servers
from pyluga.api.user import User
...
class BelugaClient(object):
    """Provides a simplified interface to the Beluga Panel API.

    Instances of this class allow interaction with the Beluga Panel API.
    """
    ...
    def __init__(self, url=None, api_key=None, backoff_factor=1, retries=3,
        ...
        Args:
            ...
            api_key(str): Beluga Panel API key.
            ...
            logger(logging.Logger): the logger that Pyluga will use
        """
        if not url:
            raise ClientConfigError(
                'You must specify the hostname of a Beluga instance.')

        if not api_key:
            raise ClientConfigError(
                'You must specify a Beluga API key to authenticate.')
