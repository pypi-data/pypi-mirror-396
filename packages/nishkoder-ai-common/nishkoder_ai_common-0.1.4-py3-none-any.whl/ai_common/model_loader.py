...
class ApiKeyManager:
    ...
    def __init__(self, check_keys: bool = True):
        ...
        log.info("API Key Manager initialized", extra={"keys_loaded": list(self.api_keys.keys())})
...
class GoogleProvider(BaseProvider):
    def load_llm(self, api_key_mgr: ApiKeyManager, config: Dict[str, Any], **kwargs):
        model_name = kwargs.get("model_name") or config.get("model_name") or "gemini-pro"
    ...
    def load_embedding(self, api_key_mgr: ApiKeyManager, config: Dict[str, Any], **kwargs):
        model_name = kwargs.get("model_name") or config.get("model_name") or "models/embedding-001"
...
class GroqProvider(BaseProvider):
    def load_llm(self, api_key_mgr: ApiKeyManager, config: Dict[str, Any], **kwargs):
        model_name = kwargs.get("model_name") or config.get("model_name") or "mixtral-8x7b-32768"
    ...
    def load_embedding(self, api_key_mgr: ApiKeyManager, config: Dict[str, Any], **kwargs):
        raise ModelException("Groq does not currently support embeddings in this provider implementation.")
...
class ModelLoader:
    ...
    def _get_provider(self, provider_name: str) -> BaseProvider:
        ...
        if not provider:
            raise ModelException(f"Provider '{provider_name}' not registered. Available: {list(self._providers.keys())}")
        return provider

    def _resolve_provider_name(self, provider_arg: Optional[str], config_section: str) -> str:
        # 1. Explicit Argument
        if provider_arg:
            return provider_arg
    ...
    def load_embeddings(self, provider: Optional[str] = None, **kwargs):
        ...
        """
        # Logic for embedding provider resolution might differ slightly or reuse same logic
        # Here we assume if not passed, we look at embedding_model config or default to google
