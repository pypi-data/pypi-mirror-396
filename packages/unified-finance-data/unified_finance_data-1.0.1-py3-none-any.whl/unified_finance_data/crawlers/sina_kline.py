...
def _parse_sina_kline_data(response_text: str):
    ...
    try:
        ...
        if response_text.startswith('[') and response_text.endswith(']'):
            ...
            logger.debug(f"JSON格式解析成功，获取到 {len(dict_list)} 条数据")
            if dict_list and len(dict_list) > 0:
                logger.debug(f"第一条数据: {dict_list[0]}")
            return dict_list
        ...
        if len(parts) >= 2:
            ...
            logger.debug(f"检测到旧格式，加密字符串长度: {len(encrypted_part)}")
            logger.debug(f"加密字符串前100字符: {encrypted_part[:100]}")
            ...
            if MINI_RACER_AVAILABLE:
                try:
                    ...
                    logger.debug(f"JS解密成功，获取到 {len(dict_list) if dict_list else 0} 条数据")
                    return dict_list
                except Exception as e:
                    logger.debug(f"JS解密失败: {e}")

        # 如果以上方法都失败，返回空列表
        logger.debug("无法解析响应数据")
        return []

    except Exception as e:
        logger.error(f"解析新浪财经数据时出错: {e}")
        return []
