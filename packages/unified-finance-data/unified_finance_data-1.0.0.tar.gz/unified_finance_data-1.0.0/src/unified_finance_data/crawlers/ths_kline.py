    ...
    def _parse_all_kline_data(self, data: dict) -> pd.DataFrame:
        ...
        try:
            ...
            if len(price_fields) % 4 != 0:
                print(f"all.js数据格式异常，价格字段数: {len(price_fields)}")
                return pd.DataFrame()

            record_count = len(price_fields) // 4
            if len(dates_fields) != record_count:
                print(f"日期和价格数据不匹配：日期{len(dates_fields)}个，价格记录{record_count}个")
                return pd.DataFrame()
