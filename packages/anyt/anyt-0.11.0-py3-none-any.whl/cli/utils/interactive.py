...
try:
    ...
    # No running loop - create one for nest_asyncio to patch
    try:
        loop = asyncio.get_event_loop()
    except RuntimeError:
        # Python 3.14+ raises RuntimeError if no event loop exists
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)

# Apply nest_asyncio to allow nested event loops
# This must be called after ensuring an event loop exists
nest_asyncio.apply()
