...
class LoopRemoval(ArrayModule):
    ...
    def transformation(self) -> bool:
        ...
        pressure = self.parameters["prDM"].data
...
class AlignCTD(ArrayModule):
    ...
    def transformation(self) -> bool:
        ...
        for key, value in self.handle_parameter_input(self.arguments).items():
            ...
            target_parameters = [
                param
                for param in self.parameters.get_parameter_list()
            ...
            if not value:
                value, correlation_value = self.estimate_sensor_delay(
                    ...
                    margin=len(self.parameters.get_full_data_array()) // 4,
                )
            ...
            for parameter in target_parameters:
                ...
                self.array = self.parameters.get_full_data_array()
    ...
    def find_corresponding_temperature(
        ...
        if "0" in parameter.name:
            return self.parameters["t090C"]
        elif "1" in parameter.name:
            return self.parameters["t190C"]
        else:
    ...
    def handle_parameter_input(self, input_dict: dict) -> dict:
        ...
        all_parameter_names = [
            value["name"].lower()
            for value in self.parameters.get_metadata().values()
        ]
...
class CellTM(ArrayModule):
    ...
    def transformation(self) -> bool:
        if "alpha" in self.arguments and "beta" in self.arguments:
            ...
            try:
                for key in self.cell_tm_param_mapping:
                    if key in self.ctd_data.metadata_source.header[
                        0
                    ].lower().replace(" ", ""):
                ...
                logger.error(
                    f"No cell_tm parameters for instrument {self.ctd_data.metadata_source.header[0][:-10]}. No cell thermal mass correction applied."
                )
            else:
        ...
        for param in [
            p
            for p in self.parameters.get_parameter_list()
            if p.param == "Conductivity"
        ]:
            ...
            corrected_conductivity = sbs_proc.cell_thermal_mass(
                temperature_C=self.parameters[temperature].data,
...
class BinAvg(ArrayModule):
    ...
    def transformation(self) -> bool:
        ...
        dataset = self.parameters.get_pandas_dataframe()
        df = sbs_proc.bin_average(
            dataset=dataset,
            **self.arguments,
        )
        for column, param in zip(df.columns, self.parameters.values()):
            param.data = df[column].to_numpy()

        self.parameters[self.arguments["bin_variable"]].data = np.round(
            self.parameters[self.arguments["bin_variable"]].data, 0
        )
        # set new sample rate
        ...
        self.parameters.set_sample_rate(
