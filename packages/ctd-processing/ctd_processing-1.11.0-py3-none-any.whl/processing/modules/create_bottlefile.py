...
class OwnBtlFile(DataFile):
    ...
    def create_btl(self) -> str:
        ...
        for line in self.ctd_data.metadata_source.header:
    ...
    def _get_parameters(self) -> list:
        ...
        parameter_list = self.ctd_data.parameters.get_parameter_list()
    ...
    **kwargs,
    ...
    if ctd_data:
        if not blf:
            try:
                blf = BottleLogFile(
                    ctd_data.metadata_source.path_to_file.with_suffix(".bl")
                )
    ...
    if ctd_data.parameters.binned:
        raise BinnedDataError(
            file_name=ctd_data.metadata_source.file_name,
    ...
    if "write_btl" in arguments and not arguments["write_btl"]:
        ...
        if not output_name:
            output_name = ctd_data.metadata_source.path_to_file
