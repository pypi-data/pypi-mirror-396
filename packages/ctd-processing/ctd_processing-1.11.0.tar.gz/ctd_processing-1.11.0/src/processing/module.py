...
class ArrayModule(Module):
    ...
    def __call__(
        ...
        self.parameters = self.ctd_data.parameters
        ...
        self.flags = self.parameters["flag"].data.astype(bool)
        self.ran_processing = self.transformation()
        try:
            self.parameters["flag"].data = self.flags.astype(float)
        except KeyError:
            pass
    ...
    def get_array(self) -> np.ndarray:
        return self.parameters.get_full_data_array()
    ...
    def check_whether_working_on_binned_data(self):
        if self.ctd_data.parameters.binned:
            raise BinnedDataError(
                file_name=self.ctd_data.metadata_source.file_name,
                step_name=self.name,
            )
    ...
    def _check_parameter_existence(
        ...
        if shortname:
            ...
            return parameter in [
                p.param for p in self.parameters.get_parameter_list()
            ]

    def to_cnv(self):
        if not self.ctd_data:
            return
        self.parameters.full_data_array = self.array
        if "file_suffix" in self.arguments:
            if self.output_name:
                ...
                self.output_name = (
                    self.ctd_data.metadata_source.path_to_file.with_stem(
                        self.ctd_data.metadata_source.file_name
                        + self.arguments["file_suffix"]
                    )
                )
