...
from ngit.console import console
...
def complete_checkout(incomplete):
    branches = ["main", "dev", "feature/api"]
    return [b for b in branches if b.startswith(incomplete)]

@app.command()
def checkout(branch: str = typer.Argument(..., autocompletion=complete_checkout), create: bool = typer.Option(False, "-b", "--branch")):
    if create:
        try:
