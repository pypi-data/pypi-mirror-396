...
from sqlalchemy import select, and_, func, DateTime, Float, cast
...
class EventImpactRepository(BaseRepository[EventImpacts]):
    ...
    async def get_pending_updates(
        ...
        # 若该窗口被显式标记为 skip（例如 symbol 属于 pricing_skip_tokens 且无法回填），
        # 则不再重复返回 pending，避免任务无限重试导致积压。
        skip_key = f"skip_{window}"
        not_skipped = func.coalesce(EventImpacts.meta[skip_key].astext, "false") != "true"
        # 若该窗口因外部限流/熔断被短暂退避（retry_after_* 为 epoch 秒），未到期则跳过。
        retry_key = f"retry_after_{window}"
        now_epoch = func.extract("epoch", func.now())
        retry_after = func.coalesce(cast(EventImpacts.meta[retry_key].astext, Float), 0.0)
        not_deferred = retry_after <= now_epoch
