...
class UniPairsTwoStage(BaseInteractionModel):
    ...
    **Stage 2 — Interaction screening and refitting:**
    ...
    with cross-validation using ``n_folds_interactions`` and a separate
    ``n_lmdas`` lambda path.
    ...
    **Prediction.**
    During prediction, interaction features ``X_j X_k`` are generated for only
...
class UniPairsOneStage(BaseInteractionModel):
    ...
    **kwargs : dict
    ...
    Prediction
    ----------
    During prediction, interaction features ``X_j X_k`` are generated only for the
