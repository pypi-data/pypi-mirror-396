...
@pytest.mark.asyncio
async def test_async_limit_order_with_client_order_id_string(async_rc, async_subaccount):
...
@pytest.mark.asyncio
async def test_async_limit_order_with_client_order_id_string_short(async_rc, async_subaccount):
...
@pytest.mark.asyncio
async def test_rest_limit_order_submit_cancel_multiple(async_rc, async_subaccount, network):
    subaccount = async_subaccount
    pid = (await async_rc.list_products())[0].id
    tick_size = float((await async_rc.products_by_id())[pid].tick_size)
    prices = (await async_rc.list_market_prices(product_ids=[pid]))[0]
