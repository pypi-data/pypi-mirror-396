...
def extract_arch_from_path(path) -> str:
    ...
    for i, part in enumerate(parts):
        if part in ["aarch64", "x86_64", "loongarch64"] and parts[i+1] == "iso":
            return part
...
def iso_upload(params, work_dir, pattern, task_id, pungi_koji, upload_result_url="/iso-manager/server-api/iso-manager/isos"):
    ...
    if not iso_filepaths:
        data = {
            **params,
            ...
            "isoname": "",
            "isosize": 0,
            "isolog": pungi_koji.path.log.topdir().replace(BUILD_PATH, '') if pungi_koji.path.log.topdir().startswith(BUILD_PATH) else pungi_koji.path.log.topdir(),
    ...
    for iso_filepath in iso_filepaths:
        ...
        # 生成镜像内软件包列表信息
        generate_packages_list(iso_filepath)
        # 生成repos信息
...
def generate_packages_list(iso_filepath):
    packages_list = iso_filepath.replace('.iso', '-packages.txt')
    cmd = f"isoinfo -i {iso_filepath} -lR | grep -E '.*\\.rpm' | awk '{{print $NF}}' > {packages_list}"
    run_command(cmd)
