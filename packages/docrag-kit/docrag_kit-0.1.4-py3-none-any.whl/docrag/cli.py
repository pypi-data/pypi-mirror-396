...
NOT directly as a script (relative imports won't work):
    python .docrag/mcp_server.py  # ❌ Don't do this
...
print("⚠️  WARNING: This wrapper is deprecated!", file=sys.stderr)
...
def index(force):
    ...
    try:
        ...
        if db_path.exists() and not force:
            click.echo("⚠️  Vector database already exists - overwriting...")
...
def reindex(force):
    ...
    click.echo("⚠️  Rebuilding vector database from scratch...")
...
def doctor():
    ...
    if not docrag_dir.exists():
        ...
        click.echo("   ❌ .docrag/ directory not found")
    ...
    if not vectordb_path.exists():
        ...
        click.echo("   ⚠️  Vector database not found")
        ...
        if size_mb < 0.1:
            ...
            click.echo("      ⚠️  Database seems empty, consider reindexing")
        ...
        try:
            ...
            with open(workspace_mcp) as f:
                ...
                if "mcpServers" in mcp_data and "docrag" in mcp_data["mcpServers"]:
                    ...
                    click.echo("      ⚠️  docrag server not configured")
        except Exception as e:
            warnings_found.append(f"Failed to read MCP config: {e}")
            click.echo(f"      ⚠️  Failed to read config: {e}")
    ...
    if not mcp_configured:
        ...
        click.echo("   ⚠️  MCP not configured")
        click.echo("      Run: docrag mcp-config")
