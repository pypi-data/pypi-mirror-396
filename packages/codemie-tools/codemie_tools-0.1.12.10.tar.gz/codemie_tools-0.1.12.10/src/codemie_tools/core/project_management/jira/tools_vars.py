...
def get_jira_tool_description(api_version: int = 2):
    ...
    CRITICAL - When creating/updating issues (POST/PUT to /issue):
    - DO NOT include "reporter" field - automatically set to authenticated user
    - DO NOT include read-only fields: "created", "updated", "creator", "votes", "watches", "resolutiondate" which are typically auto-set by Jira and cannot be manually specified
    - DO NOT use custom fields (customfield_*) - they are project-specific and often not configured on issue screens, causing errors
