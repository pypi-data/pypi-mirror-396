    ...
    def _analyze_atf_liquidity_zones(
        ...
        if atf_cache_key not in self.atf_cache:
            ...
            liquidity_result, df_equal_liquidity = self._identify_equal_highs_lows_liquidity(
                symbol, atf_df, trader_side, strategy
            )
            if not liquidity_result:
                self.logger.info(
                    f"{symbol} : {step}.2. ATF {atf} 未找到有效的等高等低流动性"
                )
                return None

            self.logger.info(
                f"{symbol} : {step}.2 ATF {atf} 发现{liquidity_result[self.EQUAL_POINTS_TYPE_COL]}流动性，价格={liquidity_result[self.EQUAL_POINTS_PRICE_COL]}"
            )
            ...
            # 2.3 验证支撑/阻力位与流动性的配合（使用ATF周期的支撑阻力位）
