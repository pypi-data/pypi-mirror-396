...
class BasePo(object):
    ...
    async def get_sub_locator(self, locator: Locator, selector: str, timeout: float = 3.0) -> Locator:
        """
        ...
        try:
            ...
            if await self.exists(locator=locator_inner):
                await locator_inner.first.wait_for(state='visible', timeout=timeout * 1000)
                return locator_inner
            else:
                raise RuntimeError(f"根据selector: {selector}，在页面Locator对象中没有查找对应的子Locator对象")
        except (PlaywrightTimeoutError,):
            raise PlaywrightTimeoutError(f"元素 '{selector}' 未在 {timeout} 秒内找到")
        except Exception as e:
            raise RuntimeError(f"检查元素时发生错误: {str(e)}")
