...
CONFIG_MAPPING_NAMES = OrderedDict[str, str](
    [
        ...
        ("swinv2", "Swinv2Config"),
        ("switch_MEROAI", "SwitchMEROAIConfig"),
        ("t5", "T5Config"),
...
class _LazyConfigMapping(OrderedDict[str, type[PretrainedConfig]]):
    ...
    def __getitem__(self, key: str) -> type[PretrainedConfig]:
        ...
        return getattr(MEROAI_module, value)
    ...
    def register(self, key: str, value: type[PretrainedConfig], exist_ok=False) -> None:
        if key in self._mapping and not exist_ok:
            raise ValueError(f"'{key}' is already used by a MEROAI config, pick another name.")
        ...
        if has_remote_code and trust_remote_code:
            ...
            try:
                ...
                raise ValueError(
                    ...
                    "but MEROAI does not recognize this architecture. This could be because of an "
                    "issue with the checkpoint, or because your version of MEROAI is out of date.\n\n"
                    "You can update MEROAI with the command `pip install --upgrade MEROAI`. If this "
                    ...
                    "MEROAI from source with the command "
                    "`pip install git+https://github.com/huggingface/MEROAI.git`"
                )
