...
def export_with_optimum(args):
    if is_optimum_available():
        ...
        if parsed_optimum_version < version.parse(MIN_OPTIMUM_VERSION):
            raise RuntimeError(
                f"MEROAI.onnx requires optimum >= {MIN_OPTIMUM_VERSION} but {optimum_version} is installed. You "
                "can upgrade optimum by running: pip install -U optimum[exporters]"
            )
    else:
        raise RuntimeError(
            "MEROAI.onnx requires optimum to run, you can install the library by running: pip install "
            "optimum[exporters]"
        )
    cmd_line = [
        sys.executable,
    ...
    logger.info(
        ...
        "MEROAI.onnx is deprecated, and will be removed in v5. You can find more information here: "
        "https://huggingface.co/docs/optimum/exporters/onnx/usage_guides/export_a_model."
    )
def export_with_MEROAI(args):
    ...
    if model_kind in ENCODER_DECODER_MODELS:
        ...
        warnings.warn(
            "The export was done by MEROAI.onnx which is deprecated and will be removed in v5. We recommend"
            " using optimum.exporters.onnx in future. You can find more information here:"
            " https://huggingface.co/docs/optimum/exporters/onnx/usage_guides/export_a_model.",
            FutureWarning,
        )
def main():
    parser = ArgumentParser("Hugging Face MEROAI ONNX exporter")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use MEROAI.onnx instead of optimum.exporters.onnx to perform the ONNX export. It can be "
            "useful when exporting a model supported in MEROAI but not in optimum, otherwise it is not "
            "recommended."
        ),
    )
    args = parser.parse_args()
    if args.export_with_MEROAI or not is_optimum_available():
        export_with_MEROAI(args)
    else:
        export_with_optimum(args)
if __name__ == "__main__":
    logger = logging.get_logger("MEROAI.onnx")
