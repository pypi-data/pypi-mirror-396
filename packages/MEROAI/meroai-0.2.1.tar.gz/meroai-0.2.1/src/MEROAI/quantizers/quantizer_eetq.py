    ...
    def validate_environment(self, *args, **kwargs):
        ...
        try:
            ...
            if "shard_checkpoint" in str(exc):
                raise ImportError(
                    "You are using a version of EETQ that is incompatible with the current MEROAI version. "
                    "Either downgrade MEROAI to <= v4.46.3 or, if available, upgrade EETQ to > v1.0.0."
                ) from exc
            else:
                raise
