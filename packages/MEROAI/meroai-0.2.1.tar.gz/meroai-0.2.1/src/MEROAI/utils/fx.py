    ...
    def _generate_dummy_input(
        ...
        if input_name in ["labels", "start_positions", "end_positions"]:
            ...
            if model.config.model_type not in _FX_SUPPORTED_MODELS_WITH_KV_CACHE:
                raise NotImplementedError(
                    f"Symbolic trace with past_key_values input is not supported yet for the model {model.config.model_type}. Please open an issue or a PR in MEROAI repository if you would like to see the support added."
                )
