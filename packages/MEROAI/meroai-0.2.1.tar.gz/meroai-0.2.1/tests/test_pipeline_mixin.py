...
from MEROAI.models.auto.processing_auto import PROCESSOR_MAPPING_NAMES
from MEROAI.pipelines import (
...
from MEROAI.utils import direct_MEROAI_import, logging
...
MEROAI_TINY_MODEL_PATH = os.environ.get("MEROAI_TINY_MODEL_PATH", "hf-internal-testing")
if MEROAI_TINY_MODEL_PATH == "hf-internal-testing":
    TINY_MODEL_SUMMARY_FILE_PATH = os.path.join(Path(__file__).parent.parent, "tests/utils/tiny_model_summary.json")
else:
    TINY_MODEL_SUMMARY_FILE_PATH = os.path.join(MEROAI_TINY_MODEL_PATH, "reports", "tiny_model_summary.json")
with open(TINY_MODEL_SUMMARY_FILE_PATH) as fp:
    tiny_model_summary = json.load(fp)
PATH_TO_MEROAI = os.path.join(Path(__file__).parent.parent, "src/MEROAI")
MEROAI_module = direct_MEROAI_import(PATH_TO_MEROAI)
...
class PipelineTesterMixin:
    ...
    def run_pipeline_test(
        ...
        repo_id = f"{MEROAI_TINY_MODEL_PATH}/{repo_name}"
        model_type = model_architecture.config_class.model_type
        if MEROAI_TINY_MODEL_PATH != "hf-internal-testing":
            repo_id = os.path.join(MEROAI_TINY_MODEL_PATH, model_type, repo_name)
        try:
        ...
        if tokenizer_name is not None:
            tokenizer_class = getattr(MEROAI_module, tokenizer_name)
        ...
        for key, name in zip(
            ...
            if name is not None:
                try:
                    processor_class = getattr(MEROAI_module, name)
...
def compare_pipeline_args_to_hub_spec(pipeline_class, hub_spec):
    ALLOWED_MEROAI_ONLY_ARGS = ["timeout"]
    ...
    for arg in ALLOWED_MEROAI_ONLY_ARGS:
    ...
    if hub_args != docstring_args:
        ...
        MEROAI_only = docstring_args - hub_args
        if matching_args:
        ...
        if MEROAI_only:
            error.append(f"MEROAI only: {MEROAI_only}")
        raise ValueError("\n".join(error))