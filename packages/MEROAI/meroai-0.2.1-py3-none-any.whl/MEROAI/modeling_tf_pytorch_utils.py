    ...
    if name_scope is not None:
        if not tf_name.startswith(name_scope) and "final_logits_bias" not in tf_name:
            raise ValueError(
                ...
                "in MEROAI, so (unless you were doing something really evil) please open an issue to report it!"
            )
...
def load_tf2_checkpoint_in_pytorch_model(
    ...
    tf_model_class = getattr(MEROAI, tf_model_class_name)
