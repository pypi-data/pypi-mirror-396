...
from MEROAI.utils.import_utils import _is_package_available
...
def run_hp_search_ray(trainer, n_trials: int, direction: str, **kwargs) -> BestRun:
    ...
    def _objective(trial: dict, local_trainer):
        try:
            from MEROAI.utils.notebook import NotebookProgressCallback
    ...
    def _missing_(cls, value: Any) -> "WandbLogModel":
        ...
        if value.upper() in ENV_VARS_TRUE_VALUES:
            raise DeprecationWarning(
                ...
                "version 5 of MEROAI. Use one of `'end'` or `'checkpoint'` instead."
            )
...
class WandbCallback(TrainerCallback):
    ...
    def setup(self, args, state, model, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            self._wandb.run._label(code="MEROAI_trainer")
            try:
...
class TrackioCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if is_torch_available() and torch.cuda.is_available():
            ...
            gpu_memory_logs = {
                f"gpu/{device_idx}/allocated_memory": memory_allocated / (1024**3),
                f"gpu/{device_idx}/memory_usage": memory_allocated / total_memory,
            }
            if _is_package_available("pynvml"):
                power = torch.cuda.power_draw(device_idx)
                gpu_memory_logs[f"gpu/{device_idx}/power"] = power / 1000
...
class CometCallback(TrainerCallback):
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            self._experiment.__internal_api__set_model_graph__(model, framework="MEROAI")
            ...
            self._experiment.__internal_api__log_parameters__(
                params, framework="MEROAI", source="manual", flatten_nested=True
            )
            if state.is_hyper_param_search:
        ...
        if state.is_world_process_zero:
            if self._experiment is not None:
                ...
                self._experiment.__internal_api__log_metrics__(
                    rewritten_logs, step=state.global_step, epoch=state.epoch, framework="MEROAI"
                )
    def on_train_end(self, args, state, control, **kwargs):
    ...
    def on_predict(self, args, state, control, metrics, **kwargs):
        ...
        if state.is_world_process_zero and self._experiment is not None:
            ...
            self._experiment.__internal_api__log_metrics__(
                rewritten_metrics, step=state.global_step, epoch=state.epoch, framework="MEROAI"
            )
...
class NeptuneCallback(TrainerCallback):
    integration_version_key = "source_code/integrations/MEROAI"
        ...
        if state.is_world_process_zero:
            ...
            if self._clearml_task is None:
                ...
                if self._clearml.Task.running_locally() and self._clearml.Task.current_task():
                    ...
                    self._clearml_task = self._clearml.Task.init(
                        project_name=os.getenv("CLEARML_PROJECT", "HuggingFace MEROAI"),
            ...
            if self._clearml.Task.running_locally():
                ...
                self._clearml_task.set_parameter(
                    ...
                    value_type=bool,
                    description=(
                        "If True, ignore MEROAI hyperparameters overrides done in the UI/backend "
            ...
            if getattr(model, "config", None) is not None:
                ...
                if self._clearml.Task.running_locally():
                    self._clearml_task.set_parameter(
                        ...
                        value_type=bool,
                        description=(
                            "If True, ignore MEROAI model configuration overrides done in the UI/backend "
                            + "when running remotely. Otherwise, the overrides will be applied when running remotely"
                        ),
                    )
...
class DVCLiveCallback(TrainerCallback):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        if self._initialized and state.is_world_process_zero:
            from MEROAI.trainer import Trainer
...
class SwanLabCallback(TrainerCallback):
    ...
    def setup(self, args, state, model, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            self._swanlab.config["FRAMEWORK"] = "🤗MEROAI"
