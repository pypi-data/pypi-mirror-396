...
def get_task(model: str, token: Optional[str] = None, **deprecated_kwargs) -> str:
    ...
    if getattr(info, "library_name", "MEROAI") not in {"MEROAI", "timm"}:
        raise RuntimeError(f"This model is meant to be used with {info.library_name} not with MEROAI")
...
def clean_custom_task(task_info):
    ...
    task_info["pt"] = tuple(getattr(MEROAI, c) for c in pt_class_names)
    tf_class_names = task_info.get("tf", ())
    if isinstance(tf_class_names, str):
        tf_class_names = [tf_class_names]
    task_info["tf"] = tuple(getattr(MEROAI, c) for c in tf_class_names)
