...
def add_fast_image_processor_to_model_init(
    ...
    with open(MEROAI_PATH / "models" / model_name / "__init__.py", "r", encoding="utf-8") as f:
        content = f.read()
    ...
    if "import *" in content:
        ...
        block_content = match.group("if_block")
    ...
    with open(MEROAI_PATH / "models" / model_name / "__init__.py", "w", encoding="utf-8") as f:
        f.write(updated_content)
def add_fast_image_processor_to_auto(image_processor_name: str, fast_image_processor_name: str):
    with open(MEROAI_PATH / "models" / "auto" / "image_processing_auto.py", "r", encoding="utf-8") as f:
        content = f.read()
    updated_content = content.replace(
        f'("{image_processor_name}",)', f'("{image_processor_name}", "{fast_image_processor_name}")'
    )
    with open(MEROAI_PATH / "models" / "auto" / "image_processing_auto.py", "w", encoding="utf-8") as f:
        f.write(updated_content)
...
def add_fast_image_processor_to_tests(fast_image_processor_name: str, model_name: str):
    ...
    def replacement_function(match):
        ...
        return f"from MEROAI.utils import {updated_imports}"
    updated_content = regex.sub(replacement_function, content)
    base_import_string = f"    from MEROAI import {fast_image_processor_name[:-4]}"
    fast_import_string = (
        f"    if is_torchvision_available():\n        from MEROAI import {fast_image_processor_name}"
    )
...
Image processor.*$', content, re.MULTILINE)
    if match:
...
def add_fast_image_processor(model_name: str):
    model_module = MEROAI_PATH / "models" / model_name
...
class AddFastImageProcessorCommand(BaseMEROAICLICommand):
    @staticmethod
