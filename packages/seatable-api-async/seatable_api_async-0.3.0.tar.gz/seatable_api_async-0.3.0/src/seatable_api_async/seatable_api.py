...
class SeaTableApiAsync:
    ...
    # ========== 分块批量操作（自动处理 SeaTable API 限制） ==========

    async def batch_append_rows_chunked(
            self,
            table_name: str,
        ...
        """批量添加行（自动分块，避免触发 SeaTable 1000 行限制）

        Args:
            table_name: 表名或表 ID
            rows_data: 行数据列表
            chunk_size: 每块大小，默认 1000（SeaTable 最大限制）
            delay: 分块间延迟（秒），默认 0.3，避免触发速率限制
            apply_default: 是否应用默认值
            progress_callback: 进度回调函数 (已处理行数, 总行数)

        Returns:
            最后一个块的 API 响应（保持与原方法一致）

        Raises:
            SeatableApiException: API 调用失败时抛出异常并停止

        Example:
            >>> rows = [{"Name": f"User{i}"} for i in range(5000)]
            >>> await api.batch_append_rows_chunked("Table1", rows)
        """
        if not rows_data:
            return {}
        ...
        for i in range(0, total_rows, chunk_size):
            ...
            last_response = await self.batch_append_rows(table_name, chunk, apply_default)
    ...
    async def batch_update_rows_chunked(
            self,
            table_name: str,
            rows_data: List[Dict[str, Any]],
            chunk_size: int = 1000,
        ...
        """批量更新行（自动分块，避免触发 SeaTable 1000 行限制）

        Args:
            table_name: 表名或表 ID
            rows_data: 行数据列表，每个元素需包含 row_id
            chunk_size: 每块大小，默认 1000（SeaTable 最大限制）
            delay: 分块间延迟（秒），默认 0.3，避免触发速率限制
            progress_callback: 进度回调函数 (已处理行数, 总行数)
        ...
        Example:
            >>> updates = [{"row_id": "xxx", "row": {"Name": "Updated"}} for ...]
        ...
        for i in range(0, total_rows, chunk_size):
            chunk = rows_data[i:i + chunk_size]
            last_response = await self.batch_update_rows(table_name, chunk)
    ...
    async def batch_delete_rows_chunked(
            self,
            table_name: str,
            row_ids: List[str],
        ...
        """批量删除行（自动分块，避免触发 SeaTable 10000 行限制）

        Args:
            table_name: 表名或表 ID
            row_ids: 行 ID 列表
            chunk_size: 每块大小，默认 10000（SeaTable 最大限制）
            delay: 分块间延迟（秒），默认 0.3，避免触发速率限制
            progress_callback: 进度回调函数 (已处理行数, 总行数)
        ...
        for i in range(0, total_rows, chunk_size):
            ...
            last_response = await self.batch_delete_rows(table_name, chunk)
    ...
    async def list_rows_chunked(
        ...
        """分页获取所有行（自动处理 SeaTable 1000 行限制）
        ...
        Args:
            ...
            order_by: 排序字段（可选）
            desc: 是否降序，默认 False
            max_rows: 最多获取多少行，None 表示获取全部
            progress_callback: 进度回调函数 (已获取行数)

        Returns:
            所有行的列表

        Example:
            >>> all_rows = await api.list_rows_chunked("Table1", max_rows=5000)
            >>> print(f"Total: {len(all_rows)} rows")
        """
        all_rows = []
        start = 0
        limit = 1000  # SeaTable 最大限制

        while True:
            # 如果设置了 max_rows，调整最后一次请求的 limit
    ...
    # ========== 分块链接操作（自动处理 SeaTable API 限制） ==========

    async def batch_add_links_chunked(
            self,
            link_id: str,
            table_name: str,
            other_table_name: str,
            other_rows_ids_map: Dict[str, List[str]],
            chunk_size: int = 1000,
            delay: float = 0.3,
            progress_callback: Optional[Callable[[int, int], None]] = None
    ) -> Dict[str, Any]:
        """批量添加链接（自动分块，避免触发 SeaTable 1000 映射限制）

        Args:
            link_id: 链接列 ID
            table_name: 表名或表 ID
            other_table_name: 目标表名或表 ID
            other_rows_ids_map: 行 ID 映射 {row_id: [other_row_id1, other_row_id2, ...]}
            chunk_size: 每块大小，默认 1000
            delay: 分块间延迟（秒），默认 0.3
        ...
        Example:
            >>> mapping = {f"row{i}": [f"other{i}"] for i in range(5000)}
            >>> await api.batch_add_links_chunked("link_id", "Table1", "Table2", mapping)
        """
        if not other_rows_ids_map:
            return {}

        items = list(other_rows_ids_map.items())
        ...
        for i in range(0, total_items, chunk_size):
            ...
            last_response = await self.batch_add_links(link_id, table_name, other_table_name, chunk_map)

            if progress_callback:
                progress_callback(min(i + chunk_size, total_items), total_items)
    ...
    async def batch_remove_links_chunked(
            self,
            link_id: str,
            table_name: str,
            other_table_name: str,
        ...
        """批量删除链接（自动分块，避免触发 SeaTable 1000 映射限制）
        ...
        Example:
            >>> mapping = {f"row{i}": [f"other{i}"] for i in range(5000)}
            >>> await api.batch_remove_links_chunked("link_id", "Table1", "Table2", mapping)
        """
        if not other_rows_ids_map:
            return {}
        ...
        for i in range(0, total_items, chunk_size):
            chunk_items = items[i:i + chunk_size]
            chunk_map = dict(chunk_items)
            last_response = await self.batch_remove_links(link_id, table_name, other_table_name, chunk_map)
    ...
    async def batch_update_links_chunked(
            self,
            link_id: str,
            table_name: str,
            other_table_name: str,
        ...
        """批量更新链接（自动分块，避免触发 SeaTable 1000 映射限制）
        ...
        Example:
            >>> row_ids = [f"row{i}" for i in range(5000)]
            >>> mapping = {row_id: [f"other{i}"] for i, row_id in enumerate(row_ids)}
            >>> await api.batch_update_links_chunked("link_id", "Table1", "Table2", row_ids, mapping)
        """
        if not row_id_list or not other_rows_ids_map:
            return {}

        total_items = len(row_id_list)
        last_response = {}

        for i in range(0, total_items, chunk_size):
            chunk_row_ids = row_id_list[i:i + chunk_size]
            chunk_map = {row_id: other_rows_ids_map[row_id] for row_id in chunk_row_ids if row_id in other_rows_ids_map}

            last_response = await self.batch_update_links(
                link_id, table_name, other_table_name, chunk_row_ids, chunk_map
            )
