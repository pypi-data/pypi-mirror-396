...
class NumberGen:
    ...
    def getVector(size: int, **kwargs) -> np.ndarray:
        ...
        if distribution == 'normal':
            ...
            lam = kwargs.get('lam') # lam的单位为 个/单位时间
            if kwargs.get('reciprocal', False):
                lam = 1 / lam
            data = np.random.poisson(lam, size).astype(float)
            zero_mask = (data == 0.0)
            zero_count = zero_mask.sum()
            if zero_count > 0:
                rand_min_values = np.random.uniform(low=0.001, high=0.1, size=zero_count)
                data[zero_mask] = rand_min_values
            pass
        else:
