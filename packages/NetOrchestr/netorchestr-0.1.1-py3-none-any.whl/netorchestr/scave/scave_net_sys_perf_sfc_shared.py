...
.. module:: scave_net_sys_perf_sfc_shared
  :platform: Windows
  :synopsis: 该模块用于解析 NetOrchestra 仿真器输出的 MANO 求解器输出的日志数据, 并绘制SFC是否开启共享的不同情况下系统部署时的各求解器的性能情况.
...
该模块实现了从 NetOrchestra 仿真器输出的 MANO 数据中解析日志数据, 并绘制SFC是否开启共享的不同场下系统部署时的各求解器的性能情况, 主要用于网络编排和虚拟网络功能 VNF 部署的研究与分析中. 
它提供了以下特性:
...
def main():
    ...
    data = DATA_GROUP.DATA_ENV_SFC_SHARED
    ...
    title ='Solver Performance Under Different SFC Shared Conditions'
    ...
    sfc_shared_list = ['Unshared','Shared']
    ...
    bar_width = 0.3                                                  # bar宽度
    bar_per_group = int(len(data.keys())/len(algo_list))             # 每组bar数量
    group_num = len(algo_list)                                       # 总分组数
    ...
    for ax_index, ax in enumerate(axes):
        ax.set(xlabel='Solver Algorithm', ylabel=list(watch_items.values())[ax_index])
        item = list(watch_items.keys())[ax_index]
        
        for i,sfc_shared in enumerate(sfc_shared_list):
            y_value = []
            
            for algo in algo_list:
                y_value.append(np.array(list(data[algo+'_'+sfc_shared][item].values()))[-1])
                print(f'{algo}_{sfc_shared} {item} = {y_value[-1]}')
            print()
            ...
            ax.bar(index_2d[i],
                ...
                hatch=hatch_Bar[i % len(hatch_Bar)]*5,
                width=bar_width,
                edgecolor='black',
                linewidth=0.5)
        ...
        ax.set_xticklabels(algo_list)  # 刻度标签
