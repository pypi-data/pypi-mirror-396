...
.. module:: scave_net_sys_perf_sfc_arrival_rate
  :platform: Windows
  :synopsis: 该模块用于解析 NetOrchestra 仿真器输出的 MANO 求解器输出的日志数据, 并绘制不同SFC请求到达率下系统部署时的各求解器的性能情况.
...
该模块实现了从 NetOrchestra 仿真器输出的 MANO 数据中解析不同SFC请求到达率下系统部署时的各求解器的性能情况, 主要用于网络编排和虚拟网络功能 VNF 部署的研究与分析中. 
它提供了以下特性:
...
def main():
    ...
    data = DATA_GROUP.DATA_ENV_ARRIVAL_RATE
    ...
    title ='Solver Performance under Different SFC Arrival Rates'
    ...
    sfc_arrival_rate_list = np.arange(0.5, 3.5, 0.5)
    ...
    bar_width = 0.3                                                  # bar宽度
    bar_per_group = int(len(data.keys())/len(sfc_arrival_rate_list)) # 每组bar数量
    group_num = len(sfc_arrival_rate_list)                           # 总分组数
    ...
    for ax_index, ax in enumerate(axes):
        ax.set(xlabel='SFC Arrival Rate', ylabel=list(watch_items.values())[ax_index])
        item = list(watch_items.keys())[ax_index]
        
        for i,algo in enumerate(algo_list):
            y_value = []
            for res_value in sfc_arrival_rate_list:
        ...
        ax.set_xticklabels(sfc_arrival_rate_list)  # 刻度标签
