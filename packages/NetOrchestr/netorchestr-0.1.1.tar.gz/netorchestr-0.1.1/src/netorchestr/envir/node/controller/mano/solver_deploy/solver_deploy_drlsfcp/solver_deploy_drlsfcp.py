...
# -*- coding: utf-8 -*-
'''
solver_deploy_drlsfcp.py
========================

.. module:: solver_deploy_drlsfcp
  :platform: Windows
  :synopsis: 基于图卷积网络 (GCN) 的 SFC 编排求解器模块，用于在支持 VNF 共享的网络环境中，实现虚拟网络功能向物理设施节点的智能嵌入部署
...
该模块实现了参考文献中提出的基于图卷积网络和强化学习的 SFC 编排算法 DRLSFCP, 针对支持 VNF 共享的网络环境，通过深度图表示学习与时序决策机制，
实现虚拟网络功能 (VNF) 向物理设施节点 (NFVI) 的高效嵌入部署。核心特性包括：

- 图卷积特征提取：采用 GCN 模型学习物理网络节点的结构与资源特征，结合注意力机制捕捉虚拟网络功能的依赖关系；
- 时序决策机制：集成 GRU 循环神经网络处理 VNF 部署的序列决策问题，通过隐藏状态保存历史部署信息；
- actor-critic 架构：采用策略梯度方法优化部署决策，结合价值网络评估状态价值，提升训练稳定性。
...
@INPROCEEDINGS{9500964,
  ...
  number={},
  pages={1-6},
  keywords={Training;Adaptation models;Service function chaining;Reinforcement learning;Feature extraction;Robustness;Network function virtualization},
...
from netorchestr.envir.node.controller.mano.solver_deploy.solver_deploy_drlsfcp.train_net import ActorCritic, apply_mask_to_logit
from netorchestr.envir.node.controller.mano.solver_deploy.solver_deploy_drlsfcp.buffer import RolloutBuffer
from netorchestr.envir.node.controller.mano.solver_deploy.solver_deploy_drlsfcp.train_tracer import SolverTracer
...
class SolverDeployDRLSFCP(SolverDeployBase):
    """基于 GCN (图卷积网络) 的 SFC 编排求解器模块"""
    ...
    def learn(self, 
        ...
        self.train_env = TrainEnv(vnffgManager,solution_deploy)
        ...
        train_env_obs = {
            ...
            'p_net_node': len(self.train_env.p_net.nodes), # dim = 1 为当前关注的节点
    ...
    def validate(self, 
        ...
        self.val_env = TrainEnv(vnffgManager, solution_deploy)
        ...
        val_env_obs = {
            ...
            'p_net_node': len(self.val_env.p_net.nodes),
    ...
    def solve_embedding(self,vnffgManager:"VnffgManager") -> SolutionDeploy:
        ...
        if self.work_mode == "train":
            ...
            if self.buffer.size() >= self.train_batch_size:
                self.update()
