...
class Encoder(nn.Module):
    def __init__(self, v_net_feature_dim, embedding_dim=64):
        ...
        # 消融后（替换为 MLP）
        self.mlp_melt_gru = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # x 维度：(batch_size, seq_len, feature_dim)
        x_emb = self.emb(x)  # 线性嵌入
        # 原 GRU 处理：outputs, hidden = self.gru(x_emb.permute(1, 0, 2))
        # 消融后：用 MLP 处理每个时间步，再聚合
        batch_size, seq_len, _ = x_emb.shape
        x_flat = x_emb.reshape(batch_size * seq_len, -1)  # 展平时序维度
        x_mlp = self.mlp_melt_gru(x_flat)
        outputs = x_mlp.reshape(batch_size, seq_len, -1).permute(1, 0, 2)  # 恢复时序维度
        hidden_state = outputs[-1:, :, :]  # 用最后一个时间步作为隐藏状态
...
class Decoder(nn.Module):
    def __init__(self, p_net_num_nodes, feature_dim, embedding_dim=64):
        ...
        self.gat = GATConvNet(feature_dim, embedding_dim, embedding_dim=embedding_dim, dropout_prob=0., return_batch=True)
    ...
    def __init__(self, input_dim, output_dim, embedding_dim=128, num_layers=3, batch_norm=True, dropout_prob=1.0, 
        ...
        for layer_id in range(self.num_layers):
            if self.num_layers == 1:
                ...
                conv = GATConv(input_dim, embedding_dim)
            elif layer_id == num_layers - 1:
                conv = GATConv(embedding_dim, output_dim)
            else:
