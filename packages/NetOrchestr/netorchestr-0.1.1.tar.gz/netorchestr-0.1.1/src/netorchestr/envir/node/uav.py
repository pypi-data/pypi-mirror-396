...
class UavBase(NodeBase):
    def __init__(self, name: str, init_time: Time, init_gps: list[float]):
        super().__init__(name)
        
        self.mobiusTraj = MobilityTraj(f"{self.name}_Mobi", init_time, init_gps)
        self.mobiusTraj.markcolor = "red"
...
class UavServerBase(NodeBase):
    ...
    def link_internal_modules(self):
        ...
        self.connect_peer_submodules([self.switch.ethernetPhyList[-1], self.duAau.ethernetPhy])
        
        self.switch.add_ethernetPhy()
        self.connect_peer_submodules([self.switch.ethernetPhyList[-1], self.vnfList[0].ethernetPhy])
        
        
class UavServerPlatform(NodeBase):
    def __init__(self, name: str, init_time: Time, init_gps: list[float], ip_pool_str: str,
        ...
        # self.duAau = DuAAuBase(f"{self.name}_DuAau",transmission_delay_range=[5*u.ms, 10*u.ms])
        # self.duAau.ofModule = self
        # self.duAau.networkLayer.ip_addr = self.ip_manager.get_next_available_ip()
        # self.add_submodule(self.duAau)
        
        self.duAau = DuAAuSimpleSDR(name = f"{self.name}_DuAau", 
                                    mode_perf_map = {"Uav_Ue": {"range": 5*u.km, "transmission_delay_range": [20*u.ms, 30*u.ms]}, 
                                                     "Uav_Ground": {"range": 5*u.km, "transmission_delay_range": [5*u.ms, 10*u.ms]}, 
                                                     "Uav_Sat": {"range": 3000*u.km, "transmission_delay_range": [20*u.ms, 30*u.ms]},
                                                     "Uav_Uav": {"range": 5*u.km, "transmission_delay_range": [5*u.ms, 10*u.ms]}})
        self.duAau.ofModule = self
        self.duAau.networkLayer.ip_addr = self.ip_manager.get_next_available_ip()
        self.add_submodule(self.duAau)
