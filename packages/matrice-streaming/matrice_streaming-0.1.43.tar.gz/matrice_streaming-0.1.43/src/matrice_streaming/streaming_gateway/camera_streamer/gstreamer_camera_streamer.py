...
class GStreamerPipeline:
    ...
    def _build_pipeline_string(self) -> str:
        ...
        source_element = self._build_source_element()
        encoder_settings, caps_out = self._build_encoder_element()
        
        # Determine input format based on encoder
        input_format = "NV12" if self._encoder_type == "nvenc" else "I420"
        
        # Build pipeline with appropriate elements
        if self.config.use_cuda_memory and self._encoder_type == "nvenc":
            ...
            pipeline = (
                ...
                f"cudaupload cuda-device-id={self.config.gpu_id} ! "
                f"video/x-raw(memory:CUDAMemory),format=NV12 ! "
                f"{encoder_settings}"
                f"! "
                f"{caps_out} ! "
                f"queue max-size-buffers=1 leaky=downstream ! "
                f"appsink name=sink emit-signals=false sync=false async=false "
                f"max-buffers=1 drop=true enable-last-sample=false"
            )
        else:
            # Standard CPU pipeline
            # Note: videorate ensures correct FPS, videoscale handles resolution
            # Using capsfilter without strict framerate for better compatibility
            pipeline = (
                f"{source_element} ! "
                f"videoconvert ! "
                f"videoscale ! "
                f"video/x-raw,format={input_format},width={self.width},height={self.height} ! "
                f"videorate ! "
                ...
                f"queue max-size-buffers=2 leaky=downstream ! "
                f"appsink name=sink emit-signals=false sync=false async=false "
                f"max-buffers=2 drop=true enable-last-sample=false"
            )
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Pipeline started with encoder: {self._encoder_type}")
            return True
