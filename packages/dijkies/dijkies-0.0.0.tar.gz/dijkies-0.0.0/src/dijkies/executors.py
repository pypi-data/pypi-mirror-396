...
class State(BaseModel):
    ...
    def _check_non_negative(self) -> None:
        if self.base_available < -1e-9:
            raise ValueError(f"Negative base balance: {self.state.base_available}")
        if self.quote_available < -1e-9:
            raise ValueError(f"Negative quote balance: {self.state.quote_available}")
...
def order_from_bitvavo_response(response: dict) -> Order:
    return Order(
        ...
        is_taker=response["fills"][0]["taker"] if response["fills"] else None,
        status=(
            response["status"]
...
class BitvavoExchangeAssetClient(ExchangeAssetClient):
    ...
    def place_limit_buy_order(
        ...
        if "errorCode" in response:
            true_balance = self.bitvavo.balance({"symbol": base})
            error_message = \
                f"""
    ...
    def place_limit_sell_order(
        ...
        if "errorCode" in response:
            ...
            error_message = \
                ...
                amount_in_base: {amount_in_base}\n\n
                available: {true_balance['available']}\n\n
                in orders: {true_balance['in_orders']}\n\n
