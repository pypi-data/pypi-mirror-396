...
def get_all_args(req):
    all_args = {}
    if req is not None:
        # if extract_request_data returns {'json': {...}}, merge it
        result = extract_request_data(req) or {}
        all_args.update(result.get('json', {}) or {})
        all_args.update(req.view_args or {})
        all_args.update(req.args or {})                    # query
        all_args.update(req.form or {})                    # ← include form
        all_args.update(req.get_json(silent=True) or {})   # json again (idempotent)
    return all_args
