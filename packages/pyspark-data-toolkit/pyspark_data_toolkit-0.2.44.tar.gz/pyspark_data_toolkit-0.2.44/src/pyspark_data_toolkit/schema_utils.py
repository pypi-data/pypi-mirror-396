...
def align_dataframe_to_table_schema(
    ...
    Args:
        ...
        table_name: Nome da tabela (ex: 'hive_metastore.bronze.cms' ou 'catalog.schema.table')
    ...
    # 3. Alinhar DataFrame ao schema final - SEM RECONSTRUIR DESNECESSARIAMENTE
    ...
    for field in final_schema.fields:
        ...
        if col_name not in df_aligned.columns:
            ...
            # Campo existe - verificar se precisa transformação
            source_field = df_source.schema[col_name]
            
            # SÓ reconstruir se os schemas forem DIFERENTES
            if not schemas_are_compatible(source_field.dataType, field.dataType):
                # print(f"Reconstruindo campo: {col_name}")
                ...
                if isinstance(field.dataType, StructType):
                    ...
                    df_aligned = df_aligned.withColumn(
                        col_name,
                        F.col(col_name).cast(field.dataType)
                    )
...
def schemas_are_compatible(source_type, target_type) -> bool:
    """
    Verifica se dois tipos de dados são compatíveis (não precisam reconstrução).
    Retorna True se são iguais ou compatíveis.
    """
    # Tipos primitivos idênticos
    if source_type == target_type:
        return True
    
    # Structs - verificar recursivamente
    if isinstance(source_type, StructType) and isinstance(target_type, StructType):
        # Verificar se todos os campos do target existem no source com mesmo tipo
        source_fields = {f.name: f.dataType for f in source_type.fields}
        
        for target_field in target_type.fields:
            if target_field.name not in source_fields:
                # Campo novo no target - precisa reconstruir
                return False
            
            if not schemas_are_compatible(source_fields[target_field.name], target_field.dataType):
                # Campo com tipo diferente - precisa reconstruir
                return False
        
        # Todos os campos do target existem no source - compatível
        # (source pode ter campos extras, mas tudo bem)
        return True
    
    # Arrays - verificar elemento type
    if isinstance(source_type, ArrayType) and isinstance(target_type, ArrayType):
        return schemas_are_compatible(source_type.elementType, target_type.elementType)
    
    # Tipos diferentes - não compatível
    return False
    ...
    for field in target_schema.fields:
        ...
        if struct_has_field(full_schema, full_path):
            ...
            # Array<Struct> - NUNCA usar cast, sempre transform
                ...
                # Debug
                # print(f"\n=== DEBUG: {full_path} ===")
                # print(f"Target espera: {field.dataType}")
                # print(f"Source tem: {source_type}")
                # print(f"Source é Array? {isinstance(source_type, ArrayType)}")
                # if isinstance(source_type, ArrayType):
                    # print(f"Source element: {source_type.elementType}")
                    # print(f"Target element: {field.dataType.elementType}")
...
def get_field_schema(schema: StructType, path: str):
    ...
    Retorna o tipo COMPLETO do campo final (incluindo Arrays).
    """
    parts = path.split(".")
    current = schema

    for i, part in enumerate(parts):
        # Desembrulhar arrays APENAS para navegação (não no campo final)
    ...
    # Retornar o tipo final SEM desembrulhar
    # Se for Array<Struct>, retorna Array<Struct>, não Struct!
    return current
