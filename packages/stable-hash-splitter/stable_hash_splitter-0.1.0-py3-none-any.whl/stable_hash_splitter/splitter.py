...
class StableHashSplit(BaseCrossValidator):
    ...
    def __init__(self, test_size=0.2, id_column='id', hash_func=None, random_state=None):
        ...
        # keep max for compatibility but prefer normalized comparison
        self._max_hash = 2**32

    def _get_identifiers(self, X):
        """Extract the identifier series from X (DataFrame, array, or Series)."""
        if isinstance(X, pd.DataFrame):
            # If id_column is None, use the DataFrame index
    ...
    def _in_test_set(self, identifier):
        ...
        hash_val = int(self.hash_func(identifier)) & 0xffffffff
        # Normalize into [0, 1) using 2**32 and compare against `test_size`.
        return (hash_val / float(2**32)) < float(self.test_size)
    ...
    def split(self, X, y=None, groups=None):
        ...
        # Validate lengths if y is provided
        if y is not None:
            check_consistent_length(ids, y)

        # Create boolean mask for the test set. Try to vectorize via numpy for speed.
        if isinstance(ids, pd.Series) or isinstance(ids, pd.Index):
            id_values = ids.values
        else:
            id_values = np.asarray(ids)

        try:
            # Use numpy.fromiter to build hash array efficiently
            hashes = np.fromiter((int(self.hash_func(v)) & 0xffffffff for v in id_values), dtype=np.uint64, count=len(id_values))
            test_mask = (hashes / float(2**32)) < float(self.test_size)
        except Exception:
            # Fallback to Python-level loop
            test_mask = np.array([self._in_test_set(v) for v in id_values], dtype=bool)
    ...
    def train_test_split(self, X, y=None, **kwargs):
        ...
        """
        # This convenience wrapper is deterministic and ignores shuffle/random_state.
        check_consistent_length(X, y)
        train_idx, test_idx = next(self.split(X, y=y))
