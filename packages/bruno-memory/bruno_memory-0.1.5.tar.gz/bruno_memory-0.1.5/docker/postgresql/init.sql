-- PostgreSQL Database Initialization for bruno-memory
-- This script is automatically executed when the container first starts
-- 
-- Purpose:
--   - Enable required PostgreSQL extensions
--   - Create the bruno-memory database schema
--   - Set up tables, indexes, and constraints
--   - Configure triggers for automatic timestamp updates
--
-- Note: This script is run by the postgres user during container initialization
...
-- Memory entries table with JSON metadata and vector support
CREATE TABLE IF NOT EXISTS memory_entries (
    ...
    embedding vector(1536)  -- pgvector support for embeddings
);
...
DO $$
BEGIN
    RAISE NOTICE 'bruno-memory database schema initialized successfully';
