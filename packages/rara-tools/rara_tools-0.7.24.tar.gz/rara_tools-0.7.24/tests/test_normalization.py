...
def test_add_nationality():
    ...
    viaf_record = normalizer._get_viaf_record(record, False, viaf_identifier="123zsxcasd")
...
def test_classified_fields_added_to_linked_record():
    ...
    classified_fields = [
        [{"670": {"ind1": " ", "ind2": "0", "subfields": [{"a": "Päikesekiri, 2021"}]}}]
    ]
...
def test_existing_record_enriched_by_elastic_identifier():
    ...
    original_record = next(iter(JSONReader(
        json.dumps([original_record_json], ensure_ascii=False)
    )))
