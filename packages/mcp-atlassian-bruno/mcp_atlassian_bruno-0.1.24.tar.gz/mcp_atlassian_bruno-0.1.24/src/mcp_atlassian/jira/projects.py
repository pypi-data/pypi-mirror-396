...
from ..exceptions import FortiGateAuthenticationRequiredError
...
class ProjectsMixin(JiraClient, SearchOperationsProto):
    ...
    def get_all_projects(self, include_archived: bool = False) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # Check if response contains FortiGate error information
            if (
                isinstance(projects, dict)
                and projects.get("error") == "fortinet_auth_required"
            ):
                raise FortiGateAuthenticationRequiredError(
                    auth_url=projects.get("auth_url"),
                    message=projects.get("message"),
                )

            return projects if isinstance(projects, list) else []

        except FortiGateAuthenticationRequiredError:
            # Re-raise FortiGate errors so they can be handled at tool level
            raise
        except Exception as e:
            # Check if this is a FortiGate error in the exception message
            error_str = str(e)
            if "fortinet_auth_required" in error_str or "fgtauth" in error_str:
                # Extract auth_url if available
                auth_url = None
                if "auth_url" in error_str:
                    import re

                    match = re.search(r'"auth_url":\s*"([^"]*)"', error_str)
                    if match:
