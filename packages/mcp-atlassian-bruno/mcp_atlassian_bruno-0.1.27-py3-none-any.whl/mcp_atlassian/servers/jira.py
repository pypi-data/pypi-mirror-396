...
from mcp_atlassian.exceptions import (
    FortiGateAuthenticationRequiredError,
...
def safe_tool_wrapper(func: Callable) -> Callable:
    """
    Wrapper that ensures all tools return valid JSON even if they error.

    This prevents tools from returning empty lists [] when unexpected errors occur.
    Instead, always returns a JSON object with error details.
    """
...
def handle_fortinet_error(error: Exception) -> str | None:
    """
    Check if an exception is a FortiGate authentication error and return JSON response.

    If the error is a FortiGateAuthenticationRequiredError, returns a JSON string
    with the authentication URL. Otherwise, returns None.

    Args:
        error: The exception to check

    Returns:
        JSON string with FortiGate auth info if applicable, None otherwise
    """
    if isinstance(error, FortiGateAuthenticationRequiredError):
        logger.warning(f"FortiGate authentication required: {error.auth_url}")
...
async def get_user_profile(
    ...
    try:
        ...
        # Check for FortiGate authentication error
        fortinet_response = handle_fortinet_error(e)
        if fortinet_response:
            return fortinet_response
...
# ============================================================================
# FortiGate Authentication Tools
# ============================================================================


@jira_mcp.tool(tags={"fortinet", "admin"})
async def set_fortinet_cookie(
    cookie_value: Annotated[
        str,
        Field(
            description="The FortiGate authentication token/cookie value extracted from the authentication URL"
        ),
    ],
) -> str:
    """
    Set the FortiGate authentication cookie for automatic injection in all requests.

    When Fortinet FortiGate SSL inspection is detected, the system attempts to
    automatically extract the authentication token from the FortiGate URL.
    If automatic extraction is successful, subsequent requests will work automatically.

    If automatic extraction fails or you need to manually set the cookie, use this tool.

    The cookie value can be obtained by:
    1. Looking at the FortiGate authentication URL in the error message
    2. The URL typically looks like: https://192.168.11.118/fgtauth?<TOKEN>
    3. Extract the <TOKEN> part (e.g., "070e0b8d9bf6c39d")
    4. Call this tool with that token value

    Args:
        cookie_value: The FortiGate authentication token (the part after "fgtauth?")

    Returns:
        JSON string with success status and next steps
    """
    from mcp_atlassian import utils

    try:
        utils.fortinet.set_fortinet_cookie(cookie_value)
        return json.dumps(
            {
                "success": True,
                "message": "FortiGate authentication cookie set successfully",
                "cookie_length": len(cookie_value),
                "next_step": "Retry your original Jira operation - it will now work with automatic cookie injection",
            },
            indent=2,
        )
    except Exception as e:
        logger.error(f"Failed to set FortiGate cookie: {str(e)}")
        return json.dumps(
            {
                "success": False,
                "error": str(e),
                "message": "Failed to set FortiGate cookie",
            },
            indent=2,
        )
...
async def clear_fortinet_cookie() -> str:
    ...
    try:
        utils.fortinet.clear_fortinet_cookie()
        return json.dumps(
            {
                "success": True,
                "message": "FortiGate authentication cookie cleared successfully",
                "next_step": "If you need to re-authenticate, the next Jira operation will prompt you again",
            },
            indent=2,
        )
    except Exception as e:
        logger.error(f"Failed to clear FortiGate cookie: {str(e)}")
        return json.dumps(
            {
                "success": False,
                "error": str(e),
                "message": "Failed to clear FortiGate cookie",
            },
            indent=2,
        )
...
@jira_mcp.tool(tags={"fortinet", "admin"})
async def auto_authenticate_fortigate(
    auth_url: Annotated[
        str,
        Field(
            description="The FortiGate authentication URL (e.g., https://192.168.11.118/fgtauth?<token>)"
        ),
    ],
    username: Annotated[
        str | None,
        Field(
            description="FortiGate username. If not provided, uses FORTIGATE_USERNAME env var. Required if env var not set.",
            default=None,
        ),
    ] = None,
    password: Annotated[
        str | None,
        Field(
            description="FortiGate password. If not provided, uses FORTIGATE_PASSWORD env var. Required if env var not set.",
            default=None,
        ),
    ] = None,
) -> str:
    """
    Automatically authenticate to FortiGate and extract the authentication cookie.

    This tool attempts to automate the authentication process by:
    1. Making an HTTP request to the FortiGate authentication portal
    2. Submitting your credentials
    3. Extracting the FGTAUTH cookie
    4. Automatically storing it for use in Jira API requests

    This approach works without requiring browser access and can be used in
    Docker containers or headless environments.

    Args:
        auth_url: The FortiGate auth URL from the fortinet_auth_required error
        username: Your FortiGate username (optional if FORTIGATE_USERNAME env var is set)
        password: Your FortiGate password (optional if FORTIGATE_PASSWORD env var is set)

    Returns:
        JSON string with success status and next steps

    Environment Variables:
        FORTIGATE_USERNAME: Default username if not provided as parameter
        FORTIGATE_PASSWORD: Default password if not provided as parameter

    Note:
        - If 2FA/MFA is enabled, this may not work (fall back to manual extraction)
        - The cookie will be automatically stored and used for all subsequent requests
        - Your credentials are NOT stored, only used for this one authentication attempt
    """
    import os
    ...
    try:
        ...
        # Step 0: Get credentials - try environment variables first, then parameters
        final_username = os.getenv("FORTIGATE_USERNAME") or username
        final_password = os.getenv("FORTIGATE_PASSWORD") or password

        # If no credentials available, ask user to provide them
            ...
            return json.dumps(
                {
                    ...
                    "message": f"FortiGate {' and '.join(missing_creds)} required for authentication",
                    "please_provide": missing_creds,
                    "next_steps": [
                        "Call the tool again with the missing parameters:",
                        f"auto_authenticate_fortigate(auth_url='{auth_url}', username='<your_username>', password='<your_password>')",
                        "",
                        "OR set environment variables and try again:",
        ...
        logger.info(
            f"Credentials found, attempting to authenticate to FortiGate: {auth_url}"
        )

        # Create a session to maintain cookies and state
        session = requests.Session()
        session.verify = False  # FortiGate often uses self-signed certs

        # Step 1: GET the auth page to extract form details and CSRF tokens
        logger.info(f"Attempting to authenticate to FortiGate: {auth_url}")
        response = session.get(auth_url, timeout=10)
        response.raise_for_status()

        # Step 2: Parse the authentication form to find required fields
        ...
        if not form:
            logger.error("No login form found in FortiGate response")
            return json.dumps(
                {
                    "success": False,
                    "error": "Form not found",
                    "message": "Could not find authentication form in FortiGate page",
                    "suggestion": "Use inspect_fortigate_structure(auth_url) to debug the page structure",
                },
                indent=2,
            )

        # Extract all hidden fields from the form (magic, redirect, etc.)
        form_data = {
            "username": final_username,
            "password": final_password,
        }

        # Find all input fields in the form and extract hidden values
        if isinstance(form, Tag):
            for inp in form.find_all("input"):
                if isinstance(inp, Tag):
                    ...
                    # Add all hidden fields (magic token, redirect, etc.)
                    if field_type == "hidden" and name:
                        form_data[name] = value
                        logger.debug(f"Added hidden field: {name}")

            logger.debug(f"Form data prepared: {list(form_data.keys())}")

            # Step 3: Determine the form action and method
            form_action_raw = form.get("action")
            form_action = form_action_raw if isinstance(form_action_raw, str) else "/"
            if not form_action.startswith("http"):
                # Make absolute URL
                parsed_url = urlparse(auth_url)
                form_action = f"{parsed_url.scheme}://{parsed_url.netloc}{form_action}"

            form_method_raw = form.get("method", "POST")
            form_method = (
                form_method_raw.upper() if isinstance(form_method_raw, str) else "POST"
            )
            logger.debug(f"Form: {form_method} {form_action}")
        else:
            return json.dumps(
                {
                    "success": False,
                    "error": "Form parsing failed",
                    "message": "Form found but could not be parsed properly",
                },
                indent=2,
            )
        ...
        try:
            logger.debug(f"Submitting login form to {form_action}")
            if form_method == "POST":
            ...
            # Check if we got the FGTAUTH cookie
            fgtauth_cookie = session.cookies.get("FGTAUTH")
            if fgtauth_cookie:
                login_success = True
                logger.info("Successfully authenticated! Got FGTAUTH cookie")
        except Exception as e:
            logger.debug(f"Login attempt failed: {str(e)}")

        # Step 5: Store the cookie if we got it
        if login_success and fgtauth_cookie:
            from mcp_atlassian import utils

            utils.fortinet.set_fortinet_cookie(fgtauth_cookie)
            return json.dumps(
                {
                    "success": True,
                    "message": "FortiGate authentication successful!",
                    "cookie_extracted": True,
                    "cookie_length": len(fgtauth_cookie),
                    "next_step": "Retry your original Jira operation - the authentication cookie is now stored and will be automatically injected in all requests",
                },
                indent=2,
            )
        else:
            return json.dumps(
                {
                    "success": False,
                    "message": "Could not extract FGTAUTH cookie from FortiGate",
                    "possible_reasons": [
                        "2FA/MFA might be enabled - requires manual authentication",
                        "Incorrect username or password",
                        "FortiGate version might use different form structure",
                        "Custom FortiGate configuration",
                    ],
                    "next_steps": [
                        "1. Verify your FortiGate credentials are correct",
                        "2. Check if 2FA/MFA is enabled (auto-auth may not work with 2FA)",
                        "3. Use inspect_fortigate_structure(auth_url) to debug the page structure",
                        "4. If auto-auth still fails, use manual extraction:",
                        f"   - Visit: {auth_url}",
                        "   - Extract FGTAUTH cookie from browser DevTools",
                        "   - Call: set_fortinet_cookie(cookie_value='<extracted_value>')",
                    ],
                    "fallback": "Use manual extraction: Visit auth_url in browser → DevTools → Application → Cookies → Copy FGTAUTH value → Call set_fortinet_cookie()",
                },
                indent=2,
            )

    except requests.exceptions.Timeout:
        logger.error("Timeout connecting to FortiGate")
        return json.dumps(
            {
                "success": False,
                "error": "Connection timeout to FortiGate",
                "message": "FortiGate server did not respond within 10 seconds",
                "auth_url": auth_url,
            },
            indent=2,
        )
        ...
        return json.dumps(
            {
                ...
                "message": f"Could not connect to FortiGate: {str(e)}",
                "auth_url": auth_url,
            },
            indent=2,
        )
        ...
        logger.error(
            f"Error during FortiGate authentication: {error_msg}", exc_info=True
        )
        return json.dumps(
            {
                "success": False,
...
@jira_mcp.tool(tags={"fortinet", "debug"})
async def inspect_fortigate_structure(auth_url: str) -> str:
    """Inspect the FortiGate authentication page structure.

    This tool helps debug FortiGate authentication by retrieving and
    parsing the HTML structure of the auth portal.

    Args:
        auth_url: The FortiGate authentication URL
        (e.g., https://192.168.11.118/fgtauth?070e0b8d9bf6c39d)

    Returns:
        JSON string containing the parsed form structure.
    """
    logger.info(f"Inspecting FortiGate structure at {auth_url}")

    try:
        response = requests.get(
            auth_url,
            verify=False,  # noqa: S501 (intentional for FortiGate MITM scenario)
            timeout=10,
            headers={
                "User-Agent": (
        ...
        if isinstance(form, Tag):
            ...
            form_method = method.upper() if isinstance(method, str) else "POST"

        form_data = {
            "form_found": form is not None,
            "form_action": form.get("action") if isinstance(form, Tag) else None,
            "form_method": form_method,
            "form_id": form.get("id") if isinstance(form, Tag) else None,
            "form_class": form.get("class") if isinstance(form, Tag) else None,
        }
        ...
        if isinstance(form, Tag):
            ...
            # Fallback: look for inputs outside form tags
        ...
        if isinstance(form, Tag):
            ...
            # Fallback: look for buttons outside form
        ...
        # Find meta tags (CSRF tokens, etc.)
        meta_tags = {}
        ...
        # Find scripts (may contain tokens)
        scripts = soup.find_all("script")
        script_content = []
        for script in scripts:
            if isinstance(script, Tag) and script.string:
                script_content.append(str(script.string)[:500])  # First 500 chars
        ...
        result = {
            ...
            "script_samples": script_content[:2] if script_content else [],
            "page_title": (
                soup.title.string
                if soup.title and isinstance(soup.title, Tag)
                else None
            ),
            "form_attributes": (
                dict(form.attrs.items()) if isinstance(form, Tag) else None
            ),
            "html_length": len(response.text),
            "html_snippet": response.text[:2000],
            "message": "FortiGate form structure successfully parsed",
        }

        logger.info("Successfully inspected FortiGate structure")
        ...
        error_msg = f"{type(e).__name__}: {str(e)}"
        logger.error(
            f"Error during FortiGate structure inspection: {error_msg}",
            exc_info=True,
        )
        return json.dumps(
            {
                "success": False,
                "error": "Structure inspection failed",
                "error_type": type(e).__name__,
                "message": str(e),
                "auth_url": auth_url,
                "debug_info": error_msg,
