...
async def auto_authenticate_fortigate(
    ...
    username: Annotated[
        str,
        Field(description="FortiGate username for authentication"),
    ],
    password: Annotated[
        str,
        Field(description="FortiGate password for authentication"),
    ],
) -> str:
    """
    ...
    try:
        ...
        # Common field names in FortiGate login forms
        form_data = {
            "username": username,
            "password": password,
        }

        # Look for CSRF token (common name patterns)
        csrf_patterns = [
            r'name=["\']csrf["\'][\s\w="\']*value=["\']([^"\']+)["\']',
            r'name=["\']token["\'][\s\w="\']*value=["\']([^"\']+)["\']',
            r'value=["\']([a-f0-9]{32,})["\']',  # Look for hex token-like values
        ]
        ...
        for pattern in csrf_patterns:
            ...
            if match:
                ...
                logger.debug("Found CSRF token in form")
                break

        # Look for nonce or other security tokens
        nonce_match = re.search(
            r'name=["\']nonce["\'][\s\w="\']*value=["\']([^"\']+)["\']',
            response.text,
            re.IGNORECASE,
        )
        if nonce_match:
            form_data["nonce"] = nonce_match.group(1)

        # Step 3: Determine the login endpoint
        # Common endpoints for FortiGate
        ...
        for endpoint in login_endpoints:
            try:
                logger.debug(f"Attempting login to {endpoint}")
                ...
                if fgtauth_cookie:
                    login_success = True
                    logger.info(
                        f"Successfully authenticated! Got FGTAUTH cookie from {endpoint}"
                    )
                    break
            except Exception as e:
                logger.debug(f"Login to {endpoint} failed: {str(e)}")
                continue

        # Step 4: If we got the cookie, store it
