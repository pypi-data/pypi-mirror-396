...
from mcp_atlassian.utils.fortinet import install_fortinet_hook
...
from mcp_atlassian.utils.ssl import (
    ...
    get_verify_ssl_for_atlassian_client,
)
    ...
    def __init__(self, config: JiraConfig | None = None) -> None:
        ...
        if self.config.auth_type == "oauth":
            ...
            self.jira = Jira(
                ...
                verify_ssl=get_verify_ssl_for_atlassian_client(
                    self.config.ssl_verify, self.config.cert_path
                ),
            )
        ...
        # Install FortiGate detection hook for on-premise deployments
        # This detects when FortiGate SSL inspection redirects to /fgtauth
        install_fortinet_hook(self.jira._session)
