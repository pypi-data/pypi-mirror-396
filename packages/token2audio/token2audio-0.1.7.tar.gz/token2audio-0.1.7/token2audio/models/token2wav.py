...
class Token2wav:
    ...
    def encode_wav(wav, sr):
        ...
        # 方案1：使用 StreamWriter（推荐）
        try:
            with io.BytesIO() as wavio:
                writer = torchaudio.io.StreamWriter(
                    dst=wavio, format="wav", sample_rate=sr, num_channels=wav.shape[0]
                )
                writer.add_audio_stream(
                    sample_rate=sr,
                    num_channels=wav.shape[0],
                    encoder="pcm_s16le",  # WAV 格式的 PCM 编码
                )
                writer.open()
                writer.write_audio_chunk(0, wav)
                writer.close()
    ...
    def _prepare_prompt_from_audio(self, prompt_audio, prompt_audio_sr):
        ...
        # 保存为临时文件
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_file:
            torchaudio.save(tmp_file.name, prompt_audio, prompt_audio_sr)
