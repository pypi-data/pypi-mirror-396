...
from analyst_klondike.state.app_state import INITIAL_STATE, AppState, get_state, select
...
class MessageBoxScreen(ModalScreen[bool]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="dialog"):
            with VerticalScroll():
                yield Static(msg_text, id="message_text")
    ...
    def watch_state(self, state: AppState) -> None:
        if state.last_action_type == DisplayMessageBoxAction.type:
            ...
            message_text.content = state.message_box.message
