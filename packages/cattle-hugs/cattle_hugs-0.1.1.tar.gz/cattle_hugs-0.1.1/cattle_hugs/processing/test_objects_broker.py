...
@pytest.fixture
def like(actor_for_test, remote_actor_id):
    return {
        "actor": actor_for_test.actor_id,
        "data": {
            "parsed": {
                "activity": {
                    "id": "http://remote.test/like",
                    "type": "Like",
                    "actor": remote_actor_id,
                    "to": [actor_for_test.actor_id, default_public],
                    "object": "http://local.test/object/id",
                }
            }
        },
    }
...
async def test_like(test_broker, like, sql_session):
    base_object = {"id": "http://local.test/object/id"}
    await register_base_object(sql_session, base_object)
    await sql_session.commit()
    ...
    result = await sql_session.scalar(select(Interaction))

    assert result


async def test_like_processed_twice(test_broker, like, sql_session):
    base_object = {"id": "http://local.test/object/id"}
    await register_base_object(sql_session, base_object)
    await sql_session.commit()
