...
class WarehouseExportOperations:
    ...
    def _collect_agent_settings(self, export_data: dict) -> None:
        ...
        try:
            ...
            # If agent settings endpoint fails (e.g., 500 error), log warning and continue with empty list
            status_code = "unknown"
            if isinstance(e, CRFAPIError) and hasattr(e, "response") and e.response is not None:
                status_code = e.response.status_code
            elif isinstance(e, requests.RequestException) and hasattr(e, "response") and e.response is not None:
                status_code = e.response.status_code
