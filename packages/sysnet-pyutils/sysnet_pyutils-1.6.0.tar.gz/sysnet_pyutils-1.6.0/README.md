...
Nový singleton **SyslogFactory**. Slouží pro odesílání logů na server **syslog**. 

### Verze 1.6.0

Nový balíček **cron_utils**. Slouží pro syntaktickou a sémantickou validaci výrazů cron. Vyžaduje knihovnu **crontier**. 
...
----------------------------------------------------------------
#### summarize_cron(expr: str, tz: str = "Europe/Prague") -> CronSummary:
...
----------------------------------------------------------------
#### has_next_occurrence(expr: str, start: datetime | None = None, horizon_years: int = 10) -> bool:
Sémantická validace
...
----------------------------------------------------------------
#### validate_cron(expr: str, horizon_years: int = 10) -> tuple[bool, list[str]]:
