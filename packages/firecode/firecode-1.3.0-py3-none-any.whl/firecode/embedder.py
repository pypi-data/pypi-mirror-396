...
from firecode.errors import (InputError, NoOrbitalError,
                             ZeroCandidatesError)
...
#            '''
    ...
    def _set_custom_orbs(self, orb_string):
        ...
        for mol_id, pairings in self.pairings_dict.items():
            for letter, (i1, i2) in pairings.items():
                if letter.isupper() and letter not in self.pairing_dists:
                    coords = self.objects[mol_id].atomcoords[0]
    ...
    def _calculator_setup(self):
        ...
        if self.options.calculator == 'AIMNET2':
            if self.options.mult != 1:
                self.log(f'--> Open shell detected: overriding theory level to use the NSE model for AIMNET2')
...
class RunEmbedding(Embedder):
    ...
    def saddle_refining(self):
        ...
        '''
        raise NotImplementedError('Removed Sella from dependencies - need to reimplement!')
