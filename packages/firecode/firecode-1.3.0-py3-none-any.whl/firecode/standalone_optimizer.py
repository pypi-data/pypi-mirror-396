    ...
    for i, name in enumerate(filenames):
        try:
            ...
            for c_n, coords in enumerate(data.atomcoords):
                ...
                if optimizer.calc in ('AIMNET2', 'UMA') and optimizer.solvent != None:
                    post = f'+ALPB({optimizer.solvent})'
                else:
                    post = ''

                print(f'{action} {name} - {i+1} of {len(filenames)}, conf {c_n+1} of {len(data.atomcoords)} ({optimizer.method}/{optimizer.calc}{post}) - CHG={charge} MULT={mult}')
                ...
                if optimizer.free_energy:
                    ...
                    print(f'Performing vibrational analysis on {name} - {i+1} of {len(filenames)}, conf {c_n+1} of {len(data.atomcoords)} ({optimizer.method})')
                    t_start = perf_counter()
