...
from firecode.graph_manipulations import d_min_bond
from firecode.utils import (graphize, loadbar, molecule_check, read_xyz,
...
def explode_search(filename, embedder, n=10, iterations=3):
    ...
    if len(embedder.objects) > 1 or len(embedder.objects[0].atomcoords) > 1:
    ...
    for bond in bonds:
        i1, i2 = bond
        e1, e2 = mol.atomnos[i1], mol.atomnos[i2]
        d_eq = d_min_bond(e1, e2, factor=1.0)
        d_max = 1.1 * d_eq

        hs = HalfSpring(i1, i2, d_max=d_max, d_eq=d_eq)
        bond_constraints.append(hs)

    nbp = NewBondPreventer(mol.atomnos, mol.atomcoords[0], graph.edges)
    ...
    for iteration in range(iterations):
        ...
        if True:
            with open(f'firecode_debug_explode_search_{iteration+1}.xyz', 'w') as f:
                write_xyz(base_coords, mol.atomnos, f, title='original')
                for s, h in zip(scrambledcoords, sc_info):
                    write_xyz(s, mol.atomnos, f, title=f'heat = {h:2f}')

        cum_time = 0
        processes, output_structures, output_energies = [], [], []
...
if __name__ == '__main__':
    ...
    scrambled, heats = _get_scrambled_coords(mol.atomcoords[0], n=20)

    with open('exploded_structs.xyz', 'w') as f:
        write_xyz(mol.atomcoords[0], mol.atomnos, f, title='original')
