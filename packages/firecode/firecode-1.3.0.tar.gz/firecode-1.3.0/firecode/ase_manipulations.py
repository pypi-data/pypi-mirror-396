...
from firecode.graph_manipulations import (d_min_bond, findPaths, graphize,
                                          neighbors)
from firecode.rmsd import get_alignment_matrix, rmsd_and_max_numba
from firecode.units import EH_TO_EV, EH_TO_KCAL, EV_TO_KCAL
from firecode.utils import (HiddenPrints, align_structures, cartesian_product,
                            clean_directory, get_double_bonds_indices,
                            molecule_check, read_xyz, scramble_check,
...
class NewBondPreventer:
    ...
    def __init__(self, atomnos, ref_coords, bonds):
    ...
    def adjust_forces(self, atoms, forces):
        ...
        for pair, dist in zip(self.nonbound_pairs, self.nonbound_dists):
            ...
            forces[i1] -= (norm(direction) * spring_force)
            forces[i2] += (norm(direction) * spring_force)
            # applying harmonic force to each atom, directed toward the other one
...
def ase_neb(
    ...
    if mep_input is not None:
        ...
        if len(mep_input) == n_images:
            images = [Atoms(atomnos, positions=coords) for coords in mep_input]
            if logfunction is not None:
            ...
            images = [Atoms(atomnos, positions=mep_input[i]) for i in picked_indices]
...
def most_different_structures_ids(structures, n):
    ...
    while True:
        ...
        for i1 in output[:-1]:
            ...
            results.append((rmsd_and_max_numba(structures[i1], structures[i2], center=True)[0], (i1, i2)))
