...
class LSTMGenerator(nn.Module):
    ...
    def forward(self, z, z_step, labels):
        ...
        init = torch.cat([z.unsqueeze(1) * c.unsqueeze(1)], dim=2).reshape(z.size(0), -1)  # [B, latent_dim*embed_dim]
        h0 = torch.tanh(self.fc(init)).unsqueeze(0)
        c0 = torch.zeros_like(h0, device=h0.device)

        # Repeat label embedding untuk setiap step
        c_rep = c.unsqueeze(1).repeat(1, self.seq_len, 1)
        step_input = torch.cat([z_step, c_rep], dim=2)  # [B, seq_len, latent_step_dim + embed_dim]
        lstm_input = self.step_proj(step_input.reshape(step_input.size(0), -1, self.latent_step_dim * self.embed_dim))
