...
def get_list_text_from_url(url_or_list):
    ...
    - Does NOT recurse infinite loops.
    """
...
def extract_identifiers_from_json(json_data, key='CID'):
    ...
    Args:
        json_data (dict): The JSON response dictionary from PubChem.
        key (str): The specific identifier to extract (e.g., 'CID', 'SID', 'AID').
                   Defaults to 'CID'.
                   
    Returns:
        list: A list of extracted identifiers, or an empty list if not found.
    """
    if not json_data:
        return []
    ...
    try:
        ...
        if info and isinstance(info, list):
            ...
            # We look for the key in the first item.
...
def _base_request(domain, identifiers, namespace, operation, default_fmt, override_fmt=None, **kwargs):
    ...
    # Define namespaces that contain complex, non-numeric characters
    structural_namespaces = ['smiles', 'inchi', 'sdf', 'formula', 'name'] 
    is_structural_input = any(ns in namespace for ns in structural_namespaces) or namespace in structural_namespaces

    # --- 2. Flatten Identifiers ---
    flat_list = []
    ...
    def flatten(args):
        ...
        if isinstance(args, (list, tuple)): 
            ...
            item_str = str(args) 
            # Encode the identifier based on namespace type
            if is_structural_input:
                flat_list.append(urllib.parse.quote(item_str))
            else:
    ...
    # List of namespaces that require single input when not looking up CIDs/SIDs directly
    STRING_INPUT_NAMESPACES = ['name', 'smiles', 'inchi', 'inchikey', 'formula']
    
    # --- 3. Batch Size Adjustment Logic ---
    current_batch_size = BATCH_SIZE
    
    # Condition 1: Gene/Pathway and Assay/Target lookup (known to fail on batches)
    if (domain == 'gene' and operation == 'pwaccs') or \
       (domain == 'assay' and operation == 'targets/geneid'):
        current_batch_size = 1
    
    # Condition 2: All Single-Input Operations (Gene Concise, Assay Record/DoseResponse, Classification, Image Retrieval, Taxonomy AIDs, and ALL CELL)
    ...
    (operation in ['PNG', 'record', 'classification'] and domain in ['compound', 'substance'] and out_fmt in ['PNG', 'PNG_3D']) or \
    ...
    (domain == 'cell'):
        current_batch_size = 1  # Force single input per request

    # Condition 3: Enforce single identifier for structural searches OR string namespaces
    elif is_structural_input or \
         (namespace in STRING_INPUT_NAMESPACES) or \
         (any(ns in namespace for ns in ['xref/'])): # Check for any XREF path
        current_batch_size = 1
    ...
    for batch in batches:
        # Check for namespaces that must be URL-encoded but cannot be batched
        if is_structural_input or namespace in STRING_INPUT_NAMESPACES or 'xref/' in namespace:
             batch_str = ",".join(batch)
             safe_item = batch_str 
        else:
             # Numeric IDs (CID, SID, AID) can still be batched up to BATCH_SIZE=50
             batch_str = ",".join(batch)
        ...
        time.sleep(THROTTLE_TIME) # Apply throttling at the URL generation level
...
def make_func(domain, ns, func_suffix, url_op, fmt):
    ...
    def dynamic_wrapper(*args, output_format=None, operation=url_op, **kwargs):
        ...
        if func_suffix:
            ...
            # Inject aids_type
            if func_suffix.endswith('_active'):
                local_kwargs['aids_type'] = 'active'
            elif func_suffix.endswith('_inactive'):
