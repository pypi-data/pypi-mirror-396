...
class FastCapture:
    ...
    def _cleanup_old_frames(self):
        """Remove old frame files, keep last 100 (enough for very slow LLM processing)."""
        try:
            ...
            if len(old_frames) > 100:
                for old_frame in old_frames[:-100]:
                    try:
                        old_frame.unlink()
            ...
            try:
                if latest_file.exists():
                    ...
                    if mtime > last_mtime:
                        ...
                        # Cleanup old frame copies (keep last 50 for slow processing)
                        all_frames = sorted(self.ramdisk_path.glob("frame_*.jpg"))
                        old_frames = [f for f in all_frames if "_opt" not in f.name]
                        if len(old_frames) > 50:
                            for old_frame in old_frames[:-50]:
                                try:
                                    old_frame.unlink()
                                    # Also remove optimized version if exists
                                    opt_frame = old_frame.with_name(old_frame.stem + "_opt.jpg")
