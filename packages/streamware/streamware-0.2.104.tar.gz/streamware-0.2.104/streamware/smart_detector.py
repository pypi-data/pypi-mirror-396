            ...
            try:
                ...
                # Use regular YOLO detector
                    ...
                    if detections:
                        ...
                        for d in detections:
                            det_id = f"{d.class_name}_{d.confidence:.2f}"
                            curr_x = d.x + d.w / 2
                            if det_id in prev_positions:
                                prev_x = prev_positions[det_id]
                                dx = curr_x - prev_x
                                if dx > 50:  # Moving right
                                    result.quick_summary = f"{d.class_name.title()} moving right"
                                elif dx < -50:  # Moving left
                                    result.quick_summary = f"{d.class_name.title()} moving left"
                            prev_positions[det_id] = curr_x
                        ...
                        if result.confidence >= yolo_skip_threshold:
                            ...
                            logger.debug(f"YOLO confident ({result.confidence:.2f} >= {yolo_skip_threshold}), skipping LLM, notify={result.should_notify}")
                            return result
                    else:
                        # YOLO found nothing
                        self._consecutive_no_target += 1
                        
                        # Notify when target disappears (diff mode support)
                        if self._prev_summary and "detected" in self._prev_summary.lower():
