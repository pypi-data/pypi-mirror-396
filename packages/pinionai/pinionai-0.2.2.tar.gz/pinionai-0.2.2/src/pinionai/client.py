_Bj="Twilio 'from' number not configured. Cannot send SMS."
_Bi='completedAt'
_Bh='Error: URL template evaluation failed.'
_Bg='contentType'
_Bf='token_type'
_Be='client_credentials'
_Bd='function_call'
_Bc='Error: AI client is not configured.'
_Bb='Cannot get Gemini response: AI client is not initialized.'
_Ba='Using environment credentials/IAM role for S3.'
_BZ='boto3 library is required for S3 operations.'
_BY='Using Application Default Credentials for GCS.'
_BX='google-cloud-storage library is required for GCS operations.'
_BW='action_action_key'
_BV='__builtins__'
_BU='destinationVariable'
_BT='sourceVariable'
_BS='promptName'
_BR='unknown_intent'
_BQ='Error: AI response was malformed (function call had no name).'
_BP='resp_mime_type'
_BO='gemini-3-pro-preview'
_BN='safetySettings'
_BM='mime_type'
_BL='variables'
_BK='accountId'
_BJ='Content-Encoding'
_BI='DEEPSEEK_API_KEY'
_BH='ANTHROPIC_API_KEY'
_BG='OPENAI_API_KEY'
_BF='EMAIL_FROM_ADDRESS'
_BE='EMAIL_API_KEY'
_BD='GEMINI_API_KEY'
_BC='GCP_REGION'
_BB='GCP_PROJECT'
_BA='WEBSOCKET_URL'
_B9='user_input'
_B8='Journey token not available.'
_B7='Accept-Encoding'
_B6='client_secret'
_B5='client_id'
_B4='grant_type'
...
_Ay='function'
_Ax='header'
_Aw='gemini-2.0-flash'
_Av='model_provider'
_Au='agentDefaultIntent'
_At='intentContainsPairsList'
_As='intentNameList'
_Ar='value'
_Aq='action_flow'
_Ap='action_type'
...
_Ag='privacy'
_Af='prompt_type'
_Ae='gcs_service_account'
_Ad='ragStoreVectorDistanceThreshold'
_Ac='ragStoreTopK'
_Ab='ragStoreResourceId'
_Aa='transferPasskeyFlag'
_AZ='transferAllowed'
_AY='access_token'
_AX='TWILIO_NUMBER'
...
class AsyncPinionAIClient:
	...
	def __init__(self,create_key,agent_id,host_url,client_id,client_secret=_A,version=_A,token=_A,session_id=_A,raw_session_data=_A,initial_engagement_data=_A,grpc_server_address=_A,gcp_project_id=_A,gcp_region=_A,gemini_api_key=_A,twilio_account_sid=_A,twilio_auth_token=_A,twilio_number=_A,email_api_key=_A,email_from_address=_A,openai_api_key=_A,anthropic_api_key=_A,deepseek_api_key=_A):
		...
		self._agent_id=agent_id;self._host_url=host_url.rstrip('/');self._client_id=client_id;self._client_secret=client_secret;self._version=version;self._grpc_server_address=grpc_server_address or os.environ.get(_BA,'localhost:50051');self._encryption_manager=EncryptionManager(client_secret=agent_id);self._gcp_project_id=gcp_project_id or os.environ.get(_BB);self._gcp_region=gcp_region or os.environ.get(_BC);self._gemini_api_key=gemini_api_key or os.environ.get(_BD);self._twilio_account_sid=twilio_account_sid or os.environ.get(_AV);self._twilio_auth_token=twilio_auth_token or os.environ.get(_AW);self._twilio_number=twilio_number or os.environ.get(_AX);self._email_api_key=email_api_key or os.environ.get(_BE);self._email_from_address=email_from_address or os.environ.get(_BF);self._openai_api_key=openai_api_key or os.environ.get(_BG);self._anthropic_api_key=anthropic_api_key or os.environ.get(_BH);self._deepseek_api_key=deepseek_api_key or os.environ.get(_BI);self._http_session=httpx.AsyncClient(base_url=self._host_url,timeout=12e1,follow_redirects=_D);self._token=token;self._session_id=session_id;self._raw_session_data=raw_session_data;self.var={};self.engagement=''
	...
	async def create_from_stream(cls,file_stream,host_url,key_secret=_A):
		...
		async with httpx.AsyncClient(base_url=host_url.rstrip('/'),timeout=12e1)as http_session:
			try:
				...
				compressed_data=gzip.compress(json.dumps(api_payload,default=_json_datetime_serializer).encode(_N));response=await http_session.post('/filesession',headers={_Q:_K,_BJ:_t},content=compressed_data);response.raise_for_status();resp_json=response.json()
		...
		decryption_key=resp_json.get(A);access_token=resp_json.get(_AY);version=resp_json.get('version_type')
		...
		agent_id=resp_json.get(_B,{}).get(_E,{}).get('agentId');session_id=resp_json.get(_B,{}).get(_E,{}).get('uid');client_id=resp_json.get(_B,{}).get(_E,{}).get(_BK)
	...
	def _configure_client_from_session_data(self):
		...
		if _B in self._raw_session_data and self._raw_session_data[_B]:
			...
			if self._rag_stores_by_name:first_store_cfg=next(iter(self._rag_stores_by_name.values()));self.var[_Ab]=first_store_cfg.get(_u,'');self.var[_Ac]=first_store_cfg.get(_v,10);self.var[_Ad]=first_store_cfg.get(_w,.5)
			else:self.var[_Ab]='';self.var[_Ac]=10;self.var[_Ad]=.5
			self._account_id=self._raw_session_data[_B][_E].get(_BK);self._customer_id=self._raw_session_data[_B][_E].get('customerId');self._agent_id_from_api=agent_data.get('agentId');self._agent_description=self._clean_text(agent_data.get(_L,''))
	...
	def _initialize_genai_client(self):
		...
		if self.var[_l]:
			...
			if conn_config and conn_config.get(_m)==_Ae:
	...
	async def process_user_input(self,user_input='',sender=_b):
		...
		if not self.transfer_requested:
			...
			if self.next_intent:self.var['prompt_resp_var']=self.next_intent;prompt_type=self.var[_Af]=_A9;user_input=self.next_intent;intent_data=self._get_intent_details(self.next_intent);self.next_intent=''
			else:
				...
				prompt_type=self.var.get(_Af);intent_data=self._get_intent_details(self.var[prompt_resp_var_name])
			self.var[_c]=''
			...
			intent_type=intent_data[_F];self.var[A]=intent_data[_C];self._privacy_level=self.var['privacy_resp']=intent_data[_Ag];self.current_pipeline=self.var['pipeline_resp']=intent_data[_x];delivery_method=intent_data[_Ah];language=intent_data[_d];intent_input_vars=self._normalize_intent_vars(intent_data.get(B,{}));context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_Ai,{})
			...
			if intent_type!=_y:
				if intent_input_vars:
					agent_vars_config=self._raw_session_data.get(_B,{}).get(_E,{}).get(_B,{}).get(_J,{}).get(_BL,[])
				...
				if self._class_intent_data:intent_data=self._class_intent_data;user_input=self._class_message;intent_type=intent_data[_F];self.var[A]=intent_data[_C];delivery_method=intent_data[_Ah];language=intent_data[_d];intent_input_vars=self._normalize_intent_vars(intent_data.get(B,{}));context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_Ai,{});self._privacy_level=intent_data[_Ag];self.current_pipeline=intent_data[_x]
			if intent_input_vars:
				...
				if waiting_for_inputs:
					...
					if final_response:self.chat_messages.append({_S:_Aj,_R:final_response})
					...
					if not self.authorized and'private'in self._privacy_level or J in self._privacy_level:
						...
						if intent_data[K]:
							...
							if customer_id and enrolled:
								pipeline_key=self.var[L]if J in self._privacy_level else self.var['authentication_pipeline'];json_payload={_Ak:pipeline_key,_o:{_f:delivery_method,_g:self.phone_number},_Al:{_Am:self.unique_id,_g:self.phone_number},_E:{_An:self._session_id},_d:language}
								if self.var.get(_p):json_payload[_Ao]=self.var[_p]
								...
								auth_response_msg=f"You are not enrolled. We are sending a link to {self.phone_number} so that you can enroll your device.";json_payload={_Ak:self.var['enroll_pipeline'],_o:{_f:delivery_method,_g:self.phone_number},_Al:{_Am:self.unique_id,_g:self.phone_number},_E:{_An:self._session_id},_d:language}
								if self.var.get(_p):json_payload[_Ao]=self.var[_p]
			...
			if not final_response:
				if intent_type=='fixed':fixed_message_template=self._clean_text(intent_data['fixedResponseMessage']);final_response=self._evaluate_f_string(fixed_message_template,self.var)
					...
					action_details_from_intent=self._map_to_standard_action_details(intent_data);action_type=action_details_from_intent.get(_Ap)
					if action_type=='journey':self.var,final_response=await self._run_journey_action(self.var,action_details_from_intent)
						...
						action_flow=action_details_from_intent.get(_Aq)
						if action_flow:
	...
	async def send_grpc_message(self,message_text):
		...
		try:recipient_id=_Aj if self.grpc_sender_id==_b else _b;request=ChatMessageRequest(thread_id=1,message=message_text,sender_id=self.grpc_sender_id,recipient_id=recipient_id,session_id=str(self._session_id));await self._grpc_stub.SendMessage(request);logger.info(f"gRPC message sent by {self.grpc_sender_id}: {message_text}")
	...
	def _extract_data_from_session(self,session_data_root):
		'Extracts variables and agent configuration into the var dictionary.';var_dict={};agent_config=session_data_root.get(_E,{}).get(_B,{}).get(_J,{});global_to_self_attr_map={_BA:'_grpc_server_address',_BB:'_gcp_project_id',_BC:'_gcp_region',_BD:'_gemini_api_key',_AV:'_twilio_account_sid',_AW:'_twilio_auth_token',_AX:'_twilio_number',_BE:'_email_api_key',_BF:'_email_from_address',_BG:'_openai_api_key',_BH:'_anthropic_api_key',_BI:'_deepseek_api_key'}
		for variable_def in agent_config.get(_BL,[]):
			name=variable_def[_C];value=variable_def[_Ar];var_type=variable_def.get(_F)
		...
		for intent in agent_config.get('intents',[]):
			...
			intent_privacy_pairs.append((intent_name,intent[_Ag]));intent_type_pairs.append((intent_name,intent[_F]));intent_contains_word_pairs.append((intent_name,intent.get('containsWord','')));intent_action_key_pairs_raw.append((intent_name,intent.get(_x,'')))
		var_dict[_As]=intent_names;var_dict['noInputIntentNameList']=no_input_intent_names;var_dict['inputIntentNameList']=input_intent_names;var_dict['intentPrivacyPairsList']=intent_privacy_pairs;var_dict['intentTypePairsList']=intent_type_pairs;var_dict[_At]=intent_contains_word_pairs;resolved_action_key_pairs=[]
		...
		var_dict['intentActionPairsList']=resolved_action_key_pairs;var_dict[_AD]=agent_config.get(_AD);var_dict[_Au]=agent_config.get('defaultIntent');return var_dict
	...
	async def _prompt_response(self,target_prompt,user_input=''):
		...
		self.var[_Af]=prompt_config[_F];prompt_resp_var_name=prompt_config[_AE];prompt_body_template=self._clean_text(prompt_config[_W]);current_prompt_text=self._evaluate_f_string(prompt_body_template,self.var,user_input=user_input);model_provider=prompt_config.get(_Av,_e).lower();model_name=prompt_config.get(_X);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',self.var)
		if model_provider==_e:
			...
			if prompt_config.get(_AF):
				...
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BM]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_AG],top_p=prompt_config[_Y],top_k=prompt_config[_AH],candidate_count=prompt_config[_AI],max_output_tokens=prompt_config[_AJ],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BN))
			if model_name in[_BO]and prompt_config.get(_i)is not _A:gen_config.thinking_config=google_genai_types.ThinkingConfig(thinking_level=prompt_config.get(_i))
			if prompt_config.get(_AK):gen_config.response_schema=prompt_config[_AK];gen_config.response_mime_type=prompt_config.get(_BP,_K)
		...
		if function_calls:
			...
			if not function_name:logger.error(f"LLM response contained a function call with no name: {function_call}");return self.var,_BQ
		...
		if llm_response_text:
			if prompt_config[_F]==_A9:
				...
				matched_intent_key=self._match_intent_from_text(raw_response,self.var.get(_At,[]),self.var.get(_Au,_BR),self.var.get(_As,[]));self.var[prompt_resp_var_name]=matched_intent_key
	...
	async def _async_prompt_response(self,target_prompt,user_input='',var_snapshot=_A):
		...
		if function_calls:
			function_call=function_calls[0];function_name=function_call.get(_C);function_args=function_call.get(_I,{})
			if not function_name:logger.error(f"LLM async response contained a function call with no name: {function_call}");return prompt_resp_var_name,_BQ
		...
		if llm_response_text:
			if prompt_config[_F]==_A9:
				...
				final_value_for_var=self._match_intent_from_text(raw_response,current_var.get(_At,[]),current_var.get(_Au,_BR),current_var.get(_As,[]))
	...
	def _get_mcp_details(self,mcp_name,current_var):
		...
		if mcp_config:
			mcp_config=self._evaluate_vars_in_structure(mcp_config,current_var);mcp_config[_L]=self._clean_text(mcp_config.get(_L,''));mcp_config[_Z]=self._evaluate_f_string(mcp_config.get(_Z,''),current_var);mcp_config[_A3]=self._clean_text(mcp_config.get(_A3,''));mcp_config['cwd']=self._clean_text(mcp_config.get('cwd',''))
	...
	async def _select_mcp_config(self,mcp_group_names,current_var,user_input,mcp_group_config):
		...
		select_mcp_declaration=FunctionDeclaration(name=B,description="Select the most appropriate MCP from the list based on the user's request.",parameters={_F:_r,_AM:{A:{_F:_AP,_L:'The name of the selected MCP.',_AO:[mcp.get(_C)for mcp in mcp_options]}},_AN:[A]});prompt_name=mcp_group_config.get(_BS,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
		if prompt_name:
			...
			prompt_model_provider=prompt_config.get('modelProvider',_e);prompt_model=_Aw
	...
	async def _run_mcp_action(self,current_var,mcp_name,user_input=_A):
		...
		try:
			...
			prompt_name=mcp_config.get(_BS,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
			if prompt_name:
				logger.info(f"MCP '{mcp_name}' is using prompt '{prompt_name}'.");prompt_config,_,_=self._get_prompt_details(prompt_name,current_var)
				if not prompt_config:error_msg=f"Error: Prompt '{prompt_name}' for MCP '{mcp_name}' not found.";logger.error(error_msg);return current_var,error_msg
			...
			mcp_transport=mcp_config.get(F,'');mcp_url=mcp_config.get(_Z,'');mcp_description=mcp_config.get(_L,'');mcp_command=mcp_config.get(_A3,'');mcp_args=mcp_config.get(_I,{});mcp_cwd=mcp_config.get('cwd','');mcp_args=mcp_config.get(_I,[]);mcp_params=mcp_config.get('parametersJson',{});mcp_env=mcp_config.get(_A4,{});mcp_name=mcp_config.get(_C,'default');required_inputs=[]
			if mcp_params:
			...
			if is_http_call:
				eval_mcp_url=mcp_url;http_headers=self._evaluate_vars_in_structure(mcp_config.get(_Ax,{}),current_var);mcp_connector=mcp_config.get(_U,'')
				if mcp_connector:
			...
			if session_context:
				async with session_context as session:
					...
					llm_tools=[]
					if mcp_tools:llm_tools.append(FunctionDeclaration(name=H,description='Calls a specific tool available on the MCP server.',parameters={_F:_r,_AM:{B:{_F:_AP,_AO:[t.name for t in mcp_tools],_L:'The name of the tool to call.'},A:{_F:_r,_L:'The arguments for the tool as a JSON object.'}},_AN:[B,A]}))
					if mcp_resources:llm_tools.append(FunctionDeclaration(name=I,description='Reads a specific resource (e.g., a file or data entry) from the MCP server.',parameters={_F:_r,_AM:{C:{_F:_AP,_AO:[r.uri for r in mcp_resources],_L:'The URI of the resource to read.'}},_AN:[C]}))
					if mcp_prompts:llm_tools.append(FunctionDeclaration(name=J,description='Gets a pre-defined, rendered prompt template from the MCP server.',parameters={_F:_r,_AM:{D:{_F:_AP,_AO:[p.name for p in mcp_prompts],_L:'The name of the prompt to get.'},E:{_F:_r,_L:'The arguments for rendering the prompt template.'}},_AN:[D]}))
					if not llm_tools:return current_var,f"Error: MCP '{mcp_name}' has no tools, resources, or prompts to use."
					logger.info(f"Asking LLM to choose a mcp activity: '{current_prompt_text[:100]}...'");model_provider=prompt_config.get(_Av,_e)if prompt_config else _e;model_to_use=_Aw
	...
	async def _run_agent_loop(self,prompt,client,session,model_id,max_tool_turns,initial_temperature,tool_call_temperature,mcp_description=_A):
		...
		while has_function_calls and turn_count<max_tool_turns:
			turn_count+=1;function_calls_to_execute=[part.function_call for part in latest_content.parts if part.function_call];tool_response_parts=await self._execute_tool_calls(function_calls_to_execute,session);contents.append(google_genai_types.Content(role=_Ay,parts=tool_response_parts));subsequent_config=google_genai_types.GenerateContentConfig(temperature=tool_call_temperature,**base_gen_config_dict);response=await client.aio.models.generate_content(model=model_id,contents=contents,config=subsequent_config)
	...
	async def _run_api_action(self,current_var,api_name):
		...
		headers={}
		if api_config.get(_Ax):
			try:headers.update(json.loads(api_config[_Ax]))
	...
	def _run_parser_action(self,current_var,parser_name):
		...
		source_var_name=parser_config.get(_BT)
	...
	def _run_merger_action(self,current_var,merger_name):
		...
		dest_var_name=merger_config.get(_BU)
	...
	async def _run_script_action(self,var,script_name):
		...
		try:
			if script_type=='javascript':
				...
				local_script_vars=var.copy();logger.warning('Executing Python scripts from the database is a security risk and is currently disabled.');exec(script_code,{_BV:{}},local_script_vars)
	...
	async def _evaluate_rule(self,expr,var):
		...
		try:
			...
			try:return eval(formatted_expr,{_BV:{}},{_z:var})
	...
	async def _execute_outcome(self,outcome_statement,outcome_type,user_input):
		...
		if outcome_type=='Set a variable'and outcome_statement.get(A):
			set_config=outcome_statement.get(A);variable_to_set=set_config.get('variable');value_to_set=set_config.get(_Ar)
			...
			auth_value=outcome_statement.get(_Ar)
	...
	async def _run_element_action(self,current_var,element_name):
		...
		if element_type=='date':generated_date=self._generate_date(element_config.get(_Az,'%Y-%m-%d'));variable_out=element_config.get(A);current_var[variable_out]=generated_date;return current_var,_A
		elif element_type=='datetime':generated_datetime=self._generate_datetime(element_config.get(_Az,'%Y-%m-%d %H:%M:%S'));variable_out=element_config.get(A);current_var[variable_out]=generated_datetime;return current_var,_A
	...
	async def _run_journey_action(self,current_var,action_details):
		"Handles a 'journey' type action, from either an intent or an action entity.";B='configuration';A='payment_description';action_key_var_name=action_details.get(_BW);action_key=current_var.get(action_key_var_name)if action_key_var_name else _A
		...
		delivery_method=action_details.get(_A_);language=action_details.get(_d);_,self.unique_id,self.phone_number=format_phone(self.phone_number);json_payload={_Ak:action_key,_o:{_f:delivery_method,_g:self.phone_number},_Al:{_Am:self.unique_id,_g:self.phone_number},_E:{_An:self._session_id},_d:language}
		if current_var.get(_p):json_payload[_Ao]=current_var[_p]
		if action_details.get(_B0)=='credit card payment'and current_var.get(A):json_payload[B]={'credit-card-payment':{'currency':current_var.get('payment_currency'),'lineItems':[{'title':current_var.get(A),'amount':current_var.get('payment_amount'),'quantity':current_var.get('payment_quantity')}]}}
		if action_details.get(_B0)=='one time passcode':json_payload[B]={'random-code':{'code':'6'}}
		if action_details.get(_A_)=='voice':json_payload[_o]={_f:delivery_method,_g:self.phone_number,'callOperator':'twilio'}
	...
	def _run_transfer_action(self,current_var,action_details):
		...
		if current_var.get(_AZ):
			...
			if current_var.get(_Aa):passkey=self._generate_password(6);current_var['transferPasskey']=passkey;sms_message=f"Your agent will validate themselves with the following shared passkey: {passkey}"
		...
		return current_var,final_response
	def _map_to_standard_action_details(self,data):'Maps fields from an intent or action entity to a standard action_details dictionary.';return{_C:data.get(_C),_Ap:data.get('actionType'),_BW:data.get(_x),_A_:data.get(_Ah),_d:data.get(_d),_B0:data.get('actionEvent'),_h:data.get('actionResponseMessage'),_Aq:data.get(_Ai,{})}
	async def _run_action_action(self,current_var,action_name,final_response_from_caller=_A,user_input=_A):
		...
		action_details=self._map_to_standard_action_details(action_entity_data);action_type=action_details.get(_Ap,'general')
		...
		action_flow=action_details.get(_Aq,{});action_produced_final_response=_A
		if action_flow:
	...
	async def _gcs_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not storage:raise ImportError(_BX)
		gcs_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_m)==_Ae:sa_info=json.loads(conn_config.get(_n,'{}'));gcs_client=storage.Client.from_service_account_info(sa_info);logger.info(f"Using GCS service account from connector '{connector_name}'.")
		if not gcs_client:gcs_client=storage.Client();logger.info(_BY)
	...
	async def _s3_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not boto3:raise ImportError(_BZ)
		s3_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_m)=='aws_iam':s3_client=boto3.client('s3',aws_access_key_id=conn_config.get(_B1),aws_secret_access_key=conn_config.get(_n));logger.info(f"Using AWS IAM credentials from connector '{connector_name}'.")
		if not s3_client:s3_client=boto3.client('s3');logger.info(_Ba)
	...
	async def _run_file_action(self,current_var,file_name):
		...
		try:
			...
			if is_write_op:
				...
				if file_format==_A7:content_bytes_to_write=json.dumps(content,indent=2).encode(_N)
	...
	def _configure_tools_for_request(self,direct_tool_configs,processed_fd_dicts,model_name):
		...
		RESTRICTED_MODELS_FOR_MIXED_TOOLS={_Aw};has_functions=bool(function_declarations);has_search=any(t.google_search for t in final_tools_for_config);model_is_restricted=model_name in RESTRICTED_MODELS_FOR_MIXED_TOOLS
	...
	def _get_gemini_response_sync(self,model_name,contents,config):
		...
		if not self._genai_client:logger.error(_Bb);return _Bc,[],_A
		try:
			...
			if response.candidates and len(response.candidates)>0:
				...
				if candidate.content and candidate.content.parts:
					fc_parts=[part.function_call for part in candidate.content.parts if hasattr(part,_Bd)and part.function_call]
					if fc_parts:
						function_calls=[]
	...
	async def _get_openai_response_async(self,model_name,messages,prompt_config,tools=_A):
		...
		http_client=httpx.AsyncClient();client=AsyncOpenAI(api_key=self._openai_api_key);openai_params={_X:model_name,_A0:messages,_A8:prompt_config.get(_AG),_B2:prompt_config.get(_AJ),_Y:prompt_config.get(_Y),'frequency_penalty':prompt_config.get('frequencyPenalty'),'presence_penalty':prompt_config.get('presencePenalty')}
		if prompt_config.get(_i):openai_params[_i]=prompt_config.get(_i)
		if tools:openai_params[_O]=tools;openai_params[_B3]='auto'
	...
	async def _get_anthropic_response_async(self,model_name,system_prompt,messages,prompt_config,tools=_A):
		...
		client=AsyncAnthropic(api_key=self._anthropic_api_key);anthropic_params={_X:model_name,_A2:system_prompt,_A0:messages,_A8:prompt_config.get(_AG),_B2:prompt_config.get(_AJ,4096),_Y:prompt_config.get(_Y),_AH:prompt_config.get(_AH)}
		if tools:anthropic_params[_O]=tools;anthropic_params[_B3]={_F:'auto'}
	...
	async def _get_token_api(self,host,client_id,client_secret):
		url='/token';headers={_Q:_K};data={_B4:_Be,_B5:client_id,_B6:client_secret}
		try:response=await self._http_session.post(url,headers=headers,json=data);response.raise_for_status();auth_response=response.json();return f"{auth_response[_Bf]} {auth_response[_AY]}"
	...
	async def _start_session_api(self,host,agent_id_to_start,token):
		url=f"/agent/{agent_id_to_start}";headers={_B7:_t,_s:_A6,_M:token}
		try:
	...
	async def _start_version_api(self,host,agent_id_to_start,token,version_str):
		url=f"/version/{agent_id_to_start}/{version_str}";headers={_B7:_t,_s:_A6,_M:token}
		try:
			response=await self._http_session.get(url,headers=headers);response.raise_for_status();data=response.json();session_uid=data.get(_B,{}).get(_E,{}).get('uid')
			if session_uid:return session_uid,data
	...
	async def _get_session_api(self,host,session_uid,token):
		url=f"/session/{session_uid}";headers={_s:_A6,_M:token,_B7:_t}
		try:
	...
	async def _get_session_lastmodified_api(self,host,session_uid,token):
		url=f"/session/{session_uid}/lastmodified";headers={_s:_A6,_M:token}
	...
	async def _post_session_api(self,host,token,session_uid,data_payload,transfer_req,transfer_acc):
		url=f"/session";headers={_Q:_K,_BJ:_t,_M:token};full_payload={'sessionUid':session_uid,_B:data_payload,'transferRequested':transfer_req,'transferAccepted':transfer_acc}
		try:
	...
	async def _get_token_for_connector(self,connector_config):
		'Gets auth token for a given connector configuration.';A='refresh_token';url=connector_config.get(_Z);client_id=connector_config.get(_B1);client_secret=connector_config.get(_n);grant_type=connector_config.get(_m,_Be);content_type=connector_config.get(_Bg,_K);use_header_payload=connector_config.get('headerPayload',_G);scope=connector_config.get(_AC,_A);refresh_token=connector_config.get('refreshToken',_A)
		...
		if use_header_payload:client_keys=f"{client_id}:{client_secret}";client_keys_b64=base64.b64encode(client_keys.encode()).decode();headers[_M]=f"Basic {client_keys_b64}";headers[_Q]='application/x-www-form-urlencoded';data_to_send=f"grant_type={grant_type}"
			...
			if _A7 in content_type.lower():
				data_to_send={_B4:grant_type,_B5:client_id,_B6:client_secret}
				...
				form_data=[(_B4,grant_type),(_B5,client_id),(_B6,client_secret)]
		...
		try:
			...
			async with httpx.AsyncClient()as client:response=await client.post(url,headers=headers,content=payload);response.raise_for_status();auth_response=response.json();token_type=auth_response.get(_Bf,'Bearer').capitalize();return f"{token_type} {auth_response[_AY]}"
	...
	async def _generic_api_post_put(self,current_var,api_config,headers,method):
		...
		if not url:logger.error(f"URL could not be evaluated for API '{api_config.get(_C)}'. Template: {url_template}");return current_var,_A,_A,_Bh
	...
	async def _generic_api_get(self,current_var,api_config,headers):
		...
		if not url:logger.error(f"URL could not be evaluated for API GET '{api_config.get(_C)}'. Template: {url_template}");return current_var,_A,_A,_Bh
	...
	async def _journey_lookup(self,unique_id_val,current_var,user_input=_A):
		...
		headers={_AT:_K,_M:bearer_token};url=f"https://app.journeyid.io/api/system/customers/lookup?account_id={account_id}&unique_id={unique_id_val}"
		try:
	...
	async def _journey_send_pipeline(self,current_var,json_payload,delivery_method,user_input=_A):
		...
		if not bearer_token:return _A,_B8
	...
	async def _journey_execution_status_check(self,execution_id,current_var,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_B8
		url=f"https://app.journeyid.io/api/system/executions/{execution_id}";headers={_AT:_K,_M:bearer_token}
		for attempt in range(max_retries):
			try:
				...
				if status_data.get(_Bi):return _D,status_data.get('outcome',{}).get(_k,'Task completed successfully.')
	...
	async def _journey_session_pipeline_status_check(self,session_id_val,current_var,pipeline_key_to_check,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_B8
		url=f"https://app.journeyid.io/api/system/sessions/lookup?external_ref={session_id_val}";headers={_AT:_K,_M:bearer_token}
		for attempt in range(max_retries):
			try:
				async with httpx.AsyncClient()as client:response=await client.get(url,headers=headers,timeout=30)
				...
				for exec_item in executions:
					if exec_item.get(A,{}).get('key')==pipeline_key_to_check or exec_item.get(A,{}).get('id')==pipeline_key_to_check:
						if exec_item.get(_Bi):return _D,exec_item.get('outcome',{}).get(_k,'Pipeline completed.')
						break
	...
	async def _twilio_sms_message(self,sms_body,to_phone,from_phone_twilio=_A):
		...
		if not twilio_from_number:logger.error(_Bj);return
	...
	async def _infobip_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		infobip_url='https://g9egk8.api.infobip.com/email/4/messages';headers={_M:token,_Q:_K,_AT:_K};payload={_A0:[{'sender':from_address,'destinations':[{'to':[{'destination':to_address}]}],_R:{_AR:subject,_P:plain_text_body,'html':html_body_content}}]}
		try:
	...
	async def _ms_graph_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		graph_api_endpoint='https://graph.microsoft.com/v1.0';url=f"{graph_api_endpoint}/users/{from_address}/sendMail";headers={_M:token,_Q:_K};email_payload={_k:{_AR:subject,_W:{_Bg:'HTML',_R:html_body_content}},'saveToSentItems':'true'}
			...
			for(key,value)in list(data_structure.items()):
				if isinstance(value,str):
					if A in value or B in value:
						try:eval_globals={_z:context_vars,C:context_vars,D:context_vars,_B9:user_input};data_structure[key]=eval(f"f'''{value}'''",eval_globals,{})
			...
			for(i,item)in enumerate(data_structure):
				if isinstance(item,str):
					if A in item or B in item or user_input is not _A and'{user_input}'in item:
						try:eval_globals={_z:context_vars,C:context_vars,D:context_vars,_B9:user_input};data_structure[i]=eval(f"f'''{item}'''",eval_globals,{})
	...
	def _evaluate_f_string(self,template_string,context_vars,user_input=_A):
		...
		try:eval_namespace={_z:context_vars,_B9:user_input or''};return eval(f"f'''{template_string}'''",eval_namespace,{})
	...
	def _create_function_declaration_from_dict(self,declaration_dict):
		...
		name=declaration_dict.get(_C);description=declaration_dict.get(_L);parameters=declaration_dict.get(_AU)
	...
	def _translate_tools_for_provider(self,provider,direct_tool_configs,processed_fd_dicts):
		...
		translated_tools=[]
		if provider==_j:
			for fd in processed_fd_dicts:translated_tools.append({_F:_Ay,_Ay:{_C:fd.get(_C),_L:fd.get(_L),_AU:fd.get(_AU)}})
		elif provider==_A1:
			for fd in processed_fd_dicts:translated_tools.append({_C:fd.get(_C),_L:fd.get(_L),'input_schema':fd.get(_AU)})
