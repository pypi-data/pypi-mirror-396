...
class Chat(cmd2.Cmd):
    ...
    def do_search(self, query: str):
        ...
        if not is_web_search_supported(default_llm):
    ...
    def _retrieve_ara_config(self):
        ...
        return ConfigManager.get_config(self._get_config_path())
    ...
    def do_CHOOSE_MODEL(self, model_name):
        ...
        if not config_path:
            error_handler.report_error(
                AraConfigurationError("Could not find ara_config.json.")
            )
            return

        self.config.default_llm = model_name
        save_data(filepath=config_path, config=self.config)
    ...
    def _get_config_path(self) -> str | None:
        """
        Finds the absolute path to the ara_config.json file.
        """
        project_root = self._find_project_root()
        if project_root:
            return os.path.join(project_root, "ara", ".araconfig", "ara_config.json")
        return None
