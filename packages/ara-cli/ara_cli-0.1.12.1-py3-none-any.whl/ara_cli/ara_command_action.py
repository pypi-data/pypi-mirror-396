...
def create_action(args):
    ...
    def handle_parent_arguments(args):
        parent_classifier = (
            args.parent_classifier if hasattr(
...
def prompt_action(args):
    ...
    def handle_chat():
        ...
        # Construct the source artefact path to pass to the agent if needed
        source_artefact_path = None
        try:
            sub_directory = Classifier.get_sub_directory(classifier)
            if sub_directory:
                # e.g., ara/features/my_feature.feature
                artefact_file_path = os.path.join(
                    "ara", sub_directory, f"{param}.{classifier}"
                )
                if os.path.isfile(artefact_file_path):
                    source_artefact_path = os.path.abspath(artefact_file_path)
        except Exception:
            # If any error occurs during path construction, default to None
            source_artefact_path = None
        ...
        initialize_prompt_chat_mode(
            ...
            source_artefact_path=source_artefact_path,
        )
