...
def initialize_prompt_chat_mode(
    ...
    artefact_data_path = os.path.join(
        "ara", sub_directory, f"{param}.data"
...
def start_chat_session(
    chat_file, reset, output_mode, append_strings, restricted, source_artefact_path=None
):
    with suppress_stdout(suppress=output_mode):
        chat = (
            Chat(chat_file, reset=reset, source_artefact_path=source_artefact_path)
            if not restricted
            else Chat(
                chat_file,
                reset=reset,
                enable_commands=whitelisted_commands,
                source_artefact_path=source_artefact_path,
            )
        )
    if append_strings:
