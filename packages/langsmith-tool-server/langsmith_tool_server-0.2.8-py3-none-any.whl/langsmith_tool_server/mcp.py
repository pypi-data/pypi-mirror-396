        ...
        for tool in tools:
            ...
            if tool_name not in seen_names:
                ...
                if "auth_provider" in tool:
                    ...
                    _meta["auth_provider"] = tool["auth_provider"]

                if "scopes" in tool:
                    mcp_tool["scopes"] = tool["scopes"]
                    annotations["scopes"] = tool["scopes"]
                    _meta["scopes"] = tool["scopes"]
                ...
                if "default_interrupt" in tool:
                    ...
                    _meta["default_interrupt"] = tool["default_interrupt"]

                if "integration" in tool:
                    annotations["integration"] = tool["integration"]
                    _meta["integration"] = tool["integration"]

                if annotations:
                    mcp_tool["annotations"] = annotations

                if _meta:
                    mcp_tool["_meta"] = _meta
