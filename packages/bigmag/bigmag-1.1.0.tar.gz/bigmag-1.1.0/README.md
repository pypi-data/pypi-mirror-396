...
If you are curious how the dashboard looks like and check its layout even before starting to work with it, please visit this link (it is not a virus, we promise!): https://bigmag.online/ (please be patient, we are using a free account in Render with 0.5 CPUs 😢)

## Dashboard
After the successful execution of the [MAGFlow](https://github.com/jeffe107/MAGFlow) or [nf-core/mag](https://nf-co.re/mag/5.3.0/parameters/#:~:text=%2D%2Dgenerate_bigmag_file-,Make%20a%20BIgMAG,-input%20file%20including), you are ready to display the dashboard and start exploring your data. For this, you will need Conda, Mamba (recommended versions 23.3.1 and 1.3.1) or [pip](https://pip.pypa.io/en/stable/installation/) installed in your local system. You can install the components with:
```bash
 pip install bigmag
```
...
The layout is composed by 8 plots labeled to which tool or process they are related to. At the bottom of the dashboard, the raw data in a format table is displayed with the possibility for the user of highlighting cells, deleting rows and filtering the columns with keywords.
