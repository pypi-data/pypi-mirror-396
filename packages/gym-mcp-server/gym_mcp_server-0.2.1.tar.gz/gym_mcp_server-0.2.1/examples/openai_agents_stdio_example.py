#!/usr/bin/env python3
"""
Example showing how to use the gym-mcp-server with OpenAI Agents SDK.

This demonstrates the recommended way to integrate gym environments with AI agents
...
async def main():
    ...
    try:
        ...
        print("=== OpenAI Agents SDK + Gym MCP Server Example ===\n")

        # Launch the gym-mcp-server as a subprocess via stdio transport
        async with MCPServerStdio(
            name="Gym Environment",
            params={
                "command": "python",
                "args": ["-m", "gym_mcp_server", "--env", "CartPole-v1"],
            },
        ) as server:
