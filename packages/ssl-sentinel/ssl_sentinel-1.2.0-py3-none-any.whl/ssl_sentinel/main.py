...
def process_hostname(hostname, threshold, expiring_soon=False):
    ...
    try:
        ...
        elif certificate["status"] == "WARNING":
            date_formated = certificate["expiry_date"].strftime("%Y-%m-%d")
            days = certificate["days_left"]
            print(f"[WARNING]: Expires in {days} days on {date_formated}.")
        elif certificate["status"] == "OK":
            date_formated = certificate["expiry_date"].strftime("%Y-%m-%d")
            days = certificate["days_left"]
            print(f"[OK]: Expires in {days} days on {date_formated}.")
