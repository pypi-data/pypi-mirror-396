...
class Certificate:
    ...
    def _fetch_certificate(self, timeout=5):
        ...
        try:
            with socket.create_connection((self._hostname, 443), timeout=timeout) as sock:
                with self._context.wrap_socket(sock, server_hostname=self._hostname) as ssock:
                    self._cert = ssock.getpeercert()
        except (OSError, ssl.SSLError, ValueError) as e:
            raise ConnectionError(f"Could not retrieve certificate for {self._hostname}: {e}")
