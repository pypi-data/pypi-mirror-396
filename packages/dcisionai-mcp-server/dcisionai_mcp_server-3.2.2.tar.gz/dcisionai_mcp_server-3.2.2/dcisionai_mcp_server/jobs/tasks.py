...
def publish_job_update(job_id: str, update: Dict[str, Any]) -> None:
    ...
    """
    if not redis_client:
        logger.debug(f"Redis not available, skipping update for job {job_id}")
        return
...
def run_optimization_job(
    ...
    def progress_callback(step: str, progress: int, details: Dict[str, Any]) -> None:
        ...
        """
        progress_update = {
            "type": "progress",  # Add type field for WebSocket protocol
            "job_id": job_id,
            "status": JobStatus.RUNNING.value,
            "progress": {
                "current_step": step,
        ...
        self.update_state(state="PROGRESS", meta=progress_update)

        logger.info(f"Job {job_id} progress: {step} ({progress}%)")
    ...
    try:
        ...
        def make_serializable(obj):
            ...
            if hasattr(obj, "type") and hasattr(obj, "content"):
                ...
                return {k: make_serializable(v) for k, v in obj.items() if k != 'llm_metrics_tracker'}
