...
async def handle_job_websocket(websocket: WebSocket, job_id: str) -> None:
    ...
    try:
        ...
        await websocket.send_json({
            ...
            "progress": 0,
            "current_step": job.get("metadata", {}).get("current_step", "Initializing")
        })
        logger.info(f"[JobWebSocket] Sent initial status for job {job_id}: {job.get('status')}")
        ...
        async for message in pubsub.listen():
            if message["type"] == "message":
                try:
                    ...
                    logger.debug(f"[JobWebSocket] Sent update to client: {update.get('type')}")
                    ...
                    logger.error(f"[JobWebSocket] Error processing update: {e}")
                    continue
