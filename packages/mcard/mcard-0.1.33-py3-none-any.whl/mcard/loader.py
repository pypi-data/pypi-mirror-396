        ...
        if metadata_only and file_io.is_problematic_file(file_path):
            ...
            try:
                ...
                try:
                    ...
                    # 2. Collision: Try relative path handle
                    if root_path:
                        try:
                            ...
                            # If even full path exists (should not happen for unique files), ignore
                            logger.warning(f"Handle collision for both '{handle}' and relative path.")
                    else:
                        logger.warning(f"Handle collision for '{handle}' and no root_path provided.")
                # ---------------------------------
