    ...
    def _send_command(self, command: str) -> str:
        ...
        while time.time() - start_time < self.timeout:
            if self.serial_conn.in_waiting:
                ...
                # Track when "Command sent successfully" appears (for MCTP commands)
                if command_sent_time is None and "command sent successfully" in response.lower():
                    command_sent_time = time.time()

            # Check if response ends with "Cmd>" (the final prompt)
            stripped = response.rstrip()
            if stripped.endswith("Cmd>"):
                # Make sure we have actual response content (not just a lone Cmd>)
                # Look for newlines indicating multi-line response
                if "\n" in stripped:
                    # For MCTP packet commands, "Receive packet" comes AFTER
                    # "Command sent successfully" - need to wait for it
                    if command_sent_time is not None:
                        # If we've seen receive packet, we're done
                        if "receive packet" in response.lower():
                            break
                        # Otherwise keep waiting (up to timeout)
                    else:
                        # Non-MCTP command - done when we see final Cmd>
                        break
        ...
        if success and response_packets:
            try:
                ...
                if len(all_bytes) >= 18:
                    # NVMe-MI Health Status Poll response structure:
                    # Bytes 0-3: MCTP header (20 0f XX 3b)
                    # Bytes 4-7: NVMe-MI message header
                    # Bytes 8-9: NVMe-MI response type (84 88)
                    # Bytes 10-11: Status
                    # Byte 12: Critical Warning
                    # Bytes 13-17: Reserved/flags
                    # Byte 18: Composite Temperature (Celsius)
                    # Bytes 19-20: Reserved
                    # Byte 21: Available Spare Threshold (%)
                    # Byte 22+: Additional data

                    # Extract critical warning (byte 12)
                    if len(all_bytes) >= 13:
                        critical_warning = all_bytes[12]

                    # Extract temperature (byte 18, in Celsius)
                    if len(all_bytes) >= 19:
                        temp_celsius = all_bytes[18]
                        if 0 < temp_celsius < 128:  # Sanity check
                            composite_temp_celsius = float(temp_celsius)
                            # Convert to Kelvin for the composite_temperature field
                            composite_temp = int(temp_celsius + 273)

                    # Extract available spare threshold (byte 21)
                    if len(all_bytes) >= 22:
                        available_spare_threshold = all_bytes[21]

                    # Note: Available spare and percentage_used locations need
                    # further investigation - not clearly present in current response
