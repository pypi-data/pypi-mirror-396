...
- `composite_temperature`: Drive temperature in Kelvin
- `critical_warning`: Critical warning bit field
- `available_spare_threshold`: Available spare threshold percentage
...
- **1.3.2** (2025-12): Fixed MCTP raw packet response parsing
  - Fixed `send_mctp_packet()` to properly wait for async "Receive packet" responses
  - Device sends response packets after "Command sent successfully" message
...
- **1.3.1** (2025-12): Fixed MCTP health status response parsing
  ...
  - Documented that `available_spare` and `percentage_used` are not available via MCTP Health Status Poll
