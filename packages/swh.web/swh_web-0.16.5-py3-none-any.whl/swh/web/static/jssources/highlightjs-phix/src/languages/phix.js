...
function hljsDefinePhix(hljs) {
    // Phix has more [nest-able] comment types and number formats than average, 
    //          with four different string types, not really that much I guess,
    //          but otherwise is pretty much dirt-simple for a syntax colourer.
    const COMMENTS = {
        className: 'comment',
        variants: [
            hljs.COMMENT('^#!','$'),
    ...
    // NB: afaict, none of this actually achieves anything at all...
    const ILLEGAL = [
        '<[\\/%\\?]',
        '\\)\\s?\\{',
        '\\+\\+',
        '===',
        // end must be followed by a single space and one of class, for, 
        // function, if, procedure, struct, switch, try, type, or while:
        // four we can do in full, three pairs we validate to 2 chars only.
        '\bend\b[^ ]',
        '\bend [^cfipstw]',
        '\bend c[^l][^a][^s][^s]',
        '\bend f[^ou]',
        '\bend i[^f]',
        '\bend p[^r][^o][^c][^e][^d][^u][^r][^e]',
        '\bend s[^tw]',
        '\bend t[^ry]',
        '\bend w[^h][^i][^l][^e]',
        // In Phix single quotes are an 8-bit char, including '\0', '\b', 
        // '\e', '\t', '\r', '\n', '\\', '\"', '\'', '\xHH', '\#HH', ' ', 
        // '0', and 'z', but not (say) 'ab' or anything longer.  (nb ^^^)
        '\\\'\\\\[^0betrn\\\"\'x#]',    // (quote backslash not-our-11)
        '[^\']\s\'[^\\][^\']' // (notquote space quote notbackslash notquote)
    ];
// best I could come up with, but gets me a "0 width match regex" error, somehow.
//  const SQUOTE_CHARACTER = '\\\'(\\\\x..|\\\\#..|\\\\.|.)\\\'';
//  const SQUOTE_CHARACTER = '\'(\\x..|\\#..|\\.|.)\'';
//  const SQUOTE_CHARACTER = '\'.\'';
//  const SQUOTE_CHARACTER = '\\\'.\\\'';
// no fatal error, but no workie... at this point I just simply have to give up.
//  const SQUOTE_CHARACTER = {
//    scope: 'string',
//      begin: '\'',
//      end: '\'',
...
//  };
    ...
    const STRINGS = {
        ...
        variants: [
          ...
          // technically squotes are chars, so 'ab' should be illegal,
          // but this'll just have to do, at least for now...
          hljs.APOS_STRING_MODE,
//        SQUOTE_CHARACTER,         // oops...
          TRIPLEQUOTE_STRINGS,
          BACKTICK_STRINGS
        ]
    };
    ...
    const NUMBERS = {
        ...
        relevance: 0,
        variants: [
          // Note this does not do ".4", not that we should encourage that, and
          // because it would spanner s[1..2] - we are not doing operators such
          // as that ".." at all here.
        
          // 0-prefixed explict (decimal) base integers, eg 0(16)FFFF or
          // o(2)101 or o(36)1ZQ [Phix supports all number bases 2..36].
          { begin: "\\b0[\\(]\\d+[\\)][0-9a-zA-Z](_?[0-9a-zA-Z])*\\b" },

          // decimal integer/float, optionally exponential
          { begin: `\\b(${digits})(\\.(${digits}))?([eE][+-]?${digits})?\\b` },

          // explicit decimal/binary/octal/hexadecimal integer
          { begin: "\\b0[dD][0-9](_?[0-9])*\\b" },
          { begin: "\\b0[bB][0-1](_?[0-1])*\\b" },
    ...
    // NB: avoid putting "|0" on anything that lua (being the closest thing) does not.
    const TYPES = [
        'string', 'nullable_string', 'atom_string', 'atom', 'bool|0', 'boolean',
        'cdCanvan', 'cdCanvas', 'complex|0', 'CURLcode', 'dictionary', 'int|0',
        'integer', 'Ihandle', 'Ihandles', 'Ihandln', 'mpfr', 'mpq', 'mpz',
        'mpz_or_string', 'number', 'rid_string', 'seq', 'sequence', 'timedate',
        'object|0'
    ];
    const KWDS = [
        'abstract|0', 'class|0', 'continue|0', 'export|0', 'extends|0', 'nullable',
        'private|0', 'public|0', 'static|0', 'struct|0', 'trace', 'format', 
        'and', 'break', 'by|0', 'case|0', 'catch|0', 'const|0', 'constant', 'debug',
        'default|0', 'do', 'else', 'elsif', 'end', 'enum|0', 'exit', 'fallthru',
        'fallthrough', 'for', 'forward', 'function', 'global', 'local', 'if', 'in',
        'include', 'js', 'javascript', 'javascript_semantics', 'let|0', 'not',
        'or', 'procedure', 'profile', 'profile_time', 'return', 'safe_mode',
        'switch|0', 'then', 'to', 'try|0', 'type', 'type_check', 'until', 'warning',
        'while', 'with', 'without', 'xor',
        'ifdef|0', 'elsifdef|0', 'elsedef|0'
    ];
    const BUILT_INS = [
        ...
        'begins|0', 'binary_search', 'bits_to_int', 'bk_color', 'bytes_to_int',
        ...
        'glVertexAttribPointer', 'glViewport', 'head|0', 'hsv_to_rgb', 'iff',
        ...
        'shift_bits', 'shuffle', 'sign|0', 'sin', 'smallest', 'sort',
        ...
        'namespace', 'new|0', 'newDib', 'open', 'open_dll', 'PE32', 'PE64',
  ...
  return {
    name: 'Phix',
//  disableAutodetect: true,    // (wouldn't really bother me.)
    keywords: KEYWORDS,
    illegal: ILLEGAL,
    contains: [
      COMMENTS,
...
module.exports.definer = hljsDefinePhix;
