...
def handle_run(args):
    ...
    if result["success"]:
        ...
        if output_directory:
            ...
            # Allow bucket from YAML (s3_bucket or s3-bucket) or env
            bucket = (
                yaml_data.get("s3_bucket")
                or yaml_data.get("s3-bucket")
                or BUCKET
            )
            if not bucket:
                print("✗ S3 bucket not configured. Set S3_BUCKET env var or add 's3_bucket' in the YAML.")
                sys.exit(1)
