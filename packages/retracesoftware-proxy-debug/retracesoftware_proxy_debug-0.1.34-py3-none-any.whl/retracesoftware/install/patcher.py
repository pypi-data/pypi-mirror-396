...
def patch_module(patcher, checkpoint, config, namespace, update_refs):
    if '__name__' in namespace:
        name = namespace['__name__']
        if name in config:
            # print(f"Patching: {namespace['__name__']}")

            # checkpoint(f"Patching module: {name}")            
            
            for phase_name, phase_config in config[name].items():
