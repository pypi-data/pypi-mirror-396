...
class DatabaseCleaner:
    ...
    async def cleanup_session(
        ...
        env_db_configs: dict[str, list[dict]] = {}
        ...
        async def fetch_db_configs(env: EnvironmentInfo):
            if env.artifact_id:
                try:
                    ...
                    valid_configs = [c for c in (configs or []) if c.get("db_database")]
                    if valid_configs:
        ...
        async def cleanup_env(env: EnvironmentInfo) -> tuple[str, EnvironmentCleanupResult]:
            ...
            if env.alias in env_db_configs:
                ...
                async def cleanup_db(config: dict) -> tuple[str, DatabaseCleanupResult]:
                    db_name = config["db_database"]
                    port = ports[db_name]
                    try:
