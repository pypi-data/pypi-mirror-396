...
def print_tips_box():
    ...
    # Header: "💡 Tips:" (8 visible chars)
    header_text = "💡 Tips:"
    header_padding = inner_width - len(header_text) - 1  # -1 for space after ║
    print(f"{Colors.ORANGE}║{Colors.RESET} {Colors.BOLD}{Colors.YELLOW}{header_text}{Colors.RESET}{' ' * header_padding}{Colors.ORANGE}║{Colors.RESET}")
    
    print(f"{Colors.ORANGE}╠{'═' * inner_width}╣{Colors.RESET}")
    
    # Tips: "  • " prefix (4 visible chars) + tip text
    for tip in tips:
        prefix = "  • "  # 4 visible chars
        tip_text = tip  # Actual text length
        total_visible = len(prefix) + len(tip_text)
        padding = inner_width - total_visible - 1  # -1 for space after ║
        print(f"{Colors.ORANGE}║{Colors.RESET}{prefix}{Colors.DIM}{tip_text}{Colors.RESET}{' ' * padding}{Colors.ORANGE}║{Colors.RESET}")
