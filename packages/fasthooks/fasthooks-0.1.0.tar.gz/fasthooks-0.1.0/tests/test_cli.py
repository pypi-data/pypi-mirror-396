...
class TestCLIRun:
    def test_cli_run_help(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "claudehooks", "run", "--help"],
            capture_output=True,
            text=True,
        )
...
class TestCLIInit:
    def test_cli_init_creates_project(self, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "claudehooks", "init", str(project_dir)],
            capture_output=True,
            text=True,
        )
    ...
    def test_cli_init_hooks_file_content(self, tmp_path):
        ...
        assert "from claudehooks import" in content
...
class TestCLIHelp:
    def test_cli_help(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "claudehooks", "--help"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
