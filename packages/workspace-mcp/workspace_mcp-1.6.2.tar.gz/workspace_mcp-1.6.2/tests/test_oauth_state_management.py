...
async def test_start_auth_flow_persists_state(monkeypatch):
    ...
    monkeypatch.setattr("auth.google_auth.get_fastmcp_session_id", lambda: "session-123")
...
def test_handle_auth_callback_validates_state(monkeypatch):
    ...
    monkeypatch.setattr("auth.google_auth.get_user_info", lambda credentials: {"email": "user@example.com"})
...
def test_ensure_session_from_access_token(monkeypatch):
    ...
    class DummyProvider:
        def __init__(self):
            ...
            self._refresh_tokens = {"refresh-token": SimpleNamespace(token="refresh-token")}
