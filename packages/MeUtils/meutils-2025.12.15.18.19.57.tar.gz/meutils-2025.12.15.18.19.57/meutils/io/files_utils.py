...
async def do_file_data(
    ...
    if isinstance(file, list):
        tasks = [do_file_data(_, input_reference_format) for _ in file]
        return await asyncio.gather(*tasks)
    ...
    if isinstance(file, str):  # url
        ...
        if input_reference_format in {"base64", "b64"}:
            file = await to_base64(await file.read(), file.content_type)

        elif input_reference_format == "oss":  # to url todo海外服务器
            file = await to_url(await file.read(), file.content_type)

        else:  # fal url
            file = await to_url_fal(await file.read(), file.content_type)
