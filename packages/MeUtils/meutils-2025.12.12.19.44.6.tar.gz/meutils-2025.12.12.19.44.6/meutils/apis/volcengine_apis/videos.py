...
async def get_valid_token(tokens: Optional[list] = None, batch_size: Optional[int] = None, seed: int = 0,
    ...
    """
    for i in range(3):  # 最多轮询5次， todo 加兜底
        _ = await polling_keys('volc')  # VOLC_API_KEY
...
async def create_task(request: Union[CompletionRequest, VideoRequest, SoraVideoRequest], api_key: Optional[str] = None):
    ...
    if isinstance(request, SoraVideoRequest):  # todo 首尾帧 参考图
        ...
        if "lite" in request.model and (request.input_reference or request.first_frame_image):
        ...
        if request.input_reference:  # url
            payload["content"] += [
                ...
                for image in request.input_reference
            ]
