...
class RawQuicClient(QuicProtocolShell):
    ...
    def __init__(self, quic: QuicConnection, datagramEndpoint, client: 'QuicClient', stream_handler):
        ...
        self._stream_sem = asyncio.Semaphore(1024)
    ...
    #     sock = self.datagramEndpoint.transport.get_extra_info("socket")
    ...
    #         self._quic.receive_datagram(data, addr, time.time())
    #         self._schedule_flush()

    async def _send_loop(self):
        while self._running:
            sid, data, end = await self._send_queue.get()
            try:
    ...
    async def request(self, request: Union[GNRequest, AsyncGenerator[GNRequest, Any]]):
        if isinstance(request, GNRequest):
            ...
            #self._schedule_flush()
            await self._send_queue.put((sid, blob, True))
