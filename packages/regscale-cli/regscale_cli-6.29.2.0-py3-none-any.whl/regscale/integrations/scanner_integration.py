...
class ScannerIntegration(ABC):
    ...
    def _populate_issue_lookup_cache(self) -> None:
        ...
        with self._issue_cache_lock:
            ...
            cache: ThreadSafeDict[str, List[regscale_models.Issue]] = ThreadSafeDict()
            indexed_count = 0
    ...
    def _get_existing_issues_for_finding(
        ...
        if cache is None:
            # Fallback if cache is still None after population attempt
            return []

        existing_issues: List[regscale_models.Issue] = cache.get(finding_id) or []
    ...
    def _process_findings_with_threading(
        ...
        logger.info(
            f"Deduplication stats: {self._dedup_stats['existing']} existing issues updated, "
            f"{self._dedup_stats['new']} new issues created (total processed: {processed_findings_count})"
        )
