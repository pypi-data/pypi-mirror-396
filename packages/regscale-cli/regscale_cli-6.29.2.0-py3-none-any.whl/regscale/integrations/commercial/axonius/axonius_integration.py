...
def _process_findings_batches(
    ...
    for batch_count, batch in enumerate(finding_batches, start=1):
        if batch_count % 2 == 0:
            logger.info("Issues Processed: %d", batch_count * 1000)
...
def _process_vulnerabilities_batches(
    ...
    for batch_count, batch in enumerate(vuln_batches, start=1):
        if batch_count % 10 == 0:
            logger.info("Vulns Processed: %d", batch_count * 1000)
...
def _process_asset_batches(
    ...
    for batch_count, batch in enumerate(asset_batches, start=1):
        if batch_count % 2 == 0:
            logger.info("Assets Processed: %d", batch_count * 1000)
...
def sync_compliance(filepath: str, fisma_filter: str) -> None:
    ...
    for fisma, ssp_id in system_ids.items():
        ...
        try:
            catalog_id = get_catalog_by_ssp(ssp_id=id)
...
def _create_compliance_finding_dict(
    ...
    :rtype: dict
    """
    # NOSONAR - Intentional placeholder CVE for compliance findings
    # TECHNICAL DEBT: Hardcoded placeholder CVE value
    # Axonius compliance posture checks (STIG assessments) are NOT vulnerability findings
    # and do not have real CVE IDs. However, the RegScale API requires a CVE field
    # in the issue payload. This placeholder value satisfies the API schema requirement.
    # Do we need vulnerabilities for stig assessment issues? discuss with the team about this one!
    # compliance findings, or use None/empty string if API accepts it.
    # See also: Line 46-64 (Axonius data structure) which shows COMPLIANCE_TABLE
    # doesn't include CVE fields.
    return {
        ...
        "source": "Axonius_Posture",
        "status": "Open",
        "description": finding["COMPLIANCE-INFO"],
        ...
        "dateFirstDetected": f"{created_date}T03:49:18.429Z",
        "recommendedActions": finding["COMPLIANCE-SOLUTION"],
        "dueDate": due_date,
        "parentId": ssp_id,
        "parentModule": "securityplans",
    }
