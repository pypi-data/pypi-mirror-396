...
def _get_qualys_api():
    ...
    if config.get("qualysMockUrl"):
        logger.info("Using Qualys Mock API at: %s", qualys_url)
...
def combine_duplicate_qualys_vulns(qualys_vulns: list[Issue]) -> list:
    """
    Combine duplicate Qualys vulnerabilities by qualysId.

    :param qualys_vulns: List of Issue objects to deduplicate
    :return: List of deduplicated Issue objects
    """
    combined_vulns = {}
    job_progress = Progress()
    deduping_task = job_progress.add_task("[cyan]Deduplicating vulnerabilities...", total=len(qualys_vulns))

    with job_progress:
        for vuln in qualys_vulns:
            if vuln.qualysId in combined_vulns:
                current_identifier = combined_vulns[vuln.qualysId].assetIdentifier
...
def inner_join(reg_list: list, qualys_list: list) -> list:
    ...
    :rtype: list
    """
    logger.info("=== DEDUPLICATION DEBUG START ===")
    logger.info("Total RegScale assets to check: %s", len(reg_list))
    logger.info("Total Qualys assets to deduplicate: %s", len(qualys_list))

    # Build deduplication sets
    plugin_id_set, qualys_id_set, asset_name_patterns = _build_deduplication_sets(reg_list)
    _log_deduplication_sets(plugin_id_set, qualys_id_set, asset_name_patterns)
    ...
    logger.info("  - Total matches found (existing assets): %s", matches_found)
    logger.info("  - Assets to create (new): %s", len(qualys_list) - matches_found)
    logger.info("=== DEDUPLICATION DEBUG END ===")
