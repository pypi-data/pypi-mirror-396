...
def auth_was_api() -> tuple[str, dict]:
    ...
    logger.debug("WAS Auth - Starting authentication")
    logger.debug("WAS Auth - Using URL: %s", qualys_url)
    if config.get("qualysMockUrl"):
        logger.info("WAS Auth - Using Mock API at: %s", qualys_url)
    ...
    logger.info("Successfully configured WAS API authentication")
