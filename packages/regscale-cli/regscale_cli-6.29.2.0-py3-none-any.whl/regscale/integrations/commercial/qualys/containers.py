...
def auth_cs_api() -> tuple[str, dict]:
    ...
    logger.debug("Container Security Auth - Starting authentication")
    logger.debug("Container Security Auth - Using URL: %s", qualys_url)
    if config.get("qualysMockUrl"):
        logger.info("Container Security Auth - Using Mock API at: %s", qualys_url)
    ...
    try:
        ...
        # Mock API doesn't need transformation
        if qualys_url and "qualysguard" in qualys_url:
            base_url = qualys_url.replace("qualysguard", "gateway")
            logger.debug("Container Security Auth - Transformed qualysguard -> gateway")
        else:
            base_url = qualys_url

        logger.debug("Container Security Auth - Base URL: %s", base_url)

        auth_url = urljoin(base_url, "/csapi/v1.3/auth")
        logger.debug("Container Security Auth - Final auth_url: %s", auth_url)
        logger.debug("Container Security Auth - Request headers: %s", auth_headers)
        logger.debug(
            "Container Security Auth - Request data: %s",
            {k: "***" if k == "password" else v for k, v in auth_data.items()},
        )
        logger.info("Container Security Auth - Sending POST request to: %s", auth_url)
