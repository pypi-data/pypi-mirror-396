...
def _query_qradar_for_identifier(qradar_client: Any, query_config: QRadarQueryConfig) -> tuple[bool, int]:
    """
    ...
    logger.info(
        ...
        f"(last {query_config.time_window_hours} hours, timeout: 900s)"
    )
    logger.debug(f"AQL query: {aql_query}")
    filtered_events = qradar_client.execute_aql_query(aql_query, query_timeout=900)
...
def _create_logging_evidence(
    ...
    evidence_title = f"QRadar AWS Account Logging Assessment - {account_id}"
    evidence_desc = (
    ...
    if created_evidence and created_evidence.id:
        ...
        filename = f"qradar_logging_evidence_{account_id}_{datetime.now(timezone.utc).strftime('%Y%m%d')}.jsonl.gz"
...
def query_events(
    ...
    try:
        ...
        identifier_found, event_count = _query_qradar_for_identifier(qradar_client, query_config)
