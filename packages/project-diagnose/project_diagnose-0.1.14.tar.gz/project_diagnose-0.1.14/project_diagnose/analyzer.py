...
def is_valid_file(rel_path: str, filename: str) -> bool:
    """
    rel_path: путь относительно ROOT, например "analysis/tune_sltp.py"
    filename: только имя файла, например "tune_sltp.py"
    """

    # пользовательские исключения: сначала по полному относительному пути,
    # потом по имени файла (на случай старых конфигов)
    if rel_path in EXCLUDE_FILES or filename in EXCLUDE_FILES:
        return False

    # explicit include: по пути или по имени
    if rel_path in INCLUDE_FILES_EXTRA or filename in INCLUDE_FILES_EXTRA:
        return True

    _, ext = os.path.splitext(filename)

    # особые JSON по имени
    if ext == ".json" and filename in SPECIAL_JSON:
        return True

    # обычный случай по расширению
