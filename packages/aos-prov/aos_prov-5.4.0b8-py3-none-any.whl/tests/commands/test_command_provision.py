...
def test_provision_unit_v4(mocker, tmp_path):
    mocker.patch(
        'aos_prov.commands.command_provision.UnitCommunicationV6.get_system_info', side_effect=GrpcUnimplemented(),
    )
    ...
    mocker.patch(
        'aos_prov.commands.command_provision.UnitCommunicationV6.get_protocol_version', side_effect=GrpcUnimplemented(),
    )
...
def test_provision_unit_unsupported_protocol(mocker, tmp_path):
    console_patched = mocker.patch(
        'aos_prov.utils.common.console',
        new=Console(file=io.StringIO(), width=120, soft_wrap=True),
    )
...
def test_provision_unit_connection_error(mocker, tmp_path):
    console_patched = mocker.patch(
        'aos_prov.utils.common.console',
        new=Console(file=io.StringIO(), width=120, soft_wrap=True),
    )
    ...
    get_protocol_version_v6 = mocker.patch(
        'aos_prov.commands.command_provision.UnitCommunicationV6.get_protocol_version', side_effect=UnitError(),
    )
    ...
    assert get_protocol_version_v6.call_count == 2
    assert get_protocol_version_v5.call_count == 0
    assert 'Connection timeout' in console_patched.file.getvalue()


def test_provision_unit_v4_additional_cert(mocker, tmp_path):
    console_patched = mocker.patch(
        'aos_prov.utils.common.console',
        new=Console(file=io.StringIO(), width=120, soft_wrap=True),
    )
