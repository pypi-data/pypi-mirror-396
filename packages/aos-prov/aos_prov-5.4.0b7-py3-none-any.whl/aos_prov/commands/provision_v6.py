...
def run_provision_v6(config: Config, unit_address: str, cloud_api: CloudAPI, nodes_count: int = 2):
    ...
    for node_id in config.node_ids:
        ...
        if node_info.node_type.strip() != node_info.node_type:
            raise UnitError(f'Node Type: {node_info.type} - containers trailing ASCII whitespaces')
        if node_info.title.strip() != node_info.title:
            raise UnitError(f'Node Name: {node_info.title} - containers trailing ASCII whitespaces')
            ...
            'cpus': [
                {
                    ...
                    'arch': cpu.arch_info.architecture + '/'+ cpu.arch_info.variant,
                    'archFamily': cpu.arch_info.architecture,
                } for cpu in node_info.cpus],  # noqa: WPS319
            'maxDmips': node_info.max_dmips,
            'totalRam': node_info.total_ram,
            'osType': node_info.os_info.os + ' ' + node_info.os_info.version,
            'attrs': attrs,
            'partitions': [
                {
                    'name': partition.name,
