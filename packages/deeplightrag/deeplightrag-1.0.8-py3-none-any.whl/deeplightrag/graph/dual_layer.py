...
class DualLayerGraph:
    ...
    def _build_cross_layer_connections(self):
        ...
        print(f"  - Created {len(self.entity_to_regions)
                             } entity→region mappings")
        print(f"  - Created {len(self.region_to_entities)
                             } region→entity mappings")
        print(f"  - Created {len(self.figure_caption_links)
                             } figure↔caption links")
        print(f"  - Created {len(self.image_to_entities)
                             } image→entity mappings")
        ...
        for entity in relevant_entities[:5]:
            ...
            for region in regions[:2]:
                if region.node_id not in seen_regions:
                    ...
                    context_parts.append(
                        f"**[{region.region.block_type.upper()
    ...
    def load(self, directory: str):
        ...
        if os.path.exists(entity_relationship_path):
            ...
            print(f"Warning: entity_relationship.json not found in {
                  directory}")
