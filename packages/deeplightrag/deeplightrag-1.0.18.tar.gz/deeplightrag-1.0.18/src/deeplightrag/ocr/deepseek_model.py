...
class DeepSeekOCR(BaseOCRProcessor):
    ...
    def process_image(self, image: Image.Image, page_num: int = 0) -> List[VisualRegion]:
        ...
        try:
            ...
            # DeepSeek-OCR prints output and may return None
            if result is None:
                # The model prints output to stdout, try to capture from last output
                text = "OCR completed (output printed above)"
                compression_stats = {}
