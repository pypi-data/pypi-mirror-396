...
class EntityRelationshipGraph:
    ...
    def extract_entities_from_ocr(self, ocr_results: List[PageOCRResult]) -> int:
                    ...
                    for entity in entities:
                        ...
                        if normalized_name not in self.entity_name_index:
                            ...
                            try:
                                ...
                                logger.warning(f"Failed to merge entity {normalized_name}: {e}")
                except Exception as e:
                    extraction_errors += 1
        ...
        if extraction_errors > 3:
            logger.warning(f"{extraction_errors - 3} more extraction errors")
    ...
    def _extract_entities_from_region(self, region: VisualRegion) -> List[Entity]:
        ...
        try:
            ...
            if len(entities) > 0:
                logger.debug(f"GLiNER extracted {len(entities)} entities from {region.block_type}")
                return entities
            else:
                logger.debug("GLiNER found no entities")
