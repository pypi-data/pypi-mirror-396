...
class DeepLightRAG:
    ...
    def index_document(
        ...
        logger.info(f"Starting indexing of {
                    pdf_path} (size: {file_size / 1024:.1f}KB)")
    ...
    def retrieve(
        ...
        print(f"\nQuery Level: {classification['level']} ({
        ...
        if is_visual_retrieval:
            ...
            print(
                f"Retrieved {retrieval_result.nodes_retrieved} nodes (Visual mode: {
                    visual_mode})"
            )
