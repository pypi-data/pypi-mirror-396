...
def copy_demo_structure(module_name: str, target_module_dir: Path, project_root: Path):
    """
    ...
    try:
        ...
        if not source_demo_path.exists() or not source_demo_path.is_dir():
            print(f'错误: 源 demo 目录不存在: {source_demo_path}', file=sys.stderr)
            sys.exit(1)
        
        # 复制整个 demo 目录树（排除 __init__.py，因为我们需要根据模块名生成）
        print(f'正在复制 demo 目录结构...')
        print(f'  源目录: {source_demo_path}')
        print(f'  目标目录: {target_module_dir}')
        
        # 检查源目录中的子目录
        source_dirs = [item.name for item in source_demo_path.iterdir() if item.is_dir()]
        print(f'  源目录包含的子目录: {", ".join(source_dirs)}')
        ...
        try:
            ...
            print(f'✓ 已复制 demo 目录结构: {target_module_dir.relative_to(project_root)}')
            
            # 验证复制的目录
            copied_dirs = [item.name for item in target_module_dir.iterdir() if item.is_dir()]
            print(f'  已复制的子目录: {", ".join(copied_dirs)}')
            
            # 检查 frontend 和 common 目录是否存在
            frontend_dir = target_module_dir / 'frontend'
            common_dir = target_module_dir / 'common'
            
            missing_dirs = []
            if not frontend_dir.exists():
                missing_dirs.append('frontend')
            if (source_demo_path / 'common').exists() and not common_dir.exists():
                missing_dirs.append('common')
            
            if missing_dirs:
                print(f'警告: 以下目录未被复制: {", ".join(missing_dirs)}', file=sys.stderr)
                print(f'尝试使用备用方法复制这些目录...', file=sys.stderr)
                # 使用备用方法复制缺失的目录
                for dir_name in missing_dirs:
                    ...
                    if source_dir.exists() and source_dir.is_dir():
                        try:
                            ...
                            print(f'✓ 已使用备用方法复制 {dir_name} 目录', file=sys.stderr)
                        except Exception as backup_error:
                            print(f'错误: 使用备用方法复制 {dir_name} 目录失败: {backup_error}', file=sys.stderr)
                            import traceback
                            traceback.print_exc()
                
        except FileExistsError as e:
            print(f'错误: 目标目录已存在: {e}', file=sys.stderr)
            print(f'提示: 请确保模块目录 {target_module_dir} 不存在', file=sys.stderr)
            sys.exit(1)
        except Exception as e:
            print(f'错误: 复制目录时出错: {e}', file=sys.stderr)
            import traceback
            traceback.print_exc()
            sys.exit(1)
        ...
        print(f'错误: 复制 demo 目录结构时出错: {e}', file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
def replace_module_names(target_dir: Path, module_name: str, project_root: Path):
    ...
    if replaced_count > 0:
        print(f'✓ 已替换 {replaced_count} 个文件中的模块名称')
            ...
            for old_name, new_name in rename_patterns:
                if file == old_name:
                    ...
                    if not new_file_path.exists():
                        ...
                        renamed_count += 1
                        print(f'✓ 重命名文件: {file_path.relative_to(project_root)} -> {new_file_path.relative_to(project_root)}')
                    break
...
def update_vite_config(frontend_dir: Path, module_name: str):
    ...
    try:
        ...
        if new_content != content:
            ...
            print(f'✓ 已更新 vite.config.ts 中的 outDir 配置')
    except Exception as e:
        print(f'警告: 更新 vite.config.ts 时出错: {e}', file=sys.stderr)


def create_module_structure(module_name: str, project_root: Path = None):
    """
    创建新模块的目录结构和文件
    ...
    if not (project_root / 'run.py').exists() and not (project_root / 'setup.py').exists():
        print(f'错误: 当前目录不是项目根目录（未找到 run.py 或 setup.py）', file=sys.stderr)
        print(f'提示: 请在项目根目录下运行此命令', file=sys.stderr)
        sys.exit(1)
    
    # 检查模块名是否合法
    if not module_name or not module_name.replace('_', '').isalnum():
        print(f'错误: 模块名只能包含字母、数字和下划线', file=sys.stderr)
        sys.exit(1)
    ...
    if module_dir.exists():
        print(f'错误: 模块 "{module_name}" 已存在: {module_dir}', file=sys.stderr)
        sys.exit(1)
    ...
    try:
        # 复制整个 demo 目录结构
        copy_demo_structure(module_name, module_dir, project_root)
...
''').substitute(module_name=module_name)
        ...
        print(f'✓ 创建文件: {init_file.relative_to(project_root)}')
        
        # 所有文件和目录已在 copy_demo_structure 中复制并处理
        
        print(f'\n✓ 模块 "{module_name}" 创建成功！')
        print(f'\n模块目录结构（已从 demo 目录完整复制）:')
        print(f'  {module_name}/')
        print(f'    ├── __init__.py              # Blueprint 定义')
        print(f'    ├── controller/              # 控制器目录')
        print(f'    ├── service/                  # 服务层目录')
        print(f'    ├── schema/                    # Schema 定义目录')
        print(f'    ├── frontend/                  # 前端项目目录')
        print(f'    └── common/                    # 通用目录（如果存在）')
        print(f'\n提示: 模块已自动注册，所有文件中的 demo/Demo 已替换为 {module_name}/{module_name_capitalized}，可以直接使用！')
        
    except Exception as e:
        print(f'\n✗ 创建模块失败: {e}', file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
示例:
  fred-create user              # 创建名为 user 的模块
  fred-create user_management   # 创建名为 user_management 的模块
  fred-create --path /path/to/project mymodule  # 在指定项目目录创建模块
    ...
    if args.path:
        ...
        if not project_root.exists():
            print(f'错误: 指定的路径不存在: {project_root}', file=sys.stderr)
            sys.exit(1)
        if not project_root.is_dir():
            print(f'错误: 指定的路径不是目录: {project_root}', file=sys.stderr)
            sys.exit(1)
    else:
        project_root = Path.cwd()
    ...
    # 执行创建模块
    try:
        create_module_structure(args.module_name, project_root)
    except Exception as e:
        print(f'\n✗ 创建模块失败: {e}', file=sys.stderr)
        sys.exit(1)
