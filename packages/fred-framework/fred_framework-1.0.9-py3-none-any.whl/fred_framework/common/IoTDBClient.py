...
class IoTDBClient:
	...
	def list_timeseries(self, path_pattern: str = "root.**") -> List[str]:
		...
		try:
			...
			logger.error(f"IoTDB 查询时序路径失败: {str(e)}, path_pattern: {path_pattern}", exc_info=True)
			return []
	...
	def query_data(self, device_path: str, measurements: Optional[List[str]] = None,
		...
		try:
			...
			if start_time is not None or end_time is not None:
				logger.info(f"IoTDB 查询 SQL: {sql}")
			...
			logger.error(f"IoTDB 查询失败: {str(e)}", exc_info=True)
			return []
		...
		try:
			...
			if '**' in path_pattern or '*' in path_pattern:
				...
				for ts_path in timeseries_list:
					try:
						...
						if "508" in error_msg or "does not exist" in error_msg or "device template" in error_msg.lower():
							logger.info(f"时序路径不存在或由设备模板表示，视为删除成功: {ts_path}")
							continue
						# 其他错误记录警告但继续删除其他路径
						logger.warning(f"删除时序路径失败: {ts_path}, 错误: {error_msg}")
						continue
			else:
				...
				if not timeseries_list:
					...
					if not path_pattern.endswith('.**'):
						...
						try:
							...
							self.iotdb_client.execute_non_query_statement(delete_sql)
							logger.info(f"删除时序路径成功: {path_pattern}")
							return True
						except Exception as e:
							error_msg = str(e)
							# 如果路径不存在（508错误）或由设备模板表示，视为成功（幂等性）
							if "508" in error_msg or "does not exist" in error_msg or "device template" in error_msg.lower():
								logger.info(f"时序路径不存在或由设备模板表示，视为删除成功: {path_pattern}")
								return True
							# 其他错误才视为失败
							logger.error(f"删除时序路径失败: {path_pattern}, 错误: {error_msg}")
							return False
					else:
						# 路径下没有时序路径，视为删除成功
						logger.info(f"路径下没有时序路径，视为删除成功: {path_pattern}")
						return True
				...
				if success_count > 0 or fail_count == 0:
					logger.info(f"删除路径完成: {path_pattern}, 成功: {success_count}, 失败: {fail_count}")
					return True
				else:
					logger.error(f"删除路径失败: {path_pattern}, 成功: {success_count}, 失败: {fail_count}")
					return False

			return True
		except Exception as e:
			logger.error(f"删除时序路径异常: {str(e)}", exc_info=True)
			return False
