...
class ImageVerification:
	...
	def get_font(self):
		"""
		获取字体对象，优先使用fonts目录中的字体文件，如果不存在则尝试系统字体，最后使用PIL默认字体
		支持通过 self.fontsize 动态调整字体大小
		"""
		# 获取当前文件所在目录
		current_dir = os.path.dirname(os.path.abspath(__file__))
		# 构建字体文件路径：从 common 目录到 fonts 目录
		font_path = os.path.join(os.path.dirname(current_dir), 'fonts', 'NotoSansSC-VariableFont_wght.ttf')
		
		# 如果字体文件存在，使用该字体
		if os.path.exists(font_path):
			try:
				return ImageFont.truetype(font_path, self.fontsize)
			except Exception:
