...
class Sqlacodegen:
	...
	def __generate_model(self, app, database_uri, bind_name):
		...
		if not tables:
			app.logger.info(f"------数据库 {bind_name} 没有有效表，跳过生成模型文件")
			return
		app.logger.info(f"------开始生成模型文件：" + model_path)
		...
		if result.returncode != 0:
			app.logger.error(f"生成模型文件失败: {result.stderr}")
			return
			
		app.logger.info(f"------模型文件生成完成：" + model_path)
		# 自动为模型类添加 __bind_key__
		if bind_name:
			app.logger.info(f"------自动为模型类添加 __bind_key__：" + model_path)
