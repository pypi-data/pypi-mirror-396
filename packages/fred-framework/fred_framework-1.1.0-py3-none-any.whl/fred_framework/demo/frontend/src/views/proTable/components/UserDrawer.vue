  ...
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}用户`">
    ...
    >
      <el-form-item label="用户头像" prop="avatar">
        <UploadImg v-model:image-url="avatarSafe" width="135px" height="135px" :file-size="3">
          <template #empty>
      ...
      <el-form-item label="用户照片" prop="photo">
        <UploadImgs v-model:file-list="photoSafe" height="140px" width="140px" border-radius="50%">
          <template #empty>
...
// 新增：安全 avatar 和 photo 计算属性，避免 undefined
const avatarSafe = computed({
  get: () => drawerProps.value.row?.avatar ?? "",
  set: v => {
    if (drawerProps.value.row) drawerProps.value.row.avatar = v;
  }
});
const photoSafe = computed({
  get: () => drawerProps.value.row?.photo ?? [],
  set: v => {
    if (drawerProps.value.row) drawerProps.value.row.photo = v;
  }
});
