...
class RsToolsOperatoer:
    ...
    def convert_jpg_to_tif(input_jpg_file, out_tif_file):
        ...
        # 自定义坐标系
        if jpeg_dataset.GetProjection() is None or str(jpeg_dataset.GetProjection()).strip() == "":
            # 如果输入的栅格数据没有地理信息的话，需要自定义坐标系，否则矢量化结果与原数据程垂直镜像对称
            spatial_reference = osr.SpatialReference()
            if src_transform[0]<180 and src_transform[0]>-180 and src_transform[3]<90 and src_transform[3]>-90:
                spatial_reference.ImportFromEPSG(4326)  # WGS84地理坐标系
            else:
                spatial_reference.ImportFromEPSG(3857) # 这里需要进一步判断，先简单估为3857
            jpeg_dataset.SetProjection(spatial_reference.ExportToWkt())
        if jpeg_dataset.GetGeoTransform() is None or str(jpeg_dataset.GetGeoTransform()).strip() == "":
            # 坐标信息
            # 左上角点坐标X
            # x方向的分辨
            # 旋转系数，如果为0，就是标准的正北向图像
