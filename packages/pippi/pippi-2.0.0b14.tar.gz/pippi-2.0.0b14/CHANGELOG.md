...
- Allow 0 input channels and/or 0 output channels for playback-only or 
  capture-only systems.

### 2.0.0 - Beta 9 - 12

Testing raspberry pi builds and wheel issues...

#### Changes

- Fixed some portability issues when using `ld` for `size_t` in string 
  formatting + added a more portable size annotation on the `LMDB_MAPSIZE` 
  constant.
- Added astrid and soudnpipe headers to MANIFEST.in
- Moved an inline declaration in astrid.c causing builds to fail on 
  raspbian bullseye/py3.9 to the top of the function scope

### 2.0.0 - Beta 8

#### Bug Fixes

- Fixed a problem with buffer normalization that could produce Inf with 
  quiet or silent buffers.

### 2.0.0 - Beta 7

Just tooling changes to support wheel building. We're vendoring soundpipe like other deps now.

### 2.0.0 - Beta 6

Removed internal POSIX message queues, replaced with Mindaugas Rasiukevicius' lock 
free ringbuffer library for all internal queues. External messages (from instrument 
to instrument) are brokered over POSIX message queues. Each instrument has a POSIX inbox 
for external listening. (With the idea in mind of adding other inboxes for things like 
sending messages over a socket, etc.)

Numerous bugfixes (many causing memory leaks in astrid instruments)

### 2.0.0 - Beta 5

Major internal refactor, moving most core DSP out of cython into libpippi / astrid C libraries.

Added a basic unit generator system for astrid instruments.

First pass experimental astrid GUIs.
