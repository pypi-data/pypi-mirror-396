...
import { getDatasetAnalytics, createRun, listPendingJudgements, judgeResult } from '../api'
...
export default function DatasetAnalytics() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getDatasetAnalytics(Number(id!)),
  ...
  const runMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['dataset-analytics', id] }),
  })

  const datasetPendingJudgements = useMemo(() => {
    if (!data || !pendingJudgements) return []
    const ids = new Set(data.matrix.map((row: MatrixRow) => row.scenario_id))
    return pendingJudgements.filter((item: any) => ids.has(item.result?.scenario_id))
  ...
  const runAllJudges = () => {
    datasetPendingJudgements.forEach((item: any) => {
  ...
  const stats = useMemo(() => {
    ...
    return { ...base, pendingJudges: datasetPendingJudgements.length }
  }, [data, datasetPendingJudgements])

  const breadcrumbs = [
    { label: 'Datasets', path: '/datasets' },
    { label: data?.dataset?.name || `Dataset ${id}`, path: `/dataset/${id}` },
    { label: 'Analytics' },
  ]
  ...
  if (isLoading) {
    return (
      <div>
        <PageHeader breadcrumbs={breadcrumbs} title="Dataset Analytics" />
  ...
  if (!data) {
    return (
      <div>
        <PageHeader breadcrumbs={[{ label: 'Datasets', path: '/datasets' }, { label: 'Not Found' }]} title="Dataset Not Found" />
        <EmptyState title="Dataset not found" description="The dataset you're looking for doesn't exist." />
      </div>
    )
  }

  const { dataset, executors, matrix } = data
  ...
  return (
    <div>
      <PageHeader
        ...
        description={`Quality matrix for "${dataset.name}"`}
        actions={<Link to={`/dataset/${id}`}><Button variant="secondary">Edit Dataset</Button></Link>}
      />
