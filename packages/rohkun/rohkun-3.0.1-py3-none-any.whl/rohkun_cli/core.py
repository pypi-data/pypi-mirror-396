...
# Knowledge graph now uses API + UI server approach
...
def run_analysis(
    ...
    try:
        ...
        # Start knowledge graph UI server (serves HTML + API)
        from .knowledge_graph.ui_server import start_knowledge_graph_ui
        kg_ui_thread, kg_ui_port, kg_api_port = start_knowledge_graph_ui(project_path, ui_port=8003, api_port=8002)
        ...
        return AnalysisResult(
            ...
            html_graph_path=html_file,  # Include HTML graph path
            snapshot=snapshot,
            project_hash=project_info['project_hash'],
            visualization_url=f"http://localhost:{port}",
            knowledge_graph_url=f"http://localhost:{kg_ui_port}",
