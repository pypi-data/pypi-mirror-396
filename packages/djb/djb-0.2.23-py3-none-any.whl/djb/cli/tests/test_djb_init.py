...
def mock_identity():
    ...
    with patch("djb.cli.init._configure_user_identity", return_value=("Test User", "test@example.com")):
        yield
...
class TestDjbInit:
    ...
    def test_init_skip_all(self, runner, mock_subprocess_run):
        ...
        result = runner.invoke(
            djb_cli,
            ["init", "--skip-brew", "--skip-python", "--skip-frontend", "--skip-secrets", "--skip-hooks"],
        )
        assert result.exit_code == 0
    ...
    def test_init_skips_homebrew_on_windows(self, runner, mock_subprocess_run):
    ...
    def test_init_produces_no_empty_lines(self, runner, mock_subprocess_run):
        ...
        assert len(non_empty_lines) >= 5, f"Expected at least 5 non-empty lines, got {len(non_empty_lines)}"
    ...
    def test_init_adds_djb_to_installed_apps(self, runner, mock_subprocess_run, tmp_path):
...
''')
    ...
    def test_init_skips_djb_when_already_in_installed_apps(self, runner, mock_subprocess_run, tmp_path):
...
class TestInitOrUpgradeSecrets:
    ...
    def test_creates_all_environments_when_none_exist(self, runner, tmp_path):
        """Test that all environments are created when none exist."""
        from djb.cli.init import SECRETS_ENVIRONMENTS

        # Patch where it's used (in secrets.init module)
        with patch("djb.secrets.init.get_default_key_path", return_value=tmp_path / ".age" / "keys.txt"):
            result = runner.invoke(
                djb_cli,
                [
                    "init",
    ...
    def test_upgrades_existing_secrets_with_missing_keys(self, runner, tmp_path):
        """Test that existing secrets get upgraded with new template keys."""
        from djb.secrets import SecretsManager, generate_age_key
        ...
        with patch("djb.secrets.init.get_default_key_path", return_value=key_path):
            result = runner.invoke(
                djb_cli,
                [
                    "init",
    ...
    def test_reports_up_to_date_when_no_changes_needed(self, runner, tmp_path):
...
''')
        ...
        assert djb_line_idx == django_components_idx + 1, "djb should be right after django_components"
...
class TestTeamMembershipChanges:
    ...
    def test_bob_first_user_creates_secrets_with_his_key(self, tmp_path):
        ...
        from djb.secrets.init import SECRETS_ENVIRONMENTS, _get_encrypted_recipients, init_or_upgrade_secrets
        ...
        with patch("djb.secrets.init.get_default_key_path", return_value=bob_key_path):
            with patch("djb.secrets.init.logger"):
        ...
        from djb.secrets.core import parse_sops_config
    ...
    def test_bob_joins_no_warning_about_self(self, tmp_path):
        ...
        from djb.secrets.init import SECRETS_ENVIRONMENTS, init_or_upgrade_secrets
        ...
        with patch("djb.secrets.init.get_default_key_path", return_value=bob_key_path):
            with patch("djb.secrets.init.logger") as mock_logger:
    ...
    def test_alice_sees_warning_when_bob_added(self, tmp_path):
        ...
        # Alice runs djb init
        with patch("djb.secrets.init.get_default_key_path", return_value=alice_key_path):
            with patch("djb.secrets.init.logger") as mock_logger:
    ...
    def test_alice_reencrypts_for_bob(self, tmp_path):
        """Test that Alice re-encrypts secrets when Bob is added.

        After Alice sees the warning, secrets should be re-encrypted for both.
        """
        from djb.secrets import SecretsManager, generate_age_key
        from djb.secrets.core import create_sops_config
        from djb.secrets.init import SECRETS_ENVIRONMENTS, _get_encrypted_recipients, init_or_upgrade_secrets
        ...
        # Verify files are only encrypted for Alice
        dev_recipients = _get_encrypted_recipients(secrets_dir / "dev.yaml")
        assert alice_public_key in dev_recipients
        assert bob_public_key not in dev_recipients
        ...
        # After re-encryption, files should be encrypted for both
        dev_recipients = _get_encrypted_recipients(secrets_dir / "dev.yaml")
        assert alice_public_key in dev_recipients, "Alice should still be a recipient"
        assert bob_public_key in dev_recipients, "Bob should now be a recipient"
    ...
    def test_warning_when_team_member_removed(self, tmp_path):
        ...
        manager = SecretsManager(secrets_dir=secrets_dir, key_path=alice_key_path)
        for env in SECRETS_ENVIRONMENTS:
            manager.save_secrets(env, {"django_secret_key": "test"}, [alice_public_key, carol_public_key])
    ...
    def test_no_warning_or_reencrypt_when_team_unchanged(self, tmp_path):
        ...
        with patch("djb.secrets.init.get_default_key_path", return_value=alice_key_path):
            with patch("djb.secrets.init.logger") as mock_logger:
                with patch("djb.secrets.init.rotate_keys") as mock_rotate:
                    init_or_upgrade_secrets(tmp_path, email="alice@example.com")
