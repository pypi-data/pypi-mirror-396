...
def update_and_restore_editable(
    repo_root: Path,
    state: EditableState,
    new_djb_version: str | None = None,
) -> None:
    """Restore editable config, optionally with updated djb version.

    For publish: restore editable mode but with the new version specifier.
    """
    pyproject_path = repo_root / "pyproject.toml"
    uv_lock_path = repo_root / "uv.lock"

    # Start with the current pyproject.toml (which may have version updates)
    current_content = pyproject_path.read_text()

    # Add back the editable source config if it was there
    ...
    # Restore uv.lock to editable state
    if state.uv_lock_content is not None:
        uv_lock_path.write_text(state.uv_lock_content)
