...
Usage:
    djb editable-djb              # Install djb in editable mode
...
def install_editable_djb(repo_root: Path | None = None, quiet: bool = False) -> bool:
    ...
    result = run_cmd(
        ...
        cwd=repo_root,
        halt_on_fail=False,
        label=f"Installing djb in editable mode from {djb_dir}",
...
def show_status(repo_root: Path | None = None) -> None:
    ...
    if is_editable:
        ...
        if djb_dir:
            ...
            logger.info("No local djb directory found for editable installation.")
