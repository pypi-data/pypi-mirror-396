...
from djb.secrets import SecretsManager, get_default_key_path
    ...
    if is_macos and has_brew:
        ...
        for package in missing:
            if is_macos:
                ...
                logger.info(f"  See: https://github.com/getsops/sops#install" if package == "sops" else f"  See: https://github.com/FiloSottile/age#installation")
        return False
...
def init(key_path: Path | None, secrets_dir: Path | None, force: bool):
    ...
    - Creates encrypted secrets files for dev/staging/heroku_prod
...
| `heroku_prod.yaml` | Heroku production secrets |
...
djb secrets edit heroku_prod   # Edit production secrets
```

### View secrets (careful in shared environments)
```bash
djb secrets view dev
djb secrets view heroku_prod --key django_secret_key
```
...
def list_environments(secrets_dir: Path | None):
    ...
    secret_files = sorted(
        f for f in secrets_dir.glob("*.yaml") if f.name != ".sops.yaml"
    )
...
@click.argument(
    "environment",
    type=click.Choice(["dev", "staging", "heroku_prod", "all"]),
    required=False,
    default="all",
)
@click.option(
