...
def publish(part: str, dry_run: bool):
    ...
    if parent_root:
        ...
        try:
            ...
            # Restore editable mode if it was active (even on error)
            if parent_editable:
                runner.action(
                    "Restore editable djb configuration",
                    lambda: update_and_restore_editable(parent_root, parent_state, new_version),
                )
                if not dry_run:
