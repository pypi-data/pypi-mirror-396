...
class TestDjbCliModeOption:
    ...
    def test_mode_option_accepts_valid_values(self, tmp_path, monkeypatch, mode_value, expected_output):
...
class TestDjbCliModePersistence:
    ...
    def test_mode_persists_to_config_file(self, tmp_path, monkeypatch):
        """Test that --mode persists to .djb/config.yaml."""
        monkeypatch.chdir(tmp_path)
        (tmp_path / "pyproject.toml").write_text(
            '[project]\nname = "test"\ndependencies = ["djb"]\n'
        )
        ...
        config_path = tmp_path / ".djb" / "config.yaml"
