...
class TestInitOrUpgradeSecrets:
    ...
    def test_creates_all_environments_when_none_exist(self, runner, project_dir):
        ...
        with patch(
            "djb.secrets.init.get_default_key_path", return_value=project_dir / ".age" / "keys.txt"
        ):
            result = runner.invoke(
                djb_cli,
                [
        ...
        with patch("djb.secrets.init.get_default_key_path", return_value=alice_key_path):
            with patch("djb.secrets.init.logger") as mock_logger:
                with patch("djb.secrets.init.rotate_keys") as mock_rotate:
                    ...
                    assert (
                        len(team_warnings) == 0
                    ), f"Expected no team warnings, got: {team_warnings}"
    ...
    def test_bob_cannot_reencrypt_for_alice(self, tmp_path):
        ...
        with patch("djb.secrets.init.get_default_key_path", return_value=bob_key_path):
            with patch("djb.secrets.init.logger") as mock_logger:
                ...
                assert (
                    charlie_warning
                ), f"Expected warning about charlie@example.com: {warning_calls}"
