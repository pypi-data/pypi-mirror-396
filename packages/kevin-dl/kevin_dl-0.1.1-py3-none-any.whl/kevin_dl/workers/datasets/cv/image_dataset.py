...
@DATASETS.register(name=":cv:Image_Dataset")
class Image_Dataset(Dataset):
    ...
    def __init__(self, **kwargs):
        """
            参数：
                ...
                task_type:                      <str> 任务类型，有 "train" "test" "val" 可选
                                                    ...
                                                    默认设置为 "train"。
                seed:                           <int>  在生成测试数据时，亦即 task_type="test" 时将模糊等操作的随机种子固定为
        ...
        # 校验参数
        # files
        if paras.get("kvt_path", None) is not None:
            # 兼容旧接口， kvt_path 等效于 ann_file 参数
            warnings.warn(
                f"The parameter 'kvt_path' is deprecated, please use 'ann_file' instead.",
                DeprecationWarning)
            paras["ann_file"] = paras["kvt_path"]
        #
        for k in ("ann_file", "prefix"):
            if not isinstance(paras[k], (list, tuple,)):
                paras[k] = [paras[k]]
            paras[k] = [os.path.abspath(os.path.expanduser(i.strip())) for i in paras[k]]  # 替换家目录
        assert len(paras["ann_file"]) == len(paras["prefix"]) > 0
        #
        assert isinstance(paras["transforms"], (dict,))

        # 读取数据
        content_ls = []
        for ann_file, prefix in zip(paras["ann_file"], paras["prefix"]):
            file_obj = StringIO(initial_value=read_file(file_path=ann_file))
            if ann_file.endswith(".json"):
                content = json_.read(file_obj=file_obj)
            elif ann_file.endswith(".kvt"):
                _, content = kevin_notation.read(file_obj=file_obj)
            else:
                raise NotImplementedError(f"Unsupported file type: {ann_file}")
            # 加前缀
            assert "image_path" in content
            content["image_path"] = [os.path.join(prefix, i) for i in content["image_path"]]
...
if __name__ == '__main__':
    a = Image_Dataset(
        kvt_path=[
            "~/Desktop/gitlab_repos/kevin_dl/kevin_dl/workers/datasets/cv/face_attrs/processing/example/example_dataset.kvt"],
        prefix=[
            "~/Desktop/gitlab_repos/kevin_dl/kevin_dl/workers/datasets/cv/face_attrs/processing/example"],
        transforms={
            "settings": [
                # 将人脸调整至中间
                {
                    "name": ':for_images:torchvision:Pad',
                    "paras": {
                        "padding": (0, 0, 0, 30)
                    }
                },
        ...
        output_contents=["FaceYaw", "FacePitch", "FaceOcclusion", "raw", "fin", "image_path"]
    )
    print(a[0])

    Image.fromarray((a[0]['raw'])).show()
    Image.fromarray((a[0]['fin'])).show()
