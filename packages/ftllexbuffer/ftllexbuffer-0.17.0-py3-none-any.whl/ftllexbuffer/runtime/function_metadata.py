...
def should_inject_locale(func_name: str, function_registry: "FunctionRegistry") -> bool:
    ...
    # Use public API get_callable() instead of accessing private _functions
    bundle_callable = function_registry.get_callable(func_name)
    global_callable = FUNCTION_REGISTRY.get_callable(func_name)

    if bundle_callable is None or global_callable is None:
        return False

    # If the callable is the same, it's the built-in; if different, it's custom
    return bundle_callable is global_callable
