...
class OpenAICompatibleProvider(OpenAIProvider):
    ...
    """
    def __init__(self, base_url: str = None, api_key: str = None, timeout: int = 300):
        ...
        if not self.base_url:
            ...
            logging.getLogger("llm_manager").warning(
                "generic_provider_config_missing", 
                message="GENERIC_BASE_URL not set. Defaulting to localhost:11434/v1 (Ollama)."
            )
    ...
    def list_models(self):
        ...
        try:
            # We can't do asyncio.run here easily if inside an async loop.
            # But list_models in simple usage is sync for now in your tool.py? 
            # Wait, `list_models` in base.py is plain def, but usually just returns a static list or calls simple sync logic.
            # For network calls, it should be async, but your interface is currently sync `list_models()`.
            # To fix this properly, we should refactor list_models to `async def`.
