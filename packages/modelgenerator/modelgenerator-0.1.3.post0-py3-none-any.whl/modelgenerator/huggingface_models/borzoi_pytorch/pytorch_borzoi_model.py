...
class Borzoi(PreTrainedModel):
    ...
    def get_embs_before_crop(self, x):
        """
        Performs the forward pass of the model until right before the final conv layers, and includes a cropping layer.
        ...
        Returns:
             torch.Tensor: Output of the model before the cropping layer with shape (N, L, dim)
        """
        x = self.conv_dna(x)
        x_unet0 = self.res_tower(x)
        x_unet1 = self.unet1(x_unet0)
    ...
    def forward(self, x, is_human = True, data_parallel_training = False, return_only_embeddings = False, delete_crop_layer=False,):
        """
        ...
        """
        if delete_crop_layer:
            x = self.get_embs_before_crop(x)
        else:
            x = self.get_embs_after_crop(x)

        x = self.final_joined_convs(x)
        if return_only_embeddings: # return embeddings before prediction head
            return x.transpose(-1,-2) # return x (b, L, d)
