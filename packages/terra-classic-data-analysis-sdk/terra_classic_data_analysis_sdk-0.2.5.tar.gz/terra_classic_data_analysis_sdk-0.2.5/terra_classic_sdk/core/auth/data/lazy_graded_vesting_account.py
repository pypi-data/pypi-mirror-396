...
class LazyGradedVestingAccount(BaseAccount):
    ...
    def __init__(self, base_vesting_account, vesting_schedules):
        # 从 base_vesting_account 中获取父类 BaseAccount 的属性
        address = base_vesting_account.base_account.address
        public_key = base_vesting_account.base_account.public_key
        account_number = base_vesting_account.base_account.account_number
        sequence = base_vesting_account.base_account.sequence
        # 调用父类的构造函数
        super().__init__(address, public_key, account_number, sequence)
        self.base_vesting_account = base_vesting_account
