...
@click.pass_context
def run(ctx: click.Context, job_name: str, tasks: Optional[str], depth: Optional[int], quiet: bool):
...
@click.pass_context
def run_all(ctx: click.Context, type: Optional[str], pattern: Optional[str], depth: Optional[int], dry_run: bool):
    ...
    Jobs are executed in sequence, and execution stops on first failure.
    ...
    try:
        ...
        # Create ONE executor instance to track completed jobs across all runs
        console.print(f"\n[bold green]Running {len(filtered_jobs)} job(s)...[/bold green]\n")
