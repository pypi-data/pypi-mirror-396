...
""",
    ...
    SegmentSeed(
        ...
        description="Contacts who clicked a link in the previous communication 6-12 hours ago (no reply/unsubscribe) and last send was 24h+ ago.",
        code="""\
...
def apply(qs, previous_comm=None):
    ...
    # Only allow weekday morning sends to mimic "next weekday morning" delivery.
    ...
    return qs.filter(
        ...
        communications__message__time_sent__lte=last_send_lte,
    ).exclude(
...
""",
    ...
    SegmentSeed(
        ...
        description="Contacts who opened once at least 24h ago, no clicks/replies/unsubscribe.",
        code="""\
from datetime import timedelta
from django.utils import timezone
...
def apply(qs, previous_comm=None):
    ...
    return qs.filter(
        ...
        communications__message__link_clicked=False,
        communications__message__time_sent__lte=sent_lte,
    ).exclude(
        communications__metadata__status__iexact='unsubscribed',
    ).distinct()
""",
    ...
    SegmentSeed(
        ...
        description="Contacts who reopened after being cold for several days; last open 6-12h ago, sent 3d+ ago, no clicks/replies.",
        code="""\
from datetime import timedelta
from django.utils import timezone
...
def apply(qs, previous_comm=None):
    ...
    return qs.filter(
        ...
        communications__message__time_sent__lte=sent_lte,
        communications__message__time_opened__gte=F('communications__message__time_sent') + timedelta(days=3),
    ).exclude(
        communications__metadata__status__iexact='unsubscribed',
    ).distinct()
""",
