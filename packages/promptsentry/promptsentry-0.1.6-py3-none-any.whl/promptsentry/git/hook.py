...
class PreCommitHook:
    ...
    def run(self, repo_path: Optional[Path] = None) -> int:
        ...
        if allowed_files:
            ...
            # Check for new issues in allowed files (warnings)
            new_issues_count = 0
            for file_path, message in allowed_files:
                if "New issues tracked" in message or "new issue(s) detected" in message:
                    console.print(f"[yellow]⚠️  {file_path}: {message}[/]")
                    new_issues_count += 1
            
            if new_issues_count > 0:
                console.print()
                console.print("[yellow]New issues have been tracked but not blocked.[/]")
                console.print("[dim]Please fix them before the next commit to avoid blocking.[/]")
            elif fixed_count == 0:
