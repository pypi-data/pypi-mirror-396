...
class AliYunOss:
    def __init__(self, **kwargs):
        ...
        bucket_name = kwargs.pop("bucket_name", "") if kwargs.get("bucket_name") else app_config.get(
            "ALIYUN_CONNECT_CONFIG").get(
            "bucket_name")
        auth_config = {
            "access_key_id": kwargs.pop("access_key_id", "") if kwargs.get(
                "access_key_id") else app_config.get("ALIYUN_CONNECT_CONFIG").get("access_key_id"),
            "access_key_secret": kwargs.pop("access_key_secret", "") if kwargs.get(
                "access_key_secret") else app_config.get("ALIYUN_CONNECT_CONFIG").get("access_key_secret")
        }
        self.auth = oss2.Auth(**auth_config)
        self.endpoint = kwargs.pop("endpoint", "") if kwargs.get("endpoint") else app_config.get(
            "ALIYUN_CONNECT_CONFIG").get(
            "endpoint")
        config = {
            "auth": self.auth,
