...
def main():
    class BakingSwap(sp.Contract):
        ...
        def deposit(self, rate, duration):
            ...
            assert not (sp.sender in self.data.ledger)
...
if "main" in __name__:
    ...
    def test():
        ...
        delegator.set_initial_balance(sp.tez(1_000))
        sc += delegator
        ...
        if sc.simulation_mode() is sp.SimulationMode.MOCKUP:
            sc.p("set delegate doesn't work in mockups")
            return
        ...
        sc.verify(delegator.balance == sp.tez(1000 + 10))
