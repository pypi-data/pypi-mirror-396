...
class Scenario:
    ...
    def _get_dependencies(self, m):
        ...
        calling_file = get_file_line_no()
        nm = self._clean(make_relative(calling_file.filename))
        sections.append((nm, [ky for ky in self._FILE_DEPENDENCIES.keys() if ky != nm]))
...
def test_scenario(name=None, modules=None):
    load_library_if_needed()
    if modules is None:
        modules = []
    scenario = Scenario()
