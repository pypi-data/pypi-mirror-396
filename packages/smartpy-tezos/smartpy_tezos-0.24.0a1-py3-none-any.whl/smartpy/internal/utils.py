...
def get_file_line_no(line_no=None):
    ...
    while frame:
        ...
        # Skip frames that are part of pytest internals
        if not is_within_smartpy and "<frozen " not in fn and "init" != fn:
            parts = list(pathlib.Path(fn).parts)
            base = pathlib.Path(fn).name
            is_pytest_internal = any(
                p.startswith("_pytest") for p in parts
            ) or base.startswith("_pytest")
            if not is_pytest_internal:
                if ":" in fn:
                    fn = fn[fn.rindex(":") + 1 :]
