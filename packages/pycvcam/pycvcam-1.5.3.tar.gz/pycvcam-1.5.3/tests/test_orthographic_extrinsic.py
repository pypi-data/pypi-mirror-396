...
def default():
    ...
    return OrthographicExtrinsic.from_rt(rvec, tvec)
...
def test_transform_2d_points(default):
    ...
    expected_normalized = cam_points[:, :2]
...
def test_jacobian_analytic_numeric_match(default):
    ...
    for i in range(len(dp_labels)):
        ...
        extrinsic_plus = OrthographicExtrinsic()
...
def test_rays_creation(default):
    ...
    def ray_one_point(point, rvec, tvec):
        ...
        unit_ray = numpy.array([0, 0, 1])  # For orthographic, rays are parallel along Z-axis
        world_ray = rmat.T @ unit_ray  # Rotate the ray direction to world coordinates
        ray = numpy.zeros((6,), dtype=numpy.float64)
        ray[:3] = world  # World coordinates of the point
        ray[3:] = world_ray  # Direction of the ray in world coordinates
        return ray
