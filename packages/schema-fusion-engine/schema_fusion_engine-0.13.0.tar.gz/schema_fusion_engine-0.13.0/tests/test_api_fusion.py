...
class TestFusionViewEndpoint:
    ...
    @patch("src.api.fusion.trino_client")
    def test_list_views(self, mock_client, client: TestClient):
    ...
    @patch("src.api.fusion.trino_client")
    def test_delete_view(self, mock_client, client: TestClient):
        ...
        assert "DROP VIEW" in mock_client.execute_query.call_args[0][0]

    @patch("src.api.fusion.trino_client")
    def test_delete_view_error(self, mock_client, client: TestClient):
        """Test error handling when deleting a view."""
        mock_client.execute_query.side_effect = Exception("View does not exist")
