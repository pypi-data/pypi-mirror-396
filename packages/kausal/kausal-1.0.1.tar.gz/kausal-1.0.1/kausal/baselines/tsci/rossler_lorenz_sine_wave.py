...
for trial in range(N_trials):
    ...
    for sigma in tqdm(sigmas):
        ...
        sine_wave = np.sin(np.linspace(0, 110, 8000)).reshape(-1, 1)
        x_signal = x_signal + sine_wave * sigma
        y_signal = y_signal + sine_wave * sigma
        ...
        r_x2y, r_y2x = tsci.tsci_nn(
            x_state, y_state, dx_state, dy_state, fraction_train=0.8
        )
...
np.savez(
    f"results/{seed_time}_sin_wave.npz",
    res_x2ys_np,
    res_y2xs_np,
    res_x2ys_cutoffs_np,
    res_y2xs_cutoffs_np,
