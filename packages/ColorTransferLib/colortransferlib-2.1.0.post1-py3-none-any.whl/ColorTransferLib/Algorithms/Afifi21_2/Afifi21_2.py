...
from .third_party.utils.face_preprocessing import face_extraction
from .third_party.rehistoGAN import train_from_folder
...
class Afifi21_2:
    ...
    def apply(src, ref, opt):
        ...
        if src.get_type() == "Image":
            out_obj = Afifi21_2.__apply_image(src, ref, opt)
        elif src.get_type() == "LightField":
            out_obj = Afifi21_2.__apply_lightfield(src, ref, opt)
        elif src.get_type() == "Video":
            out_obj = Afifi21_2.__apply_video(src, ref, opt)
        elif src.get_type() == "VolumetricVideo":
            out_obj = Afifi21_2.__apply_volumetricvideo(src, ref, opt)
        elif src.get_type() == "Mesh":
            out_obj = Afifi21_2.__apply_mesh(src, ref, opt)
        else:
    ...
    def __color_transfer(img_src, img_ref, opt):
        opt.models_dir = os.path.join(get_cache_dir(), "Afifi21_2")
        opt.histGAN_models_dir = os.path.join(opt.models_dir, "Afifi21_2")
    ...
    def __apply_image(src, ref, opt):
        ...
        out_colors = Afifi21_2.__color_transfer(src_img, ref_img, opt)
    ...
    def __apply_video(src, ref, opt): 
        ...
        for i, src_raw in enumerate(src_raws):
            ...
            out_colors = Afifi21_2.__color_transfer(src_raw, ref_raw, opt)
