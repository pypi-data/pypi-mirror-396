...
from .third_party.psnet import PSNet
...
class Cao20:
    ...
    def apply(src, ref, opt):
        ...
        if src.get_type() == "PointCloud":
            out_obj = Cao20.__apply_pointcloud(src, ref, opt)
        else:
            out_obj = None
    ...
    def __color_transfer(src, ref, opt): 
        ...
        model_file_paths = init_model_files("Cao20", [
        ...
        np.random.seed(42)
        # trained_model = "Models/Cao20/model"
        trained_model = os.path.join(get_cache_dir(), "Cao20/model")

        config = configparser.ConfigParser()
        config.read(model_file_paths["base_config.txt"])
    ...
    def __apply_pointcloud(src, ref, opt):
        ...
        # sys.stdout = devnull
        out = Cao20.__color_transfer(src, ref, opt)
