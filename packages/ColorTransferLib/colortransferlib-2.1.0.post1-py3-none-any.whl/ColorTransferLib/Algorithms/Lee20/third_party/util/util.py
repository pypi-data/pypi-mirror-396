...
def LAB2RGB(I):
    ...
    lab = np.dstack([l, a, b]).astype(np.float64)

    # Clamp to a valid Lab range before conversion to avoid negative/invalid XYZ
    lab[..., 0] = np.clip(lab[..., 0], 0.0, 100.0)
    lab[..., 1] = np.clip(lab[..., 1], -86.1830297444, 98.2330538631)
    lab[..., 2] = np.clip(lab[..., 2], -107.857300207, 94.4781222765)
