...
class YOLOSegmenter(YOLOWorker):
    ...
    def offset_elements(self, parent: Element, elements: list[dict]) -> None:
    ...
    def process_element(self, element: Element) -> None:
        ...
        if children:
            children = list(map(asdict, children))
            self.offset_elements(element, children)
            logger.info("Publishing elements on Arkindex...")
            self.create_elements(parent=element, elements=children)
        else:
