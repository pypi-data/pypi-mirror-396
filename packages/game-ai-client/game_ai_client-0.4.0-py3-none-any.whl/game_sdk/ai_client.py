...
class TurnBasedGame(ABC):
    ...
    def ai_vs_ai_difficulty_selection(
        ...
        difficulty settings play against each other using AIGameClient instances.
        ...
        if client is None:
            raise ValueError(
                "ai_vs_ai_difficulty_selection now requires a GameClient instance. "
                "Please pass a valid GameClient instead of None."
            )

        # Map difficulty to iterations and epsilon (randomness)
        def get_difficulty_config(difficulty: str) -> tuple[int, float]:
            """Returns (iterations, epsilon) for a given difficulty."""
            difficulty_map = {
                "easy": (10, 0.85),    # 10 iterations, 85% random moves
                "medium": (1000, 0.0), # 1000 iterations, no randomness
                "hard": (5000, 0.0),   # 5000 iterations, no randomness
            }
            config = difficulty_map.get(difficulty.lower())
            if config is None:
                raise ValueError(
                    f"Unknown difficulty '{difficulty}'. "
                    f"Available options: {', '.join(difficulty_map.keys())}"
                )
            return config

        # Create the apply_move function for AIGameClient
        ...
        iterations1, epsilon1 = get_difficulty_config(difficulty1)
        iterations2, epsilon2 = get_difficulty_config(difficulty2)

        # Create two AIGameClient instances (one for each player)
        ai_client1 = AIGameClient(
            game_id=game_id,
            api_key=f"{client.api_key}-P1",
        ...
        ai_client2 = AIGameClient(
            game_id=game_id,
            api_key=f"{client.api_key}-P2",
            apply_move_fn=apply_move_fn,
            base_url=client.base_url,
            epsilon=epsilon2,
            exploration_c=1.4,
        ...
        while not current_game.is_game_over():
            ...
            # Select appropriate AI client and iterations
            ai_client = ai_client1 if current_player_idx == 0 else ai_client2
            iterations = iterations1 if current_player_idx == 0 else iterations2

            if verbose:
                print(f"Move {move_count + 1}: {current_player['id']} at '{current_difficulty}' difficulty")

            # Standard AIGameClient flow: send_state -> best_move -> log_move
            ai_client.send_state(match_id, state)
            move = ai_client.best_move(match_id, iterations=iterations)
            ...
            # Record move with state for database persistence
            move_history.append({
                "player": current_player["id"],
                "move": move,
                "difficulty": current_difficulty,
                "state": state  # Add state for database logging
            })
            ...
            # Log move (now after send_state, following standard pattern)
        ...
        # Build final_state for the result (even if no client provided)
        final_state_for_result = game_to_state_fn(current_game, {
            "game_id": game_id,
            "players": players_with_difficulty,
            "turn_index": current_player_idx,
            "move_count": move_count,
        ...
        # Add result to final_state if not present
        if "result" not in final_state_for_result:
            result_dict_inner = {}
            if winner:
                for idx, p in enumerate(players_with_difficulty):
                    result_dict_inner[p.get("id", f"P{idx+1}")] = 1.0 if p.get("symbol") == winner else 0.0
            else:
                for p in players_with_difficulty:
                    result_dict_inner[p.get("id")] = 0.5
            final_state_for_result["result"] = result_dict_inner
        ...
        result_dict = {
            ...
            "final_state": final_state_for_result  # Add final_state for database persistence
        }
