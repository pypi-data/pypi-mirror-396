...
class MCTSStrategy(SearchAlgorithmStrategy):
    ...
    def __init__(
        ...
        # Tree reuse: cache the root node between searches
        self.enable_tree_reuse = enable_tree_reuse
    ...
    def search(self, root_state: Any, iterations: int, root_player: Optional[int] = None) -> Any:
        ...
        Supports optional tree reuse: if your game implements __eq__, the tree
        will be reused between searches for better performance. If not, it works
        normally without tree reuse.
        ...
        # Try to reuse tree from previous search (automatic if __eq__ implemented)
        root = self._find_or_create_root(root_state, root_player)
        ...
        # Cache the best child for next search (if tree reuse enabled)
        if self.enable_tree_reuse:
            self._cached_root = best_child
            # Detach from parent to make it the new root and avoid memory leaks
            best_child.parent = None

        # Return the original move dict stored in the child
        return best_child.move_from_parent

    def _find_or_create_root(self, root_state: Any, root_player: int) -> Node:
        """
        Try to reuse tree from previous search, or create new root.

        Tree reuse is AUTOMATIC and OPTIONAL:
        - If game implements __eq__: tree reuse works automatically
        - If game doesn't implement __eq__: silently creates new root (no errors)

        Returns:
            Node to use as root for this search
        """
        if not self.enable_tree_reuse or self._cached_root is None:
            # No cache or tree reuse disabled
            return self._create_new_root(root_state, root_player)

        # Try exact match: cached root is current state
        if self._states_match(self._cached_root.state, root_state):
            # Tree reuse! (only happens if __eq__ is implemented)
            return self._cached_root

        # Try subtree reuse: current state might be in cached root's children
        # This happens when opponent played a move we explored
        for child in self._cached_root.children.values():
            if self._states_match(child.state, root_state):
                # Subtree reuse! (only happens if __eq__ is implemented)
                child.parent = None  # Detach from old parent
                return child

        # No match found or __eq__ not implemented - create new root
        return self._create_new_root(root_state, root_player)

    def _create_new_root(self, root_state: Any, root_player: int) -> Node:
        """Create a new root node from scratch."""
        return Node(
            state=root_state,
    ...
    def _states_match(self, state1: Any, state2: Any) -> bool:
        """
        Check if two game states are equivalent.

        Gracefully handles cases where __eq__ is not implemented.
        Tree reuse only works if game objects properly implement __eq__.
        ...
        Returns:
            True if states are equivalent, False otherwise (including if __eq__ not implemented)
        """
        # Identity check (fastest)
        if state1 is state2:
            return True

        # Try equality check (requires __eq__ implementation)
        try:
            # Check if __eq__ is actually implemented (not just inherited from object)
            if hasattr(type(state1), '__eq__') and type(state1).__eq__ is not object.__eq__:
                return state1 == state2
        except (TypeError, AttributeError, Exception):
            # Comparison failed or not implemented properly - that's OK!
            pass

        # No __eq__ implemented or comparison failed - assume different
        # This is fine! Game just won't benefit from tree reuse
        return False
    ...
    def clear_cache(self):
        ...
        Call this between games or when you want to force a fresh search.
        """
...
class MCTS:
    ...
    def __init__(
        ...
        Args:
            ...
            enable_tree_reuse: Enable tree reuse between searches (default: True)
                              Automatic: works if game implements __eq__, no errors otherwise
        """
        if env is not None:
            import warnings
    ...
    def clear_cache(self):
        """Clear the cached tree. Call between games or to force fresh search."""
