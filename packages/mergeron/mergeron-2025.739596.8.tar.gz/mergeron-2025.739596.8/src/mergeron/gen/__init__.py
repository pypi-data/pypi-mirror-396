...
class MarketsData:
    ...
    def _appd(_i: MarketsData) -> ArrayDouble:
    ...
    @fcounts.default
    def _fcd(_i: MarketsData) -> ArrayINT:
    ...
    def _nfsd(_i: MarketsData) -> ArrayDouble:
        retval: ArrayDouble = np.empty_like(_i.frmshr_array[:, :1], float)
        retval.fill(np.nan)
        return retval
    ...
    @hhi_post.default
    def _hpd(_i: MarketsData) -> ArrayDouble:
        retval: ArrayDouble = np.empty_like(_i.frmshr_array[:, :1], float)
        retval.fill(np.nan)
        return retval
...
class UPPTestsCounts:
    ...
    ByFirmCount: ArrayBIGINT = field(eq=cmp_using(eq=np.array_equal))
    ByDelta: ArrayBIGINT = field(eq=cmp_using(eq=np.array_equal))
    ByHHIandDelta: ArrayBIGINT = field(eq=cmp_using(eq=np.array_equal))
