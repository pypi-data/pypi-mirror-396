...
#SBATCH hetjob
#SBATCH -N 2 --exclusive --cpus-per-task=2
#SBATCH hetjob
#SBATCH -N 1 --exclusive
...
# and client are given an exclusive node each. Although they generally do not
# need the resources of an exclusive node, there are currently unidentified
# stability issues when using shared nodes for the scheduler and client.
...
MEM_PER_WORKER=$(echo "$SLURM_CPUS_PER_TASK_HET_GROUP_1 * $SLURM_MEM_PER_CPU_HET_GROUP_1 * .9" | bc -l)
