...
class IncrementalVocEvaluator:
    ...
    def optimize_serialized_size(self) -> None:
        """
        Optimize the member variables to take up as little memory as possible when the object is serialized.

        The partial results are stored as lists of numpy.ndarray's (one array per image) and concatenation is done at
        the end rather than for each image. Pickling an object of the `IncrementalVocEvaluator` class wastes large
        amounts of space if the number of images is large, as each array incurs significant overhead. This method
        concatenates the arrays and makes subsequent pickling efficient.
        """

        def concatenate_to_single_array(list_of_arrays):
            return [np.concatenate(list_of_arrays)]

        self._all_matched_classes_and_scores = concatenate_to_single_array(self._all_matched_classes_and_scores)

        for c in range(self._num_classes):
            self._tp_fp_labels_per_class[c] = concatenate_to_single_array(self._tp_fp_labels_per_class[c])
            self._scores_per_class[c] = concatenate_to_single_array(self._scores_per_class[c])
            self._image_indexes_per_class[c] = concatenate_to_single_array(self._image_indexes_per_class[c])
