...
def validate_gpu_sku(device: str, validate_free_gpu_mem: bool = True, min_gpu_mem: float = MIN_GPU_MEM) -> None:
    ...
    # Reducing another 200 MB since yolo weights are loaded before this check.
    # Checking if atleast 10800 MB is availble.
    min_free_gpu_memory = max(0, min_gpu_mem - 600 * MEGABYTE)
    pynvml.nvmlInit()
    try:
        is_valid = True
        per_device_infos = []
