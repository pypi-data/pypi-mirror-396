...
class RuleBasedDefectDetector:
    ...
    def _evaluate_spectrum_rule(
        ...
        if has_sidebands and sidebands:
            ...
            if modulating_freq_type:
                ...
                # Если группа пустая, результат не учитывается (None), а не True
                left_passed = check_sideband_group(left_sidebands) if left_sidebands else None
                right_passed = check_sideband_group(right_sidebands) if right_sidebands else None
                
                # Применяем оператор между группами
                # Логика:
                # - AND: основная частота И все левые И все правые (все непустые группы должны пройти)
                # - OR: (основная частота И все левые) ИЛИ (основная частота И все правые) (хотя бы одна непустая группа должна пройти)
                if sideband_operator == 'OR':
                    # OR: хотя бы одна из непустых групп должна пройти вместе с основной
                    left_result = (main_passed and left_passed) if left_passed is not None else False
                    right_result = (main_passed and right_passed) if right_passed is not None else False
                    # Если обе группы пустые, правило не выполнено
                    if left_passed is None and right_passed is None:
                        sidebands_passed = False
                    else:
                        sidebands_passed = left_result or right_result
                else:  # AND (по умолчанию)
                    # AND: все непустые группы должны пройти вместе с основной
                    left_result = left_passed if left_passed is not None else True
                    right_result = right_passed if right_passed is not None else True
                    sidebands_passed = main_passed and left_result and right_result
    ...
    def _calculate_defect_strength_by_criteria(
        ...
        # ВАЖНО: Критерии вычисляются только если корневая группа прошла оценку
        # Если корневая группа не прошла (например, оператор AND и не все правила выполнены),
        # то дефект не обнаружен, независимо от индивидуальных правил
        if not evaluation_result.root_group_result.passed:
            return None, {}
