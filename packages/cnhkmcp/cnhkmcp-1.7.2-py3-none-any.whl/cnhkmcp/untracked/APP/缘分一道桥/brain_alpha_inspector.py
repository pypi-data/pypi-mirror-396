...
try:
    from ace_lib import start_simulation, simulation_progress
...
def find_common_availabilities(
    ...
    for field in datafields:
        ...
        # If field not found or has no availability, the intersection is empty
        if info.get("error") or not info.get("availability"):
            print(f"Field '{field}' has no availability or not found. Intersection is empty.")
            return []
...
def get_alpha_variants(
    session: requests.Session, alpha: Dict, operators: List[Dict]
) -> Dict:
    ...
    for new_setting in common_settings:
        ...
        if not is_duplicate:
            # Construct full simulation payload
...
def main() -> None:
    ...
    for i, alpha in enumerate(alphas):
        new_alphas = process_alpha_variants(session, alpha, operators)
