...
function updateSettingsUIWithOptions() {
    ...
    // 2. Convert Universe and Neutralization to Select if they are inputs
    convertInputToSelect('universe');
    convertInputToSelect('neutralization');
...
function convertInputToSelect(settingName) {
    const input = document.querySelector(`input[data-setting="${settingName}"]`);
    if (input) {
        const select = document.createElement('select');
        select.className = 'setting-value-input';
        select.setAttribute('data-setting', settingName);
        ...
        select.addEventListener('change', updateTotalCombinations);
    }
}
