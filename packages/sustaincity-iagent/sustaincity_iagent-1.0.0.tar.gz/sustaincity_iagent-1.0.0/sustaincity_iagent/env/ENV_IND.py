...
class MSWEnv(gym.Env):
    ...
    def load_params_from_excel(self, file_path):
        df = pd.read_excel(file_path, sheet_name='per_IND', header=None)
        disposal_params = {
        ...
        transport_params.update({
            ...
            'collect_energy_input': df.iat[12, 6],   
            'collect_carbon':       df.iat[16, 6] + df.iat[19, 6],    
            'collect_cost':         61.88,   

            
            'classify_energy_input': df.iat[12, 6],
            'classify_carbon':       df.iat[16, 6] + df.iat[19, 6],
            'classify_cost':         61.88,
        })
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.SSP5C_IP1_MIN = 0.8         
        self.SSP5C_COLLECT_PEN_W = 500.0   

        
        self.SSP5C_CLASS_W = 20.0          
        ...
        self.SSP4_CARBON_W = 50     
        self.SSP4_SLACK_W  = 0      

    
        self.SSP4_ENERGY_W   = 0   # total_energy_output
        self.SSP4_COST_W     = 100.0   
        self.SSP4_RESOURCE_W = 0   # total_resource_output
        ...
        self.SSP4_ENERGY_INPUT_W = 50
        self.SSP4_CAPS.update({
            'energy_in_norm_cap': 5.0,    
        })
    # ============================================

        self.SSP3_CARBON_W = 100.0   
        self.SSP3_SLACK_W  = 0    
        ...
        # —— SSP4: force re-init each episode
        self._ssp4wm_inited = False
    ...
    def step(self, action):
        ...
        if self.reward_mode == 'carbon':
            ...
            # Build a metrics namespace for the potential-based reward
            curr_metrics = SimpleNamespace(
                total_carbon=float(total_carbon),
                total_cost=float(total_cost),
                total_energy_input=float(total_energy_input)
            )
            # done flag if this is the last time step in the episode
            done_flag = (self.current_step >= self.max_steps)
            reward = self._reward_ssp4_weighted_multi(curr_metrics, action, done_flag)

            # Optional: expose targets/budgets for logging
            try:
                ssp4_info = {
                    'c_score': float(max(0.0, (total_carbon - self.C_target) / (self.C0 - self.C_target + 1e-8))),
                    'c_overshoot': float(max(0.0, (total_carbon - self.C0) / (self.C0 + 1e-8))),
                    'cost_ratio': float(total_cost / (self.K_budget + 1e-8)),
                    'ein_ratio': float(total_energy_input / (self.E_budget + 1e-8)),
                }
            except Exception:
                pass
    ...
    def compute_metrics(self, state):
        ...
        tp = self.transport_params

        
        if controlled_mass > 0:
            total_energy_input += controlled_mass * float(tp.get('collect_energy_input', 0.0))
            total_carbon       += controlled_mass * float(tp.get('collect_carbon', 0.0))
            total_cost         += controlled_mass * float(tp.get('collect_cost', 0.0))

        
        if classified_mass > 0:
            total_energy_input += classified_mass * float(tp.get('classify_energy_input', 0.0))
            total_carbon       += classified_mass * float(tp.get('classify_carbon', 0.0))
            total_cost         += classified_mass * float(tp.get('classify_cost', 0.0))
    ...
    def _delta_incineration_pos(self) -> float:
        ...
        # ===== SSP4 weighted multi · potential-based reward helpers =====
    def _ssp4wm_default_params(self):
        # Targeted low-carbon level (platform), budgets, and weights
        self.theta = getattr(self, 'theta', 0.50)  # target carbon = theta * C0
        self.carbon_feasible_floor_ratio = getattr(self, 'carbon_feasible_floor_ratio', 0.20)

        # Budgets as fractions of episode baselines (unless explicitly set)
        self.k_budget_frac = getattr(self, 'k_budget_frac', 0.65)  # ≤ 80% of baseline cost
        self.e_budget_frac = getattr(self, 'e_budget_frac', 0.65)  # ≤ 80% of baseline energy input

        # Potential weights (constraints slightly heavier than carbon)
        self.wC = getattr(self, 'wC', 1.0)
        self.wK = getattr(self, 'wK', 2.5)
        self.wE = getattr(self, 'wE', 2.5)

        # Reward shaping
        self.tau = getattr(self, 'tau', 0.20)            # tanh scale
        self.R_done = getattr(self, 'R_done', 0.80)      # terminal bonus when all satisfied
        self.lambda_term = getattr(self, 'lambda_term', 1.0)  # terminal penalty multiplier

        # Small action-smoothing penalty
        self.rho = getattr(self, 'rho', 0.002)
        # —— new: floors & weights for IP1/IP2
        self.i1_floor = getattr(self, 'i1_floor', 0.10)  
        self.i2_floor = getattr(self, 'i2_floor', 0.10)  
        self.wI1      = getattr(self, 'wI1', 1.2)        
        self.wI2      = getattr(self, 'wI2', 0.8)        

        self.soft_margin_K = getattr(self, 'soft_margin_K', 0.90)  
        self.soft_margin_E = getattr(self, 'soft_margin_E', 0.90)  
        self.wC_overshoot  = getattr(self, 'wC_overshoot', 1.0)    

        # Floors for collection/classification (soft)
        self.i1_floor = getattr(self, 'i1_floor', 0.10)   
        self.i2_floor = getattr(self, 'i2_floor', 0.05)   

        # Penalty weights for floors & vacuous classification
        self.wI1 = getattr(self, 'wI1', 2.0)   
        self.wI2 = getattr(self, 'wI2', 0.8)   
        self.wVac = getattr(self, 'wVac', 1.2) 

        
        self.anchor_w = getattr(self, 'anchor_w', 0.08)

    def _ssp4wm_clamp01(self, x: float) -> float:
        if x < 0.0:
            return 0.0
        if x > 1.0:
            return 1.0
        return x
    def _ssp4wm_cscore(self, carbon: float) -> float:
        EPS = 1e-8
        if self.C0 <= self.C_target + EPS:
            return 0.0
        return self._ssp4wm_clamp01((float(carbon) - self.C_target) / (self.C0 - self.C_target + EPS))

    def _ssp4wm_begin_episode(self):
        EPS = 1e-8
        # Baseline scales
        self.C0 = max(EPS, abs(float(getattr(self, 'initial_carbon', 0.0))))
        self.C_feasible_floor = self.carbon_feasible_floor_ratio * self.C0
        self.C_target = max(self.C_feasible_floor, self.theta * self.C0)

        # Budgets: prefer explicit values if provided; otherwise derive from baselines
        init_cost = max(EPS, float(getattr(self, 'initial_cost', 0.0)))
        init_ein  = max(EPS, float(getattr(self, 'initial_energy_input', 0.0)))

        if not hasattr(self, 'K_budget') or self.K_budget is None:
            self.K_budget = self.k_budget_frac * init_cost
        if not hasattr(self, 'E_budget') or self.E_budget is None:
            self.E_budget = self.e_budget_frac * init_ein

        # Start previous metrics from baselines
        self.prev_metrics = SimpleNamespace(
            total_carbon=float(getattr(self, 'initial_carbon', 0.0)),
            total_cost=float(getattr(self, 'initial_cost', 0.0)),
            total_energy_input=float(getattr(self, 'initial_energy_input', 0.0))
        )
        self.prev_action = None
        self._ssp4wm_inited = True

    def _ssp4wm_score(self, metrics) -> float:
        EPS = 1e-8
        carbon = float(metrics.total_carbon)
        cost   = float(metrics.total_cost)
        energy = float(metrics.total_energy_input)

        
        wC   = getattr(self, 'wC', 1.0)
        wC2  = getattr(self, 'wC_overshoot', 1.0)   
        wK   = getattr(self, 'wK', 2.5)
        wE   = getattr(self, 'wE', 2.5)
        i1_floor = getattr(self, 'i1_floor', 0.10)
        i2_floor = getattr(self, 'i2_floor', 0.05)
        wI1 = getattr(self, 'wI1', 2.0)    
        wI2 = getattr(self, 'wI2', 0.8)    
        wVac = getattr(self, 'wVac', 1.2)  
        mK = getattr(self, 'soft_margin_K', 0.90)   
        mE = getattr(self, 'soft_margin_E', 0.90)

        
        if self.C0 <= self.C_target + EPS:
            c_score = 0.0
        else:
            c_score = max(0.0, (carbon - self.C_target) / (self.C0 - self.C_target + EPS))

        
        c_overshoot = max(0.0, (carbon - self.C0) / (self.C0 + EPS))

        
        k_violation = max(0.0, (cost   - mK * self.K_budget) / ((1.0 - mK) * self.K_budget + EPS))
        e_violation = max(0.0, (energy - mE * self.E_budget) / ((1.0 - mE) * self.E_budget + EPS))

        
        ip1 = float(np.clip(getattr(self, 'ip1', 0.0), 0.0, 1.0))
        ip2 = float(np.clip(getattr(self, 'ip2', 0.0), 0.0, 1.0))

        
        i1_deficit = max(0.0, (i1_floor - ip1) / max(i1_floor, EPS))
        phi_i1 = wI1 * (i1_deficit ** 2)

        
        gate_ip2 = 1.0 if ip1 >= (0.5 * i1_floor) else 0.0
        i2_deficit = max(0.0, (i2_floor - ip2) / max(i2_floor, EPS))
        phi_i2 = wI2 * (i2_deficit ** 2) * gate_ip2

        
        vacuous_pen = wVac * (ip2 * max(0.0, 1.0 - ip1))
    ...
    def _reward_ssp4_weighted_multi(self, curr_metrics, curr_action, done: bool) -> float:
        # Lazy init of params and per-episode baselines
        if not hasattr(self, '_ssp4wm_inited') or not self._ssp4wm_inited:
            self._ssp4wm_default_params()
            self._ssp4wm_begin_episode()

        Phi_prev = self._ssp4wm_score(self.prev_metrics)
        Phi_curr = self._ssp4wm_score(curr_metrics)

        # Reward is potential drop, squashed to (-1,1)
        r = math.tanh((Phi_prev - Phi_curr) / (self.tau + 1e-8))

        # Dense carbon anchor: discourage staying at very high carbon even if no change
        try:
            c_anchor = self._ssp4wm_cscore(curr_metrics.total_carbon)
            r -= self.anchor_w * c_anchor
        except Exception:
            pass

        # Small action variation penalty
        if getattr(self, 'prev_action', None) is not None and curr_action is not None:
            try:
                r -= self.rho * float(np.abs(np.asarray(curr_action) - np.asarray(self.prev_action)).sum())
            except Exception:
                pass
        ...
        if done:
            EPS = 1e-8
            ok_cost   = float(curr_metrics.total_cost)        <= self.K_budget + EPS
            ok_energy = float(curr_metrics.total_energy_input) <= self.E_budget + EPS
            ok_carbon = float(curr_metrics.total_carbon)      <= self.C_target + EPS
            if ok_cost and ok_energy and ok_carbon:
                r += self.R_done
            else:
                r -= self.lambda_term * self._ssp4wm_score(curr_metrics)
