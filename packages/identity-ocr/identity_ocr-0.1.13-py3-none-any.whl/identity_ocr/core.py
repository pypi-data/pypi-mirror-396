...
def read_passport(image_path, crop_mrz=True, extra_fields=False):
    ...
    try:
        ...
        # Reverting to 1000px as 600px caused accuracy issues (misread separators)
        ...
        # Optimize: Crop out the photo (usually on the left).
        # We assume the photo takes up the left ~35% of the width.
        # This saves ~35% of OCR processing on the visual zone.
        vz_h, vz_w = visual_zone_img.shape[:2]
        visual_zone_img = visual_zone_img[:, int(vz_w * 0.35):]
        
        # Further optimize: Resize visual zone if it's too big
        # 350px width is usually enough for "Place of Issue" (large text)
        vz_h, vz_w = visual_zone_img.shape[:2]
        if vz_w > 350:
            scale = 350 / vz_w
