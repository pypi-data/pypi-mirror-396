...
class StorageService(OSSMixin):
    ...
    def upload_directory(
        ...
        for i, file_path in enumerate(files, 1):
            try:
                ...
                # 上传单个文件（key已经包含完整路径，传递task_hash以跳过重复检测）
                result = self.upload_file(
                    ...
                    key,
                    sample_id,
                    task_hash=task_hash,  # 传递已检测的 task_hash，避免重复检测
                )
    ...
    def _build_oss_key(
        ...
        - 有 task_hash：biofile/{env}/{project_id}/{analysis_node}/{bucket}/{suffix}/{filename}
        - 无 task_hash：biofile/{env}/{project_id}/{base_path}/{filename}
        ...
        Args:
            ...
            sample_id: 样本ID（可选，有 task_hash 时在 OSS 路径中被忽略）
        ...
        Example:
            ...
            "biofile/media/proj_001/2f/0075a6/data.csv"  # 有 task_hash 时
        """
        # 使用环境相关的路径前缀
        ...
        if task_hash:
            # 策略 A：有 task_hash，用 task_hash 替代 base_path/sample_id
            if analysis_node:
