...
class BaseAgent(Protocol):
    ...
    agent: BaseAgent = SimpleAgent(name="assistant", llm=llm)
...
def create_agent(
    ...
    **kwargs,
    ...
    Args:
        ...
        tools: 工具列表
        agent_type: Agent 类型
            - "simple": SimpleAgent（基础递归）
            - "react": ReactAgent（推理+行动）
    ...
    if agent_type == "simple":
        ...
        return SimpleAgent(name=name, llm=llm, tools=tools, **kwargs)

    elif agent_type == "react":
        from loom.agents.react import ReactAgent

        return ReactAgent(name=name, llm=llm, tools=tools, **kwargs)
