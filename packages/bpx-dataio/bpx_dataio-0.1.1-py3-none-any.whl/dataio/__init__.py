...
class SAGEMAKER_INFO:
    ...
    def get_sagemaker_session(self):
        """
        Get SageMaker session-like object (using boto3 only).
        Returns the boto3 session as a dictionary-like object for compatibility.
        """
        # Get default bucket name (SageMaker convention: sagemaker-{region}-{account-id})
        # Or use environment variable if available
        default_bucket_name = os.environ.get('SAGEMAKER_BUCKET')
        if not default_bucket_name:
            default_bucket_name = f"sagemaker-{self.region}-{self.account_id}"
        
        self.default_bucket = default_bucket_name
        
        # Create a simple class to mimic sagemaker session
        # Use a class to allow method calls like .default_bucket()
        class SageMakerSessionWrapper:
            def __init__(self, boto_session, default_bucket, region):
        ...
        self.sagemaker_session = SageMakerSessionWrapper(
            self.boto_session, 
            self.default_bucket,
            self.region
        )
    ...
    def get_featurestore_session(self):
        """
        Get SageMaker Feature Store runtime client.
        Returns a dict-like object with the featurestore runtime client for compatibility.
        """
        # feature store runtime
        self.featurestore_runtime = self.boto_session.client(
            service_name='sagemaker-featurestore-runtime',  
            region_name=self.region
        )
        
        # Create a simple dict-like object to mimic sagemaker Session
        self.featurestore_session = {
    ...
    def get_execution_role(self):
        """
        Get SageMaker execution role ARN by resolving from IAM roles.
        Looks for roles starting with 'AmazonSageMaker-ExecutionRole-'.
        """
        try:
            ...
            if self.verbose:
                print(f'Error retrieving role_arn: {e}')
            return None
