...
""".strip()
    ...
    if existing_reports:
        sarif_paths = [
            normalize_sonar_path(f"reports/{report}") for report in existing_reports
        ]
...
def run_dotnet_scan(project_key, sonar_url, token, project_root, env):
    ...
    try:
        # Step 1: Begin
        click.echo("1️⃣  Beginning .NET SonarScanner...")
        begin_cmd = [
            "dotnet",
            "sonarscanner",
            "begin",
        ...
        # Step 2: Build
        click.echo("2️⃣  Building .NET project...")
        subprocess.run(["dotnet", "build"], cwd=project_root, env=env, check=True)
