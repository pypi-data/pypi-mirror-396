...
## 🐍 API FiloAni (Python)
Paquete publicado en PyPI: `filoani`. Ofrece un cliente para JkAnime y una API unificada con AnimeFLV.

### Instalación
```shell
pip install filoani
pip install filoani[server]           # Endpoints HTTP opcionales
pip install filoani[animeflv]         # Soporte proveedor AnimeFLV
pip install filoani[fuzzy]            # Coincidencia difusa de títulos
```
...
with JkAnime() as api:
    print(api.search("tokyo ghoul"))

with UnifiedAnimeAPI('jkanime') as api:
    # Unificar y validar embeds de proveedores
    print(api.unified_servers("One Piece", 1, validate=True))
```
...
```
Endpoints clave:
- `GET /search?q=naruto&provider=jkanime|animeflv`
- `GET /unified/servers?title=One%20Piece&episode=1&validate=true`
