...
class Database(object):
    ...
    def add_tables(self, filepaths_dic, get_summaries=False):
        ...
        for table in filepaths_dic:
            ...
            self.tables[table] = DataFrameWithInfo(
                table, read_df(pattern, dbsystem="spark", sqlContext=self.sqlContext)
            )
        ...
        if to_memory:
            ...
            if ".csv" in output_file:
                if ".gz" in output_file:
                    sqlDF.to_csv(output_file, compression="gzip", index=False)
                else:
                    sqlDF.to_csv(output_file, index=False)
            ...
            if ".csv" in output_file:
                if ".gz" in output_file:
                    sqlDF.write.csv(output_file, compression="GZIP", header=True)
                else:
    ...
    def update_station_data(self, t0, t1, output_folder):
        ...
        if "GAUGE_RETRIEVAL" in self.config:
            ...
            raise ValueError(
                """Make sure you have a "GAUGE_RETRIEVAL" section in
                          your config file!"""
            )
        ...
        try:
            ...
            current_tab = read_df(
                output_folder + "*.csv*", dbsystem="spark", sqlContext=self.sqlContext
            )
        ...
        if old_config and old_data_ok:
            if _compare_config(
                old_config, self.config, ["GAUGE_RETRIEVAL", "NO_DATA_FILL"]
            ):
                # Old data same format as new, don't overwrite
                overwrite = 0
            if old_config != self.config:
                # Special case
                tsteps_proc = np.array(
                    current_tab.select("timestamp").collect(), dtype=int
                )
                ...
                if tstamp_start > tstamp_start_old or tstamp_end < tstamp_end_old:
                    warning = """
                    ...
                    """.format(
                        str(datetime.fromtimestamp(tstamp_start_old, timezone.utc)),
        ...
        for i, stations in enumerate(stations_sub):
            ...
            file.write(
                'python -u {:s}/retrieve_dwh_data_5min.py "{:s}" "{:s}" {:f} "{:s}" "{:s}" {:s} {:d} {:d}'.format(
                    cwd,
                    t0,
                    t1,
    ...
    def update_reference_data(self, gauge_table_name, output_folder, t0=None, t1=None):
        """
        ...
        if gauge_table_name not in self.tables.keys():
            raise ValueError(
                """No table with name {:s} was found in the
                             loaded tables, make sure to add that table
                             with load_tables first""".format(
                    gauge_table_name
                )
            )
        ...
        if "REFERENCE_RETRIEVAL" in self.config:
            ...
            raise ValueError(
                """Make sure you have a "REFERENCE_RETRIEVAL" section in
                          your config file!"""
            )
        ...
        try:
            # Try to read old data
            current_tab = read_df(output_folder + "*.parquet", dbsystem="dask")
            old_data_ok = True  # valid data present
        except Exception:
            old_data_ok = False
            pass
        ...
        if t0 is not None and t1 is not None and t1 > t0:
            logging.info(
                "Limiting myself to time period {:s} - {:s}".format(str(t0), str(t1))
            )
        ...
        ttuples = [
            datetime.fromtimestamp(float(t), timezone.utc).timetuple()
        ...
        if not overwrite:
            ...
            for f in files:
                ...
                if current_day in days_to_process:
                    logging.warning(
                        "Day {:s} was already computed, ignoring it...".format(f)
                    )
        ...
        # Create job file
        fname = tmp_folder + "/getdata_reference_{:d}_{:d}.job".format(
            0, len(task_files) - 1
        )
        ...
        file.write(
            "#SBATCH --array={:d}-{:d}%{:d}\n\n".format(
                0, len(task_files), max_simul_jobs
            )
        )
        file.write(self.config["PYTHON_HEADER"])
        file.write(
            "python -u {:s}/retrieve_reference_data_5min.py -c {:s} -t {:s} -o {:s} \n".format(
                cwd, self.config_file, tf, output_folder
            )
        )
        file.close()
    ...
    def update_radar_data(self, gauge_table_name, output_folder, t0=None, t1=None):
        """
        ...
        if "RADAR_RETRIEVAL" in self.config:
            ...
            raise ValueError(
                """Make sure you have a "RADAR_RETRIEVAL" section in
                          your config file!"""
            )

        logging.info("Load data... (may take a moment)")
        tab = (
            self.tables[gauge_table_name].df.select(["STATION", "TIMESTAMP"]).toPandas()
        )
        ...
        if t0 is not None and t1 is not None and t1 > t0:
            ...
            tab = tab.loc[
                (tab["TIMESTAMP"] > tstamp_start) & (tab["TIMESTAMP"] <= tstamp_end)
            ]
        ...
        if old_config and old_data_ok:
            if _compare_config(
                old_config,
                self.config,
                ["GAUGE_RETRIEVAL", "RADAR_RETRIEVAL", "NO_DATA_FILL"],
            ):
                # Old data same format as new, don't overwrite
                overwrite = 0
            else:
        ...
        # Create job file
        fname = tmp_folder + "/getdata_radar_{:d}_{:d}.job".format(
            0, len(task_files) - 1
        )
        logging.info(f"Writing job file {fname}")
        file = open(fname, "w")
        ...
        file.write(self.config["PYTHON_HEADER"])
        file.write(
            "python -u {:s}/retrieve_radar_data_5min.py -c {:s} -t {:s} -o {:s} \n".format(
                cwd, self.config_file, tf, output_folder
            )
        )
        file.close()
...
def _n_running_jobs(user="$USER", job_name="getdata*"):
    ...
    out = subprocess.check_output("squeue -u {:s}".format(user), shell=True)
    ...
    count = 0
    for myline in out[1:-1]:
        myline = myline.split()
        if len(fnmatch.filter([myline[2]], job_name)):
            count += 1
    return count
