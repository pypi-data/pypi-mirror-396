...
class Updater(object):
    ...
    def get_row_length(self):
        length = (
            ...
            + len(self.neighb_x)
            * len(self.neighb_y)
            * len(self.radar_variables)
            * len(self.agg_methods)
        ) * len(self.sweeps)
        return length
    ...
    def process_all_timesteps(self):
        ...
        for i, tstep in enumerate(all_timesteps):
            ...
            for r in self.radars:
                ...
                try:
                    rad_files = self.retrieve_radar_files(
                        r, dtime, dtime, include_vpr, include_status
                    )
                    ...
                    empty = (
                        np.zeros((len(stations_to_get), len_row), dtype=np.float32)
                        + np.nan
                    )
            ...
            if i == len(all_timesteps) - 1:
                ...
                next_tstep = datetime.datetime.fromtimestamp(
                    all_timesteps[i + 1], datetime.timezone.utc
                )
                ...
                try:
                    ...
                    if not file_exists:
                        ...
                        df_join = df_old._append(df).drop_duplicates(
                            subset=["TIMESTAMP", "STATION", "NX", "NY"], keep="last"
                        )
...
def _data_at_station(radar_object, variables, idx, methods=["mean"], tidx=None):
    ...
    for v in variables:
        ...
        if v == "TCOUNT":
            ...
            data = np.ma.filled(radar_object.get_field(0, v)[ix, iy], fill_value=np.nan)
            for m in methods:
                if m == "mean":
