...
def read_cart(cart_file):
    ...
    if extension == ".gif" or "CPC" in cart_file:
        ...
        if size == nbins_x * nbins_y:
            DN = True
        if DN:
            data = np.fromfile(cart_file, dtype="B").reshape(nbins_x, nbins_y)
            data = constants.SCALE_CPC[data]  # Convert to float
        else:
            data = np.fromfile(cart_file, dtype=np.float32).reshape(nbins_x, nbins_y)
    else:
...
def save_gif(gif_file, precip):
    ...
    pil_im.save(fp=gif_file, loop=0, palette=cmap)
...
def read_vpr(xml_file, radar=None):
    ...
    vpr = [
        float(doc["VPR"]["DATA"]["slice"][i]["value"])
    ...
    vpr_norm[vpr_norm < 1.0 / corr_max_lin] = 1 / corr_max_lin
    vpr_norm[vpr_norm > corr_max_lin] = corr_max_lin

    maxval_lin = 10 ** (0.1 * constants.MAX_VPR_CORRECTION_DB)
    interp = interp1d(alt, vpr_norm, bounds_error=False, fill_value=maxval_lin)
