...
METSTATIONS = pd.read_csv(
    ObjStorage.check_file(str(Path(metadata_folder, "data_stations.csv"))),
    sep=";",
    encoding="latin-1",
)
...
RADARS = pd.read_csv(
    ObjStorage.check_file(str(Path(metadata_folder, "data_radars.csv"))),
    sep=";",
    encoding="latin-1",
)
...
LOCAL_BIAS = np.fromfile(
    ObjStorage.check_file(str(Path(metadata_folder, "lbias_qpegrid.dat"))),
    dtype=np.float32,
).reshape(NBINS_Y, NBINS_X)

GLOBAL_BIAS = {"A": 3.5, "D": 2.0, "L": 4.0, "P": 2.0, "W": 1.5}
...
# Graphics
###############
path_shp = ObjStorage.check_file(
    str(Path(metadata_folder, "swiss_border_shp", "Border_CH.shp"))
)
...
if ("lom" in socket.gethostname()) or ("meteoswiss" in socket.gethostname()):
    ...
    TIMES_COSMO1_T = np.array(
        [datetime.datetime.strptime(f[-13:-3], "%Y%m%d%H") for f in FILES_COSMO1_T]
    )
    FILES_COSMO1E_T = sorted(glob.glob(FOLDER_COSMO1E_T + "*.nc"))
    TIMES_COSMO1E_T = np.array(
        [datetime.datetime.strptime(f[-13:-3], "%Y%m%d%H") for f in FILES_COSMO1E_T]
    )
...
HYDRO_CENTROIDS["A"] = [
    ...
    [50.6186, -0.0649, 0.0946, 0.9904, 1179.9],
]  # IH/HDG
HYDRO_CENTROIDS["L"] = [
...
HYDRO_CENTROIDS["D"] = [
    ...
    [46.715, 0.030477, 0.16994, 0.9969, 976.56],
]  # IH/HDG
HYDRO_CENTROIDS["P"] = [
    ...
    [44.7896, 0.0015, 0.1349, 0.9968, 1116.7],
]  # IH/HDG
HYDRO_CENTROIDS["W"] = [
    ...
    [45.9686, 0.0845, 0.0963, 0.9940, 0867.4],
]  # IH/HDG
