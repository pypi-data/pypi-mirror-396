...
def _autolabel(ax, rects):
    ...
    for rect in rects:
        ...
        ax.annotate(
            "{:3.2f}".format(height),
            rotation=90 + 180 * (height < 0),
            xy=(rect.get_x() + rect.get_width() / 2, height),
            xytext=(
                0,
                int(height < 0) * -27 + int(height > 0) * 3,
            ),  # 3 points vertical offset
            textcoords="offset points",
            ha="center",
            va="bottom",
            color=rect._facecolor,
        )
...
class MidpointNormalize(Normalize):
    ...
    def __call__(self, value, clip=None):
        ...
        x.extend(np.linspace(self.transition + 1e-6, self.vmax, 30))
        y = [0, 0.25]
...
def meteoswiss_cmap():
    ...
    # build the colormap
    colors = np.array([c[i] for i in range(len(bounds))], dtype=float)
    colors[:, :3] /= 255.0  # normalize RGB to 0-1
    cmap = ListedColormap(colors[:-1])  # last color reserved for over-range
...
def qpe_plot(
    ...
    **kwargs:
    ...
    Returns
    -------
    fig, ax, the figure and axis handlers
    """
    # TODO: currently display of colormap ticks not working properly
    ...
    if np.any(figsize is None):
        figsize = (4 * subplots[1], 3 * subplots[0] + 1)

    fig, ax = plt.subplots(
        subplots[0], subplots[1], sharex=True, sharey=True, figsize=figsize
    )
    ...
    if len(ax) < n:
        raise ValueError(
            "The total number of subplots is smaller than the number of QPE models to plot!"
        )
    ...
    for i, dd in enumerate(data):
        m = ax[i].imshow(dd, extent=extent, cmap=cmap_qpe, norm=norm, **kwargs)

        if ch_border:
            for shape in constants.BORDER_SHP.shapeRecords():
                x = [i[0] / 1000.0 for i in shape.shape.points[:]]
                y = [i[1] / 1000.0 for i in shape.shape.points[:]]
                ax[i].plot(x, y, "k", linewidth=1.0)
    if xlim is not None:
        plt.xlim(xlim)
    else:
        plt.xlim([400, 900])
    ...
    if cbar_orientation == "horizontal":
        ...
        cbar = plt.colorbar(
            m, format="%.2f", orientation="horizontal", cax=cbar_ax, extend="max"
        )
        ...
        cbar = plt.colorbar(
            m, format="%.2f", orientation="vertical", cax=cbar_ax, extend="max"
        )

    if vmax <= 50:
        ticks = np.array(
            [vmin, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, transition, vmax]
        )
    else:
        ticks = np.array(
            [
...
def qpe_plot_mch(
    ...
    **kwargs,
    ...
    """Plots one or multiple QPE realizations using the default MCH color map
    ...
    **kwargs:
    ...
    for i, dd in enumerate(data):
        m = ax[i].imshow(dd, extent=extent, cmap=cmap, norm=norm, **kwargs)
...
def score_plot(scores, title_prefix="", figsize=(10, 5)):
    ...
    scorenames.remove("N")
    fig, ax = plt.subplots(2, int(np.ceil(len(precip_ranges) / 2)), figsize=figsize)
    ax = ax.ravel()
    for i, precip_range in enumerate(precip_ranges):
        ...
        for j, s in enumerate(scorenames):
            for k, m in enumerate(models_reordered):
                ...
                rec = ax[i].bar([offset * j + k], [sc], color=colors[k], width=1)
                _autolabel(ax[i], rec)
        ...
        ax[i].set_title(
            "precip_range = {:s}, N = {:d} samples".format(
                precip_range, scores[m][precip_range]["N"]
            )
        )
        fig.legend(
            models_reordered,
...
def qpe_scatterplot(qpe_est, ref, title_prefix="", figsize=(10, 7.5)):
    ...
    if len(models) > 3:
        fig, ax = plt.subplots(
            2, int(np.ceil(len(models) / 2)), figsize=figsize, sharey=True, sharex=True
        )
    else:
        fig, ax = plt.subplots(
            1, len(models), figsize=figsize, sharey=True, sharex=True
        )
...
def plot_fit_metrics(metrics, output_folder):
    ...
    intensity_ranges = list(
        metrics[aggregations[0]][fractions[0]][precip_types[0]].keys()
    )
    metric_names = list(
        metrics[aggregations[0]][fractions[0]][precip_types[0]][
            intensity_ranges[0]
        ].keys()
    )
    ...
    for ag in aggregations:
        for f in fractions:
            fig, ax = plt.subplots(
                len(metric_names), len(intensity_ranges), figsize=(14, 14)
            )
            for i, metric in enumerate(metric_names):
                for j, ir in enumerate(intensity_ranges):
                    ax[i, j].bar(
                        precip_types,
                        [metrics[ag][f][p][ir][metric] for p in precip_types],
                        color="C0",
                    )
                    ax[i, j].grid()
            ...
            plt.savefig(
                os.path.join(output_folder, nfile), dpi=300, bbox_inches="tight"
            )
    return list_figures
...
def plot_crossval_stats(stats, output_folder):
    ...
    boundtype = {}
    boundtype["10min"] = list(
        stats["10min"][qpetype[0]][veriftype[0]][preciptype[0]].keys()
    )
    boundtype["60min"] = list(
        stats["60min"][qpetype[0]][veriftype[0]][preciptype[0]].keys()
    )
    scoretype = list(
        stats[aggtype[0]][qpetype[0]][veriftype[0]][preciptype[0]][
            boundtype["10min"][0]
        ].keys()
    )
    ...
    for a in aggtype:
        for b in boundtype[a]:
            for v in veriftype:
                ...
                fig.legend(qpetype)
                plt.suptitle(
                    "{:s} errors, Agg : {:s}, R-range {:s} \n Nsamples = {:d}".format(
                        v, a, b, int(stats[a][q][v][p][b]["N"]["mean"])
                    )
                )

                nfile = "{:s}_{:s}_{:s}".format(v, a, b) + ".png"
                plt.savefig(output_folder + "/" + nfile, dpi=300, bbox_inches="tight")
