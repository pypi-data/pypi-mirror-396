...
def envyaml(filename):
    ...
    for mylist in file.readlines():
        matches = re.findall(pattern, mylist)
        for m in matches:
            mylist = mylist.replace(m, os.environ[m.replace("${", "").replace("}", "")])
        filebuff.write(mylist)
    filebuff.seek(0)
...
def timefromfilename(fname):
    ...
    tstr = bname[3:12]
    time = datetime.datetime.strptime(tstr, "%y%j%H%M").replace(
...
def idx_cart(x, y):
    ...
    idx = [(np.max(x_qpe) - x_llc).astype(int), (y_llc - np.min(y_qpe)).astype(int)]
...
def dict_flatten(mydict):
    ...
    for key, value in mydict.items():
        if isinstance(value, dict):
            _dict = {
                ":".join([key, str(_key)]): _value
                for _key, _value in dict_flatten(value).items()
            }
...
def nested_dict_values(d):
    ...
    for value in listvals:
        if isinstance(value, list) or isinstance(value, np.ndarray):
            ...
            listvals_unwrapped.append(value)
...
def aggregate_multi(array_3d, agg_operators):
    ...
    for i, op in enumerate(op_un):
        out[:, idx == i] = constants.AVG_METHODS[op](array_3d[:, :, idx == i], axis=0)
...
def read_df(pattern, dbsystem="dask", sqlContext=None):
    ...
    if dbsystem == "spark" and sqlContext is None:
    ...
    if ".parq" in files[0] or ".parquet" in files[0]:
        ...
        if dbsystem == "spark":
            df = sqlContext.read.csv(files, header=True, inferSchema=True)
        else:
            if ".gz" in files[0]:
                df = dd.read_csv(
                    pattern, compression="gzip", dtype={"TIMESTAMP": "float64"}
                )
            else:
                df = dd.read_csv(pattern)
    else:
        logger.error(
            """Invalid data, only csv and parquet files are accepted.
...
def nearest_time(dt, reference):
    ...
    dt2 = dt - datetime.timedelta(
        minutes=dt.minute % reference, seconds=dt.second, microseconds=dt.microsecond
    )
    if dt2 != dt:
...
def get_qpe_files(input_folder, t0=None, t1=None, time_agg=None, list_models=None):
    """
    ...
    for sub in glob.glob(input_folder + "/*"):
        ...
        for f in files:
            try:
                ...
                t = datetime.datetime.strptime(t, "%y%j%H%M").replace(
                    tzinfo=datetime.timezone.utc
                )
...
def get_qpe_files_multiple_dirs(
    input_folder, t0=None, t1=None, time_agg=None, list_models=None
):
    """
