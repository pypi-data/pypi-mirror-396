...
from rainforest.common.radarprocessing import (
    HZT_hourly_to_5min,
    Radar,
    hydroClass_single,
    hydroClass_single_over_iso,
)
from rainforest.common.retrieve_data import (
    retrieve_hzt_prod,
    retrieve_prod,
    get_COSMO_T,
    get_COSMO_variables,
...
class Updater(object):
    def __init__(self, task_file, config_file, output_folder, debug=False):
        ...
        if (self.temp_ref == "ISO0_HEIGHT") and (
            "ISO0_HEIGHT" not in self.other_variables
        ):
            logging.error(
                'Temperature reference "temp_ref" is ISO0_HEIGHT,'
                " but HZT fields are missing in radar_variables"
            )
        if (self.temp_ref == "TAIR") and ("T" not in self.cosmo_variables):
            logging.error(
                'Temperature reference "temp_ref" is TAIR,'
                " but T fields are missing in cosmo_variables"
            )

    def retrieve_radar_files(
        self, radar, start_time, end_time, include_vpr=True, include_status=True
    ):
        """
        ...
        try:
            ...
            if include_vpr:
                ...
                if len(files_v) > 2:
                    ...
                    if len(files_v) > 2:
                        files_temp = []
                        files_hours = np.array(
                            [ff.split("/")[-1][-10:-6] for ff in files_v]
                        )
                        for hhmm in np.unique(files_hours):
                            # Take the latest one with the highest quality flag
                            files_temp.append(
                                files_v[np.where(files_hours == hhmm)[-1][0]]
                            )
                        files_v = files_temp
            ...
            logging.error(
                """Retrieval for radar {:s} at timesteps {:s}-{:s}
                      failed""".format(
                    radar, str(start_time), str(end_time)
                )
            )
        return files_rad
    ...
    def retrieve_iso0_files(self, start_time, end_time):
        ...
        try:
            files_hzt = retrieve_hzt_prod(
                self.config["TMP_FOLDER"], start_time, end_time
            )
            ...
            raise
            logging.error(
                """Retrieval of hzt files at timesteps {:s}-{:s}
                      failed""".format(
    ...
    def process_single_timestep(self, list_stations, radar_object, tidx):
        ...
        ncols_by_sweep = (
            self.dims["no"]
            + self.dims["nc"]
            + (self.dims["nnx"] * self.dims["nny"] * self.dims["nrv"]) * self.dims["nm"]
        )
        ...
        if not valid_data:
            logging.info(
                "No need to process radar {:s} no measurement above stations...".format(
                    radar
                )
            )
            return all_data
        ...
        if "ZH_VISIB" in self.radar_variables or "ZV_VISIB" in self.radar_variables:
            radar_object.visib_mask(
                self.radar_cfg["VISIB_CORR"]["MIN_VISIB"],
            ...
            for j, sta in enumerate(list_stations):
                ...
                try:
                    ...
                    if "RADPRECIP" in self.other_variables:
                        ...
                        try:
                            wetradome = radar_object.status["status"]["sweep"][-1][
                                "RADAR"
                            ]["STAT"]["WET_RADOME"]

                            if wetradome is None:
                                radprecip = 0
                            else:
                    ...
                    if "ISO0_HEIGHT" in self.other_variables[-2::]:
                        ...
                        tmp = _data_at_station(
                            radsweeps[sweep], self.other_variables[-2::], idx
                        )
                        all_data[
                            j, idx0_col : idx0_col + len(self.other_variables[-2::])
                        ] = tmp
                        idx0_col += len(self.other_variables[-2::])

                    # COSMO data
                    idx = lut_coords[sta][sweep]["00"]
                    tmp = _data_at_station(radsweeps[sweep], self.cosmo_variables, idx)
    ...
    def process_all_timesteps(self):
        ...
        for i, tstep in enumerate(all_timesteps):
            ...
            tstart = datetime.datetime.fromtimestamp(
                float(tstep), tz=datetime.timezone.utc
                ...
                if hour_of_year != current_hour:
                    ...
                    try:
                        if only_cosmo_T:
                            ...
                            cosmo_data = get_COSMO_variables(
                                tstart,
            ...
            if (self.temp_ref == "ISO0_HEIGHT") or (
                "ISO0_HEIGHT" in self.other_variables
            ):
                ...
                if i == 0:
                    ...
                    # Interpolate between timesteps
                    hztfields.update(
                        HZT_hourly_to_5min(tstart, hzt_files, tsteps_min=5)
                    )

            # Create array for one timestep
            data_one_tstep = np.empty((len(stations_to_get), 0), dtype=np.float32)
            # MAIN LOOP
            for r in self.radars:
                ...
                if not np.any(np.isin(stations_to_get, visible_stations)):
                    logging.info(
                        "No need to process radar {:s} for these stations...".format(r)
                    )
                    continue
                ...
                try:
                    ...
                    rad_files = self.retrieve_radar_files(
                        r, tstart, tend, include_vpr, include_status
                    )

                    for tidx, tstamp in enumerate(
                        rad_files["radar"].keys()
                    ):  # 2 timesteps to make 10 min
                        # Create radar object
                        ...
                        tmp = self.process_single_timestep(
                            stations_to_get, radar, tidx + 1
                        )
                    ...
                    data_one_tstep = np.append(
                        data_one_tstep,
                        aggregate_multi(np.array(data_one_rad), temp_agg_op),
                        axis=1,
                    )
                except Exception as e:
                    logging.error(e)
                    logging.error(
                        """Data retrieval for radar {:s} and timestep
                    ...
                    empty = (
                        np.zeros(
                            (len(stations_to_get), len(temp_agg_op)), dtype=np.float32
                        )
                        + np.nan
                    )
            ...
            try:
                data_remapped, colnames = self._remap(
                    data_one_tstep, tstep, stations_to_get, compute_hydro
                )
                ...
                if len(all_data_daily) != 0:
                    logging.info(
                        "***SHAPE of all_data_daily at tstep: "
                        + str(tstep)
                        + " "
                        + str(np.shape(all_data_daily)[0])
                        + "/"
                        + str(np.shape(all_data_daily)[1])
                    )
            ...
            if i == len(all_timesteps) - 1:
                ...
                next_tstep_day = datetime.datetime.strftime(next_tstep, "%Y%m%d%H")[
                    0:-2
                ]
            ...
            if save_output and len(all_data_daily):
                ...
                try:
                    ...
                    for c, col in enumerate(colnames):
                        ...
                        isin_listcols = [
                            c == col.split("_")[0] for c in constants.COL_TYPES.keys()
                        ]
                    ...
                    if not file_exists:
                        ...
                        logging.info(
                            "Saving file: "
                            + name
                            + " as it already exists, I will append to it"
                        )
            ...
            for i in range(self.dims["nr"]):  # Loop on radar
                for j in range(self.dims["ns"]):  # Loop on sweeps
                    ...
                    for k in range(self.dims["nnx"]):
                        for ll in range(self.dims["nny"]):
                            dslice = []
                            # Add to each row COSMO and OTHER vars from nx = ny = 0
                            dslice.extend(
                                row[
                                    idx_sweep_start : idx_sweep_start
                                    + self.dims["nc"]
                                    + self.dims["no"]
                                ]
                            )
            ...
            try:
                for m in self.agg_methods:
                    ...
                    if self.temp_ref == "TAIR":
                        ...
                        hydro = hydroClass_single_over_iso(
                            rearranged[:, 2],  # radar
                            rearranged[:, zh_idx].astype(float),
                            rearranged[:, zdr_idx].astype(float),
                            rearranged[:, kdp_idx].astype(float),
                ...
                logging.error(e)
                logging.error(
                    """Could not compute hydrometeor classes, make
...
def _data_at_station(radar_object, variables, idx, methods=["mean"], tidx=None):
    ...
    for v in variables:
        ...
        if v == "TCOUNT":
            ...
            data = np.ma.filled(
                radar_object.get_field(0, v)[idx[:, 0], idx[:, 1]], fill_value=np.nan
            )
            for m in methods:
                if m == "mean":
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="task_file",
        default=None,
        help="Specify the task file to process",
        metavar="TASK",
    )
