...
class MedicalRoom8M(MedicalRoomBase):
    def __init__(self, mic_type='mono'):
        super().__init__(area=8, mic_type=mic_type)
    ...
    def _add_furniture(self, room):
        length, width = self.dimensions
        exam_table = objects.ExaminingTable("ExamTable", [length - 0.95, width - 0.4, 0], rotation_z=90)
        room.add_furniture(exam_table)
        stool = objects.MedicalStool("Stool", [0.6, width - 1.0, 0])
        room.add_furniture(stool)
        sink = objects.Sink("Sink", [length - 0.5, 0.3, 0], rotation_z=0)
        room.add_furniture(sink)
        bin = objects.SquareBin("Bin", [length - 1.0, 0.3, 0])
        room.add_furniture(bin)
        cabinets = objects.HorizontalCabinets("Cabinets", [length - 0.8, 0.23, 1.6], rotation_z=0)
        room.add_furniture(cabinets)
        door = objects.Door("Door", [0.5, 0, 0], rotation_z=0, handle_side='positive')
        room.add_furniture(door)
        desk = objects.Desk("Desk", [0.6, width - 0.35, 0], rotation_z=180)
        room.add_furniture(desk)
        imac = objects.iMac("iMac", [0.35, width - 0.35, 0.75], rotation_z=220)
        room.add_furniture(imac)
        keyboard = objects.Keyboard("Keyboard", [0.6, width - 0.50, 0.75], rotation_z=15)
        room.add_furniture(keyboard)

        # Add a person behind src1
        src1_pos = [length - 1.35, width * 0.8, 1.2]
        person1_pos = [src1_pos[0] + 0.5, src1_pos[1], 0.94]
        person1 = objects.LayingPerson("Person1", person1_pos, rotation_z=90)
