...
class MedicalRoom32M(MedicalRoomBase):
    def __init__(self, mic_type='mono'):
        super().__init__(area=32, mic_type=mic_type)
    ...
    def _add_furniture(self, room):
        ...
        cabinets = objects.HorizontalCabinets("Cabinets", [0.1, 0.1, 1.0], rotation_z=90)
        ...
        doctor = objects.Person("Doctor", [length * 0.2, width * 0.8, 0], rotation_z=180)
        room.add_furniture(doctor)
        wheelchair = objects.Wheelchair("Wheelchair", [length - 1, width - 1, 0], rotation_z=-45)
        room.add_furniture(wheelchair)
