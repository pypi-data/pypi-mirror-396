...
DEFAULT_API_URL = "https://auralis.dev/api"
...
class AuralisWorker:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.api_url}/worker/me",
                    headers=self.headers
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        logger.info(f"✅ Authenticated as: {data.get('name', 'Worker')}")
                        return True
                    else:
    ...
    async def get_available_jobs(self) -> list:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.api_url}/worker/jobs/available",
                    headers=self.headers
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(
                    f"{self.api_url}/worker/job/{job_id}/claim",
                    headers=self.headers
                ) as resp:
    ...
    async def get_job_info(self, job_id: str) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.api_url}/worker/job/{job_id}",
                    headers=self.headers
                ) as resp:
                    if resp.status == 200:
    ...
    async def update_status(self, job_id: str, status: str, output_url: str = None):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(
                    f"{self.api_url}/worker/job/{job_id}/status",
                    headers=self.headers,
                    data=data
                ) as resp:
                    return resp.status == 200
    ...
    async def append_log(self, job_id: str, content: str):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(
                    f"{self.api_url}/worker/job/{job_id}/log",
                    headers=self.headers,
                    data=data
                ) as resp:
                    return resp.status == 200
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(download_url) as resp:
                if resp.status == 200:
                    ...
                    logger.info(f"📥 Downloaded {resp.content_length or 'unknown'} bytes")
                    return local_path
                else:
    ...
    async def run_container(
        ...
        try:
            ...
            if not container_output_dir.startswith("/"):
            ...
            async for line in process.stdout:
                ...
                if elapsed > timeout_seconds:
                    process.kill()
                    await self.append_log(job_id, f"⏰ Timed out after {timeout_hours}h")
                    return False
    ...
    async def execute_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            await self.append_log(job_id, "📥 Downloading input files...")
            ...
            await self.append_log(job_id, "📦 Extracting project files...")
            project_dir = self.extract_archive(input_file, job_dir)
            if not project_dir:
                await self.append_log(job_id, "❌ No Dockerfile found")
                await self.update_status(job_id, "FAILED")
                return False
            ...
            await self.append_log(job_id, "🔨 Building Docker image...")
            
            if not await self.build_docker_image(project_dir, image_name, job_id):
                await self.append_log(job_id, "❌ Docker build failed")
                await self.update_status(job_id, "FAILED")
                return False
            
            await self.append_log(job_id, "✅ Docker image built")
            
            # Run container
            logger.info(f"▶️ Running container ({timeout}h timeout)...")
            await self.append_log(job_id, f"▶️ Running container...")
            
            if not await self.run_container(
                image_name, output_dir, output_directory, timeout, job_id
            ):
                await self.append_log(job_id, "❌ Container execution failed")
                await self.update_status(job_id, "FAILED")
                return False
            
            await self.append_log(job_id, "✅ Container finished")
            
            # Upload outputs
            logger.info("📤 Uploading outputs...")
            await self.append_log(job_id, "📤 Uploading output files...")
            await self.upload_outputs(output_dir, upload_url)
            
            # Mark complete
            await self.update_status(job_id, "COMPLETED")
            await self.append_log(job_id, "🎉 Job completed!")
            ...
            await self.append_log(job_id, f"❌ Error: {str(e)}")
            await self.update_status(job_id, "FAILED")
            return False
        finally:
            # Cleanup
...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="auralis-worker",
        description="Auralis Worker Node - Execute distributed compute jobs"
    )
