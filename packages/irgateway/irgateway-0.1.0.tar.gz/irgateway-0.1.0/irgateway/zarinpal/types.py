...
class PaymentRequest(DictMixin):
    ...
    Attributes:
        ...
        fee_type (Optional[str]): Type of transaction fee applied, such as 'Percent' or 'Rial'.
...
class PaymentVerification(DictMixin):
    ...
    Attributes:
        ...
        fee_type (Optional[str]): Type of fee deducted from the transaction ('Percent' or 'Rial').
...
@dataclass
class RefundResult(DictMixin):
    """
    Response from ZarinPal's refund endpoint (pg/v4/payment/refund.json).

    Models the result of processing a partial or full refund for a verified transaction.
    Refunds are processed asynchronously; check status via transaction inquiry if needed.

    Attributes:
        code (int): Refund status code. 100 indicates successful refund initiation.
        message (str): Descriptive message about the refund result (e.g., "Refund processed").

    Properties:
        is_success (bool): True if refund was initiated successfully (code == 100).
    ...
    Note: Refunds deduct from the merchant's settlement and credit the user's card.
    Reference: ZarinPal API Documentation - Refunds (inferred from SDKs like Node.js SDK).
    """
    code: int
    message: str
...
class FeeCalculation(DictMixin):
    ...
    Attributes:
        ...
        fee_type (str): Fee calculation type ('Percent' or 'Rial').
