...
class LmsCmd:
    def __init__(self, cfg: LmsCfg) -> None:
        ...
        self.cmd_map = {
            ...
            '61': lambda: self.lms.resource_group_list(),
            '62': lambda: self.select_resource_group(self.lms.resource_group_describe),
            '63': lambda: self.resource_group_update(),
    ...
    def print_cmds(self):
        ...
        print("*              31.load all  32.release all   33.describe all  34.delete all  35.print all")
        print("*       index: 51.describe  52.get-progress  53.delete | rs-group: 61.list  62.describe  63.update")
        print("*    yaml cmd: 41.search    42.insert        43.create-collection  44.query_cnt")
        print("*              45.query     46.create-index  47.delete_entity")
    ...
    def resource_group_update(self):
        def _update(name):
            try:
                node_num = int(input('Please input new node_num:'))
                return self.lms.resource_group_update(name, node_num)
            except:
                logger.warning(f"Input node_num Error: {traceback.format_exc()}")
        return self.select_resource_group(_update)

    def select_resource_group(self, func):
        datas = self.lms.resource_group_list()['data']
        datas.sort()
        ...
        cmd = input('==> Please select resource-group: ').strip()
        if cmd == 'q':
            return
        else:
            try:
