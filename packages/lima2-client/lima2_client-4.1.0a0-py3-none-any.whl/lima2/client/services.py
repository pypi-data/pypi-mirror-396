...
class Detector:
    ...
    def version(self) -> Version:
        ...
        payload = self.session.get("/version/detector_plugin").json()
        return Version(str(payload))
...
class Pipeline:
    ...
    def version(self, processing_name: str) -> Version:
        """Get the version of a processing pipeline class.

        Raises:
          Lima2NotFound: The class name isn't valid.
        """
        payload = self.session.get(f"/pipeline/class/{processing_name}/version").json()
        return Version(str(payload))
