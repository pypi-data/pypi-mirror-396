...
class WidgetGroup(Widget):
    ...
    def append(self, child: Widget) -> Self:
        children = self._children + (child,)
        return replace(self, _children=children)

    def remove(self, child: Widget) -> Self:
        children = tuple(
            c for c in self._children if c.spec.metadata != child.spec.metadata
        )
        return replace(self, _children=children)

    def replace(self, child: Widget) -> Self:
        children = tuple(
            child if c.spec.metadata == child.spec.metadata else c
