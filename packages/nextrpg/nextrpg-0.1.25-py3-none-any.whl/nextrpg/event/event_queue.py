...
class EventQueue:
    ...
    def tick(self, time_delta: Millisecond, state: GameState) -> Self:
        ...
        for pygame_event in self.events:
            if callable(predicate := pygame_event.delay):
                if predicate(state):
                    res = replace(pygame_event, delay=0)
                else:
                    res = pygame_event
            else:
                if pygame_event.delay <= 0:
                    continue
                delay = pygame_event.delay - time_delta
                res = replace(pygame_event, delay=delay)
            events.append(res)
