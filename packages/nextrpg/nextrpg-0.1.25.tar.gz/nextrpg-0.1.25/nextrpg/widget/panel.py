...
class Panel(WidgetGroup):
    ...
    def _more_after_icon(self) -> tuple[DrawingOnScreen, ...]:
        if self._visible.stop == len(self._children) or self.spec.loop:
            return ()
    ...
    def _to_next_child(self, current_child: DrawingOnScreens) -> Width | Height:
        if self._is_vertical:
            return current_child.height + self.spec.config.padding.height
        return current_child.width + self.spec.config.padding.width

    def _child_drawing(
        self, child: SizableWidget, coordinate: Coordinate
    ) -> DrawingOnScreens:
