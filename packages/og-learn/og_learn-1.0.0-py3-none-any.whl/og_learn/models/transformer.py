"""
FTTransformer (Feature Tokenizer Transformer) for regression.
"""
...
class FTTransformer(nn.Module):
    def __init__(self, num_features, emb_dim=32, depth=4, num_heads=4, dropout=0.1, global_feature=True, local_feature=True,pooling=False):
        super().__init__()
        self.global_feature = global_feature
        self.local_feature = local_feature
        self.pooling=pooling
        if not global_feature and not local_feature:
            raise ValueError("At least one of global_feature or local_feature must be True")
            
        # Case 1: global_feature=True, local_feature=True - Both embeddings
        # Case 2: global_feature=True, local_feature=False - Only global_feature embedding
        # Case 3: global_feature=False, local_feature=True - Only local_feature embedding
        
        if global_feature:
            self.global_embedding = nn.Linear(num_features, emb_dim)
            
        if local_feature:
            self.feature_embeddings = nn.ModuleList([nn.Linear(1, emb_dim) for _ in range(num_features)])
            
        self.transformer = nn.Sequential(*[FTTransformerBlock(emb_dim, num_heads, dropout) for _ in range(depth)])
        self.head = nn.Linear(emb_dim, 1)

    def forward(self, x):
        if self.global_feature and not self.local_feature:
            # Case 2: Only global_feature
            x = self.global_embedding(x).unsqueeze(1)
            
        elif not self.global_feature and self.local_feature:
            # Case 3: Only local_feature
            tokens = [emb(x[:, i:i+1]) for i, emb in enumerate(self.feature_embeddings)]
            x = torch.stack(tokens, dim=1)
            
        else:
            # Case 1: Both global_feature and local_feature
            global_x = self.global_embedding(x).unsqueeze(1)
            tokens = [emb(x[:, i:i+1]) for i, emb in enumerate(self.feature_embeddings)]
            feature_x = torch.stack(tokens, dim=1)
            x = torch.cat([global_x, feature_x], dim=1)
        
        x = self.transformer(x)

        if self.global_feature and not self.pooling:
            x = x[:, 0]   
        else:
            x = x.mean(dim=1)
        return self.head(x)
...
class FTTransformerRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, num_features, emb_dim=32, depth=4, num_heads=4, dropout=0.1, lr=1e-3, batch_size=1024, 
    epochs=10, eval_every_samples=50, memory_size=10000,   device=None,global_feature=True, local_feature=True,pooling=False,
    density=None):
    ...
    def plot_training_history(self, train_history, val_history,epoch_history, save_path='training_history.png'):
        plt.close('all')  
        ...
        ax.set_title('R2 Score during Training')
    ...
    def fit(self, X, y, X_valid=None, y_valid=None, OG_componment=False, oscillation=0.05, log_path=None, sampling_alpha=0.1, density=None, progress_bar=False, early_stopping=False, patience=5, monitor_curve=False, eval_every_samples=None, eval_every_seconds=None, eval_every_epochs=None, model_type="lgb",
        ...
        if tensorboard_dir is not None:
            try:
                ...
                run_name = tensorboard_name or f"Transformer_{time.strftime('%Y%m%d_%H%M%S')}"
                tb_writer = SummaryWriter(log_dir=os.path.join(tensorboard_dir, run_name))
            except ImportError:
        ...
        for epoch in range(self.epochs):
            if OG_componment == True:
                X_mem, y_mem = generate_OG_componment(model, X, oscillation=oscillation, density_col=density, alpha=sampling_alpha, label='pseudo_label')
                train_mem_loader = self.dataloader(X_mem, y_mem, shuffle=True)
                train_loader = train_mem_loader

            self.model_.train()
                
            # 根据progress_bar参数决定是否显示tqdm
            if progress_bar:
            ...
            for batch_idx, (xb, yb) in pbar:
                ...
                # Forward pass
                preds = self.model_(combined_xb)
                loss = loss_fn(preds, combined_yb)
        ...
        # If early stopping was used but not triggered, still load the best model
    ...
    def predict(self, X):
        ...
        # Process data in batches of 100k
        batch_size = 100000
        n_samples = len(X)
        predictions = []
        
        # Create progress bar, disappear if finished
        pbar = tqdm(total=(n_samples // batch_size) + (1 if n_samples % batch_size else 0), 
                   desc='Predicting',disable=True)
    ...
    def predict_with_uncertainty(self, X, n_samples=50):
        ...
        predictions = np.array(predictions)  # shape: (n_samples, n_samples, 1)
