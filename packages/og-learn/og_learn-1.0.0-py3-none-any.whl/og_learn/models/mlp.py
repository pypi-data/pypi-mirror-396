...
class MLPNetwork(nn.Module):
    def __init__(self, num_features, hidden_dims=[512, 256, 128], dropout=0.1, activation='relu'):
        super().__init__()
...
class MLPRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, num_features, hidden_dims=[512, 256, 128], dropout=0.1, activation='relu',
                 lr=1e-3, batch_size=1024, epochs=10, device=None):
    ...
    def plot_training_history(self, train_history, val_history, epoch_history, save_path='mlp_training_history.png'):
        plt.close('all')  
        ...
        ax.set_title('MLP R2 Score during Training')
    ...
    def fit(self, X, y, X_valid=None, y_valid=None, OG_componment=False, oscillation=0.05, 
        ...
        if tensorboard_dir is not None:
            try:
                ...
                run_name = tensorboard_name or f"MLP_{time.strftime('%Y%m%d_%H%M%S')}"
        ...
        last_eval_time = time.time()  # Fixed: Initialize unconditionally to avoid UnboundLocalError
        ...
        # Initialize display for training curve if needed
            ...
            for batch_idx, (xb, yb) in pbar:
                ...
                if val_loader is not None:
                    if eval_every_epochs is not None:
                        # Epoch-based evaluation (only at end of epoch)
                        # Evaluate at epoch 0, then every eval_every_epochs
                        if batch_idx == len(train_loader) - 1:
                            if epoch == 0 or (epoch + 1) % eval_every_epochs == 0:
                ...
                if should_evaluate:
                    ...
                    # Early stopping check (moved here to align with evaluation)
                    if early_stopping:
                    ...
                    # Generate and display training curve if monitor_curve is enabled
                    if monitor_curve:
                    ...
                    # Only reset samples_processed if evaluation was triggered by samples
