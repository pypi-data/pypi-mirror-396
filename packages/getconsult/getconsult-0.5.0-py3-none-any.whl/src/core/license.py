...
Tier System:
- FREE_BYOK: $0/month, 5 queries/day, user provides API keys
- PRO_BYOK: $9/month, 100 queries/day, user provides API keys
...
License Key Format (v1):
  ...
  - CSL1: Version prefix (Consult License v1)
  - tier: pro, free, or ent
  - issued_ts: Unix timestamp when key was generated
  - expires_ts: Unix timestamp when key expires (0 = never)
  - nonce: 8-char random string (prevents guessing)
...
# License version prefix
_LICENSE_VERSION = "CSL1"
...
def generate_license_key(
    ...
    payload = f"{_LICENSE_VERSION}_{tier}_{issued_ts}_{expires_ts}_{nonce}"
...
def validate_license_key(license_key: str) -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    # Expected: CSL1_tier_issued_expires_nonce_checksum = 6 parts
    if len(parts) != 6:
...
class LicenseManager:
    ...
    def _validate_license(self, license_key: str) -> LicenseInfo:
        ...
        # Extract expiry from key if present
