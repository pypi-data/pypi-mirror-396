...
class TUIEventHandler(EventListener):
    ...
    def _handle_consensus_check(self, data: dict) -> None:
        """Handle consensus check results."""
        ...
        if reached:
            self._log_header("✅", f"CONSENSUS REACHED: {pct}% approval", "bold green")
            self._log_detail(f"Threshold ({threshold_pct}%) met!", "green")
            if interpretation:
                self._log_detail(f"{interpretation}", "dim green")
            self._log_detail("Experts approved each other's solutions - proceeding to synthesis", "dim")
        else:
            self._log_header("⚠️", f"APPROVAL SCORE: {pct}%", "bold yellow")
            self._log_detail(f"Below threshold ({threshold_pct}%)", "yellow")
            if interpretation:
