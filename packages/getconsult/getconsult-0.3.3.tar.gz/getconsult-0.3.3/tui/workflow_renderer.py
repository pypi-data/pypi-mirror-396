            ...
            if self._current_iteration in self._iteration_widgets:
                ...
                if reached:
                    ...
                    if 3 in self._phase_widgets:
                        ...
                        # Build transparent explanation with pairwise approvals
                        pct = int(score * 100)
                        explanation = Text()
                        explanation.append("✓ CROSS-EXPERT APPROVAL ACHIEVED\n", style="bold green")
                        explanation.append(f"Aggregate: {pct}%", style="green")
                        if interpretation:
                            explanation.append(f" — {interpretation}", style="dim green")
                        explanation.append("\n\n", style="dim")

                        # Show individual pairwise approvals with reasoning
                        ...
                        if pending_approvals:
                            for approval in pending_approvals:
                                ...
                                endorsements = approval.get('endorsements', [])
                                concerns = approval.get('concerns', [])
                                ...
                                explanation.append(f"{icon} {evaluator} → {target}: ", style=style)
                                explanation.append(f"{int(appr_score * 100)}%\n", style=f"bold {style}")

                                # Show top endorsement/concern
                                if endorsements and len(endorsements) > 0:
                                    top = endorsements[0][:60] + "..." if len(endorsements[0]) > 60 else endorsements[0]
                                    explanation.append(f"   ✓ {top}\n", style="dim green")
                                if concerns and len(concerns) > 0:
                                    top = concerns[0][:60] + "..." if len(concerns[0]) > 60 else concerns[0]
                                    explanation.append(f"   ⚠ {top}\n", style="dim yellow")
                        else:
                            explanation.append("Experts approved each other's solutions.\n", style="dim")

                        explanation.append("\n→ Proceeding to final synthesis", style="bold green")
