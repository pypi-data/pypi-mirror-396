...
# Uses sbit if available with setuid, otherwise falls back to sudo
run_supervisorctl() {
    ...
    # Fallback to sudo supervisorctl (silently - no warnings during status checks)
    sudo supervisorctl "$@"
}
...
check_git_changes() {
    ...
    # Set global version variables for log prefix
...
setup_directories() {
    ...
    if [ -f "$TARGET_DIR/sbit" ] && [ -u "$TARGET_DIR/sbit" ]; then
        # sbit exists with setuid - use it to fix permissions (it preserves its own setuid bit)
        print_status "Using sbit to fix permissions (preserves setuid automatically)..."
        "$TARGET_DIR/sbit" fix-permissions || {
            print_warning "sbit fix-permissions failed, falling back to manual chmod"
            chown -R root:root /sphere
            chmod -R 755 /sphere
            chmod -R 755 "$LOG_DIR"
            # Manually restore sbit setuid bit
            chmod 4755 "$TARGET_DIR/sbit" 2>/dev/null || true
        }
    else
    ...
    if [ -n "$SBIT_SOURCE" ]; then
        ...
        if [ -f "$TARGET_DIR/sbit" ] && [ -u "$TARGET_DIR/sbit" ]; then
            ...
            if gcc -o "$TEMP_SBIT" "$TARGET_DIR/sbit.c"; then
                ...
                # Use existing sbit to install the new one (this preserves setuid bit automatically)
                if "$TARGET_DIR/sbit" install "$TEMP_SBIT"; then
                    print_status "✅ New sbit installed using sbit install command"
                    rm -f "$TEMP_SBIT"
                else
                    print_error "Failed to install new sbit using 'sbit install'"
                    rm -f "$TEMP_SBIT"
                    exit 1
                fi
            else
...
kill_all_old_sphere_processes() {
    ...
    ( timeout 2 sudo pkill -9 -f "cli.py watch-queue" 2>/dev/null || true ) 2>/dev/null
    ( timeout 2 sudo pkill -9 -f "watch-queue.*--queue-name" 2>/dev/null || true ) 2>/dev/null
    ...
    ( timeout 2 sudo pkill -9 -f "system_monitor.py" 2>/dev/null || true ) 2>/dev/null
    ( timeout 2 sudo pkill -9 -f "auto_upgrade_monitor.py" 2>/dev/null || true ) 2>/dev/null
    ( timeout 2 sudo pkill -9 -f "gc_cleanup.py" 2>/dev/null || true ) 2>/dev/null
    ( timeout 2 sudo pkill -9 -f "featrix_watchdog.py" 2>/dev/null || true ) 2>/dev/null
...
kill_all_celery_workers() {
    ...
    ( timeout 2 sudo pkill -9 -f "celery.*worker" 2>/dev/null || true ) 2>/dev/null
    ( timeout 2 sudo pkill -9 -f "celery -A celery_app" 2>/dev/null || true ) 2>/dev/null
    ( timeout 2 sudo pkill -9 celery 2>/dev/null || true ) 2>/dev/null
...
kill_uvicorn_processes() {
    ...
    if [ -n "$UVICORN_PIDS" ]; then
        ...
        ( timeout 2 sudo pkill -9 uvicorn 2>/dev/null || true ) 2>/dev/null
        ( timeout 2 sudo pkill -9 -f "uvicorn.*api:app" 2>/dev/null || true ) 2>/dev/null
        ( timeout 2 sudo pkill -9 -f "uvicorn.*--port" 2>/dev/null || true ) 2>/dev/null
...
show_final_status() {
    ...
    print_status "Verifying all services are running..."
    FAILED_SERVICES=()
    STATUS_OUTPUT=$(run_supervisorctl status 2>&1)
    ...
    # Check if sbit has setuid bit set
    local SBIT_PATH="/sphere/app/sbit"
    if [ -f "$SBIT_PATH" ]; then
        if [ ! -u "$SBIT_PATH" ]; then
            add_prefix ""
            add_prefix "${YELLOW}⚠️  IMPORTANT: sbit is not configured for passwordless operations${NC}"
            add_prefix ""
            add_prefix "${YELLOW}   The sbit binary exists but doesn't have the setuid bit set.${NC}"
            add_prefix "${YELLOW}   This means you'll need sudo password for service management.${NC}"
            add_prefix ""
            add_prefix "${YELLOW}   To enable passwordless service management, run:${NC}"
            add_prefix "${GREEN}   sudo chown root:root $SBIT_PATH${NC}"
            add_prefix "${GREEN}   sudo chmod +s $SBIT_PATH${NC}"
            add_prefix ""
            add_prefix "${YELLOW}   After that, you can use:${NC}"
            add_prefix "${YELLOW}   • sbit services status   (instead of sudo supervisorctl status)${NC}"
            add_prefix "${YELLOW}   • sbit services restart  (instead of sudo supervisorctl restart all)${NC}"
            add_prefix "${YELLOW}   • sbit services up       (instead of sudo supervisorctl start all)${NC}"
            add_prefix "${YELLOW}   • sbit services down     (instead of sudo supervisorctl stop all)${NC}"
            add_prefix ""
        else
            add_prefix ""
    ...
    if [ -f "$SBIT_PATH" ] && [ -u "$SBIT_PATH" ]; then
        ...
        add_prefix "   • Logs: sudo supervisorctl tail -f api_server"
    else
        add_prefix "   • Status: sudo supervisorctl status"
        add_prefix "   • Logs: sudo supervisorctl tail -f api_server"
        add_prefix "   • Restart: sudo supervisorctl restart all"
    fi
