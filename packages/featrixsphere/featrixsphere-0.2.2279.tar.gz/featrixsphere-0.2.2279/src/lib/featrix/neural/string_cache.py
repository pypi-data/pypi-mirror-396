...
class StringCache:
    ...
    def _add_single_cache_entry(self, value):
        ...
        # ALWAYS encode via string server with retry logic
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    delay = base_delay * (2 ** attempt)
                    logger.warning(f"⚠️  String encoding attempt {attempt + 1}/{max_retries} failed: {str(e)}, retrying in {delay}s...")
                    time.sleep(delay)
                else:
                    # Final attempt failed - raise the error
                    logger.error(f"❌ String encoding failed after {max_retries} attempts: {str(e)}")
                    raise
