    ...
    def forward(self, batch_data):
        ...
        if missing_columns:
            ...
            for col_name in missing_columns:
                # Create a batch of NOT_PRESENT tokens (None values that TokenBatch converts to NOT_PRESENT)
...
def sample_marginal_masks(batch_mask, min_mask_ratio=0.40, max_mask_ratio=0.60):
    """
    ...
    Returns:
        (new_mask_A, new_mask_B): Two complementary masks
...
class FeatrixTableEncoder(nn.Module):
    def __init__(self, col_codecs, config: FeatrixTableEncoderConfig, min_mask_ratio=0.40, max_mask_ratio=0.60, hybrid_groups=None):
        super().__init__()
        ...
        self.joint_encoder = JointEncoder(
            ...
            hybrid_groups=self.hybrid_groups,  # Pass hybrid groups to joint encoder
        )
    ...
    def forward(self, column_batches):
        ...
        mask_1, mask_2 = sample_marginal_masks(token_status_mask, self.min_mask_ratio, self.max_mask_ratio)
