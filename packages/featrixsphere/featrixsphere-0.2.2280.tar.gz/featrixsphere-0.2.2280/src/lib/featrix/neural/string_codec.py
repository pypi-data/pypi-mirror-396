...
def _init_string_server_client():
    ...
    # Default URLs based on client behavior
    default_primary = "http://taco.local:9000"
    default_fallback = "https://sphere-api.featrix.com/strings/encode"
    
    logger.info("🌐🌐🌐 ATTEMPTING TO INITIALIZE STRING SERVER CLIENT 🌐🌐🌐")
    logger.info(f"   Expected primary URL: {default_primary}")
    logger.info(f"   Expected fallback URL: {default_fallback}")
    if potential_urls:
    ...
    try:
        ...
        # CRITICAL: Diagnose EXACTLY what's wrong
        import subprocess
        import sys
        import socket
        import requests
        ...
        logger.error("=" * 80)
        logger.error("❌❌❌ STRING SERVER CLIENT INITIALIZATION FAILED ❌❌❌")
        logger.error("=" * 80)
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pip", "show", "featrix-string-server-client"],
                capture_output=True,
                text=True,
                timeout=5
            )
            if result.returncode == 0:
                ...
                logger.error("   ✅ Package IS installed")
                logger.error(f"   Package details:\n{package_info}")
            else:
                logger.error("   ❌ Package is NOT installed")
                logger.error(f"   pip show output: {result.stderr}")
        except Exception as check_err:
            logger.error(f"   ⚠️  Could not check package status: {check_err}")
        
        # Step 2: Try to import and show exact error
        logger.error("\n🔍 IMPORT TEST:")
        try:
            import featrix_string_server_client
            logger.error("   ✅ Import succeeded")
            try:
                from featrix_string_server_client import StringServerClient
                logger.error("   ✅ StringServerClient class found")
            except ImportError as class_err:
                logger.error(f"   ❌ StringServerClient class not found: {class_err}")
        except ImportError as import_test_err:
            logger.error(f"   ❌ Import failed: {import_test_err}")
            logger.error(f"   Error type: {type(import_test_err).__name__}")
            logger.error(f"   Error message: {str(import_test_err)}")
        
        # Step 3: Check URL accessibility
        logger.error("\n🔍 URL ACCESSIBILITY TEST:")
        for url_name, url in [("Primary", default_primary), ("Fallback", default_fallback)]:
            try:
                ...
                logger.error(f"   Testing {url_name} URL: {url}")
                logger.error(f"      Hostname: {hostname}, Port: {port}")
                ...
                try:
                    ...
                    logger.error(f"      ❌ Port test failed: {port_err}")
                
                # Test HTTP connectivity (for fallback URL)
                if url == default_fallback:
                    try:
                        test_url = f"{url.rstrip('/encode')}/health" if url.endswith('/encode') else f"{url}/health"
                        response = requests.get(test_url, timeout=3)
                        logger.error(f"      ✅ HTTP request succeeded: {response.status_code}")
                    except Exception as http_err:
                        logger.error(f"      ❌ HTTP request failed: {http_err}")
            except Exception as url_test_err:
                logger.error(f"   ❌ URL test failed for {url_name}: {url_test_err}")
        
        # Step 4: Print fix commands
        logger.error("\n🔧 FIX COMMANDS:")
        if not package_installed:
            logger.error(f"   📦 INSTALL PACKAGE:")
            logger.error(f"      {sys.executable} -m pip install featrix-string-server-client")
        else:
            logger.error(f"   📦 PACKAGE IS INSTALLED - TRY REINSTALLING:")
            logger.error(f"      {sys.executable} -m pip uninstall featrix-string-server-client -y")
            logger.error(f"      {sys.executable} -m pip install featrix-string-server-client")
        
        logger.error(f"\n🌐 EXPECTED URLS:")
        logger.error(f"   Primary: {default_primary}")
        logger.error(f"   Fallback: {default_fallback}")
        
        logger.error("\n" + "=" * 80)
        
        # CRITICAL: Crash immediately with clear error message
        # Package is installed from private bits server
        install_cmd = "python3 -m pip install --upgrade --extra-index-url https://bits.featrix.com/pypi/simple --trusted-host bits.featrix.com featrix-string-server-client"
        uninstall_cmd = "python3 -m pip uninstall featrix-string-server-client -y"
        
        if not package_installed:
            error_msg = (
                f"\n"
                f"================================================================================\n"
                f"❌ CRITICAL ERROR: featrix-string-server-client NOT INSTALLED\n"
                f"================================================================================\n"
                f"\n"
                f"The string server client package is required but not installed.\n"
                f"\n"
                f"📦 INSTALL COMMAND (from private bits server):\n"
                f"\n"
                f"    {install_cmd}\n"
                f"\n"
                f"🌐 Expected URLs:\n"
            ...
            raise RuntimeError(
                f"featrix-string-server-client package not installed.\n"
                f"Install with: {install_cmd}\n"
                f"Expected URLs: Primary={default_primary}, Fallback={default_fallback}"
            )
        else:
            # Package is installed but import failed - different error
            error_msg = (
                f"\n"
                f"================================================================================\n"
                f"❌ CRITICAL ERROR: featrix-string-server-client IMPORT FAILED\n"
                f"================================================================================\n"
                f"\n"
                f"The package is installed but cannot be imported.\n"
                f"This may be a version compatibility issue.\n"
                f"\n"
                f"🔧 TRY REINSTALLING (from private bits server):\n"
                f"\n"
                f"    {uninstall_cmd}\n"
                f"    {install_cmd}\n"
                f"\n"
            ...
            logger.critical(error_msg)
            raise RuntimeError(
                f"featrix-string-server-client package installed but import failed.\n"
                f"Try reinstalling: {uninstall_cmd} && {install_cmd}"
            )
    except Exception as e:
        logger.error(f"❌❌❌ INITIALIZATION FAILED ❌❌❌")
        logger.error(f"   Would use primary URL: {default_primary}")
        logger.error(f"   Would use fallback URL: {default_fallback}")
