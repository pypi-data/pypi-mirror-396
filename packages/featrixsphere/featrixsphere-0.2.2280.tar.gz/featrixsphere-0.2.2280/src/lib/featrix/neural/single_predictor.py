...
class FeatrixSinglePredictor:
    ...
    async def train(
        ...
        self._structured_logger = StructuredLogger(logger, target_col_name=self.target_col_name)
                ...
                try:
                    ...
                    for epoch_idx in range(n_epochs):
                        ...
                        for batch_idx, column_batch in enumerate(train_dataloader):
                            ...
                            try:
                                ...
                                if epoch_idx == 0 and batch_idx == 0:
                                    logger.info(f"{log_prefix}🔍 ENCODER DIAGNOSTIC [epoch={epoch_idx}, batch={batch_idx}]:")
                                ...
                                if epoch_idx == 0 and batch_idx == 0:
                                    ...
                                    logger.info(f"{log_prefix}🔍 ENCODING RESULT [epoch={epoch_idx}, batch={batch_idx}]:")
                            ...
                            if (out.abs() > 20.0).any():
                                ...
                                if batch_idx % 10 == 0:  # Don't spam logs
                                    logger.warning(f"⚠️  [epoch={epoch_idx}, batch={batch_idx}] Clamped {n_clamped} extreme logits (max={max_logit:.1f}) to prevent loss explosion")
                            ...
                            if batch_idx == 0:
                                ...
                                if scheduler is not None and isinstance(scheduler, OneCycleLR):
                                    ...
                                    logger.info(f"📈 OneCycleLR [epoch={epoch_idx}]: {phase} phase ({progress_pct:.1f}% complete, LR={lr_value:.6e})")
                                elif scheduler is not None:
                                    logger.info(f"📈 LR Schedule [epoch={epoch_idx}]: LR={lr_value:.6e}")
                                else:
                    ...
                    if hasattr(self, '_feature_tracker') and self._feature_tracker:
                        try:
                            ...
                            # Export history to output directory
                            if self._output_dir:
                                history_path = os.path.join(self._output_dir, "feature_suggestion_history.json")
                            else:
                                history_path = "feature_suggestion_history.json"
    ...
    def compute_classification_metrics(self, queries, ground_truth, pos_label, epoch_idx=0, n_epochs=None):
        ...
        with PredictorEvalModeContextManager(fsp=self, debugLabel="compute_classification_metrics"):
            ...
            if not is_binary:
                ...
                if 'pos_probs_array' in locals() and 'raw_logits_array' in locals():
                ...
                if 'threshold_pred_counts' in locals() and threshold_pred_counts:
