...
class EmbeddingSpace(object):
    def __init__(
        ...
        self.n_transformer_layers = n_transformer_layers if n_transformer_layers is not None else 3
        self.n_attention_heads = n_attention_heads if n_attention_heads is not None else 8
    ...
    def get_default_joint_encoder_config(
        self, d_model: int, n_cols: int, col_order, dropout: float
    ):
    ...
    def _queue_project_training_movie_frame(self, epoch_idx):
        ...
        try:
            ...
            if not redis_available:
                logger.warning(f"⚠️  Redis not available - skipping movie frame generation for epoch {epoch_idx}")
                logger.warning(f"   Movie frames require Redis/Celery for async processing")
                logger.warning(f"   Start Redis with: redis-server (or systemctl start redis-server)")
                return
    ...
    def _log_marginal_loss_breakdown(self, loss_dict, epoch_idx, batch_idx):
        ...
        try:
            ...
            logger.debug(f"📊 [epoch={epoch_idx}, batch={batch_idx}] MARGINAL LOSS BREAKDOWN:")
            ...
            if column_losses:
                ...
                if set_adaptive_count == 0:
                    ...
                    for strategy_name, code in set_strategy_order:
                        header_parts.append(f"{code:>5s}")
                    if has_deltas:
                        header_parts.append("   Δ")  # Delta from initial
                    header_parts.append("Dominant")
                    logger.info("      " + "   ".join(header_parts))
                    ...
                    for col_name, learned_weight, semantic_weight in set_mixture_data:
                        ...
                        # Format: Column(45) LRN(6) SEM(6) Delta(8) Dominant
                        ...
                        if has_deltas and col_name in set_mixture_deltas:
                            ...
                            if abs(delta) < 0.001:
                                ...
                                if delta > 0:
                                    ...
                                    delta_str = f"  {GREEN}+{delta_pct:>4.1f}%{RESET}"
                                else:
                                    # Moving toward semantic (negative)
                                    delta_str = f"  {CYAN}{delta_pct:>5.1f}%{RESET}"
                        
                        # Build final row with consistent spacing
                        logger.info(f"      {display_name:<45s}  {lrn_colored}  {sem_colored}{delta_str}  {YELLOW}{dominant_str}{RESET}")
                    ...
                    if has_deltas:
                        if abs(mean_delta) < 0.001:
                            ...
                            delta_color = GREEN if mean_delta > 0 else CYAN
                            mean_parts.append(f"{delta_color}{delta_pct:+.1f}%{RESET}")
                    logger.info("      " + "   ".join(mean_parts))
                    ...
                    if has_deltas:
                        ...
                        if delta_values and len(delta_values) > 1:
                            ...
                            std_parts.append(f"{GRAY}±{delta_std:.1f}%{RESET}")
                        else:
    ...
    def _get_default_curriculum_config() -> CurriculumLearningConfig:
        ...
        Phase 1 (0-30%): Spread focus
        Phase 2 (30-60%): Marginal focus (reduced weight)
        Phase 3 (60-90%): Joint focus
        Phase 4 (90-100%): Spread + Joint focus
        """
        return CurriculumLearningConfig(
            ...
            phases=[
                ...
                CurriculumPhaseConfig(
                    ...
                    end_progress=0.60,
                    spread_weight=0.1,
                    marginal_weight=0.5,  # Reduced compared to spread/joint
                    joint_weight=0.1,
                    transition_width=0.05,
                ),
                CurriculumPhaseConfig(
    ...
    def train(
        ...
        with profiler_ctx_mngr as profiler:
            for epoch_idx in range(base_epoch_index, n_epochs):
                ...
                spread_weight, marginal_weight, joint_weight = self._compute_loss_weights(epoch_idx, n_epochs)
                ...
                # Update all three weights in the encoder's loss config
                ...
                if isinstance(val_loss, (int, float)) and val_loss > 0:
                    ...
                    logger.info(f"[epoch={epoch_idx}] {'LR':8s}: {' '.join(lr_nums)}")
                    ...
                    logger.info(f"[epoch={epoch_idx}] {'Marg Wt':8s}: {' '.join(mw_nums)}")
