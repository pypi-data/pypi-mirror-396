...
class Downloader:
    ...
    @staticmethod
    def _is_download_needed(file_path: str, sha256hash: Optional[str]) -> bool:
        """
        Delete as needed stuff at the file_path.

        :param file_path: File path to check and remove if needed.
        :param sha256hash: sha256hash of the expected file. Do not remove if the hash matches.
        ...
        if os.path.exists(file_path):
            ...
            elif sha256hash is not None and sha256hash == Downloader.sha256(file_path):
                logger.info("sha256hash matched. No download necessary.")
                return False
    ...
    def get_download_path(cls, file_name: str, target_dir: str,
                          prefix: Optional[str] = None, sha256: Optional[str] = None) -> str:
        """
        ...
        """
        if os.environ.get('AZUREML_AUTOML_OPTIMIZED_DOWNLOADS') and sha256:
            tmpDir = tempfile.gettempdir()
            os.makedirs(os.path.join(tmpDir, 'automl_downloads'), exist_ok=True)
            result = os.path.join(tmpDir, 'automl_downloads', sha256)
    ...
    @classmethod
    def download(cls, download_prefix: str, file_name: str, target_dir: str, sha256: Optional[str] = None,
        ...
        :param sha256hash: sha256hash of the file being downloaded to prevent random files from getting downloaded.
        ...
        if not Downloader._is_download_needed(download_path, sha256):
            logger.info("File already found at {}. Skipping download".format(download_path))
            return download_path

        if cls._download_with_retries(download_url, download_path, retries, sha256=sha256):
            return download_path
        else:
            return None
    ...
    @classmethod
    def _download(cls, download_url: str, download_path: str, sha256: Optional[str] = None) -> bool:
        ...
        if sha256 and download_hash != sha256:
            error_message = "sha256 validation failed. This downloaded file will not be used " \
                            "and will be removed."
            Downloader.remove_file(tmp_download_path)
    ...
    @classmethod
    def sha256(cls, file_path: str) -> Optional[str]:
        """
        Calculate sha256hash of the given file name if the path exists. Else, return None.

        :param file_path: Path to the file.
        :return: sha256digest or None
        """
