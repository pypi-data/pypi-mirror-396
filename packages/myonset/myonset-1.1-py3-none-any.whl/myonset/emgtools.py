...
def detector_var(data_trial, times, th=3.5, time_limit=.025, 
                 ...
                 baseline='pre_stim', sf=None, use_derivative=False,
                 moving_avg_size=1):
    ...
    mbsl : float | None
        If None, use the mean baseline value of the trial (defined by 'baseline'
        parameter). To use global mbsl, use getGlobalVariance function, applied
        on same data type as input signal (e.g., raw EMG, teager-kayser...)
        (default None).
    stbsl : float | None
        If None, use the baseline standard deviation of the trial (defined by 
        'baseline' parameter). To use global stbsl, apply getGlobalVariance on 
        the same data type as input signal (e.g., raw EMG, teager-kayser...)
        (default None).
    baseline : list | array
        Start and end time latencies (in s) to compute baseline mean and standard
        deviation values of the trial (default 'pre_stim' computes mean and 
        standard deviation between first and time 0 samples).
    sf : float
        Sampling frequency.
    use_derivative : bool
    ...
    if baseline == 'pre_stim':
        tbsl = range(find_times(0, times))
    else:
        tbsl = range(find_times(baseline[0], times), find_times(baseline[1], times))
    
    data_trial = moving_avg(data_trial, moving_avg_size)
    data_trial = np.abs(data_trial)

    if mbsl is None: mbsl = data_trial[tbsl].mean()
    if stbsl is None: stbsl = data_trial[tbsl].std()
    ...
    if len(onsets) > 0 or len(offsets) > 0 :
        ...
        if use_derivative and len(onsets) > 0 and onsets[0] < (data_trial.shape[0] - 1): # we exclude if onset is on last sample
            ...
            mbsl = data_trial[tbsl].mean()
            stbsl = data_trial[tbsl].std()
...
def detector_dbl_th(data_trial, times, th=3, window_size=.020, 
                    ...
                    mbsl=None, stbsl=None, baseline='pre_stim', sf=None):
    """Return the indices of time windows containing signal exceeding the 
    double threshold.    
    ...
    mbsl : float | None
        If None, use the mean baseline value of the trial (on the time interval 
        defined by 'baseline' parameter). To use global mbsl, use 
        getGlobalVariance function, applied on same data type as input signal 
        (e.g., raw EMG, teager-kayser...) (default None).
    stbsl : float | None
        If None, use the baseline standard deviation of the trial (on the time 
        interval defined by 'baseline' parameter). To use global stbsl, apply 
        getGlobalVariance on the same data type as input signal (e.g., raw EMG,
        teager-kayser...) (default None).
    baseline : list | array
        Start and end time latencies (in s) to compute baseline mean and standard
        deviation values of the trial (default 'pre_stim' computes mean and 
        standard deviation between first time sample and time 0 sample).
    sf : float
        Sampling frequency.
...
def get_onsets(data_trial, times, 
    ...
    params : dict
        ...
        - mbsl_raw : float | None
            If None, use the mean baseline value of the raw signal trial (on the 
            time interval defined by 'baseline' parameter). To use global mbsl, 
        ...
        - stbsl_raw : float | None
            ...
            (on the time interval defined by 'baseline' parameter). To use global
            stbsl, apply 'global_var' (default is None).
        - th_tkeo : float
            Threshold to multiply by variance, used in 'detector_var' applied
        ...
        - mbsl_tkeo : float | None
            ...
            transformation of data_trial signal (on the time interval defined by
            'baseline' parameter). To use global mbsl, use 'global_var' function
            on the Teager-Kaiser signal transformation (default is None).
        - stbsl_tkeo : float | None
            If None, use the baseline standard deviation of the Teager-Kaiser
            transformation of data_trial signal (on the time interval defined by
            'baseline' parameter). To use global stbsl, apply 'global_var' on 
        ...
        - mbsl_raw : float | None
            If None, use the mean baseline value of raw signal (on the time
            interval defined by 'baseline' parameter). To use global mbsl, use
            'global_var' (default is None).
        - stbsl_raw : float | None
            If None, use the baseline standard deviation of raw signal (on the 
            time interval defined by 'baseline' parameter). To use global stbsl,
    ...
    if method == 'single_threshold':
        if use_raw: 
            ...
            active_sections_raw = detector_var(data_trial, times, 
                                               ...
                                               varying_min=varying_min_raw, 
                                               mbsl=mbsl_raw, stbsl=stbsl_raw, baseline=baseline,
                                               sf=sf)
        else: 
        ...
        if use_tkeo: 
            ...
            active_sections_tkeo = detector_var(tkeo(data_trial), times, th=th_tkeo, time_limit=time_limit_tkeo, min_samples=min_samples_tkeo,
                                                varying_min=varying_min_tkeo, 
                                                mbsl=mbsl_tkeo, stbsl=stbsl_tkeo, baseline=baseline,
                                                sf=sf)
        else: 
        ...
        if use_raw: 
            ...
            active_sections_raw = detector_dbl_th(data_trial, times, th=th_raw, window_size=window_size, 
                                                  min_above_threshold=min_above_threshold, min_samples=min_samples, 
                                                  mbsl=mbsl_raw, stbsl=stbsl_raw, baseline=baseline,
                                                  sf=sf)
        else: 
            active_sections_raw = np.transpose(([],[]))
