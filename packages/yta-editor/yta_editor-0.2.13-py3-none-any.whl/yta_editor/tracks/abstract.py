...
from yta_editor.utils.cache import SilentAudioFrameCache, BlackFrameCache
...
class _TrackWithAudio(_Track):
    ...
    #) -> list['AudioFrameWrapped']:
        ...
        if not hasattr(self, '_audio_silent'):
            self._audio_silent = SilentAudioFrameCache().get(
...
class _TrackWithVideo(_Track):
    ...
    def video_empty_frame(
        ...
        """
        video_frame = BlackFrameCache().get(
            size = self.size,
            video_fps = self.fps
            # TODO: We need the transparency to apply
        )

        # TODO: Handle the transparency
