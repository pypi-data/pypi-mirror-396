...
class QAssistant:
    ...
    def __init__(self, user_id: str, org_id: str, orchestrator: Optional[Orchestrator] = None) -> None:
        ...
        *agentfoundry* is not available (e.g., in lightweight environments or
        ...
        if not self.orchestrator:
            try:
                ...
                agent_tools_map = getattr(self.tool_registry, 'agent_tools', None)
                if not isinstance(agent_tools_map, dict) or not agent_tools_map:
                    self.tool_registry.agent_tools = {"general_agent": available_tools}
    ...
    def answer_question(
        ...
        try:
            ...
            with client_context_scope(client_context or {}):
                ...
                response = self.orchestrator.process_request(messages, checkpointer=self.memory, config=self.config)
