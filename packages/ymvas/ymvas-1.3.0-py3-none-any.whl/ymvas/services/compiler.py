...
from ymvas.config.components.ref import Tag, Ref
...
class Compiler:
    ...
    def _compile_endpoints( self , config ):
        ...
        def _render(tag:Tag) -> Tag:
            ...
            module = settings.modules.get(str(repo))
            if module is None:
                return tag
            ...
            if tag.ref.src == rsrc:
                ...
                ref = cache.get(rsrc, Ref(rsrc))
            
            tag.sync_slots_from_ref( ref )
            tag.set_replace( ref.to_string( _render, depth, into) )
            
            # store refs for next iteration
            if not tag.ref.src in cache:
                cache[tag.ref.src] = tag.ref
            if not rsrc in cache:
                cache[rsrc] = ref
