...
#[pyfunction]
fn share(x: u32) -> PyResult<(u32, u32)> {
    let mut rng = rand::thread_rng();
    let x0: u32 = rng.gen();
    let x1: u32 = x.wrapping_sub(x0);
    Ok((x0, x1))
}
...
#[pyfunction]
fn add_64(a: u64, b: u64) -> PyResult<u64>{
    Ok(a.wrapping_add(b))
}
...
#[pyfunction]
fn sub_64(a: u64, b: u64) -> PyResult<u64> {
    Ok(a.wrapping_sub(b))
}
...
#[pyfunction]
fn mult_64(a: u64, b: u64) -> PyResult<u64> {
    Ok(a.wrapping_mul(b))
}

#[pyfunction]
fn secureml_mult(idx: u32, ai: u32, bi: u32, zi: u32, e: u32, f: u32) -> PyResult<u32> {
    let mut ci: u32 = (f.wrapping_mul(ai)).wrapping_add(e.wrapping_mul(bi));
    ci = ci.wrapping_add(zi);
    let result: u32 = match idx{
        1 => {
            let t = ci.wrapping_sub(e.wrapping_mul(f));
            //let t = (t as u32) >> (16);
            let t = t >> 12;
            t
        }
        0 => {
            let t = ci.wrapping_neg();
            //let t = (t as u32) >> (16);
            let t = t >> 12;
            let t = t.wrapping_neg();
            t
        }
        _ => {
...
#[pyfunction]
fn beaver_triples_mult(idx: u32, ai: u32, bi: u32, zi: u32, e: u32, f: u32) -> PyResult<u32> {
    let mut ci: u32 = (f.wrapping_mul(ai)).wrapping_add(e.wrapping_mul(bi));
    ci = ci.wrapping_add(zi);
    let result: u32 = match idx{
        1 => {
            ci.wrapping_sub(e.wrapping_mul(f))
        }
        0 => {
            ci
        }
...
#[pyfunction]
fn left_shift(a: u32, b: u32) -> PyResult<u32> {
    Ok(a << b)
}
...
#[pyfunction]
fn float2fix_scale(x: f64) -> PyResult<u32> {
    let y: f64 = x;
    let z: i32 = y as i32;
    let k: u32 = z as u32;
    Ok(k)
...
#[pyfunction]
fn fix2float_64(x: u64) -> PyResult<f64> {
    let y: i64 = x as i64;
    let z: f64 = y as f64;
    let factor: f64 = 2_f64.powf(-12.0);
    let k: f64 = factor * z;
...
#[pyfunction]
fn convert_64(input: Vec<u8>) -> PyResult<u64> {
    // input: Vec<u8> each element represent the hex's decimal representation, e.g. ed -> 237
    // println!("Integer: {:?}", (input[0], input[1], input[2], input[3]));
    let group_element = u64::from_be_bytes([input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7]]);
    Ok(group_element )
}
...
fn universe(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(share, m)?)?;
    m.add_function(wrap_pyfunction!(float2fix_scale, m)?)?;
    m.add_function(wrap_pyfunction!(beaver_triples_mult, m)?)?;
    m.add_function(wrap_pyfunction!(secureml_mult, m)?)?;
