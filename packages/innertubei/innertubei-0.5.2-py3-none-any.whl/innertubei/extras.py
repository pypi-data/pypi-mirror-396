...
from innertubei.core.channel import ChannelCore
from innertubei.core.constants import ChannelRequestType, ResultMode
from innertubei.core.hashtag import HashtagCore
from innertubei.core.playlist import PlaylistCore
from innertubei.core.suggestions import SuggestionsCore
from innertubei.core.transcript import TranscriptCore
from innertubei.core.video import VideoCore
...
class Video:
    ...
    async def get(
        ...
        """
        try:
            from innertubei.core.componenthandler import getVideoId
            ...
            if video.result and video.result.get("id"):
                if result_mode == ResultMode.json:
                    import orjson

                    return orjson.dumps(video.result).decode("utf-8")
                return video.result
            else:
                return None
        except Exception:
...
class Playlist:
    ...
    async def getNextVideos(self) -> None:
        if not self.info:
            ...
            if (
                self.__playlist.playlistComponent
                and "info" in self.__playlist.playlistComponent
            ):
                self.info = copy.deepcopy(self.__playlist.playlistComponent["info"])
                self.videos = self.__playlist.playlistComponent.get("videos", [])
            else:
                self.info = (
                    copy.deepcopy(self.__playlist.playlistComponent)
                    if self.__playlist.playlistComponent
                    else {}
                )
                self.videos = (
                    self.__playlist.playlistComponent.get("videos", [])
                    if self.__playlist.playlistComponent
                    else []
                )

            self.hasMoreVideos = self.__playlist.continuationKey != None
            if self.info and "videos" in self.info:
    ...
    async def get(playlistLink: str) -> Union[dict, str, None]:
        ...
        """
        try:
            if not playlistLink.startswith("http") and playlistLink.startswith("PL"):
                playlistLink = f"https://www.youtube.com/playlist?list={playlistLink}"

            playlist = PlaylistCore(playlistLink, None, ResultMode.dict, 2)
            await playlist.async_create()

            if playlist.playlistComponent and "info" in playlist.playlistComponent:
                result = playlist.playlistComponent["info"].copy()
                result["videos"] = playlist.playlistComponent.get("videos", [])
                return result
    ...
    async def getVideos(playlistLink: str) -> Union[dict, str, None]:
        ...
        try:
            ...
            if playlist.playlistComponent and "videos" in playlist.playlistComponent:
                return playlist.playlistComponent["videos"]
            return playlist.playlistComponent
        except Exception:
            return None
...
class Transcript:
    ...
    async def get(videoLink: str, params: str = None):
        try:
            ...
            return getattr(transcript_core, "result", None)
        except Exception:
            return None
...
class Channel(ChannelCore):
    ...
    @staticmethod
    async def get(
        channel_id: str, request_type: str = ChannelRequestType.playlists
    ) -> Union[dict, None]:
        try:
            channel_core = ChannelCore(channel_id, request_type)
            await channel_core.async_create()
            return getattr(channel_core, "result", None)
        except Exception:
            return None
