...
from innertubei.core.constants import ResultMode, requestPayload, searchKey
...
class ChannelSearchCore(RequestCore, ComponentHandler):
    ...
    async def _asyncRequest(self) -> None:
        ...
        try:
            self.response = orjson.loads(request.content)
        except:
    ...
    def result(self, mode: int = ResultMode.dict) -> Union[str, dict]:
        ...
        if mode == ResultMode.json:
            return orjson.dumps(
                {"result": self.response}, option=orjson.OPT_INDENT_2
            ).decode("utf-8")
