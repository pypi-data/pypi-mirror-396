...
from innertubei.core.constants import ResultMode, searchKey
...
class VideoCore(RequestCore):
    ...
    async def async_html_create(self):
        ...
        response = await self.asyncPostRequest()
        self.HTMLresponseSource = orjson.loads(response.content)

    def __parseSource(self) -> None:
        try:
            self.responseSource = orjson.loads(self.response)
    ...
    def __result(self, mode: int) -> Union[dict, str]:
        if mode == ResultMode.dict:
            ...
            return orjson.dumps(
                self.__videoComponent, option=orjson.OPT_INDENT_2
            ).decode("utf-8")
