...
def produce_nb_grad(r=1):
    def nb_grad(pred, act):
        return (act+r)/(pred+r) - act/pred  # when r high, reduces to (p-a)/p
    return nb_grad
...
links = {"Unity": Unity_link, "Root": Root_link,
         "Log": Log_link, "Logit": Logit_link}
linkgrads = {"Unity": Unity_link_grad, "Root": Root_link_grad,
             "Log": Log_link_grad, "Logit": Logit_link_grad}
delinks = {"Unity": Unity_delink, "Root": Root_delink,
           "Log": Log_delink, "Logit": Logit_delink}
...
def Max_mlink_2(x1, x2):
    # I am aware that this line in particular is an abomination, and refuse to care. Rewrite it at your whim, dear reader.
...
addsmoothing_LRAs = [addsmoothing_LRA_0, addsmoothing_LRA_1,
                     addsmoothing_LRA_2, addsmoothing_LRA_3, addsmoothing_LRA_4]
...
addsmoothing_LRAs_erry = [addsmoothing_LRA_0_erry, addsmoothing_LRA_1_erry,
                          addsmoothing_LRA_2_erry, addsmoothing_LRA_3_erry, addsmoothing_LRA_4_erry]
...
def get_effect_of_this_cont_col(ser, cont):
    ...
    # Everything too early gets with the program
    effectOfCol.loc[(x <= cont[0][0])] = cont[0][1]
    for i in range(len(cont)-1):
        x1 = cont[i][0]
        x2 = cont[i+1][0]
        y1 = cont[i][1]
        y2 = cont[i+1][1]
        effectOfCol.loc[(x >= x1) & (x <= x2)] = (
            (x-x1)*y2 + (x2-x)*y1)/(x2 - x1)
    # Everything too late gets with the program
    effectOfCol.loc[x >= cont[-1][0]] = cont[-1][1]
    return effectOfCol
...
def get_dodel_lossgrad(model, df, pfname):
    def dodel_lossgrad(pri, lia, mar):
        ...
        prob = Logit_link(comb)

        dpdc = Logit_link_grad(comb)
        dcde = 1
...
def get_gradient_of_this_cont_col(ser, cont):
    x = ser
    # at the points themselves, and everywhere not covered, grad is 0
    gradOfCol = pd.Series([0.0]*len(ser))
    for i in range(len(cont)-1):
        x1 = cont[i][0]
        x2 = cont[i+1][0]
        y1 = cont[i][1]
