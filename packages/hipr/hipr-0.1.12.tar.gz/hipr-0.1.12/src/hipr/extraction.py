"""Parameter extraction utilities for hipr."""
...
from hipr.constraints import PatternConstraint, validate_constraint_conflicts
from hipr.models import MakeableModel
from hipr.typedefs import DefaultSentinel, HyperMarker
...
def create_field_info(
  ...
  if isinstance(default_value, DefaultSentinel):
    ...
    if is_sequence_origin(origin):
      # Use list as the default factory for all sequence types
      return (transformed_type, Field(default_factory=list, **constraint_kwargs))
    if is_mapping_origin(origin):
      # Use dict as the default factory for all mapping types
...
def get_type_hints_safe(obj: Any) -> dict[str, Any]:
  ...
  try:
    ...
    ns = globalns or {}
    if "Hyper" not in ns:
      from hipr.typedefs import Hyper
...
def extract_function_hyper_params(
  ...
  for name, param in param_list:
    ...
    # Only extract Hyper-annotated parameters
    if not has_hyper_marker(field_type):
      continue
