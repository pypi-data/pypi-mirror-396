        ...
        for source in raw_sources:
            ...
            if source_path.is_file():
                ...
                try:
                    ...
                    temp_fd, temp_archive = tempfile.mkstemp(suffix='.tar.gz', prefix='warp_')
                    os.close(temp_fd)
                    ...
                    compressed_size = Path(temp_archive).stat().st_size
                    ...
                    # Note: Registry update for content_hash skipped during backup
                    # (would require write access which may conflict with read-only mode)
        ...
        for f in uploaded_files:
            if f["type"] == "resource":
                ...
                manifest_raw_data.append({
                    ...
                    "compression_format": "tar.gz" if f.get("compressed_size") else None,
                    "uri": s3_uri,
                })
            ...
            for source in raw_sources:
                ...
                if needs_download:
                    if is_compressed:
                        ...
                        temp_fd, temp_archive = tempfile.mkstemp(suffix='.tar.gz', prefix='warp_dl_')
                        import os
                        os.close(temp_fd)
                        ...
                        if expected_size is not None and file_size != expected_size:
                            print(f"  ✗ Downloaded raw data size mismatch: {file_size} != {expected_size}")
                            restore_path.unlink()
                            raise ValueError(f"Downloaded file size mismatch for raw data {source_path.name}")
...
def register_remote_dataset(
    ...
    return _register(dataset_id, resources=resource_uris, schema=schema, file_format=file_format, metadata={
...
def _upload_sync_manifest(
    ...
    cloud_manifest = build_cloud_manifest(
        ...
        raw_data=[],    # TODO: track raw data in sync
