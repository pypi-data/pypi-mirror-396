...
def _try_auto_register_from_remote(workspace: str, name: str, version: str) -> Optional[Dict[str, Any]]:
    """
    Try to auto-register a dataset from remote S3 storage.
    ...
    try:
        from .storage import register_remote_dataset, DEFAULT_BUCKET, normalize_bucket_name

        dataset_id = f"warpdata://{workspace}/{name}"
        version_arg = None if version == "latest" else version
        ...
        # Try to register from bucket
        print(f"  📥 Auto-registering {workspace}/{name} from s3://{bucket}...")
        register_remote_dataset(dataset_id, bucket=bucket, version=version_arg)
