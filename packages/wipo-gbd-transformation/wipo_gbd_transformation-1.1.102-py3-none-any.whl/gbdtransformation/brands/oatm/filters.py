...
def translate_type(header):
    ...
    if code == 'MD Marque internationale': return 'TRADEMARK'
    if code == 'MR Marque régionale': return 'TRADEMARK'
...
def translate_kind(trademark, header):
    ...
    if subcode == 'MR Marque de services': return ['Individual']
    if subcode == 'MR Marque de produits': return  ['Individual']
    if subcode == 'MR Marque collective': return ['Collective']
    if subcode == 'MD Marque protocole Madrid': return ['Individual']
    if subcode == 'MR-BG2015 Marque régionale':  return ['Individual']
...
def translate_event(event):
    if ' ' in event:
        event = event.split()[1].capitalize()
