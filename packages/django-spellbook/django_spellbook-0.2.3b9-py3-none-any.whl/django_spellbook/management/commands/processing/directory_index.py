...
class DirectoryIndexBuilder:
    ...
    def _get_parent_directory_info(self, directory: Path) -> Tuple[Optional[str], Optional[str]]:
        ...
        # Add url_prefix to the URL for proper navigation
        if self.url_prefix:
            if parent_url:
    ...
    def _detect_subdirectories(
        ...
        for subdir_name, page_count in sorted(subdirs.items()):
            ...
            # Add url_prefix to subdirectory URL for proper absolute navigation
            if self.url_prefix and subdir_url:
                subdir_url = f"{self.url_prefix}/{subdir_url}"
            elif self.url_prefix and not subdir_url:
    ...
    def _collect_page_metadata(self, files: List[ProcessedFile]) -> List[dict]:
        ...
        for pf in files:
            ...
            # Add url_prefix for absolute navigation in directory index
            if self.url_prefix and page_url:
                page_url = f"{self.url_prefix}/{page_url}"
            elif self.url_prefix and not page_url:
