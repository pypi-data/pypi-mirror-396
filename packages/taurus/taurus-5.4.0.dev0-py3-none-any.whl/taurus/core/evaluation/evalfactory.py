...
class EvaluationFactory(Singleton, TaurusFactory, Logger):
    ...
    elementTypesMap = {
        TaurusElementType.Authority: EvaluationAuthority,
    ...
    def getAttribute(self, attr_name, **kwargs):
        ...
        evaluator device associated to this attribute will also be created if
        necessary.
    ...
    def _storeAttr(self, attr):
        ...
        if exists is not None:
            if exists == attr:
                ...
                self.debug(
                    "%s has already been registered with a different object!" % name
                )
                raise DoubleRegistration
