...
from taurus.core.evaluation.evalvalidator import (
    ...
    EvaluationAuthorityNameValidator,
...
@names(
    name="//localhost",  # using implicit scheme!
    out=("eval://localhost", "//localhost", "localhost"),
)
...
@valid(
    name='eval:@c=mymod.MyClass("a/b",q=())',
...
# check names
@names(name="eval://localhost/@foo", out=("eval://localhost/@foo", "@foo", "foo"))
@names(name="eval://dev=foo", out=("eval://localhost/@foo", "@foo", "foo"))
...
@valid(
    name='eval://db=localhost;dev=taurus.core.evaluation.dev_example.FreeSpaceDevice;getFreeSpace("/")/1024/1024',  # noqa
    strict=False,
)
@valid(name="eval:1")
# =========================================================================
...
@valid(
    name="eval://rand(256, 128)",
    groups={
        "attrname": "rand(256, 128)",
        "_expr": "rand(256, 128)",
        "_subst": None,
        "fragment": None,
        "__STRICT__": False,
...
@valid(
    name="eval:foo=10;rand(256, 128)+foo#label",
    groups={
        "attrname": "foo=10;rand(256, 128)+foo",
...
@valid(
    name="eval://rand(256, 128)?configuration=label",
    groups={
        "attrname": "rand(256, 128)",
        "_expr": "rand(256, 128)",
        "_subst": None,
...
@valid(
    name="eval:foo=1;bar=2;tar=3;foo+bar+tar#label",
    groups={
        "attrname": "foo=1;bar=2;tar=3;foo+bar+tar",
        "_expr": "foo+bar+tar",
...
@valid(
    name="eval:a={eval:1#f};a+{eval:2#foo}#bar",
    groups={
        "attrname": "a={eval:1#f};a+{eval:2#foo}",
        "_expr": "a+{eval:2#foo}",
        "_subst": "a={eval:1#f};",
        "query": None,
        "fragment": "bar",
...
@valid(
    name="eval:a={tango:a/b/c/d};x=2;a*x",
    groups={
        "attrname": "a={tango:a/b/c/d};x=2;a*x",
        "_expr": "a*x",
...
@names(
    name="eval:a={tango:a/b/c/d};x=2;a*x",
    out=(
        "eval://localhost/@DefaultEvaluator/"
...
@names(
    name="eval:a={eval:1#f};a+{eval:2#foo}#bar",
    out=(
        "eval://localhost/@DefaultEvaluator/a="
...
@names(
    name='eval:a={eval:"a"};{eval:0}*a*{eval:1+{eval:a=2;a*a}}',
    out=(
        "eval://localhost/@DefaultEvaluator/"
...
@names(
    name="eval:@Foo/{NonExistingAlias/foo}+5",
    out=(None, "@Foo/{NonExistingAlias/foo}+5", "foo+5"),
)
@names(
    name="eval:{eval:1}+{eval:2}",
    out=(
        "eval://localhost/@DefaultEvaluator/"
        + "{eval://localhost/@DefaultEvaluator/1}+"
        + "{eval://localhost/@DefaultEvaluator/2}",
...
@names(
    name="eval:{eval:{eval:foo=1;foo}}",
    out=(
        "eval://localhost/@DefaultEvaluator/{eval://localhost/"
...
@names(
    name="eval:x=0;x+x-x*x/x+cos(x)-xxx*xy+yx+yxz+x+{w/x/y/x}*x*{eval:x}",
    out=(
        "eval://localhost/@DefaultEvaluator/"
...
@names(
    name="eval://dev=foo;x*y?x=2;y=3",
    out=("eval://localhost/@foo/x=2;y=3;x*y", "@foo/x=2;y=3;x*y", "x*y"),
)
# old syntax gets transformed into new one!
@names(
    name="eval://x+x-x*x/x+cos(x)-xxx*xy+yx+yxz+x+{w/x/y/x}*x*{eval:x}?x=0",
    out=(
        "eval://localhost/@DefaultEvaluator/"
        + "x=0;x+x-x*x/x+cos(x)-xxx*xy+yx+yxz+x+"
        + "{tango://%s/w/x/y/x}*x*" % default_tango_authority
...
@valid(
    name="eval:a={tango:a/b/c/d};{eval:0}+a+{eval:1+{eval:a=2;a*a}}",
    groups={"_evalrefs": ["tango:a/b/c/d", "eval:0", "eval:1+{eval:a=2;a*a}"]},
)
@valid(
    name='eval:a="{eval:0}";b={eval:1};a*b+"{eval:2}"*{eval:3}#{attr.label}',
    groups={"_evalrefs": ["eval:1", "eval:3"]},
)
@valid(
    name='eval:a="{eval:1}";b={eval:1};a*b+"{eval:3}"*{eval:3}#{attr.label}',
    groups={"_evalrefs": ["eval:1", "eval:3"]},
)
@valid(
    name='eval:"{eval:1}"+"{eval:1}"*{eval:1}*{eval:3}',
    groups={"_evalrefs": ["eval:1", "eval:3"]},
)
@valid(
    name='eval:"{eval:1}"*{eval:1}+"{eval:1}"*{eval:3}',
...
@valid(
    name="eval:@c=mymod.MyClass()/c.foo",
    groups={"devname": "@c=mymod.MyClass()", "attrname": "c.foo"},
)
@valid(
    name="eval:@c=mymod.MyClass()/c.foo()",
    groups={"devname": "@c=mymod.MyClass()", "attrname": "c.foo()"},
...
@valid(
    name="eval:@c=mymod.MyClass(1)/float(c.foo())",
    groups={"devname": "@c=mymod.MyClass(1)", "attrname": "float(c.foo())"},
)
@valid(
    name='eval:@c=mymod.MyClass("a/b",q=())/float(c.foo())',
    groups={
        "devname": '@c=mymod.MyClass("a/b",q=())',
...
@names(
    name="eval:1#units",
    out=("eval://localhost/@DefaultEvaluator/1", "1", "1", "units"),
)
@names(
    name="eval:@Foo/a={tango:a/b/c/d};x=2;a*x#",
    out=(
        "eval://localhost/@Foo/a={tango://%s/a/b/c/d};x=2;a*x"
...
# old syntax gets transformed into new one!
@names(
    name="eval://1?configuration=units",
    out=("eval://localhost/@DefaultEvaluator/1", "1", "1", "units"),
)
# old syntax gets transformed into new one!
@names(
    name="eval://dev=Foo;a*x?a={tango:a/b/c/d};x=2?configuration",
    out=(
        "eval://localhost/@Foo/a={tango://%s/a/b/c/d};x=2;a*x"
        % default_tango_authority,
        "@Foo/a={tango:a/b/c/d};x=2;a*x",
...
# old syntax gets transformed into new one!
@names(
    name="eval://dev=Foo;a*x?a={tango:a/b/c/d};x=2?configuration=label",
    out=(
        "eval://localhost/@Foo/a={tango://%s/a/b/c/d};x=2;a*x"
        % default_tango_authority,
        "@Foo/a={tango:a/b/c/d};x=2;a*x",
