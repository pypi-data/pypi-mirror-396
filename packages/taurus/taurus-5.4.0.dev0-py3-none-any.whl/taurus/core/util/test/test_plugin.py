...
from taurus.core.util.plugin import EntryPointAlike, selectEntryPoints
...
def test_selectEntryPoints():
    ...
    mock_entry_point(lines=["{0} = obj_{0}".format(n) for n in names], group=group)
    ...
    some = selectEntryPoints(group, exclude=("foo2",), include=("bar2", "b.*1", "f.*"))
