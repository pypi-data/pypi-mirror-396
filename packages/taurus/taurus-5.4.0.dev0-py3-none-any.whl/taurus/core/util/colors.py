...
TAURUS_STATE_COLOR_OVERRIDES.update(
    {"DevState.{}".format(k): v for k, v in TAURUS_STATE_COLOR_OVERRIDES.items()}
)

policy = getattr(tauruscustomsettings, "TANGO_DEVICE_STATE_COLOR_POLICY", "Taurus")
if policy == "Taurus":
    DEVICE_STATE_DATA.update({**TANGO_STATE_COLORS, **TAURUS_STATE_COLOR_OVERRIDES})
else:  # Tango policy
