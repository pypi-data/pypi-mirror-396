...
class TaurusFactory(object):
    ...
    def getAuthorityNameValidator(self):
        raise NotImplementedError(
            "getAuthorityNameValidator cannot be called for abstract TaurusFactory"
        )

    def getDeviceNameValidator(self):
        raise NotImplementedError(
            "getDeviceNameValidator cannot be called for abstract TaurusFactory"
        )

    def getAttributeNameValidator(self):
        raise NotImplementedError(
            "getAttributeNameValidator cannot be called for abstract TaurusFactory"
        )
    ...
    def readAttributeFirstTime(self, attribute: TaurusAttribute):
        """Register a :class:`~taurus.core.taurusattribute.TaurusAttribute` for a
        one-shot *first read*.

        This method lazily creates a :class:`TaurusFirstRead` helper the first time
        it is needed and enqueues the given *attribute* so it is read once as part
        of the initialization/startup sequence. It is useful to prime the value
        cache before regular subscriptions/polling kick in.

        :param attribute: Taurus attribute to be read once on initialization.
        :type attribute: :class:`~taurus.core.taurusattribute.TaurusAttribute`
        :return: None
        :rtype: None
        """
        if not self.first_time_reader:
            self.first_time_reader = TaurusFirstRead()
        self.first_time_reader.addAttribute(attr=attribute)
