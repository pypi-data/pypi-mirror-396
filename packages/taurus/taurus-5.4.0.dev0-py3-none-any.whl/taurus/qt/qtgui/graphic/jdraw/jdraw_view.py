...
class TaurusJDrawSynopticsView(Qt.QGraphicsView, TaurusBaseWidget):
    ...
    def fitting(self, ADJUST_FRAME=False):
        ...
        if ADJUST_FRAME:  # adjust the "white" frame around the synoptic
            self.debug(
                "\tResizing: " + "size(%s,%s),hint(%s,%s),srect(%s,%s),parent(%s,%s)",
                *self.get_sizes(),
            )
    ...
    def resizeEvent(self, event):
        ...
        try:
            self.debug(
                "In TaurusJDrawSynopticsView(" + self._fileName + ").resizeEvent()"
            )
    ...
    @Qt.pyqtSlot("QString")
    def setModel(self, model, alias=None, delayed=False, trace=False, **kwargs):
