...
def test_form_itemFactory_selection(qtbot):
    ...
    assert type(select2[0]) is EntryPoint
    ...
    assert type(select3[0]) is EntryPointAlike
...
@pytest.mark.forked
def test_form_cwidget_bck_compat(qtbot, nodb_dev):  # noqa: F811
    ...
    try:
        ...
        qtbot.wait_until(lambda: len(w) == 2, timeout=3200)
        assert type(w[0]) is _DummyTV
        assert type(w[1]) is TaurusValue
        ...
        w.setModel([nodb_dev, "eval:1"])
        qtbot.wait_until(lambda: len(w) == 2, timeout=3200)
        assert type(w[0]) is TaurusValue
        assert type(w[1]) is TaurusValue
        assert w.getCustomWidgetMap() == {}
    finally:
...
def test_form_itemFactory_loading(qtbot):
    ...
    assert type(w[1]) is _DummyTV
    # errored in _BadFactory, ignored by _DummyItemFactory
    assert type(w[2]) is TaurusValue
