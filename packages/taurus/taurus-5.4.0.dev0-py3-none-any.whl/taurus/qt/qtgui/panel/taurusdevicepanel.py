...
from taurus.qt.qtgui.panel.taurusform import TaurusCommandsForm, TaurusForm
from taurus.qt.qtgui.taurusgui.taurusgui import TaurusGui
...
class TaurusDevicePanel(TaurusWidget):
    ...
    def setModel(self, model, pixmap=None, **kwargs):
        ...
        self.trace(
            "In TaurusDevicePanel.setModel(%s(%s),%s)" % (raw, modelclass, pixmap)
        )
        ...
        try:
            ...
            if hasattr(self, "_statelabel"):
                self._statelabel.setModel(model + "/state")  # TODO: Tango-centric
    ...
    def get_comms_form(self, device, form=None, parent=None):
        ...
        try:
            ...
            if params:
                ...
                form.setViewFilters(
                    [
                        lambda c: str(c.cmd_name).lower() not in ("state", "status")
                        and any(searchCl(s[0], str(c.cmd_name)) for s in params)
                    ]
                )
...
@click.option(
    ...
    "--filter",
    help=(
        "regexp to filter for attributes to show " + "(it can be passed more than once)"
    ),
    multiple=True,
)
