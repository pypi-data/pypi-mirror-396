...
from taurus.external.qt import Qt, QtWidgets, compat
...
class TaurusGui(TaurusMainWindow):
    ...
    **Example 3: mixing declarative and programmatic ways**
    ...
    def __init__(
        ...
        msg = "%s is ready" % QtWidgets.QApplication.instance().applicationName()
        self.newShortMessage.emit(msg)

        if self.defaultConfigRecursionDepth >= 0:
            self.newShortMessage.emit("Running in Safe Mode. Settings not loaded")
            self.warning(
                "Safe mode: \n"
    ...
    def loadConfiguration(self, confname):
        ...
        try:
            if confname is None:
                ...
                confname = str(QtWidgets.QApplication.instance().applicationName())
                self._confDirectory = ""
                ...
                try:
                    spec = importlib.util.spec_from_file_location(name, confname)
    ...
    def _loadOrgName(self, conf, xmlroot):
        orgname = getattr(
            ...
            self.__getVarFromXML(
                xmlroot,
                "ORGANIZATION",
                str(QtWidgets.QApplication.instance().organizationName()) or "Taurus",
            ),
        )
    ...
    def _loadCustomLogo(self, conf, xmlroot):
        custom_logo = getattr(
            ...
            getattr(
                conf,
                "LOGO",
                self.__getVarFromXML(xmlroot, "CUSTOM_LOGO", "logos:taurus.png"),
            ),
        )
    ...
    def _loadSingleInstance(self, conf, xmlroot):
        ...
        single_inst = getattr(
            ...
            "SINGLE_INSTANCE",
            (
                self.__getVarFromXML(xmlroot, "SINGLE_INSTANCE", "True").lower()
                == "true"
            ),
        )
        if single_inst:
    ...
    def _loadInstrumentsFromPool(self, conf, xmlroot, macro_server_name):
        ...
        instruments_from_pool = getattr(
            ...
            (
                self.__getVarFromXML(xmlroot, "INSTRUMENTS_FROM_POOL", "False").lower()
                == "true"
            ),
        )
