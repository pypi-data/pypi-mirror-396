...
from taurus import Release, tauruscustomsettings
...
class TaurusMainWindow(Qt.QMainWindow, TaurusBaseContainer):
    ...
    SAVE_SETTINGS_ON_CLOSE = getattr(tauruscustomsettings, "SAVE_SETTINGS_ON_CLOSE")
    ...
    def _onSpawnRpdb2(self):
        try:
            ...
            Qt.QMessageBox.warning(
                self,
                "Rpdb2 not installed",
                "Cannot spawn debugger: Rpdb2 is not installed on your system.",
            )
            return
    ...
    def _onSpawnRConsole(self):
        try:
            ...
            Qt.QMessageBox.warning(
                self,
                "rfoo not installed",
                "Cannot spawn debugger: rfoo is not installed on your system.",
            )
            return
        ...
        if hasattr(self, "_rconsole_port"):
            Qt.QMessageBox.information(
                self,
                "rconsole running",
                "A rconsole is already running on port %d" % self._rconsole_port,
            )
            return
    ...
    def getFactorySettingsFileName(self):
        ...
        basename = "%s.ini" % str(QtWidgets.QApplication.instance().applicationName())
    ...
    def showHelpAbout(self):
        ...
        appversion = str(QtWidgets.QApplication.instance().applicationVersion())
