...
from taurus.qt.qtgui.container import TaurusScrollArea, TaurusWidget
...
class TaurusForm(TaurusWidget):
    ...
    def chooseModels(self):
        ...
        if self.__modelChooserDlg is None:
            ...
            self.__modelChooserDlg.modelChooser.modelsAdded.connect(self._onModelsAdded)
    ...
    def _onModelsAdded(self, models):
        self.__currentModelChooser = self.__modelChooserDlg.modelChooser.currentWidget()
        ...
        for i, model in enumerate(self.getModel()):
            ...
            for ep in self._itemFactories:
                ...
                if widget is not None:
                    self.debug("widget for '%s' provided by '%s'", model, ep.name)
                    break
...
@click.option(
    ...
    type=click.STRING,
    help=(
        "Enable item factories matching INC pattern" + " (can be passed multiple times)"
    ),
)
@click.option(
    "--exc-fact",
...
@click.option("--ls-fact", is_flag=True, help="List the available item factories")
