...
class TaurusModelModel(Qt.QAbstractListModel):
    ...
    def dropMimeData(self, data, action, row, column, parent):
        ...
        if data.hasFormat(TAURUS_ATTR_MIME_TYPE):
            ...
            items = (
                bytes(data.data(TAURUS_MODEL_LIST_MIME_TYPE)).decode("utf-8").split()
            )
        elif data.hasText():
            items = [str(data.text())]
        else:
...
class TaurusModelList(Qt.QListView):
    ...
    def __init__(self, parent=None, items=None, designMode=False):
        ...
        self.editDisplayAction = Qt.QAction("Edit the display (leave the source)", self)
        self.editDisplayAction.setIcon(Qt.QIcon("actions:format-text-italic.svg"))
