...
class ProjectPage(BasePage):
    ...
    def validatePage(self):
        ...
        configs_found = glob.glob(
            os.path.join(dirname, "tgconf_*", self.wizard().getXmlConfigFileName())
        )
...
class CustomLogoPage(BasePage):
    ...
    def _setImage(self, image):
        if type(image) is Qt.QPixmap:
            self._customLogo.setPixmap(image.scaled(60, 200, Qt.Qt.KeepAspectRatio))
        elif type(image) is Qt.QImage:
            self._customLogo.setPixmap(
                Qt.QPixmap().fromImage(image).scaled(60, 200, Qt.Qt.KeepAspectRatio)
            )
        else:
...
class PanelsPage(BasePage):
    ...
    def fromXml(self, xml):
        ...
        for child in panelNodes:
            ...
            if name:
                self._panels.append((name, etree.tostring(child, encoding="unicode")))
    ...
    def generateXml(self):
        ...
        if synopticList:
            ...
            for src in synopticList:
                ...
                etree.SubElement(synoptics, "synoptic", str=os.path.basename(dst))
