...
import numpy
from guiqwt.image import INTERP_NEAREST, ImageItem, RGBImageItem, XYImageItem
...
class TaurusTrend2DScanItem(TaurusTrend2DItem):
    ...
    def _scanLineReceived(self, recordData):
        ...
        if self._zBuffer is None:
            self._zBuffer = ArrayBuffer(
                numpy.zeros((min(16, self.maxBufferSize), chval.size), dtype="d"),
                maxSize=self.maxBufferSize,
            )
...
def taurusImageMain():
    ...
    try:
        ...
        from guiqwt.tools import AnnotatedHCursorTool, AnnotatedVCursorTool

        VCursorTool, HCursorTool = AnnotatedVCursorTool, AnnotatedHCursorTool
    except ImportError:
        from guiqwt.tools import HCursorTool, VCursorTool
