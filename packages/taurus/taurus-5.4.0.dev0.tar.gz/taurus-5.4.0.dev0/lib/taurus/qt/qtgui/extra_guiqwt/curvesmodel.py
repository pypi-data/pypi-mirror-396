...
from guiqwt.styles import AxesParam, CurveParam, update_style_attr
...
class TaurusCurveItemTableModel(Qt.QAbstractTableModel):
    ...
    def flags(self, index):  # use this to set the editable flag when fix is selected
        ...
        if column in (X, Y):
            ...
            return Qt.Qt.ItemIsEnabled | Qt.Qt.ItemIsEditable | Qt.Qt.ItemIsDragEnabled
        return Qt.Qt.ItemIsEnabled | Qt.Qt.ItemIsEditable | Qt.Qt.ItemIsDragEnabled
...
class CurveItemConfDlg(Qt.QWidget):
    ...
    def _onEditParsAction(self):
        ...
        if row >= 0:
            ...
            params = [
                p for p in (c.curveparam, c.axesparam, c.taurusparam) if p is not None
            ]
            if params:
