        ...
        for exp in expressions:
            ...
            for dev in devs:
                if any(c in attribute for c in ".*[]()+?"):
                    ...
                    try:
                        taurus_dp = TaurusManager().getFactory()().getDevice(dev)
...
class TaurusGrid(QtWidgets.QFrame, TaurusBaseWidget):
    ...
    def setModel(
        ...
        **kwargs,
        ...
        if isinstance(model, dict):
            ...
            self.debug(
                ("In TaurusGrid.setModel(...): modelNames are %s" % (self._modelNames))[
                    :100
                ]
                + "..."
            )
    ...
    def setRowLabels(self, rows):
        ...
        try:
            ...
            for i in range(len(self.rows)):
                section = self.rows[i]
                self.table.setVerticalHeaderItem(i, QtWidgets.QTableWidgetItem(section))
        except Exception as e:
    ...
    def create_widgets_table(self, models):
        ...
        for i in range(len(self.columns)):
            equipment = self.columns[i]
            self.table.setHorizontalHeaderItem(i, QtWidgets.QTableWidgetItem(equipment))
    ...
    def setItemSelected(self, item_name="", selected=True):
        ...
        if isinstance(item_name, TaurusValue):
            ...
            self.trace(
                "In TaurusGrid.setItemSelected(%s,%s)" % (str(item_name), selected)
            )
            if item_name:
