...
class CaselessList(list):
    ...
    def findentry(self, item):
        ...
        if not isinstance(item, str):
            raise TypeError(
                'Members of this object must be strings. You supplied "%s"' % type(item)
            )
        for entry in self:
    ...
    def extend(self, item):
        ...
        if not isinstance(item, list):
            raise TypeError(
                'You can only extend lists with lists. You supplied "%s"' % type(item)
            )
        for entry in item:
            if not isinstance(entry, str):
                raise TypeError(
    ...
    def insert(self, i, x):
        ...
        if not isinstance(x, str):
            raise TypeError(
                'Members of this object must be strings. You supplied "%s"' % type(x)
            )
