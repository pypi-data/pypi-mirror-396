...
from taurus import info, warning
...
def x_axis_mode_option(choices=("t", "n")):
    ...
    o = click.option(
        ...
        default=choices[0],
        show_default=True,
        help=("X axis mode: " + ", ".join([k + " for " + hlp[k] for k in choices])),
    )
    return o
...
def plot_cmd(models, config_file, x_axis_mode, demo, window_name, use_alt, ls_alt):
...
def trend_cmd(
    ...
    try:
        TTrend, epname = _load_class_from_group(EP_GROUP_TREND, include=[use_alt])
    except Exception:
        _print_alts(EP_GROUP_TREND)
        sys.exit(1)
...
def trend2d_cmd(
    ...
    # get the selected alternative
    try:
        TTrend2D, epname = _load_class_from_group(EP_GROUP_TREND2D, include=[use_alt])
    except Exception:
        _print_alts(EP_GROUP_TREND2D)
        sys.exit(1)
...
def image_cmd(model, demo, window_name, color_mode, use_alt, ls_alt):
    ...
    # get the selected alternative
    try:
        TImage, epname = _load_class_from_group(EP_GROUP_IMAGE, include=[use_alt])
    except Exception:
        _print_alts(EP_GROUP_IMAGE)
        sys.exit(1)
