...
from ibm_watsonx_data_integration.services.datastage.models.enums import FILTER
from pydantic import Field
from typing import ClassVar


class filter(BaseStage):
    ...
    buf_mode: FILTER.BufMode | None = Field(FILTER.BufMode.default, alias="buf_mode")
    buf_mode_ronly: FILTER.BufModeRonly | None = Field(FILTER.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: FILTER.CollType | None = Field(FILTER.CollType.auto, alias="coll_type")
    collation_sequence: FILTER.CollationSequence | None = Field(
        FILTER.CollationSequence.OFF, alias="collation_sequence"
    )
    combinability: FILTER.Combinability | None = Field(FILTER.Combinability.auto, alias="combinability")
    ...
    execmode: FILTER.Execmode | None = Field(FILTER.Execmode.default_par, alias="execmode")
    first: FILTER.First | None = Field(FILTER.First.false, alias="first")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    hide: bool | None = Field(False, alias="hide")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    key_col_select: FILTER.KeyColSelect | None = Field(FILTER.KeyColSelect.default, alias="keyColSelect")
    ...
    nulls: FILTER.Nulls | None = Field(FILTER.Nulls.first, alias="nulls")
    ...
    part_type: FILTER.PartType | None = Field(FILTER.PartType.auto, alias="part_type")
    ...
    preserve: FILTER.Preserve | None = Field(FILTER.Preserve.default_propagate, alias="preserve")
    ...
    reject: FILTER.Reject | None = Field(FILTER.Reject.false, alias="reject")
    ...
    where_properties: list | None = Field([], alias="whereProperties")

    def _validate_parameters(self) -> tuple[set, set]:
        include = set()
        exclude = set()

        include.add("nulls") if (()) else exclude.add("nulls")
        ...
        (
            include.add("where_properties")
            if (self.output_count and self.output_count > 0)
            else exclude.add("where_properties")
        )
