...
from ibm_watsonx_data_integration.services.datastage.models.enums import SQLSERVER
from pydantic import Field
from typing import ClassVar


class sqlserver(BaseStage):
    ...
    connection: SqlserverConn = SqlserverConn()
    ...
    buf_mode_ronly: SQLSERVER.BufModeRonly | None = Field(SQLSERVER.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: SQLSERVER.BufferingMode | None = Field(SQLSERVER.BufferingMode.default, alias="buf_mode")
    ...
    collecting: SQLSERVER.Collecting | None = Field(SQLSERVER.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: SQLSERVER.CombinabilityMode | None = Field(
        SQLSERVER.CombinabilityMode.auto, alias="combinability"
    )
    ...
    decimal_rounding_mode: SQLSERVER.DecimalRoundingMode | None = Field(
        SQLSERVER.DecimalRoundingMode.floor, alias="decimal_rounding_mode"
    )
    ...
    execution_mode: SQLSERVER.ExecutionMode | None = Field(SQLSERVER.ExecutionMode.default_par, alias="execmode")
    existing_table_action: SQLSERVER.ExistingTableAction | None = Field(
        SQLSERVER.ExistingTableAction.append, alias="existing_table_action"
    )
    ...
    partition_type: SQLSERVER.PartitionType | None = Field(SQLSERVER.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: SQLSERVER.PreservePartitioning | None = Field(
        SQLSERVER.PreservePartitioning.default_propagate, alias="preserve"
    )
    ...
    read_method: SQLSERVER.ReadMethod | None = Field(SQLSERVER.ReadMethod.general, alias="read_mode")
    ...
    reject_uses: SQLSERVER.RejectUses | None = Field(SQLSERVER.RejectUses.rows, alias="reject_uses")
    ...
    sampling_type: SQLSERVER.SamplingType | None = Field(SQLSERVER.SamplingType.none, alias="sampling_type")
    ...
    sorting_key: SQLSERVER.KeyColSelect | None = Field(SQLSERVER.KeyColSelect.default, alias="keyColSelect")
    ...
    table_action: SQLSERVER.TableAction | None = Field(SQLSERVER.TableAction.append, alias="table_action")
    ...
    version_number: int | None = Field(None, alias="version_number")
    write_mode: SQLSERVER.WriteMode | None = Field(SQLSERVER.WriteMode.insert, alias="write_mode")
    ...
    def _validate_source(self) -> tuple[set, set]:
        ...
        (
            ...
            if (
                ...
                and ((not self.user_defined_function) or (not self.user_defined_function))
            )
            else exclude.add("version_number")
        )
        (
        ...
        (
            ...
            and (
                (self.user_defined_function and "false" in str(self.user_defined_function))
                or (not self.user_defined_function)
            )
            else exclude.add("version_number")
        )
        (
