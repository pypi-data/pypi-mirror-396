"""Module for Teradata connection."""
...
class TeradataConn(BaseConnection):
    """Connection class for Teradata."""

    model_config = ConfigDict(populate_by_name=True)

    datasource_type: ClassVar[str] = "96ec8f53-a818-4ba1-bd8d-c86cc33a0b45"
    asset_id: str | None = None
    proj_id: str | None = None
    name: str = Field(None, alias="name")
    authentication_method: TERADATA_CONNECTION.AuthenticationMethod | None = Field(
        TERADATA_CONNECTION.AuthenticationMethod.td2, alias="authentication_method"
    )
    client_character_set: str | None = Field(None, alias="client_charset")
    ...
    port: int | None = Field(1025, alias="port")
    query_band_expression: str | None = Field(None, alias="query_band")
    session_username: str | None = Field(None, alias="session_username")
    ...
    enable_trusted_session: bool | None = Field(False, alias="use_trusted_session")
    ...
    def _validate(self) -> tuple[set[str], set[str]]:
        ...
        (
            include.add("enable_trusted_session")
            if (self.hidden_dummy_property1)
            else exclude.add("enable_trusted_session")
        )
        include.add("vaulted_properties") if (self.hidden_dummy_property1) else exclude.add("vaulted_properties")
        include.add("session_username") if (self.hidden_dummy_property1) else exclude.add("session_username")
        (
    ...
    @classmethod
    def _from_dict(cls, properties: dict[str, Any]) -> "TeradataConn":
