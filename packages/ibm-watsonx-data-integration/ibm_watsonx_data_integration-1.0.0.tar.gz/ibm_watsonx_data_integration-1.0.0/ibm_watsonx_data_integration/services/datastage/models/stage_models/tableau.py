...
from ibm_watsonx_data_integration.services.datastage.models.enums import TABLEAU
from pydantic import Field
from typing import ClassVar


class tableau(BaseStage):
    ...
    connection: TableauConn = TableauConn()
    buf_free_run_ronly: int | None = Field(50, alias="buf_free_run_ronly")
    buf_mode_ronly: TABLEAU.BufModeRonly | None = Field(TABLEAU.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: TABLEAU.BufferingMode | None = Field(TABLEAU.BufferingMode.default, alias="buf_mode")
    byte_limit: str | None = Field(None, alias="byte_limit")
    collecting: TABLEAU.Collecting | None = Field(TABLEAU.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: TABLEAU.CombinabilityMode | None = Field(TABLEAU.CombinabilityMode.auto, alias="combinability")
    ...
    execution_mode: TABLEAU.ExecutionMode | None = Field(TABLEAU.ExecutionMode.default_par, alias="execmode")
    ...
    invalid_data_handling: TABLEAU.InvalidDataHandling | None = Field(
        TABLEAU.InvalidDataHandling.fail, alias="invalid_data_handling"
    )
    ...
    partition_type: TABLEAU.PartitionType | None = Field(TABLEAU.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: TABLEAU.PreservePartitioning | None = Field(
        TABLEAU.PreservePartitioning.default_propagate, alias="preserve"
    )
    ...
    read_mode: TABLEAU.ReadMode | None = Field(TABLEAU.ReadMode.read_single, alias="read_mode")
    ...
    sorting_key: TABLEAU.KeyColSelect | None = Field(TABLEAU.KeyColSelect.default, alias="keyColSelect")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            ...
            if ((self.infer_schema) or (self.infer_schema and "#" in str(self.infer_schema)))
    ...
    def _validate_source(self) -> tuple[set, set]:
        ...
        include.add("type_mapping") if (self.infer_schema) else exclude.add("type_mapping")
        (
            include.add("type_mapping")
            if (self.infer_schema == "true" or self.infer_schema)
            else exclude.add("type_mapping")
        )
        (
