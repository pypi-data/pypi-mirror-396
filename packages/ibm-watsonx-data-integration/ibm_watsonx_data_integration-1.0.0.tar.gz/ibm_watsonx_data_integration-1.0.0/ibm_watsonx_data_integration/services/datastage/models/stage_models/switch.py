...
from ibm_watsonx_data_integration.services.datastage.models.enums import SWITCH
from pydantic import Field
from typing import ClassVar


class switch(BaseStage):
    ...
    buf_mode: SWITCH.BufMode | None = Field(SWITCH.BufMode.default, alias="buf_mode")
    buf_mode_ronly: SWITCH.BufModeRonly | None = Field(SWITCH.BufModeRonly.default, alias="buf_mode_ronly")
    case: list | None = Field([], alias="case")
    ci_cs: SWITCH.CiCs | None = Field(SWITCH.CiCs.cs, alias="ci-cs")
    coll_type: SWITCH.CollType | None = Field(SWITCH.CollType.auto, alias="coll_type")
    collation_sequence: SWITCH.CollationSequence | None = Field(
        SWITCH.CollationSequence.OFF, alias="collation_sequence"
    )
    combinability: SWITCH.Combinability | None = Field(SWITCH.Combinability.auto, alias="combinability")
    current_output_link_type: str = Field("PRIMARY", alias="currentOutputLinkType")
    discard: int | None = Field(None, alias="discard")
    ...
    execmode: SWITCH.Execmode | None = Field(SWITCH.Execmode.default_par, alias="execmode")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    hash_selector: str = Field("hashSelector", alias="hashSelector")
    hide: bool | None = Field(False, alias="hide")
    if_not_found: SWITCH.IfNotFound | None = Field(SWITCH.IfNotFound.fail, alias="ifNotFound")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    key: SWITCH.Key = Field(SWITCH.Key.custom, alias="key")
    key_col_select: SWITCH.KeyColSelect | None = Field(SWITCH.KeyColSelect.default, alias="keyColSelect")
    ...
    part_type: SWITCH.PartType | None = Field(SWITCH.PartType.auto, alias="part_type")
    ...
    preserve: SWITCH.Preserve | None = Field(SWITCH.Preserve.default_propagate, alias="preserve")
    ...
    selection: SWITCH.Selection | None = Field(SWITCH.Selection.user, alias="selection")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        include.add("ci_cs") if (()) else exclude.add("ci_cs")
        (
            include.add("case")
            if (
                (
                    self.selection
                    and (
                        (hasattr(self.selection, "value") and self.selection.value != "hash")
                        or (self.selection != "hash")
                    )
                )
                and (
                    self.selection
                    and (
                        (hasattr(self.selection, "value") and self.selection.value != "auto")
                        or (self.selection != "auto")
                    )
                )
                and (
                    self.selection
                    and (
                        (hasattr(self.selection, "value") and self.selection.value == "user")
                        or (self.selection == "user")
                    )
                )
            )
            else exclude.add("case")
        )
        (
            include.add("discard")
            if (
            ...
            else exclude.add("discard")
        )
        (
            include.add("if_not_found")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value != "hash") or (self.selection != "hash")
                )
            )
            else exclude.add("if_not_found")
        )
        (
            include.add("hash_selector")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value == "hash") or (self.selection == "hash")
            ...
            else exclude.add("hash_selector")
        )
    ...
    def _get_parameters_props(self) -> dict:
        ...
        props = {
            ...
            "additional_properties_set_user_defined_mapping",
