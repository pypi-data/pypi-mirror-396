# -------------------------------------------------------------------
# This file was auto-generated. Do not edit directly.
# Generated by: StageGeneratorV2
# -------------------------------------------------------------------

"""This module defines the Data Rules stage."""

from ibm_watsonx_data_integration.services.datastage.models.flow.dag import StageNode
from ibm_watsonx_data_integration.services.datastage.models.stage_models.data_rules import data_rules
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from ibm_watsonx_data_integration.services.datastage.models.flow.dag import DAG


class DataRulesStage(StageNode):
    """This class defines the Data Rules stage."""

    def __init__(self, dag: "DAG", configuration: data_rules = None, *, label: str = None) -> None:
        """Initializes an instance of a Data Rules stage."""
        super().__init__(dag, configuration=configuration, label=label)
        self.configuration: data_rules = configuration or data_rules()
