...
from ibm_watsonx_data_integration.services.datastage.models.enums import WATSONX_DATA
from pydantic import Field
from typing import ClassVar


class watsonx_data(BaseStage):
    ...
    connection: WatsonxDataConn = WatsonxDataConn()
    buf_free_run_ronly: int | None = Field(50, alias="buf_free_run_ronly")
    buf_mode_ronly: WATSONX_DATA.BufModeRonly | None = Field(WATSONX_DATA.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: WATSONX_DATA.BufferingMode | None = Field(WATSONX_DATA.BufferingMode.default, alias="buf_mode")
    byte_limit: str | None = Field(None, alias="byte_limit")
    catalog_name: str | None = Field(None, alias="catalog_name")
    collecting: WATSONX_DATA.Collecting | None = Field(WATSONX_DATA.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: WATSONX_DATA.CombinabilityMode | None = Field(
        WATSONX_DATA.CombinabilityMode.auto, alias="combinability"
    )
    ...
    ds_java_heap_size: int | None = Field(512, alias="_java._heap_size")
    ...
    execution_engine_id: str | None = Field(None, alias="execution_engine_id")
    execution_mode: WATSONX_DATA.ExecutionMode | None = Field(WATSONX_DATA.ExecutionMode.default_par, alias="execmode")
    ...
    part_size: int | None = Field(20, alias="write_part_size")
    ...
    partition_type: WATSONX_DATA.PartitionType | None = Field(WATSONX_DATA.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: WATSONX_DATA.PreservePartitioning | None = Field(
        WATSONX_DATA.PreservePartitioning.default_propagate, alias="preserve"
    )
    ...
    read_method: WATSONX_DATA.ReadMethod | None = Field(WATSONX_DATA.ReadMethod.general, alias="read_mode")
    ...
    show_success_notification: bool | None = Field(False, alias="show_success_notification")
    sort_instructions: str | None = Field("", alias="sortInstructions")
    sort_instructions_text: str | None = Field("", alias="sortInstructionsText")
    sorting_key: WATSONX_DATA.KeyColSelect | None = Field(WATSONX_DATA.KeyColSelect.default, alias="keyColSelect")
    ...
    table_action: WATSONX_DATA.TableAction | None = Field(WATSONX_DATA.TableAction.create, alias="table_action")
    ...
    def _validate_target(self) -> tuple[set, set]:
        ...
        include.add("catalog_name") if (not self.static_statement) else exclude.add("catalog_name")
        include.add("schema_name") if (not self.static_statement) else exclude.add("schema_name")
        include.add("table_name") if (not self.static_statement) else exclude.add("table_name")
        (
            include.add("static_statement")
            if ((not self.catalog_name) and (not self.schema_name) and (not self.table_name))
            else exclude.add("static_statement")
        )
        (
            include.add("catalog_name")
            if ((not self.static_statement) or (self.static_statement and "#" in str(self.static_statement)))
            else exclude.add("catalog_name")
        )
        (
            include.add("schema_name")
