...
from ibm_watsonx_data_integration.services.datastage.models.enums import AZURESYNAPSE
from pydantic import Field
from typing import ClassVar


class azuresynapse(BaseStage):
    ...
    connection: AzuresynapseConn = AzuresynapseConn()
    ...
    buf_mode_ronly: AZURESYNAPSE.BufModeRonly | None = Field(AZURESYNAPSE.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: AZURESYNAPSE.BufferingMode | None = Field(AZURESYNAPSE.BufferingMode.default, alias="buf_mode")
    byte_limit: str | None = Field(None, alias="byte_limit")
    call_procedure_statement: str | None = Field(None, alias="call_statement")
    collecting: AZURESYNAPSE.Collecting | None = Field(AZURESYNAPSE.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: AZURESYNAPSE.CombinabilityMode | None = Field(
        AZURESYNAPSE.CombinabilityMode.auto, alias="combinability"
    )
    ...
    decimal_rounding_mode: AZURESYNAPSE.DecimalRoundingMode | None = Field(
        AZURESYNAPSE.DecimalRoundingMode.floor, alias="decimal_rounding_mode"
    )
    ...
    execution_mode: AZURESYNAPSE.ExecutionMode | None = Field(AZURESYNAPSE.ExecutionMode.default_par, alias="execmode")
    existing_table_action: AZURESYNAPSE.ExistingTableAction | None = Field(
        AZURESYNAPSE.ExistingTableAction.append, alias="existing_table_action"
    )
    ...
    hashing_column: str = Field(None, alias="hashing_column")
    hashing_required: bool | None = Field(False, alias="hashing_required")
    hide: bool | None = Field(False, alias="hide")
    infer_schema: bool | None = Field(True, alias="rcp")
    inout_parameters: str | None = Field("false", alias="inout_parameters")
    ...
    partition_type: AZURESYNAPSE.PartitionType | None = Field(AZURESYNAPSE.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: AZURESYNAPSE.PreservePartitioning | None = Field(
        AZURESYNAPSE.PreservePartitioning.default_propagate, alias="preserve"
    )
    ...
    read_method: AZURESYNAPSE.ReadMethod | None = Field(AZURESYNAPSE.ReadMethod.general, alias="read_mode")
    ...
    reject_uses: AZURESYNAPSE.RejectUses | None = Field(AZURESYNAPSE.RejectUses.rows, alias="reject_uses")
    ...
    sampling_type: AZURESYNAPSE.SamplingType | None = Field(AZURESYNAPSE.SamplingType.none, alias="sampling_type")
    ...
    sorting_key: AZURESYNAPSE.KeyColSelect | None = Field(AZURESYNAPSE.KeyColSelect.default, alias="keyColSelect")
    ...
    table_action: AZURESYNAPSE.TableAction | None = Field(AZURESYNAPSE.TableAction.append, alias="table_action")
    ...
    write_mode: AZURESYNAPSE.WriteMode | None = Field(AZURESYNAPSE.WriteMode.insert, alias="write_mode")
    ...
    def _validate_target(self) -> tuple[set, set]:
        ...
        (
            include.add("hashing_column")
            if ((self.hashing_required) and (self.hashing_required))
            else exclude.add("hashing_column")
        )
        (
        ...
        (
            ...
            else exclude.add("hashing_required")
        )
        (
            include.add("table_name")
            if (
        ...
        (
            include.add("hashing_column")
            if (
                ((self.hashing_required) or (self.hashing_required and "#" in str(self.hashing_required)))
                and ((self.hashing_required) or (self.hashing_required and "#" in str(self.hashing_required)))
            )
            else exclude.add("hashing_column")
        )
        (
        ...
        (
            ...
            if (
                ...
                and ((not self.create_statement) or (self.create_statement and "#" in str(self.create_statement)))
                and (
                    (
                        (
                            self.write_mode
        ...
        (
            include.add("hashing_column")
            if (self.hashing_required) and (self.hashing_required == "true" or self.hashing_required)
            else exclude.add("hashing_column")
        )
        (
            include.add("create_statement")
