...
class dataset(BaseStage):
    ...
    buf_mode_ronly: DATASET.BufModeRonly | None = Field(DATASET.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: DATASET.BufferingMode | None = Field(DATASET.BufferingMode.default, alias="buf_mode")
    collecting: DATASET.Collecting | None = Field(DATASET.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: DATASET.CombinabilityMode | None = Field(DATASET.CombinabilityMode.auto, alias="combinability")
    ...
    execution_mode: DATASET.ExecutionMode | None = Field(DATASET.ExecutionMode.default_par, alias="execmode")
    file: str = Field(None, alias="dataset")
    ...
    missing_columns_mode: DATASET.MissingColumnsMode | None = Field(
        DATASET.MissingColumnsMode.custom, alias="missingcolmode"
    )
    ...
    partition_type: DATASET.PartitionType | None = Field(DATASET.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: DATASET.PreservePartitioning | None = Field(
        DATASET.PreservePartitioning.default_propagate, alias="preserve"
    )
    ...
    sorting_key: DATASET.KeyColSelect | None = Field(DATASET.KeyColSelect.default, alias="keyColSelect")
    ...
    update_policy: DATASET.UpdatePolicy | None = Field(DATASET.UpdatePolicy.overwrite, alias="datasetmode")
    ...
    def _get_input_ports_props(self) -> dict:
        ...
        props = {"create_data_asset", "data_asset_name", "file", "runtime_column_propagation", "update_policy"}
    ...
    def _get_output_ports_props(self) -> dict:
        include, exclude = self._validate()
        props = {"file", "missing_columns_mode"}
        required = {"file"}
        props = props & (self.model_fields_set | required)
        conflict = exclude & self.model_fields_set & props
        if conflict:
