...
class VerticaConn(BaseConnection):
    """Connection class for Vertica."""

    model_config = ConfigDict(populate_by_name=True)

    datasource_type: ClassVar[str] = "vertica"
    ...
    the_certificate_for_a_key: str | None = Field(None, alias="key_certificate")
    password: str = Field(None, alias="password")
    port: int = Field(5433, alias="port")
    ...
    def _validate(self) -> tuple[set[str], set[str]]:
        ...
        (
            include.add("the_certificate_for_a_key")
            if ((self.port_is_ssl_enabled) and (self.private_key))
            else exclude.add("the_certificate_for_a_key")
        )
        (
        ...
        include.add("private_key") if (self.port_is_ssl_enabled) else exclude.add("private_key")
        (
        ...
        (
            include.add("the_certificate_for_a_key")
            if (self.port_is_ssl_enabled == "true" or self.port_is_ssl_enabled) and (self.private_key)
            else exclude.add("the_certificate_for_a_key")
        )
        return include, exclude

    @classmethod
    def _from_dict(cls, properties: dict[str, Any]) -> "VerticaConn":
