...
class Date(BaseField):
    """Class for date schema field."""

    def __init__(self, name: str) -> None:
        """Initializes a schema field."""
        super().__init__(name)
        self._set_model_type("date")
        self._set_app_type_code("DATE")
        self._set_app_odbc_type("DATE")
        self.configuration.metadata.min_length = 0
        self.configuration.metadata.max_length = 10

    def _create_from_model(self, model: models.FieldModelComplex) -> T:
        field = Date(model.name)
    ...
    def days_since(self, days_since: int) -> "BaseField":
        """Sets days since."""
        return self._days_since(days_since)
    ...
    def use_current_date(self) -> "BaseField":
        """Sets use current date."""
        return self._use_current_date()
