...
from ibm_watsonx_data_integration.services.datastage.models.flow.dag import DAG, SuperNodeRef, SuperNode
    ...
    def _export_flow(self, master_gen: MasterCodeGenerator, f_content: bytes, f_info: ZipInfo):
        ...
        if flow_model.external_paramsets:
            for paramset in flow_model.external_paramsets:
                try:
                    ...
                    ps_data = {"entity": json.loads(ps_content)}
                    param_set = ParameterSet(project=None, **ps_data)
                    ...
                    # if we cannot find the parameter set we can just move on, it will be handled later
                    pass
        ...
        def replace_super_node_refs(dtc: DAG) -> None:
            for node in dtc.nodes():
                if isinstance(node, SuperNodeRef) or (isinstance(node, Subflow) and not node.is_local):
                    ...
                    try:
                        ...
                        super_node = Subflow(parent_dag=dtc, dag=DAG(), name=f"{subflow_name} MISSING", label=node.label, is_local=False)
        ...
        #     if isinstance(node, Subflow) and not node.is_local:
        #         subflow_name = node.name
        #         subflow_json = json.loads(self.zip_importer.find_subflow(subflow_name))
        #         subflow_model = models.Flow(**subflow_json)
        #         subflow_dag_gen = DAGGenerator(subflow_model)
