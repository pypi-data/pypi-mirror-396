...
class BigsqlConn(BaseConnection):
    """Connection class for Bigsql."""

    model_config = ConfigDict(populate_by_name=True)

    datasource_type: ClassVar[str] = "2bdd9544-f13a-47b6-b6c3-f5964a08066a"
    asset_id: str | None = None
    proj_id: str | None = None
    name: str = Field(None, alias="name")
    access_token: str = Field(None, alias="access_token")
    alternate_hostname: str = Field(None, alias="alternate_host")
    alternate_port: str = Field(None, alias="alternate_port")
    ...
    use_alternate_server: bool | None = Field(None, alias="use_alternate_server")
    ...
    def _validate(self) -> tuple[set[str], set[str]]:
        ...
        (
            include.add("alternate_hostname")
            if (
                (not self.satellite_location_id)
                and (not self.satellite_connector_id)
                and (not self.secure_gateway_id)
                and (self.use_alternate_server)
            )
            else exclude.add("alternate_hostname")
        )
        (
        ...
        (
            ...
            else exclude.add("alternate_port")
        )
        (
        ...
        (
            include.add("alternate_hostname")
            if (not self.satellite_location_id)
            and (not self.satellite_connector_id)
            and (not self.secure_gateway_id)
            and (self.use_alternate_server == "true" or self.use_alternate_server)
            else exclude.add("alternate_hostname")
        )
        (
    ...
    @classmethod
    def _from_dict(cls, properties: dict[str, Any]) -> "BigsqlConn":
