...
T = TypeVar("T", bound="Bit")


class Bit(BaseField):
    """Class for bit schema field."""

    def __init__(self, name: str) -> None:
        """Initializes a schema field."""
        super().__init__(name)
        self._set_model_type("boolean")
        self._set_app_type_code("BOOLEAN")
        self._set_app_odbc_type("BIT")
        self.configuration.metadata.min_length = 6
        self.configuration.metadata.max_length = 6

    def _create_from_model(self, model: models.FieldModelComplex) -> T:
        field = Bit(model.name)
