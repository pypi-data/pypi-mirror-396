"""Module for tinyint schema field."""
...
T = TypeVar("T", bound="TinyInt")


class TinyInt(BaseField):
    """Class for tinyint schema field."""

    def __init__(self, name: str) -> None:
        """Initializes a schema field."""
        super().__init__(name)
        self._set_model_type("integer")
        self._set_app_type_code("INT8")
        self._set_app_odbc_type("TINYINT")
    ...
    def _create_from_model(self, model: models.FieldModelComplex) -> T:
        field = TinyInt(model.name)
        field.configuration = model
        return field

    def cluster_key_change(self) -> "BaseField":
        """Sets cluster key change."""
        return self._cluster_key_change()

    def difference(self) -> "BaseField":
        """Sets difference."""
        return self._difference()

    def unsigned(self) -> "BaseField":
