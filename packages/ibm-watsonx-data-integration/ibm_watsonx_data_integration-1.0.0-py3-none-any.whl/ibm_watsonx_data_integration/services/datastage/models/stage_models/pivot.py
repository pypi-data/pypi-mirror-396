...
from ibm_watsonx_data_integration.services.datastage.models.enums import PIVOT
from pydantic import Field
from typing import ClassVar


class pivot(BaseStage):
    ...
    array_size: int | None = Field(1, alias="array_size")
    ...
    buf_mode: PIVOT.BufMode | None = Field(PIVOT.BufMode.default, alias="buf_mode")
    buf_mode_ronly: PIVOT.BufModeRonly | None = Field(PIVOT.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: PIVOT.CollType | None = Field(PIVOT.CollType.auto, alias="coll_type")
    combinability: PIVOT.Combinability | None = Field(PIVOT.Combinability.auto, alias="combinability")
    ...
    execmode: PIVOT.Execmode | None = Field(PIVOT.Execmode.default_par, alias="execmode")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    hide: bool | None = Field(False, alias="hide")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    key_col_select: PIVOT.KeyColSelect | None = Field(PIVOT.KeyColSelect.default, alias="keyColSelect")
    ...
    part_type: PIVOT.PartType | None = Field(PIVOT.PartType.auto, alias="part_type")
    ...
    pivot_custom_input_o_s_h: str | None = Field("", alias="PivotCustomInputOSH")
    pivot_custom_o_s_h: str | None = Field("", alias="PivotCustomOSH")
    pivot_index: bool | None = Field(False, alias="pivotIndex")
    pivot_properties: list | None = Field([], alias="pivotProperties")
    pivot_type: PIVOT.PivotType | None = Field(PIVOT.PivotType.pivot, alias="pivot_type")
    preserve: PIVOT.Preserve | None = Field(PIVOT.Preserve.default_propagate, alias="preserve")
    ...
    vertical_pivot_custom_o_s_h: str | None = Field("", alias="VerticalPivotCustomOSH")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            ...
            if (
                self.pivot_type
                and (
                    (hasattr(self.pivot_type, "value") and self.pivot_type.value == "verticalpivot")
                    or (self.pivot_type == "verticalpivot")
                )
            )
            else exclude.add("array_size")
        )
        (
            include.add("vertical_pivot_custom_o_s_h")
            if ((not self.vertical_pivot_custom_o_s_h) and (self.vertical_pivot_custom_o_s_h))
            else exclude.add("vertical_pivot_custom_o_s_h")
        )
        (
            include.add("pivot_custom_o_s_h")
            if ((not self.vertical_pivot_custom_o_s_h) and (self.vertical_pivot_custom_o_s_h))
            else exclude.add("pivot_custom_o_s_h")
        )
        (
            include.add("pivot_custom_input_o_s_h")
            if ((not self.vertical_pivot_custom_o_s_h) and (self.vertical_pivot_custom_o_s_h))
            else exclude.add("pivot_custom_input_o_s_h")
        )
