...
from ibm_watsonx_data_integration.services.datastage.models.enums import MYSQL_AMAZON
from pydantic import Field
from typing import ClassVar


class mysql_amazon(BaseStage):
    ...
    connection: MysqlAmazonConn = MysqlAmazonConn()
    abort_after: int | None = Field(0, alias="abort_after_percent")
    abort_after_rows: int | None = Field(0, alias="reject_number")
    abort_when: MYSQL_AMAZON.AbortWhen | None = Field(MYSQL_AMAZON.AbortWhen.rows, alias="reject_uses")
    add_procedure_return_value_to_schema: bool | None = Field(False, alias="add_proccode_column")
    add_to_reject_row: MYSQL_AMAZON.RejectRowsPropertiesSet | None = Field(None, alias="rejectRowsPropertiesSet")
    batch_size: int | None = Field(2000, alias="batch_size")
    buf_free_run_ronly: int | None = Field(50, alias="buf_free_run_ronly")
    buf_mode_ronly: MYSQL_AMAZON.BufModeRonly | None = Field(MYSQL_AMAZON.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: MYSQL_AMAZON.BufferingMode | None = Field(MYSQL_AMAZON.BufferingMode.default, alias="buf_mode")
    byte_limit: str | None = Field(None, alias="byte_limit")
    call_procedure_statement: str | None = Field(None, alias="call_statement")
    collecting: MYSQL_AMAZON.Collecting | None = Field(MYSQL_AMAZON.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: MYSQL_AMAZON.CombinabilityMode | None = Field(
        MYSQL_AMAZON.CombinabilityMode.auto, alias="combinability"
    )
    ...
    credentials_input_method_ssl: MYSQL_AMAZON.CredentialsInputMethodSsl | None = Field(
        MYSQL_AMAZON.CredentialsInputMethodSsl.enter_credentials_manually, alias="credentials_input_method_ssl"
    )
    ...
    decimal_rounding_mode: MYSQL_AMAZON.DecimalRoundingMode | None = Field(
        MYSQL_AMAZON.DecimalRoundingMode.floor, alias="decimal_rounding_mode"
    )
    ...
    execution_mode: MYSQL_AMAZON.ExecutionMode | None = Field(MYSQL_AMAZON.ExecutionMode.default_par, alias="execmode")
    existing_table_action: MYSQL_AMAZON.ExistingTableAction | None = Field(
        MYSQL_AMAZON.ExistingTableAction.append, alias="existing_table_action"
    )
    ...
    input_method: MYSQL_AMAZON.InputMethod | None = Field(
        MYSQL_AMAZON.InputMethod.enter_credentials_manually, alias="credentials_input_method"
    )
    ...
    partition_type: MYSQL_AMAZON.PartitionType | None = Field(MYSQL_AMAZON.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: MYSQL_AMAZON.PreservePartitioning | None = Field(
        MYSQL_AMAZON.PreservePartitioning.default_propagate, alias="preserve"
    )
    proc_message: bool | None = Field(False, alias="no_procmessage_column")
    ...
    read_method: MYSQL_AMAZON.ReadMethod | None = Field(MYSQL_AMAZON.ReadMethod.general, alias="read_mode")
    ...
    reject_from_link: MYSQL_AMAZON.RejectFromLink | None = Field(-1, alias="reject_from_link")
    reject_rows_based_on_selected_condition: MYSQL_AMAZON.RejectConditionPropertiesOption | None = Field(
    ...
    sampling_type: MYSQL_AMAZON.SamplingType | None = Field(MYSQL_AMAZON.SamplingType.none, alias="sampling_type")
    ...
    sorting_key: MYSQL_AMAZON.KeyColSelect | None = Field(MYSQL_AMAZON.KeyColSelect.default, alias="keyColSelect")
    stable: bool | None = Field(None, alias="part_stable")
    stage_description: list | None = Field("", alias="stageDescription")
    start_count_after_rows: int | None = Field(1, alias="reject_threshold")
    static_statement: str = Field(None, alias="static_statement")
    stored_procedure_name: str | None = Field(None, alias="procedure_name")
    table_action: MYSQL_AMAZON.TableAction | None = Field(MYSQL_AMAZON.TableAction.append, alias="table_action")
    ...
    write_mode: MYSQL_AMAZON.WriteMode | None = Field(MYSQL_AMAZON.WriteMode.insert, alias="write_mode")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            include.add("abort_when")
            if (self.reject_from_link and self.reject_from_link > -1)
            else exclude.add("abort_when")
        )
        (
            include.add("abort_after_rows")
            if (self.reject_from_link and self.reject_from_link > -1)
            else exclude.add("abort_after_rows")
        )
        (
        ...
        (
            include.add("start_count_after_rows")
            if (self.abort_when == "percent")
            else exclude.add("start_count_after_rows")
        )
    ...
    def _validate_target(self) -> tuple[set, set]:
        ...
        include.add("abort_after") if (self.abort_when == "Percent") else exclude.add("abort_after")
        (
            include.add("start_count_after_rows")
            if (self.abort_when == "Percent")
            else exclude.add("start_count_after_rows")
        ...
        include.add("abort_after_rows") if (self.abort_when == "Rows") else exclude.add("abort_after_rows")
        (
            include.add("runtime_column_propagation")
    ...
    def _get_target_props(self) -> dict:
        ...
        props = {
            ...
            "queue_upper_size_ronly",
            "reject_from_link",
            "reject_rows_based_on_selected_condition",
            "runtime_column_propagation",
            "schema_name",
            "show_coll_type",
            "show_part_type",
