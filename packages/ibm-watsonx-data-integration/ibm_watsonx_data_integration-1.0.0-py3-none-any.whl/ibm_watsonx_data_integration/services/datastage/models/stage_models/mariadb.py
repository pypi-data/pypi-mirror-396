...
from ibm_watsonx_data_integration.services.datastage.models.enums import MARIADB
from pydantic import Field
from typing import ClassVar


class mariadb(BaseStage):
    ...
    connection: MariadbConn = MariadbConn()
    batch_size: int | None = Field(2000, alias="batch_size")
    buf_free_run_ronly: int | None = Field(50, alias="buf_free_run_ronly")
    buf_mode_ronly: MARIADB.BufModeRonly | None = Field(MARIADB.BufModeRonly.default, alias="buf_mode_ronly")
    buffer_free_run_percent: int | None = Field(50, alias="buf_free_run")
    buffering_mode: MARIADB.BufferingMode | None = Field(MARIADB.BufferingMode.default, alias="buf_mode")
    byte_limit: str | None = Field(None, alias="byte_limit")
    collecting: MARIADB.Collecting | None = Field(MARIADB.Collecting.auto, alias="coll_type")
    column_metadata_change_propagation: bool | None = Field(None, alias="auto_column_propagation")
    combinability_mode: MARIADB.CombinabilityMode | None = Field(MARIADB.CombinabilityMode.auto, alias="combinability")
    ...
    decimal_rounding_mode: MARIADB.DecimalRoundingMode | None = Field(
        MARIADB.DecimalRoundingMode.floor, alias="decimal_rounding_mode"
    )
    ...
    execution_mode: MARIADB.ExecutionMode | None = Field(MARIADB.ExecutionMode.default_par, alias="execmode")
    existing_table_action: MARIADB.ExistingTableAction | None = Field(
        MARIADB.ExistingTableAction.append, alias="existing_table_action"
    )
    ...
    partition_type: MARIADB.PartitionType | None = Field(MARIADB.PartitionType.auto, alias="part_type")
    ...
    preserve_partitioning: MARIADB.PreservePartitioning | None = Field(
        MARIADB.PreservePartitioning.default_propagate, alias="preserve"
    )
    ...
    read_method: MARIADB.ReadMethod | None = Field(MARIADB.ReadMethod.general, alias="read_mode")
    ...
    sampling_type: MARIADB.SamplingType | None = Field(MARIADB.SamplingType.none, alias="sampling_type")
    ...
    sorting_key: MARIADB.KeyColSelect | None = Field(MARIADB.KeyColSelect.default, alias="keyColSelect")
    ...
    table_action: MARIADB.TableAction | None = Field(MARIADB.TableAction.append, alias="table_action")
    ...
    write_mode: MARIADB.WriteMode | None = Field(MARIADB.WriteMode.insert, alias="write_mode")
            ...
            if (
                (
                    (
                        self.write_mode
                        and (
                            (hasattr(self.write_mode, "value") and self.write_mode.value == "delete_insert")
                            or (self.write_mode == "delete_insert")
                        )
                    )
                    or (
                        self.write_mode
            ...
            if (
                ...
                and (
                    (
                        hasattr(self.write_mode, "value")
                        and self.write_mode.value
                        and "delete_insert" in str(self.write_mode.value)
                    )
                    or ("delete_insert" in str(self.write_mode))
                )
                and self.write_mode
                and (
                    (
