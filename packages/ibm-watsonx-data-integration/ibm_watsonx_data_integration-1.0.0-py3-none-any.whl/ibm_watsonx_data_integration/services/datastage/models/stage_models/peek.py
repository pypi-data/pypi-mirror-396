...
from ibm_watsonx_data_integration.services.datastage.models.enums import PEEK
from pydantic import Field
from typing import ClassVar


class peek(BaseStage):
    ...
    all: PEEK.All | None = Field(PEEK.All.false, alias="all")
    ...
    buf_mode: PEEK.BufMode | None = Field(PEEK.BufMode.default, alias="buf_mode")
    buf_mode_ronly: PEEK.BufModeRonly | None = Field(PEEK.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: PEEK.CollType | None = Field(PEEK.CollType.auto, alias="coll_type")
    columns: PEEK.Columns | None = Field(PEEK.Columns.true, alias="columns")
    combinability: PEEK.Combinability | None = Field(PEEK.Combinability.auto, alias="combinability")
    current_output_link_type: str = Field("PRIMARY", alias="currentOutputLinkType")
    dataset: PEEK.Dataset | None = Field(PEEK.Dataset.false, alias="dataset")
    delim: str | None = Field(None, alias="delim")
    ...
    execmode: PEEK.Execmode | None = Field(PEEK.Execmode.default_par, alias="execmode")
    ...
    key_col_select: PEEK.KeyColSelect | None = Field(PEEK.KeyColSelect.default, alias="keyColSelect")
    ...
    name: PEEK.Name | None = Field(PEEK.Name.true, alias="name")
    nrecs: int | None = Field(10, alias="nrecs")
    ...
    part_type: PEEK.PartType | None = Field(PEEK.PartType.auto, alias="part_type")
    ...
    preserve: PEEK.Preserve | None = Field(PEEK.Preserve.default_propagate, alias="preserve")
    ...
    selection: PEEK.Selection | None = Field(PEEK.Selection.true, alias="selection")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            include.add("nrecs")
            if (self.all and ((hasattr(self.all, "value") and self.all.value != "all") or (self.all != "all")))
            else exclude.add("nrecs")
        )
        (
            include.add("part")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value == "part") or (self.selection == "part")
                )
            )
            else exclude.add("part")
        )
        (
            include.add("field")
            if (
                self.columns
                and ((hasattr(self.columns, "value") and self.columns.value == "field") or (self.columns == "field"))
            )
            else exclude.add("field")
        )
        include.add("period") if (()) else exclude.add("period")
        include.add("skip") if (()) else exclude.add("skip")
        include.add("delim") if (()) else exclude.add("delim")
    ...
    def _get_parameters_props(self) -> dict:
        ...
        props = {
            ...
            "additional_properties_set_rows",
            "all",
