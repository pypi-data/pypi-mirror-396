...
from ibm_watsonx_data_integration.services.datastage.models.enums import BLOOM_FILTER
from pydantic import Field
from typing import ClassVar


class bloom_filter(BaseStage):
    ...
    buf_mode: BLOOM_FILTER.BufMode | None = Field(BLOOM_FILTER.BufMode.default, alias="buf_mode")
    buf_mode_ronly: BLOOM_FILTER.BufModeRonly | None = Field(BLOOM_FILTER.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: BLOOM_FILTER.CollType | None = Field(BLOOM_FILTER.CollType.auto, alias="coll_type")
    combinability: BLOOM_FILTER.Combinability | None = Field(BLOOM_FILTER.Combinability.auto, alias="combinability")
    current_output_link_type: str = Field("PRIMARY", alias="currentOutputLinkType")
    data_date: str | None = Field(None, alias="data_date")
    ...
    drop_old: BLOOM_FILTER.DropOld | None = Field(BLOOM_FILTER.DropOld.false, alias="drop_old")
    ...
    execmode: BLOOM_FILTER.Execmode | None = Field(BLOOM_FILTER.Execmode.default_par, alias="execmode")
    filter: str = Field(None, alias="filter")
    filter_phases: int | None = Field(5, alias="filter_phases")
    filter_size: str = Field(None, alias="filter_size")
    flag_column: BLOOM_FILTER.FlagColumn | None = Field(BLOOM_FILTER.FlagColumn.false, alias="flag_column")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    hide: bool | None = Field(False, alias="hide")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    key_col_select: BLOOM_FILTER.KeyColSelect | None = Field(BLOOM_FILTER.KeyColSelect.default, alias="keyColSelect")
    ...
    part_type: BLOOM_FILTER.PartType | None = Field(BLOOM_FILTER.PartType.auto, alias="part_type")
    ...
    preserve: BLOOM_FILTER.Preserve | None = Field(BLOOM_FILTER.Preserve.default_propagate, alias="preserve")
    previous_days: int | None = Field(1, alias="previous_days")
    ...
    ref_date: str | None = Field("", alias="ref_date")
    runtime_column_propagation: bool | None = Field(None, alias="runtime_column_propagation")
    selection: BLOOM_FILTER.Selection | None = Field(BLOOM_FILTER.Selection.create, alias="selection")
    ...
    truncate: BLOOM_FILTER.Truncate | None = Field(BLOOM_FILTER.Truncate.false, alias="truncate")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        include.add("previous_days") if ((()) or (()) or (())) else exclude.add("previous_days")
        include.add("data_date") if (()) else exclude.add("data_date")
        include.add("flag_column") if (()) else exclude.add("flag_column")
        include.add("ref_date") if (()) else exclude.add("ref_date")
        include.add("drop_old") if (()) else exclude.add("drop_old")
        include.add("filter_phases") if (()) else exclude.add("filter_phases")
        include.add("truncate") if (()) else exclude.add("truncate")
        ...
        (
            include.add("outputlink_ordering_list")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value == "process")
                    or (self.selection == "process")
                )
            )
            else exclude.add("outputlink_ordering_list")
        )
