...
from ibm_watsonx_data_integration.services.datastage.models.enums import AGGREGATOR
from pydantic import Field
from typing import ClassVar


class aggregator(BaseStage):
    ...
    buf_mode: AGGREGATOR.BufMode | None = Field(AGGREGATOR.BufMode.default, alias="buf_mode")
    buf_mode_ronly: AGGREGATOR.BufModeRonly | None = Field(AGGREGATOR.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: AGGREGATOR.CollType | None = Field(AGGREGATOR.CollType.auto, alias="coll_type")
    collation_sequence: AGGREGATOR.CollationSequence | None = Field(
        AGGREGATOR.CollationSequence.OFF, alias="collation_sequence"
    )
    combinability: AGGREGATOR.Combinability | None = Field(AGGREGATOR.Combinability.auto, alias="combinability")
    count_field_properties: list | None = Field([], alias="countFieldProperties")
    current_output_link_type: str = Field("PRIMARY", alias="currentOutputLinkType")
    default_to_decimal: str | None = Field("8,2", alias="defaultToDecimal")
    ...
    execmode: AGGREGATOR.Execmode | None = Field(AGGREGATOR.Execmode.default_par, alias="execmode")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    hide: bool | None = Field(False, alias="hide")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    key_col_select: AGGREGATOR.KeyColSelect | None = Field(AGGREGATOR.KeyColSelect.default, alias="keyColSelect")
    ...
    method: AGGREGATOR.Method | None = Field(AGGREGATOR.Method.hash, alias="method")
    nul_res: AGGREGATOR.NulRes | None = Field(AGGREGATOR.NulRes.false, alias="nul_res")
    ...
    part_type: AGGREGATOR.PartType | None = Field(AGGREGATOR.PartType.auto, alias="part_type")
    ...
    preserve: AGGREGATOR.Preserve | None = Field(AGGREGATOR.Preserve.default_set, alias="preserve")
    ...
    reduce_properties: list | None = Field([], alias="reduceProperties")
    rereduce_properties: list | None = Field([], alias="rereduceProperties")
    runtime_column_propagation: bool | None = Field(None, alias="runtime_column_propagation")
    selection: AGGREGATOR.Selection | None = Field(AGGREGATOR.Selection.reduce, alias="selection")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            include.add("reduce_properties")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value == "reduce")
                    or (self.selection == "reduce")
                )
            )
            else exclude.add("reduce_properties")
        )
        (
            include.add("rereduce_properties")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value == "rereduce")
                    or (self.selection == "rereduce")
                )
            )
            else exclude.add("rereduce_properties")
        )
        (
            include.add("count_field_properties")
            if (
                self.selection
                and (
                    (hasattr(self.selection, "value") and self.selection.value == "countField")
                    or (self.selection == "countField")
                )
            )
            else exclude.add("count_field_properties")
        )

        (
            include.add("default_to_decimal")
            if (
                (
                    self.selection
                    and (
            ...
            else exclude.add("default_to_decimal")
        )
    ...
    def _get_parameters_props(self) -> dict:
        include, exclude = self._validate()
        props = {
            "additional_properties_set_aggregations",
