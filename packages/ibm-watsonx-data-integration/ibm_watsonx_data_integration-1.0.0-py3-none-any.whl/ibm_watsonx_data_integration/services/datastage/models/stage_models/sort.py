...
from ibm_watsonx_data_integration.services.datastage.models.enums import SORT
from pydantic import Field
from typing import ClassVar


class sort(BaseStage):
    ...
    buf_mode: SORT.BufMode | None = Field(SORT.BufMode.default, alias="buf_mode")
    buf_mode_ronly: SORT.BufModeRonly | None = Field(SORT.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: SORT.CollType | None = Field(SORT.CollType.auto, alias="coll_type")
    collation_sequence: SORT.CollationSequence | None = Field(SORT.CollationSequence.OFF, alias="collation_sequence")
    combinability: SORT.Combinability | None = Field(SORT.Combinability.auto, alias="combinability")
    ...
    execmode: SORT.Execmode | None = Field(SORT.Execmode.default_par, alias="execmode")
    flag_cluster: SORT.FlagCluster | None = Field(SORT.FlagCluster.false, alias="flagCluster")
    flag_key: SORT.FlagKey | None = Field(SORT.FlagKey.false, alias="flagKey")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    hide: bool | None = Field(False, alias="hide")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    key_col_select: SORT.KeyColSelect | None = Field(SORT.KeyColSelect.default, alias="keyColSelect")
    ...
    memory: int | None = Field(20, alias="memory")
    operator: SORT.Operator | None = Field(SORT.Operator.tsort, alias="operator")
    ...
    part_type: SORT.PartType | None = Field(SORT.PartType.auto, alias="part_type")
    ...
    preserve: SORT.Preserve | None = Field(SORT.Preserve.default_set, alias="preserve")
    ...
    stable: SORT.Stable | None = Field(SORT.Stable.stable, alias="stable")
    stage_description: list | None = Field("", alias="stageDescription")
    stats: SORT.Stats | None = Field(SORT.Stats.false, alias="stats")
    unique: SORT.Unique | None = Field(SORT.Unique.false, alias="unique")
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            include.add("flag_cluster")
            if (
                self.operator
                and ((hasattr(self.operator, "value") and self.operator.value == "tsort") or (self.operator == "tsort"))
            )
            else exclude.add("flag_cluster")
        )
        (
            include.add("flag_key")
            if (
                self.operator
                and ((hasattr(self.operator, "value") and self.operator.value == "tsort") or (self.operator == "tsort"))
            )
            else exclude.add("flag_key")
        )
        (
            include.add("collation_sequence")
            if (
                self.operator
                and ((hasattr(self.operator, "value") and self.operator.value == "tsort") or (self.operator == "tsort"))
            )
            else exclude.add("collation_sequence")
        )
        (
            include.add("unique")
            if (
                self.operator
                and (
                    (hasattr(self.operator, "value") and self.operator.value != "psort -sorter unix")
                    or (self.operator != "psort -sorter unix")
                )
            )
        ...
        (
            ...
            else exclude.add("stats")
        )

        include.add("preserve") if (self.output_count and self.output_count > 0) else exclude.add("preserve")
        include.add("workspace") if (()) else exclude.add("workspace")
        include.add("memory") if (()) else exclude.add("memory")
