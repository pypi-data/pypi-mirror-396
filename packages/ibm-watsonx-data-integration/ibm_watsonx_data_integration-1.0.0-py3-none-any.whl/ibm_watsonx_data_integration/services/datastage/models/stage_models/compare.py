...
from ibm_watsonx_data_integration.services.datastage.models.enums import COMPARE
from pydantic import Field
from typing import ClassVar


class compare(BaseStage):
    ...
    abort_on_difference: COMPARE.AbortOnDifference | None = Field(
        COMPARE.AbortOnDifference.false, alias="abortOnDifference"
    )
    ...
    buf_mode: COMPARE.BufMode | None = Field(COMPARE.BufMode.default, alias="buf_mode")
    buf_mode_ronly: COMPARE.BufModeRonly | None = Field(COMPARE.BufModeRonly.default, alias="buf_mode_ronly")
    coll_type: COMPARE.CollType | None = Field(COMPARE.CollType.auto, alias="coll_type")
    collation_sequence: COMPARE.CollationSequence | None = Field(
        COMPARE.CollationSequence.OFF, alias="collation_sequence"
    )
    combinability: COMPARE.Combinability | None = Field(COMPARE.Combinability.auto, alias="combinability")
    ...
    eq: int | None = Field(0, alias="eq")
    execmode: COMPARE.Execmode | None = Field(COMPARE.Execmode.default_par, alias="execmode")
    field_properties: list | None = Field([], alias="fieldProperties")
    first: int | None = Field(-2, alias="first")
    flow_dirty: str | None = Field("false", alias="flow_dirty")
    gt: int | None = Field(1, alias="gt")
    hide: bool | None = Field(False, alias="hide")
    input_count: int | None = Field(0, alias="input_count")
    input_link_description: list | None = Field("", alias="inputLinkDescription")
    inputcol_properties: list | None = Field([], alias="inputcolProperties")
    inputlink_ordering_list: list | None = Field([], alias="InputlinkOrderingList")
    key_col_select: COMPARE.KeyColSelect | None = Field(COMPARE.KeyColSelect.default, alias="keyColSelect")
    key_cols_coll: list | None = Field([], alias="keyColsColl")
    key_cols_coll_ordered: list | None = Field([], alias="keyColsCollOrdered")
    key_cols_coll_robin: list | None = Field([], alias="keyColsCollRobin")
    key_cols_none: list | None = Field([], alias="keyColsNone")
    key_cols_part: list | None = Field([], alias="keyColsPart")
    lt: int | None = Field(-1, alias="lt")
    ...
    part_type: COMPARE.PartType | None = Field(COMPARE.PartType.auto, alias="part_type")
    ...
    preserve: COMPARE.Preserve | None = Field(COMPARE.Preserve.default_propagate, alias="preserve")
    ...
    second: int | None = Field(-2, alias="second")
    ...
    warn_record_count_mismatch: COMPARE.WarnRecordCountMismatch | None = Field(
        COMPARE.WarnRecordCountMismatch.false, alias="warnRecordCountMismatch"
    )
    ...
    def _validate_parameters(self) -> tuple[set, set]:
        ...
        (
            include.add("warn_record_count_mismatch")
            if (self.abort_on_difference != "abortOnDifference")
            else exclude.add("warn_record_count_mismatch")
        )
        include.add("gt") if ((()) and (self.abort_on_difference != "abortOnDifference")) else exclude.add("gt")
        include.add("eq") if ((()) and (self.abort_on_difference != "abortOnDifference")) else exclude.add("eq")
        include.add("lt") if ((()) and (self.abort_on_difference != "abortOnDifference")) else exclude.add("lt")
        include.add("first") if ((()) and (self.abort_on_difference != "abortOnDifference")) else exclude.add("first")
        (
            include.add("second")
            if ((()) and (self.abort_on_difference != "abortOnDifference"))
            else exclude.add("second")
        )
        include.add("field_properties") if (()) else exclude.add("field_properties")
