...
def _build_output(ref_from, ref_to, repo_url, prs, commits, pr_commit_map):
    ...
    for issue_id, issue in all_issues.items():
        ...
        if issue.get('source') == 'jira':
            issues_dict[issue_id] = {'source': issue['source'], 'key': issue['key'], 'url': issue['url'], 'summary': issue['summary'], 'status': issue['status'], 'created_at': issue.get('created_at'), 'updated_at': issue.get('updated_at'), 'description': issue['description'], 'comments': issue['comments'], 'pull_requests': pr_numbers, 'commits': commit_hashes}
        else:
