...
def fetch_jira_issue(key, jira_url, jira_token, content_limit=DEFAULT_ISSUE_LIMIT):
    ...
    try:
        response = requests.get(api_url, headers=headers, params={'fields': 'summary,status,description,comment,created'}, timeout=HTTP_TIMEOUT, verify=SSL_VERIFY)
    ...
    issue = {'source': 'jira', 'key': key, 'url': f'{base_url}/browse/{key}', 'summary': fields.get('summary'), 'status': fields.get('status', {}).get('name'), 'created_at': fields.get('created'), 'description': fields.get('description') or '', 'comments': []}
