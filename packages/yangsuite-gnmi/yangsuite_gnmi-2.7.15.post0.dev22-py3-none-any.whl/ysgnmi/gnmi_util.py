...
class GnmiMessageConstructor:
            ...
            if val is not None:
                if 'jsonIetfVal' in val:
                    if base64_encode:
                        ...
                        try:
                            json.loads(val['jsonIetfVal'])
                            gnmi_update.val.json_ietf_val = val['jsonIetfVal'].encode('utf-8')
                    ...
                    if base64_encode:
                        ...
                        try:
                            json.loads(val['jsonVal'])
                            gnmi_update.val.json_val = val['jsonVal'].encode('utf-8')
                        except (json.JSONDecodeError, TypeError):
                        ...
                        if len(jval) == 0:
                            if node.get('nodetype', '') == 'leaf-list':
                                if jval.get(elem) is None:
                                    jval[elem] = [LeafListVal(val) for val in node['value']]
                                else:
    ...
    def _adjust_value_to_datatype(self, node, value=None):
        ...
        * https://tools.ietf.org/html/rfc6241
        ...
        for node in nodes:
            if node.get('nodetype') == 'leaf-list':
                ...
                if xpath not in leaf_list_map:
                    node['value'] = [node['value']]
                    leaf_list_map[xpath] = node
                    nodes_to_keep.append(node)
                else:
                    leaf_list_map[xpath]['value'].append(node['value'])
            else:
                nodes_to_keep.append(node)
        ...
        for node in nodes:
            if "xpath" not in node:
                ...
                if nodetype == 'leaf-list':
                    ...
                    value = [
                        self._adjust_value_to_datatype(node, value=val)
                        for val in values
                    ]
                else:
                    value = node.get('value', None)
                    value = self._adjust_value_to_datatype(node, value=value)
