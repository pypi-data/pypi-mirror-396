...
from .memory.graph import GraphStore # Import for type hinting if needed or direct usage
from .workflow import Workflow, Step # Import for workflow endpoints
...
async def list_workflows():
    ...
    return {
        "workflows": [
            {"id": "research", "name": "Deep Research", "description": "Research a topic and summarize findings"},
            {"id": "content_creation", "name": "Content Creation", "description": "Generate blog post from topic"},
        ]
    }
...
async def run_workflow(workflow_id: str, user_id: str, input_data: Dict[str, Any]):
    ...
    try:
        agent = get_or_create_agent(user_id)
        from mem_llm.workflow import Workflow, Step
...
def _get_workflow(workflow_id, agent):
    ...
    if workflow_id == "research":
        ...
        workflow.add_step(Step("Research", agent=agent, action="Research the topic provided.", input_key="topic", output_key="facts", description="Researching topic deeply..."))
        workflow.add_step(Step("Summarize", agent=agent, action="Summarize the facts.", input_key="facts", output_key="summary", description="Summarizing findings..."))
        return workflow
    elif workflow_id == "content_creation":
        workflow = Workflow("Content Creation")
        workflow.add_step(Step("Draft", agent=agent, action="Draft a blog post about the topic.", input_key="topic", output_key="draft", description="Drafting blog post..."))
        return workflow
    else:
