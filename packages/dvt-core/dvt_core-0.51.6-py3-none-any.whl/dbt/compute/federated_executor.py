...
class FederatedExecutor:
    ...
    def _rewrite_sql_for_compute(
        ...
        for source_meta in source_tables:
            ...
            # 2. Double-quoted (PostgreSQL style): "EXIM_EDWH_DEV"."ods"."cbs_f_country"
            parts = qualified_name.split(".")
            quoted_qualified_name = ".".join([f'"{part}"' for part in parts])
            rewritten_sql = rewritten_sql.replace(quoted_qualified_name, alias)

            # 3. Single string quoted: "EXIM_EDWH_DEV.ods.cbs_f_country"
            rewritten_sql = rewritten_sql.replace(f'"{qualified_name}"', alias)

            # 4. Backtick-quoted (Spark/Databricks style): `EXIM_EDWH_DEV`.`ods`.`cbs_f_country`
            backtick_qualified_name = ".".join([f'`{part}`' for part in parts])
            rewritten_sql = rewritten_sql.replace(backtick_qualified_name, alias)
