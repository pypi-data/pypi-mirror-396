...
class ModelRunner(CompileRunner):
    ...
    def execute(self, model, manifest):
        ...
        # DVT v0.51.4: Get target connection for cross-adapter detection
        # Target can be from model config or CLI --target flag
        model_target = model.config.target if hasattr(model.config, 'target') else None
        target_connection = model_target or self.config.target_name
        ...
        # DVT v0.51.4: Views cannot be federated - they are SQL definitions
        # Views must execute on target adapter directly (pushdown to target)
        materialization = model.get_materialization()
        force_target_pushdown = False
        if materialization == 'view' and natural_analysis.is_federated:
            # View with cross-adapter scenario: force pushdown to target adapter
            # Views are SQL definitions stored on target - they can't be "federated"
            # The view SQL will run on target, referencing sources via database links
            # Note: This may fail if target can't access cross-database sources
            force_target_pushdown = True
            user_compute = None
            analysis = natural_analysis  # Keep analysis for reference, but we'll force pushdown

        if not force_target_pushdown:
            # Only apply CLI override if:
            # 1. Model explicitly has compute= config, OR
            # 2. Model naturally requires federation (multi-source or cross-adapter)
            if model_compute:
                # Model has compute config - CLI can override
                user_compute = cli_compute or model_compute
            elif natural_analysis.is_federated:
                # Model requires federation - CLI can override compute engine choice
                user_compute = cli_compute  # Will use CLI override or default from selector
            else:
                # Model is pushdown-eligible and has no compute config
                # Do NOT apply CLI override - keep it pushdown
                user_compute = None

            # Re-analyze with determined user_compute (or use natural analysis)
            if user_compute:
                analysis = analyzer.analyze(
                    model,
                    user_compute_override=user_compute,
                    target_connection=target_connection
                )
            else:
        ...
        if analysis.is_pushdown or force_target_pushdown:
            # Pushdown execution: Use source adapter directly, or target adapter for views
            if force_target_pushdown:
                # DVT v0.51.4: Views in cross-adapter scenarios use target adapter
                # Views are SQL definitions that run on the target database
                execution_adapter = target_adapter
            else:
                # Standard pushdown: use source adapter
            ...
            try:
                ...
                return RunResult(
                    ...
                    message=f"Federated execution: {rows_msg}",
                    failures=None,
                    node=model,
                    agate_table=None,
                )
