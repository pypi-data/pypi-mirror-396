...
class ModelRunner(CompileRunner):
    ...
    def execute(self, model, manifest):
        ...
        # DVT v0.51.5: Handle views that read from direct sources in cross-adapter scenarios
        # Views are SQL definitions - they can't span databases.
        # If a view has direct source() refs AND target is different database, convert to table
        ...
        if materialization == 'view' and natural_analysis.is_federated:
            # View with direct source refs and cross-adapter: convert to federated table
            # The view SQL will be executed via Spark, and results materialized as a table
            convert_view_to_table = True

        if True:  # Always evaluate compute override logic
        ...
        # DVT v0.51.5: Use target_connection (includes CLI --target) not just model.config.target
