...
class ProviderContext(ManifestContext):
    ...
    def _select_adapter_for_node(self, node, config: RuntimeConfig):
        ...
        If node has 'target' config override, use that adapter.
        Otherwise, use the default adapter from RuntimeConfig.

        This enables adapter-specific Jinja functions to work correctly
        in federated mode with target overrides.
        """
        # Check if node has target override in config
        if hasattr(node, "config") and hasattr(node.config, "target") and node.config.target:
