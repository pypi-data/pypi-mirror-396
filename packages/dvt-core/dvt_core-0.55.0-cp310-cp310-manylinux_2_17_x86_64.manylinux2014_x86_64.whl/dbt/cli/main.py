...
# DVT snap command - capture metadata snapshots
@cli.command("snap")
@click.option("--source", "-s", help="Snapshot a specific source only")
@click.option("--all", "-a", "include_all", is_flag=True, help="Include models in snapshot (Phase 2)")
@click.pass_context
@p.profiles_dir
@p.project_dir
def snap(ctx, source, include_all, profiles_dir, project_dir, **kwargs):
    """Capture metadata snapshot from source databases.

    Reads source definitions from your project and captures table/column
    metadata into .dvt/metadata.duckdb for type mapping and optimization.
    ...
    Examples:
        ...
        dvt snap --source my_src   # Snapshot specific source
        dvt snap --all             # Include models (Phase 2)

    The metadata store must be initialized first with 'dvt init'.
    """
    from dbt.task.snap import SnapTask

    # Create a simple args object for the task
    class Args:
        def __init__(self):
            self.source = source
            self.all = include_all
