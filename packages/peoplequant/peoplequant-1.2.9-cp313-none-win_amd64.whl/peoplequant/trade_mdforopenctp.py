...
class PeopleQuantApi():
    ...
    def open_close(self,symbol,kaiping='',lot=0,price=None,block=True,n_price_tick=1,che_time=0,order_info='无',signal_price=float('nan'),order_close_chan=True,HedgeFlag:str="1",WaitReturn=False,OrderMemo="",_print=True,**kw):
        '''
            ...
            if kaiping== 'pingduo': #交易方向为平多
                ...
                if ExchangeID in ["SHFE","INE"]:
                    if 0 < lot <= available_today: #小于等于今仓，平今
                        ping_jin = self.insert_order(ExchangeID,symbol,'Sell','CloseToday', lot, price_sell,OrderMemo=OrderMemo,_print=False)
                        if isinstance(ping_jin,str): last_msg += ping_jin
                    elif 0 == available_today < lot <= available_his: #只有昨仓
                        ping_zuo = self.insert_order(ExchangeID,symbol,'Sell','Close',lot,price_sell,OrderMemo=OrderMemo,_print=False) 
                        if isinstance(ping_zuo,str): last_msg += ping_zuo
                    elif 0 < available_today < lot <= available_pos :  #优先平今,剩余仓位平昨   
                        ping_zuo = self.insert_order(ExchangeID,symbol,'Sell','Close',lot-available_today,price_sell,OrderMemo=OrderMemo,_print=False) #先平昨再平今 
                        ping_jin = self.insert_order(ExchangeID,symbol,'Sell','CloseToday',available_today,price_sell,OrderMemo=OrderMemo,_print=False)
                        if isinstance(ping_zuo,str): last_msg += ping_zuo
                        if isinstance(ping_jin,str): last_msg += ping_jin
                ...
                if ExchangeID in ["SHFE","INE"]:
                    if 0 < lot <= available_today : #小于等于今仓，平今
                        ping_jin=self.insert_order(ExchangeID,symbol,'Buy','CloseToday',lot,price_buy,OrderMemo=OrderMemo,_print=False)    
                        if isinstance(ping_jin,str): last_msg += ping_jin
                    elif 0 == available_today < lot <= available_his:      
                        ping_zuo=self.insert_order(ExchangeID,symbol,'Buy','Close',lot,price_buy,OrderMemo=OrderMemo,_print=False)
                        if isinstance(ping_zuo,str): last_msg += ping_zuo
                    elif 0 < available_today < lot <= available_pos:      
                        ping_zuo=self.insert_order(ExchangeID,symbol,'Buy','Close',lot-available_today,price_buy,OrderMemo=OrderMemo,_print=False)      
                        ping_jin=self.insert_order(ExchangeID,symbol,'Buy','CloseToday',available_today,price_buy,OrderMemo=OrderMemo,_print=False)
                        if isinstance(ping_zuo,str): last_msg += ping_zuo
                        if isinstance(ping_jin,str): last_msg += ping_jin
                    else: last_msg += "平仓数量不足,下单数量:{},可用今仓:{},可用昨仓:{}" .format(lot,available_today,available_his)
                else: #其他交易所不区分今昨仓
                ...
                order = self.insert_order(ExchangeID,symbol,'Buy','Open',lot,price_buy,OrderMemo=OrderMemo,_print=False)   
                if isinstance(order,str): last_msg += order
            elif kaiping=='kaikong': #交易方向为开空
                order = self.insert_order(ExchangeID,symbol,"Sell","Open",lot,price_sell,OrderMemo=OrderMemo,_print=False)
                if isinstance(order,str): last_msg += order
