...
class DQFramework:
    ...
    def _split_dataframe(
        ...
        if not include_validation_details:
            # Only return original columns (plus failure details for bad_df)
            qualified_df = qualified_df.select(*original_columns)
            # Keep failure detail columns in bad_df
            bad_columns = original_columns + ["_dq_failed_expectations", "_dq_failed_columns", "_dq_failure_reasons", "_dq_is_warning"]
            bad_df = bad_df.select(*[c for c in bad_df.columns if c in bad_columns])
