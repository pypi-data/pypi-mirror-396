...
class RMSLE(Metric):
    ...
    def update_state(self, inputs: Mapping[str, np.ndarray], act: np.ndarray, pred: Sequence[np.ndarray]) -> None:
        """
        Updates the streaming Root Mean Squared Log Error state.
        ...
        if np.any(act < 0) or np.any(pred < 0):
            ...
            self._tot += np.sum(np.square(np.log1p(pred) - np.log1p(act)))
        self._count += len(pred)

    def reset_state(self) -> None:
        """Resets the state of Root Mean Squared Log Error at the end of an epoch."""
        self._count = 0.0
        self._tot = 0.0
        self._is_negative = 0

    def result(self) -> float:
        """Computes the Root Mean Squared Log Error over the last epoch."""
        tot = DistributedHelper.allreduce_sum(self._tot)
        count = DistributedHelper.allreduce_sum(self._count)
        is_negative = DistributedHelper.allreduce_sum(self._is_negative)
        if count == 0 or is_negative > 0:
            return clip_metric(np.nan, constants.RMSLE)
        return clip_metric(np.sqrt(tot / count), constants.RMSLE)
