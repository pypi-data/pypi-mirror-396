...
class NMAE(Metric):
    ...
    def update_state(self, inputs: Mapping[str, np.ndarray], act: np.ndarray, pred: Sequence[np.ndarray]) -> None:
        ...
        for grain_idx, sample_pred, sample_act in zip(grain_indices, pred, act):
            self._tot[grain_idx] += np.sum(np.abs(sample_pred.reshape(-1) - sample_act.reshape(-1)))
    ...
    def result(self) -> float:
        ...
        if np.sum(count) == 0:
            return clip_metric(np.nan, constants.NORM_MEAN_ABS_ERROR)
        non_zero = count > 0
        tot = tot[non_zero]
        ...
        metric_per_series = tot / count / max_min_div
        if np.isnan(metric_per_series).sum() == len(metric_per_series):
            return clip_metric(np.nan, constants.NORM_MEAN_ABS_ERROR)
        metric_per_series[metric_per_series > 1] = 1.0
        return clip_metric(float(np.nanmean(metric_per_series)), constants.NORM_MEAN_ABS_ERROR)
