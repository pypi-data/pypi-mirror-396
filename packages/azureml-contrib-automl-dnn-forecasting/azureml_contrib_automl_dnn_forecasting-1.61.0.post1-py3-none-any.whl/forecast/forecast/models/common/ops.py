...
class CausalConv1d(RFModule):
    ...
    def __init__(
        ...
        *,
        ...
        stride: int, optional
            The convolution's stride, defaults to 1
        bias: bool, optional
            Should the conv include a bias term, defaults to True
        use_wn: bool. optional
            If True, apply weight norm. Defaults to True.
        ...
        c = Conv1d(
            input_channels, output_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias
        )
        self.conv = weight_norm(c) if use_wn else c
...
class LayerNorm2ndDim(RFModule):
    def __init__(self, num_features: int):
        super().__init__()
        self._ln = nn.LayerNorm(num_features)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return torch.transpose(self._ln(torch.transpose(x, -1, -2)), -1, -2)
