...
class EmbeddingPremix(AbstractPremix):
    ...
    def __init__(self, config: EmbeddingPremixConfig):
        ...
        self._conv = nn.Conv1d(in_channels=conv_in_ch, out_channels=self._output_channels, kernel_size=1)
    ...
    def forward(self, state: torch.Tensor, future_state: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        """
        # map embed indices to embeddings
        embeds = [
            e(state[:, c.feature_index, :].long()).transpose(1, 2)
            for e, c in zip(self._embeddings, self.config.embeddings)
        ]

        # select non-embed features and concat with embeddings at the end
        non_embed_inds = cast(torch.Tensor, self._non_embed_ind)
        out = torch.cat([state.index_select(1, non_embed_inds), *embeds], dim=1)
    ...
    def __post_init__(self) -> None:
        ...
        for embed in self.embeddings:
            ...
            if ind >= self.input_channels:
                raise ValueError(
                    f"Requested embedding for input feature {ind} which is >= # of model input channels "
        ...
        if len(inds) != len(set(inds)):
            ...
            raise ValueError(
                "Multiple embeddings for a single feature is not permitted; "
                f"requested multiple embeddings for features {repeats}."
            )

    def create_premix(self) -> EmbeddingPremix:
        """Creates a `EmbeddingPremix` based on the specified config."""
        return EmbeddingPremix(self)
