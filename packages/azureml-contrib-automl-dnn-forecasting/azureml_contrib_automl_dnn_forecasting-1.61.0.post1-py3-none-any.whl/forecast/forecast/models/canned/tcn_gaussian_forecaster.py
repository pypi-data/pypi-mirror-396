...
def create_tcn_gaussian_forecaster(
    input_channels: int,
    ...
    premix_config = ConvPremixConfig(
        input_channels=input_channels, output_channels=num_channels, kernel_size=1, dilation=1, stride=1
    )

    base_cell = CausalConvResidConfig(
        num_prev_cell_inputs=1, kernel_size=2, dilation=init_dilation, stride=1, dropout=dropout_rate
    )
    ...
    mean_head_config = UnboundedScalarForecastHeadConfig(horizon, dropout=dropout_rate)
    std_head_config = StrictlyPositiveScalarForecastHeadConfig(horizon, dropout=dropout_rate)

    return ForecastingModel(premix_config, backbone_config, [mean_head_config, std_head_config])
