...
def msis(
    insample: np.ndarray, actual: np.ndarray, lb: np.ndarray, ub: np.ndarray, samples_per_period: int, sig: float = 0.05
) -> float:
    ...
    """
    masep = np.mean(np.abs(insample[samples_per_period:] - insample[:-samples_per_period]))
    pen = np.mean((ub - lb) + 2 / sig * np.maximum(0, lb - actual) + 2 / sig * np.maximum(0, actual - ub))
    return pen / masep


def owa(
    insample: List[np.ndarray], actual: List[np.ndarray], pred: List[np.ndarray], samples_per_pd: List[int]
) -> float:
    ...
    return (
        ...
        / 2
        * (
            np.mean([smape(a, p) for a, p in zip(actual, pred)]) / 0.15201
