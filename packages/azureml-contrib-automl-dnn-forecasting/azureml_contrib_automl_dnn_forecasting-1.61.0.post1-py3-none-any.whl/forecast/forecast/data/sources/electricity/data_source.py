...
class ElectricityDataSource(AbstractDataSource):
    ...
    def get_config(self) -> DataSourceConfig:
        ...
        if self._featurizers:
            encodings = [
                EncodingSpec(feature_index=i + 1, num_vals=f.num_values)
                for i, f in enumerate(self._featurizers)
                if f.num_values > 2
            ]

        return DataSourceConfig(
            feature_channels=1 + len(self._featurizers),  # station_id + any date features
            forecast_channels=1,
            encodings=encodings,
        )
    ...
    def _load_data(self) -> None:
        ...
        if not osp.exists(data_path):
            fetch_zipped_data(
                data_path, "https://archive.ics.uci.edu/ml/machine-learning-databases/00321/LD2011_2014.txt.zip"
            )
