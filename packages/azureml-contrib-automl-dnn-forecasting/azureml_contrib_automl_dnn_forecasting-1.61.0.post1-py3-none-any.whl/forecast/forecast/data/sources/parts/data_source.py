...
class PartsDataSource(AbstractDataSource):
    ...
    def get_config(self) -> DataSourceConfig:
        ...
        if self._featurizers:
            encodings += [
                EncodingSpec(feature_index=i + 1, num_vals=f.num_values)  # date features + part encoding
    ...
    def _build_dataset(
        self, data: np.ndarray, window_size: int, one_hot: bool = False, drop_first: Optional[bool] = None
    ) -> TimeSeriesDataset:
