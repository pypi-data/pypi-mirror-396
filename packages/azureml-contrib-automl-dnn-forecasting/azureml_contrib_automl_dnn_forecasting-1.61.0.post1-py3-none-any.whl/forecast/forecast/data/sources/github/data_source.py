...
class GithubDataSource(AbstractDataSource):
    ...
    def __init__(
        ...
        if date_featurizers is None:
            self._featurizers = [dfeat.DayOfWeekFeaturizer(), dfeat.MonthOfYearFeaturizer(), dfeat.HolidayFeaturizer()]
        ...
        *,
        ...
        if one_hot and config.encodings:
            ...
            ohes: List[tfs.AbstractTransform] = [
                tfs.OneHotEncode(
                    [e.feature_index for e in config.encodings], [e.num_vals for e in config.encodings], drop_first=drop
                ),
                tfs.OneHotEncode(
    ...
    def get_dataset(
        self, window_size: int, one_hot: bool = False, drop_first: Optional[bool] = None
    ...
    def get_config(self) -> DataSourceConfig:
        ...
        if self._featurizers:
            ...
            encodings = [
                EncodingSpec(feature_index=i, num_vals=f.num_values)
        ...
        return DataSourceConfig(feature_channels=num_date_features, forecast_channels=1, encodings=encodings)
