    ...
    def __init__(
        ...
        if isinstance(feature_index, int) and isinstance(num_values, int):
            ...
            if len(feature_index) != len(num_values):
                raise ValueError(
                    f"{len(feature_index)} indices were provided, but {len(num_values)} dimensions " "were provided."
                )
            ...
            raise ValueError(
                f"Incompatible types for feature_index ({type(feature_index)}) and num_values " f"({type(num_values)})"
            )
...
def unbatch_and_undo(
    batch: Mapping[str, torch.Tensor], tf: Optional[AbstractTransform], prediction: Optional[torch.Tensor] = None
) -> List[dict]:
