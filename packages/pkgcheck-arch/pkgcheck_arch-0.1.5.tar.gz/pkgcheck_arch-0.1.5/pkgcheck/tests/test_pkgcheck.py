...
Tests for `pkgcheck` module.
"""

import mock

from pkgcheck import pkgcheck
from pkgcheck import messages
from pkgcheck.tests import base


MESSAGES = messages.MESSAGES


class Testpkgcheck(base.TestCase):

    def setUp(self):
        super(Testpkgcheck, self).setUp()
        self.run = pkgcheck.pkgcheckRun()

    @mock.patch('pkgcheck.pkgcheck.pkgcheckRun')
    def test_main_no_files(self, m_pkgcheckrun):
        m_run_obj = mock.MagicMock()
        m_run_obj.error_count = 0
        m_run_obj.warning_count = 0
        m_pkgcheckrun.return_value = m_run_obj

        result = pkgcheck.main([])
        expected_return = 1
        self.assertEqual(expected_return, result)

    @mock.patch('pkgcheck.pkgcheck.pkgcheckRun')
    def test_main_return_one_on_errors(self, m_pkgcheckrun):
        m_run_obj = mock.MagicMock()
    ...
    @mock.patch('pkgcheck.pkgcheck.pkgcheckRun')
    def test_main_return_one_on_ioerror(self, m_pkgcheckrun):
        m_run_obj = mock.MagicMock()
        m_run_obj.error_count = 0
        m_run_obj.check_files = mock.Mock(side_effect=IOError)
        m_pkgcheckrun.return_value = m_run_obj

        result = pkgcheck.main(['--verbose',
    ...
    def test_multi_ignore_with_slash(self):
        ...
        pkgcheck.check_no_trailing_whitespace("if ", self.run)
        pkgcheck.check_if_then("if ", self.run)
    ...
    def test_multi_ignore_mixed(self):
        ...
        pkgcheck.check_no_trailing_whitespace("if ", self.run)
        pkgcheck.check_if_then("if ", self.run)
        pkgcheck.check_indents("  echo", self.run)
    ...
    @mock.patch('pkgcheck.pkgcheck.pkgcheckRun.print_error')
    def test_while_check_for_do(self, m_print_error):
        test_line = 'while `do something args`'
        pkgcheck.check_for_do(test_line, self.run)

        m_print_error.assert_called_once_with(
            MESSAGES['E010'].msg % 'while', test_line)


class TestpkgcheckSamples(base.TestCase):
    """End to end regression testing of pkgcheck against script samples."""

    def setUp(self):
        super(TestpkgcheckSamples, self).setUp()
        log_error_patcher = mock.patch(
            'pkgcheck.pkgcheck.pkgcheckRun.log_error')
    ...
    def test_sample_E001(self):
        test_files = ['pkgcheck/tests/samples/E001_bad.sh']
        self.run.check_files(test_files, False)

        self.assert_error_found('E001', 4)

    def test_sample_E002(self):
        test_files = ['pkgcheck/tests/samples/E002_bad.sh']
        self.run.check_files(test_files, False)

        self.assert_error_found('E002', 3)

    def test_sample_E003(self):
        test_files = ['pkgcheck/tests/samples/E003_good.sh']
        self.run.check_files(test_files, False)
        self.assertEqual(0, self.run.error_count)

    def test_sample_E003_bad(self):
        test_files = ['pkgcheck/tests/samples/E003_bad.sh']
        self.run.check_files(test_files, False)
    ...
    def test_sample_E004_bad(self):
        test_files = ['pkgcheck/tests/samples/E004_bad.sh']
        self.run.check_files(test_files, False)
        self.assert_error_found('E004', 3)

    def test_sample_E010_good(self):
        test_files = ['pkgcheck/tests/samples/E010_good.sh']
        self.run.check_files(test_files, False)

        self.assertEqual(self.run.error_count, 0)

    def test_sample_E010(self):
        test_files = ['pkgcheck/tests/samples/E010_bad.sh']
        self.run.check_files(test_files, False)
    ...
    def test_sample_E011_good(self):
        test_files = ['pkgcheck/tests/samples/E011_good.sh']
        self.run.check_files(test_files, False)

        self.assertEqual(self.run.error_count, 0)

    def test_sample_E011(self):
        test_files = ['pkgcheck/tests/samples/E011_bad.sh']
        self.run.check_files(test_files, False)
    ...
    def test_sample_E012(self):
        test_files = ['pkgcheck/tests/samples/E012_bad.sh']
        self.run.check_files(test_files, False)
    ...
    def test_sample_E041(self):
        test_files = ['pkgcheck/tests/samples/E041_bad.sh']
        self.run.check_files(test_files, False)

        self.assert_error_found('E041', 4)

    def test_sample_W042(self):
        test_files = ['pkgcheck/tests/samples/W042_bad.sh']
        self.run.register_errors('W042')
        self.run.check_files(test_files, False)

        self.assert_error_found('W042', 2)
        self.assert_error_found('W042', 6)
        self.assert_error_found('W042', 10)
        self.assert_error_found('W042', 11)

    def test_sample_W043(self):
        test_files = ['pkgcheck/tests/samples/W043_bad.sh']
        self.run.register_errors('W043')
        self.run.check_files(test_files, False)

        self.assert_error_found('W043', 6)

    def test_sample_for_loops(self):
        test_files = ['pkgcheck/tests/samples/for_loops.sh']
        self.run.check_files(test_files, False)
    ...
    def test_sample_comments(self):
        test_files = ['pkgcheck/tests/samples/comments.sh']
        self.run.check_files(test_files, False)

        self.assertEqual(0, self.run.error_count)

    def test_sample_E040(self):
        test_files = ['pkgcheck/tests/samples/E040_syntax_error.sh']
    ...
    def test_sample_E044(self):
        test_files = ['pkgcheck/tests/samples/E044_bad.sh']
        self.run.check_files(test_files, False)
    ...
    def test_sample_warning(self):
        ...
        test_files = ['pkgcheck/tests/samples/E011_bad.sh',
                      'pkgcheck/tests/samples/E041_bad.sh']
    ...
    def test_ignore_heredoc(self):
        test_files = ['pkgcheck/tests/samples/heredoc_ignore.sh']
        self.run.check_files(test_files, False)
        self.assertEqual(0, self.run.error_count)
    ...
    def test_pre_zero_dot_one_sample_file(self):
        ...
        release of the pkgcheck pypi package. There were no tests
        ...
        test_files = ['pkgcheck/tests/samples/legacy_sample.sh']
        self.run.check_files(test_files, False)
