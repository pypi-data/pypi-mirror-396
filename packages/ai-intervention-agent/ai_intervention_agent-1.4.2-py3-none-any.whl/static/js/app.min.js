...
if(file.type==='image/png'){mimeCandidates.push('image/png')}else if(file.type==='image/webp'){mimeCandidates.push('image/webp','image/jpeg')}else{mimeCandidates.push('image/jpeg')}
...
if(blob.size<file.size){const compressedFile=new File([blob],finalName,{type:finalMimeType,lastModified:file.lastModified})
console.log(`图片压缩: ${file.name} ${(file.size / 1024).toFixed(2)}KB → ${(
                    blob.size / 1024
                  ).toFixed(2)}KB (压缩率: ${((1 - blob.size / file.size) * 100).toFixed(1)}%)`)
resolve(compressedFile)}else{resolve(file)}},outType,quality)}
tryToBlob(0)})}
...
if(selectedImages.length>0){container.style.display='block'}else{container.style.display='none'}}
...
if(file){await addImageToList(file)}}
updateImagePreviewVisibility()
