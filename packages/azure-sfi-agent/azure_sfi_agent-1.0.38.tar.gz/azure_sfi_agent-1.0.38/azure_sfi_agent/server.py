...
def run_command(command: list[str], timeout: int = 120) -> str:
    ...
    WRAPPER FIX: Automatically wraps 'az' commands in PowerShell 
    to avoid [WinError 2] if 'az' is not in the system PATH.
    """
    # If the command starts with 'az', wrap it in PowerShell
    if command and command[0] == "az":
        # Join the command for PowerShell (e.g. "az group show ...")
        # specific quoting might be needed for complex args, but usually basic joining works for these helpers
        az_cmd_str = " ".join(command)
        
        ps_executable = "pwsh" if shutil.which("pwsh") else "powershell"
        
        # Construct PowerShell command: cmd /c "az ..." is often safest on Windows from PS
        # But for simple reads, just running the command string works if az is in PS path
        ps_args = [
            ps_executable, "-NoProfile", "-NonInteractive", 
            "-Command", az_cmd_str
        ]
        ...
        try:
            ...
            return f"Error running command '{az_cmd_str}': {e.stderr}"
        except Exception as e:
            return f"Error executing command: {str(e)}"

    # Original logic for non-az commands (if any)
    try:
        result = subprocess.run(
            command, 
            capture_output=True, 
...
def _deploy_bicep(resource_group: str, resource_type: str, parameters: Dict[str,str]) -> str:
    ...
    try:
        ...
        # 5. Execute with longer timeout for deployment
        deploy_result = _run_powershell_script(deploy_script_path, script_args, timeout=300)
