...
# Deployment Scripts (PowerShell scripts for deploying resources)
DEPLOYMENT_SCRIPTS = {
...
def _deploy_bicep(resource_group: str, resource_type: str, parameters: Dict[str,str]) -> str:
    ...
    # Get absolute path to the template so PowerShell can find it easily
    ...
    # This avoids issues with spaces, quotes, or complex values on the command line.
    import json
    params_json = json.dumps(parameters)

    # 4. Prepare Arguments for the PowerShell Script
    script_args = {
        "ResourceGroupName": resource_group,
    ...
    # 5. Execute via _run_powershell_script
    # This uses your existing helper which correctly launches pwsh/powershell
    deploy_result = _run_powershell_script(deploy_script_path, script_args)
    
    # 6. Compliance Orchestration (NSP / Log Analytics)
