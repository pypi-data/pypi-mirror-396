...
# Execute using cmd /c to safely wrap the batch file execution on Windows
# This bypasses issues where calling 'az' directly in PowerShell sometimes fails to capture streams
$Process = Start-Process -FilePath "cmd.exe" -ArgumentList "/c az $($AzArgs -join ' ')" -NoNewWindow -PassThru -Wait
