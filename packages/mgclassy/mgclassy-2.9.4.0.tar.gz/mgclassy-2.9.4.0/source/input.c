 ...
 * modif. related to MG by Ziad Sakr 2021
 */
...
// background test option 
  class_call(parser_read_string(pfc, "mg_bckg", 
                                &(string1),
                                &(flag1),
                                errmsg),
             errmsg, errmsg);
  ...
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      ...
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        ...
        class_stop(errmsg,"incomprehensible input '%s' for the field 'mg_bckg'",string1);
      }
    }
  }

 // Redshift at which MG evolution should take over
  class_call(parser_read_double(pfc, "mg_z_init", &param1, &flag1, errmsg),
             errmsg, errmsg);
  if (flag1 == _TRUE_) {
    pba->mg_z_init = param1;
  }else{
    pba->mg_z_init = 6.0; // Default value: z=6 does not influence if mg_ansatz is left = GR by default
  } // end z_init check

  // no lensing slip option 
  class_call(parser_read_string(pfc, "mg_no_lens",
                                &(string1),
                                &(flag1),
                                errmsg),
             errmsg, errmsg);
  ...
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pba->mg_no_lens = _TRUE_;
    }
    else {
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        pba->mg_no_lens = _FALSE_;
      }
      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'mg_no_lens'",string1);
      }
    }
  }


  // Choose which MG function ansatz to use
  class_call(parser_read_string(pfc, "mg_ansatz", &string1, &flag1, errmsg),
             errmsg, errmsg);
  if (flag1 == _TRUE_) {

    if (strstr(string1,"EFT_alpha_QSL") != NULL) {
        // EFT alpha -like ansatz 
        pba->mg_ansatz = EFT_alpha_QSL;
    }
    else if (strstr(string1,"EFT_alpha_QSE") != NULL) {
        // EFT alpha -like ansatz 
        pba->mg_ansatz = EFT_alpha_QSE;
    }
    else if (strstr(string1,"horndeski") != NULL) {
        // Horndeski-like ansatz
        pba->mg_ansatz = horndeski;
    }else if (strstr(string1,"tanh") != NULL) {
        // tanh ansatz
        pba->mg_ansatz = mgtanh;
    }else if (strstr(string1,"omegade") != NULL) {
        // Horndeski-like Omega_DE ansatz
        pba->mg_ansatz = omegade;
    }else if (strstr(string1,"cpl") != NULL) {
        // Horndeski-like CPL ansatz
        pba->mg_ansatz = cpl;
    }else if (strstr(string1,"plk_late") != NULL) {
        // plk_late ansatz
        pba->mg_ansatz = plk_late;
    }else if (strstr(string1,"plk_norm_late") != NULL) {
        // plk_norm_late ansatz
        pba->mg_ansatz = plk_norm_late;
    }else if (strstr(string1,"plk_early") != NULL) {
        // plk_early ansatz
        pba->mg_ansatz = plk_early;
    }else if (strstr(string1,"plk_musigma_norm_late") != NULL) {
        // plk_late norm musigma ansatz
        pba->mg_ansatz = plk_musigma_norm_late;
    }else if (strstr(string1,"BZ") != NULL) {
        // BZ ansatz
        pba->mg_ansatz = BZ;
    }else if (strstr(string1,"Geff") != NULL) {
        // Geff ansatz
        pba->mg_ansatz = Geff;
    }else if (strstr(string1,"z_flex_early") != NULL) {
        // z_flex_early ansatz
        pba->mg_ansatz = z_flex_early;
    }else if (strstr(string1,"z_flex_late") != NULL) {
        // z_flex_late ansatz
        pba->mg_ansatz = z_flex_late;
    }else if (strstr(string1,"z_flex_norm_late") != NULL) {
        // z_flex_norm_late ansatz
        pba->mg_ansatz = z_flex_norm_late;
    }else if (strstr(string1,"z_xpans_early") != NULL) {
        // z_xpans_early ansatz
        pba->mg_ansatz = z_xpans_early;
    }else if (strstr(string1,"z_xpans_late") != NULL) {
        // z_xpans_late ansatz
        pba->mg_ansatz = z_xpans_late;
    }else if (strstr(string1,"QR") != NULL) {
        // QR ansatz
        pba->mg_ansatz = QR;
    }else if (strstr(string1,"FR") != NULL) {
        // FR ansatz
        pba->mg_ansatz = FR;
    }else if (strstr(string1,"diL") != NULL) {
        // diL ansatz
        pba->mg_ansatz = diL;
    }else if (strstr(string1,"syM") != NULL) {
        // syM ansatz
        pba->mg_ansatz = syM;
    }else if (strstr(string1,"JBD") != NULL) {
        // JBD ansatz
        pba->mg_ansatz = JBD;
    }
    else if (strstr(string1,"nDGP") != NULL) {
        // nDGP ansatz
        pba->mg_ansatz = nDGP;
    }
    else if (strstr(string1,"kmoufl") != NULL) {
        // k-moufl ansatz
        pba->mg_ansatz = kmoufl;
    }
    else if (strstr(string1,"GI") != NULL) {
        // GI ansatz
        pba->mg_ansatz = GI;
    }
    else if (strstr(string1,"simple") != NULL) {
        // simple ansatz
        pba->mg_ansatz = simple;
    }
  else {if (pba->mg_ansatz != GR)
    class_stop(errmsg,"mgclass didn't recognized your model");
  } 
 } // end ansatz check
    ...
    printf("       Modifications active for z < %3.3f    \n", pba->mg_z_init);
    printf("       For the %s model\n", &string1);
    printf("    ******************************************\n");}

  // Load parameters for chosen ansatz
  if (pba->mg_ansatz == EFT_alpha_QSL) {

    // EFT alpha ansatz
    pba->mg_EFT_alpha_M = 1.e-9;
    pba->mg_EFT_alpha_b = 1.e-9;

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_EFT_alpha_M", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_EFT_alpha_M = param1; }

    class_call(parser_read_double(pfc, "mg_EFT_alpha_b", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_EFT_alpha_b = param1; }

  }
  else if (pba->mg_ansatz == EFT_alpha_QSE) {
  ...
  }else if (pba->mg_ansatz == horndeski) {

    // Horndeski ansatz
    // Set default values (GR limit by default)
    pba->mg_horndeski_M  = 0.;
    pba->mg_horndeski_p1 = 1.;
    pba->mg_horndeski_p2 = 0.;
    pba->mg_horndeski_q2 = 1.;
    pba->mg_horndeski_t1 = 1.;
    pba->mg_horndeski_t2 = 0.;


    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_horndeski_p1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_horndeski_p1 = param1; }

    class_call(parser_read_double(pfc, "mg_horndeski_p2", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_horndeski_p2 = param1; }

    class_call(parser_read_double(pfc, "mg_horndeski_q2", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_horndeski_q2 = param1; }

    class_call(parser_read_double(pfc, "mg_horndeski_t1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_horndeski_t1 = param1; }

    class_call(parser_read_double(pfc, "mg_horndeski_t2", &param1, &flag1, errmsg),
	  errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_horndeski_t2 = param1; }

    class_call(parser_read_double(pfc, "mg_horndeski_M", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_horndeski_M = param1; }

  }else if (pba->mg_ansatz == mgtanh) {

    // tanh ansatz
    // Set default values (GR limit by default)
    pba->mg_tanh_Bmu  = 0.; // force mu to GR
    pba->mg_tanh_Bgam = 0.; // force gamma to GR
    pba->mg_tanh_Wmu  = 1.;
    pba->mg_tanh_Wgam = 1.;

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_tanh_Bmu", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_tanh_Bmu = param1; }

    class_call(parser_read_double(pfc, "mg_tanh_Bgam", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_tanh_Bgam = param1; }

    class_call(parser_read_double(pfc, "mg_tanh_Wmu", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_tanh_Wmu = param1; }

    class_call(parser_read_double(pfc, "mg_tanh_Wgam", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_tanh_Wgam = param1; }

    if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"neither mg_tanh_Bgam nor mg_tanh_Wgam should be varied with mg_no_lens option");}
    ...
    // Set default values (GR limit by default)
    pba->mg_lambda = 1.;
    pba->mg_mu_lambda  = 0.; // force mu to GR
    pba->mg_gam_lambda = 0.; // force gamma to GR

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_lambda", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_lambda = param1; }

    class_call(parser_read_double(pfc, "mg_mu_lambda", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_mu_lambda = param1; }

    class_call(parser_read_double(pfc, "mg_gam_lambda", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_gam_lambda = param1; }

    if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"mg_gam_lambda should not be varied with mg_no_lens option");}
    ...
    class_call(parser_read_double(pfc, "mg_mu0", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_mu0 = param1; }

    class_call(parser_read_double(pfc, "mg_mu1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_mu1 = param1; }

    class_call(parser_read_double(pfc, "mg_gam0", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_gam0 = param1; }

    class_call(parser_read_double(pfc, "mg_gam1", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_gam1 = param1; }

    if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"neither mg_gam0 nor mg_gam1 should be varied with mg_no_lens option");}

  }else if (pba->mg_ansatz == plk_late) {
    ...
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_E11", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_E11 = param1; }

    class_call(parser_read_double(pfc, "mg_E22", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_E22 = param1; }
    
    class_call(parser_read_double(pfc, "mg_c1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_c1 = param1; }

    class_call(parser_read_double(pfc, "mg_c2", &param1, &flag3, errmsg),
               errmsg, errmsg);
    if (flag3 == _TRUE_){ pba->mg_c2 = param1; }

  if ((pba->mg_no_lens == _TRUE_) && ((flag2 == _TRUE_) || (flag3 == _TRUE_)))
       {class_stop(errmsg,"neither mg_E22 nor mg_c2 should be varied with mg_no_lens option");}
  ...
  if ((pba->mg_no_lens == _TRUE_) && ((flag2 == _TRUE_) || (flag3 == _TRUE_)))
    ...
    class_call(parser_read_double(pfc, "mg_E22", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_E22 = param1; }

    class_call(parser_read_double(pfc, "mg_c1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_c1 = param1; }

    class_call(parser_read_double(pfc, "mg_c2", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_c2 = param1; }


 }else if (pba->mg_ansatz == plk_early) {
    ...
    class_call(parser_read_double(pfc, "mg_E12", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_E12 = param1; }
    ...
    class_call(parser_read_double(pfc, "mg_E21", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_E21 = param1; }
   ...
   class_call(parser_read_double(pfc, "mg_c2", &param1, &flag3, errmsg),
    ...
    if ((pba->mg_no_lens == _TRUE_) && ((flag1 == _TRUE_) || (flag2 == _TRUE_) || (flag3 == _TRUE_)))
       {class_stop(errmsg,"neither mg_E21 nor mg_E22 nor mg_c2 should be varied with mg_no_lens option");}

  }else if (pba->mg_ansatz == BZ) {


    // Set default values (GR limit by default)
    pba->mg_B1 = 0.0; // ~ 0 closest to GR
    pba->mg_B2 = 0.0; // ~ 0 closest to GR
    pba->mg_lambda1_2 = 0.; // force mu to GR
    pba->mg_lambda2_2 = 0.; // force gamma to GR
    pba->mg_ss = 0.0;   // ~ 0 closest to GR 

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_B1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_B1 = param1; }

    class_call(parser_read_double(pfc, "mg_B2", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_B2 = param1; }

    class_call(parser_read_double(pfc, "mg_lambda1_2", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_lambda1_2 = param1; }

    class_call(parser_read_double(pfc, "mg_lambda2_2", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_lambda2_2 = param1; }

    class_call(parser_read_double(pfc, "mg_ss", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_ss = param1; }

    if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"neither mg_B2 nor mg_lambda2_2 should be varied with mg_no_lens option");}

 }else if (pba->mg_ansatz == Geff) {


    // Set default values (GR limit by default)
    pba->mg_ga = 0.0; // ~ 0 force to GR
    pba->mg_gn = 0.0; // ~ 0 closest to GR

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_ga", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_ga = param1; }

    class_call(parser_read_double(pfc, "mg_gn", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_gn = param1; }

}else if (pba->mg_ansatz == z_flex_early) {


    // Set default values (GR limit by default)
    pba->mg_muz  = 0.0; // ~ 0 force to GR
    pba->mg_gamz = 0.0; // ~ 0 focce to GR
    pba->mg_zzn  = 0.0; // ~ 0 closest to GR

    // Load values from parameter file, if specified

    class_call(parser_read_double(pfc, "mg_muz", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_muz = param1; }

    class_call(parser_read_double(pfc, "mg_zzn", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_zzn = param1; }

    class_call(parser_read_double(pfc, "mg_gamz", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_gamz = param1; }

  if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"mg_gamz should not be set with mg_no_lens option");}
  ...
  if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_) )
    ...
    // Set default values (GR limit by default)
    pba->mg_T1  = 0.0; // ~ 0 force to GR
    pba->mg_T2 = 0.0; // ~ 0 focce to GR
    pba->mg_T3  = 0.0; // ~ 0 force to GR
    pba->mg_T4 = 0.0; // ~ 0 focce to GR
    pba->mg_zzn  = 0.0; // ~ 0 closest to GR

    // Load values from parameter file, if specified

    class_call(parser_read_double(pfc, "mg_T1", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_T1 = param1; }
    
    class_call(parser_read_double(pfc, "mg_T2", &param2, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_T2 = param2; }

    class_call(parser_read_double(pfc, "mg_zzn", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_zzn = param1; }

    class_call(parser_read_double(pfc, "mg_T3", &param3, &flag3, errmsg),
               errmsg, errmsg);
    if (flag3 == _TRUE_){ pba->mg_T3 = param3; }

    class_call(parser_read_double(pfc, "mg_T4", &param4, &flag4, errmsg),
               errmsg, errmsg);
    if (flag4 == _TRUE_){ pba->mg_T4 = param4; }

  if ((pba->mg_no_lens == _TRUE_) && ((flag3 == _TRUE_) || (flag4 == _TRUE_)))
       {class_stop(errmsg,"neither mg_T3 nor mg_T4 should be varied with mg_no_lens option");}
  ...
  if ((pba->mg_no_lens == _TRUE_) && ((flag3 == _TRUE_) || (flag4 == _TRUE_)))
 ...
 }else if (pba->mg_ansatz == QR) {

    // Set default values (GR limit by default)
    pba->mg_Q = 1.0; // force mu to GR
    pba->mg_R = 1.0; // force gamma to GR
    pba->mg_ss = 0.0;  // ~ 0 closest to GR

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_Q", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_Q = param1; }
    class_call(parser_read_double(pfc, "mg_R", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_R = param1; }
    class_call(parser_read_double(pfc, "mg_QRss", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_ss = param1; }

    if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"mg_R should not be varied with mg_no_lens option");}

 }else if (pba->mg_ansatz == FR) {
    ...
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "F_R0", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->F_R0 = param1; }

    class_call(parser_read_double(pfc, "FRn", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->FRn = param1; }

 }else if (pba->mg_ansatz == diL) {


    // Set default values (GR limit by default)
    pba->beta0 = 0.0; // force the expression to GR
    pba->xi0 = 0.0001; // ~ 1.0 closest to GR
    pba->DilS = 0.240; // ~ 0.0 closest to GR
    pba->DilR = 1.499;   // ~ 1.5 closest to GR

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "beta0", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->beta0 = param1; }
    class_call(parser_read_double(pfc, "xi0", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->xi0 = param1; }
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "DilS", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->DilS = param1; }

    class_call(parser_read_double(pfc, "DilR", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->DilR = param1; }

 }else if (pba->mg_ansatz == syM) {

    // Set default values (GR limit by default)
    pba->beta_star = 0.0; // force the expression to GR
    pba->a_star = 0.1; // ~ 0.0 closest to GR ~ 1.0 diverge
    pba->xi_star = 0.001; // ~ 1.0 closest to GR

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "beta_star", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->beta_star = param1; }
    class_call(parser_read_double(pfc, "a_star", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->a_star = param1;pba->mg_z_init = 1.0/param1-1.0; }
    class_call(parser_read_double(pfc, "xi_star", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->xi_star = param1;}


 }else if (pba->mg_ansatz == JBD) {
    ...
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "w_JBD", &param1, &flag1, errmsg),
               errmsg, errmsg);


    if (flag1 == _TRUE_){ pba->w_JBD = param1;}

}
else if (pba->mg_ansatz == nDGP) {
    ...
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "rc", &param1, &flag1, errmsg),
               errmsg, errmsg);


    if (flag1 == _TRUE_){ pba->rc = param1;  }


 }

else if (pba->mg_ansatz == kmoufl) {


    // Set default values (GR limit only at z = 0)
    pba->beta_kmfl = 0.0;

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "beta_kmfl", &param1, &flag1, errmsg),
               errmsg, errmsg);


    if (flag1 == _TRUE_){ pba->beta_kmfl = param1;  }

     pba->k0_kmfl = 1.0;  // just to nullify its effect

    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "k0_kmfl", &param1, &flag1, errmsg),
               errmsg, errmsg);


    if (flag1 == _TRUE_){ pba->k0_kmfl = param1;  }
    ...
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "gamGI", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->gamGI = param1; }


 }
else if (pba->mg_ansatz == simple) {

    // simple mu, gamma ansatz
    // Set default values (GR limit by default)
    pba->mg_A_mu = 1.;
    pba->mg_A_gamma = 1.;
    
    // Load values from parameter file, if specified
    class_call(parser_read_double(pfc, "mg_A_mu", &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_){ pba->mg_A_mu = param1; }

    class_call(parser_read_double(pfc, "mg_A_gamma", &param1, &flag2, errmsg),
               errmsg, errmsg);
    if (flag2 == _TRUE_){ pba->mg_A_gamma = param1; }

    if ((pba->mg_no_lens == _TRUE_) && (flag2 == _TRUE_))
       {class_stop(errmsg,"mg_A_gamma should not be varied with mg_no_lens option");}    

  } // end loading MG variables
...
//MGmod ENDS    
  ...
  if (flag1 == _FALSE_) {
    ...
    if ((pba->mg_bckg == _TRUE_) && (pba->mg_ansatz != GR)){

   //MGmod STARTS
  double eff_0, eff_dot_0;


    struct modgrav pmg;
	
    class_call(modgrav_init(pba, &pmg),
                   pba->error_message,
                   pba->error_message);
   
    eff_0 = 2.0 / (pmg.mu_0+pmg.mu_0*pmg.gamma_0);
    eff_dot_0 = - (2.0*(pmg.mu_dot_0*(1.0+pmg.gamma_0)+pmg.mu_0*pmg.gamma_dot_0))/(pow(pmg.mu_0+pmg.mu_0*pmg.gamma_0,2.0)) ;
    eff_dot_0 = eff_dot_0 / (_c_/1.e5);

    double rho_tot_0 ;  
   
    rho_tot_0 = (pba->Omega0_g +pba->Omega0_b +pba->Omega0_cdm +pba->Omega0_ncdm_tot) * pow(pba->H0,2.0);
    pba->Omega0_lambda = (eff_0*pow(pba->H0,2.0) +3.0*eff_dot_0*pba->H0 - rho_tot_0)/pow(pba->H0,2.0);

    }

    if (pba->mg_ansatz == kmoufl){

    
    double A_kmfl,X_kmfl,k_mfl,k_prime_mfl,epsl2_kmfl;

    A_kmfl = 1.0 + pba->beta_kmfl*1.0;
    
    epsl2_kmfl = 1.0*pba->beta_kmfl/(1.0+pba->beta_kmfl*1.0);
    
    pba->Omega0_lambda =(0.5 *(6.0+12.0 *pba->beta_kmfl+2.0 *pow(pba->beta_kmfl,2.0)-4.0 *pow(pba->H0,4.0) *pba->k0_kmfl-16.0 *pba->beta_kmfl *pow(pba->H0,4.0) *pba->k0_kmfl-24.0 *pow(pba->beta_kmfl,2.0) *pow(pba->H0,4.0) *pba->k0_kmfl-16.0 *pow(pba->beta_kmfl,3.0) *pow(pba->H0,4.0) *pba->k0_kmfl-4.0 *pow(pba->beta_kmfl,4.0) *pow(pba->H0,4.0) *pba->k0_kmfl-4.0 *Omega_tot-16.0 *pba->beta_kmfl *Omega_tot-24.0 *pow(pba->beta_kmfl,2.0) *Omega_tot-16.0 *pow(pba->beta_kmfl,3.0) *Omega_tot-4.0 *pow(pba->beta_kmfl,4.0) *Omega_tot
+
sqrt(-4.0 *(-1.0-4.0 *pba->beta_kmfl-6.0 *pow(pba->beta_kmfl,2.0)-4.0 *pow(pba->beta_kmfl,3.0)-1.0 *pow(pba->beta_kmfl,4.0)) *(4.0+4.0 *pow(pba->H0,4.0)+8.0 *pba->beta_kmfl *pow(pba->H0,4.0)+4.0 *pow(pba->beta_kmfl,2.0) *pow(pba->H0,4.0)) *pba->k0_kmfl+
pow(-6.0-12.0 *pba->beta_kmfl-2.0 *pow(pba->beta_kmfl,2.0)+4.0 *pow(pba->H0,4.0) *pba->k0_kmfl+16.0 *pba->beta_kmfl *pow(pba->H0,4.0) *pba->k0_kmfl+24.0 *pow(pba->beta_kmfl,2.0) *pow(pba->H0,4.0) *pba->k0_kmfl+16.0 *pow(pba->beta_kmfl,3.0) *pow(pba->H0,4.0) *pba->k0_kmfl+4.0 *pow(pba->beta_kmfl,4.0) *pow(pba->H0,4.0) *pba->k0_kmfl+4.0 *Omega_tot+16.0 *pba->beta_kmfl *Omega_tot+24.0 *pow(pba->beta_kmfl,2.0) *Omega_tot+16.0 *pow(pba->beta_kmfl,3.0) *Omega_tot+4.0 *pow(pba->beta_kmfl,4.0) *Omega_tot,2.0))))
/
(4.0+4.0 *pow(pba->H0,4.0)+8.0 *pba->beta_kmfl *pow(pba->H0,4.0)+4.0 *pow(pba->beta_kmfl,2.0) *pow(pba->H0,4.0));
    
    }

  if (pba->mg_ansatz == JBD){

    double phi_jbd_0,g0xphi0,d_JBD,ga_0,phi_jbd_dot_0,omegaM_0,omegaMdot_0,rho_tot_0;
   
    rho_tot_0 = (pba->Omega0_g +pba->Omega0_b +pba->Omega0_cdm +pba->Omega0_ncdm_tot) * pow(pba->H0,2.0);

    omegaM_0    = (pba->Omega0_cdm+pba->Omega0_b);
    omegaMdot_0 = -omegaM_0*(3.*1.0*pba->H0*(_c_ /1.e5)+2.*(-1.5*rho_tot_0)/pba->H0*(_c_ /1.e5));

    d_JBD = 2.0*pba->w_JBD + 3.0;
    g0xphi0 = ((4.0+2.0*pba->w_JBD)/(3.0+2.0*pba->w_JBD)) * 1./pow((2.-(pba->Omega0_cdm+pba->Omega0_b)),2./(3.*d_JBD)) ;
    ga_0 =  pow(1.0,2./d_JBD)*pow(2.*pow(1.0,3.)*(1.-omegaM_0)+omegaM_0,2./(3.*d_JBD)) ;
    phi_jbd_0 = g0xphi0*ga_0;
    phi_jbd_dot_0 = g0xphi0 *( 2./d_JBD*pba->H0*(_c_ /1.e5)*ga_0 + ga_0*1./(2.*pow(1.0,3.)*(1.-omegaM_0)+omegaM_0)* 2./(3.*d_JBD)*(6.*pba->H0*(_c_ /1.e5)*pow(1.0,3.)*(1.-omegaM_0)-2.*pow(1.0,3.)*omegaMdot_0+omegaMdot_0)   );

    pba->Omega0_lambda = 
(pow(2.0*pba->H0*(_c_ /1.e5) + (phi_jbd_dot_0/phi_jbd_0),2.0) -pow((phi_jbd_dot_0/phi_jbd_0),2.0)* (1.0+2.0*pba->w_JBD/3.0)) * phi_jbd_0/(pow(pba->H0*(_c_ /1.e5),2)*4.0) - (pba->Omega0_g +pba->Omega0_b +pba->Omega0_cdm +pba->Omega0_ncdm_tot);
   
   }

 if (pba->mg_ansatz == nDGP) { 
   pba->Omega0_lambda= 1.- pba->Omega0_k - Omega_tot + 2./(pba->H0*(_c_ /1.e5)) * sqrt(1.0/(4.*pow(pba->rc,2.)))*(_c_ /1.e5);
