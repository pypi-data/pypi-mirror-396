 ...
 * * routines related to MG written by Z. SAKR in 2021
...
int background_functions(
  ...
  //MGmod STARTS
  double phi_jbd,g0xphi0,d_JBD,ga,phi_jbd_dot,rho_rc,X_kmfl,k_mfl,k_prime_mfl,A_kmfl,epsl2_kmfl,rho_tot_kmfl,p_tot_kmfl,omegaM,omegaMdot,Ez, H,aH,aHdot,Hdot,eff,eff_dot,eff_dot_dot,mu_dt,mu_dt_dt,gamma_dt,gamma_dt_dt,eff_dt,eff_dt_dt,eff_0, eff_dot_0;
...
if ((a > 1./(1. + pba->mg_z_init)) && (pba->mg_bckg == _TRUE_)  && (pba->mg_ansatz != GR)) {

struct modgrav pmg;
double k;
double tau;
    class_call(modgrav_functions(pba, pvecback, 1.e-9, tau, &pmg),
                   pba->error_message,
                   pba->error_message);

H = pvecback[pba->index_bg_H];

eff = 2.0 / (pmg.mu+pmg.mu*pmg.gamma);
eff_dot = - (2.0*(pmg.mu_dot*(1.0+pmg.gamma)+pmg.mu*pmg.gamma_dot))/(pow(pmg.mu+pmg.mu*pmg.gamma,2.0)) ;
eff_dot_dot = 4.0 *pow(pmg.mu_dot + pmg.gamma*pmg.mu_dot + pmg.mu*pmg.gamma_dot,2.0)
                     /pow(pmg.mu + pmg.mu*pmg.gamma,3.0) 
              - 2.0 *(2.0*pmg.mu_dot*pmg.gamma_dot + pmg.mu_dot_dot + pmg.gamma*pmg.mu_dot_dot + pmg.mu*pmg.gamma_dot_dot)
                     /pow(pmg.mu + pmg.mu*pmg.gamma,2.0);
eff_dot = eff_dot/(_c_/1.e5);
eff_dot_dot = eff_dot_dot/(_c_/1.e5);

}

if ((pba->mg_ansatz == JBD) && (a > 1./(1. + pba->mg_z_init))){
  H = pvecback[pba->index_bg_H];
  Ez = H / pba->H0;
  Hdot= pvecback[pba->index_bg_H_prime];
  

  omegaM    = (1. / Ez / Ez)*pow(a,-3.0)*(pba->Omega0_cdm+pba->Omega0_b);
  ...
  if ((pba->mg_ansatz == kmoufl) && (a > 1./(1. + pba->mg_z_init))) {
  ...
  pvecback[pba->index_bg_rho_lambda] = pba->Omega0_lambda * pow(pba->H0,2);
    ...
    k_mfl = -1.0 + X_kmfl + pba->k0_kmfl * pow(X_kmfl,2.0);
    k_prime_mfl = 1.0 + 2.0*pba->k0_kmfl * X_kmfl;
    epsl2_kmfl = a*pba->beta_kmfl/(1.0+pba->beta_kmfl*a);

       rho_tot_kmfl = ( -k_mfl + pow( pow(H*a,2.0),2.0)*pow(A_kmfl,2.0)*k_prime_mfl ) *pvecback[pba->index_bg_rho_lambda]/pow(A_kmfl,4.0);
       p_tot_kmfl = (k_mfl*pvecback[pba->index_bg_rho_lambda])/pow(A_kmfl,4.0);
    }

  else if (pba->mg_ansatz == nDGP && (a > 1./(1. + pba->mg_z_init))  ) {
    ...
    else  if ((a > 1./(1. + pba->mg_z_init)) && (pba->mg_bckg == _TRUE_)  && (pba->mg_ansatz != GR)) {
...
//MGmod STARTS
  ...
  if (pba->mg_ansatz == nDGP && (a > 1./(1. + pba->mg_z_init))  ) {
      
      rho_rc = 1.0/(4.*pow(pba->rc,2.));
      pvecback[pba->index_bg_H] = sqrt(rho_tot +rho_rc) - sqrt(rho_rc) ; 
      pvecback[pba->index_bg_H_prime] = -3.0/2.0 *(pvecback[pba->index_bg_H]*( pow(pvecback[pba->index_bg_H],2.0) +pvecback[pba->index_bg_H]/pba->rc +p_tot )) /(2.0 *pvecback[pba->index_bg_H] +1.0/pba->rc)*a;

     
  }

 else if ((pba->mg_ansatz == JBD) && (a > 1./(1. + pba->mg_z_init))){
      	pvecback[pba->index_bg_H] = 0.5 * ( -(phi_jbd_dot/phi_jbd) + sqrt( pow((phi_jbd_dot/phi_jbd),2.0) * (1.0+2.0*pba->w_JBD/3.0) + 4.0*(rho_tot/phi_jbd) ) );
	
        pvecback[pba->index_bg_H_prime] = (+pvecback[pba->index_bg_H]*(phi_jbd_dot/phi_jbd) -pba->w_JBD/3.0*pow((phi_jbd_dot/phi_jbd),2.0) - 1.0/phi_jbd*(3.0*pba->w_JBD*p_tot+rho_tot*(3.0+pba->w_JBD))/(2.0*pba->w_JBD+3.0) -pow(pvecback[pba->index_bg_H],2.0)) *a;
  }

else  if ((pba->mg_ansatz == kmoufl) && (a > 1./(1. + pba->mg_z_init))) { 
     
	pvecback[pba->index_bg_H] = sqrt(pow(A_kmfl,2.0)/(1.0-pow(epsl2_kmfl,2.0))*(rho_tot+rho_tot_kmfl));
	pvecback[pba->index_bg_H_prime] = 4.0*pow(A_kmfl,3.0)*pba->beta_kmfl*pvecback[pba->index_bg_H]*a*pvecback[pba->index_bg_H] *a - (3./2.) * (rho_tot+rho_tot_kmfl +p_tot_kmfl) * pow(A_kmfl,4.0)*a  ;

}

else  if ((a > 1./(1. + pba->mg_z_init)) && (pba->mg_bckg == _TRUE_)  && (pba->mg_ansatz != GR)) { 
     
	pvecback[pba->index_bg_H] = (0.5*( -3.0*eff_dot/a + sqrt(9.0*eff_dot/a*eff_dot/a+4.0*pvecback[pba->index_bg_rho_lambda]*eff+4.0*eff*(rho_tot-pvecback[pba->index_bg_rho_lambda]) ) ) )/ eff;
        
	pvecback[pba->index_bg_H_prime] = - (3./2.) * ((rho_tot + p_tot)*a  +eff_dot_dot/a -pvecback[pba->index_bg_H]*eff_dot -pvecback[pba->index_bg_H]*eff_dot/a) /eff ;
...
//print to remove?
    ...
    class_call(modgrav_bckg_test(&mg_bckg_test_enabled),
               pba->error_message,
               pba->error_message
              );
        printf("    * Modified Gravity Background enabled: %d\n", mg_bckg_test_enabled);
