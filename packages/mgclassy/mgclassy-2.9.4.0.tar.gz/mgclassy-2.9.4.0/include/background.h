...
//MGmod
/** list of possible ansatzes for MG theory */
enum mg_possible_ansatzes {GR, EFT_alpha_QSL, EFT_alpha_QSE, horndeski, mgtanh, omegade, cpl, plk_early, plk_late, plk_norm_late, plk_musigma_norm_late, BZ, Geff, z_flex_early, z_flex_late, z_flex_norm_late, z_xpans_early, z_xpans_late, QR, FR, diL, syM, JBD, nDGP, kmoufl, GI, simple};
...
struct background
{
  ...
  // MG background option
  short mg_bckg;

  // Defines which MG functional ansatz to use
  enum mg_possible_ansatzes mg_ansatz;

  // Modified gravity function parameters (simple mu, gamma ansatz)
  double mg_A_mu;
  double mg_A_gamma;
  
   // Modified gravity function parameters (EFTalpha -inspired ansatz)
  double mg_EFT_alpha_M;
  double mg_EFT_alpha_b;

  // Modified gravity function parameters (Horndeski-inspired ansatz)
  double mg_horndeski_M;
  double mg_horndeski_p1;
  double mg_horndeski_p2;
  double mg_horndeski_q2;
  double mg_horndeski_t1;
  double mg_horndeski_t2;

  // Modified gravity function parameters (tanh ansatz)
  double mg_tanh_Bmu;
  double mg_tanh_Bgam;
  double mg_tanh_Wmu;
  double mg_tanh_Wgam;

  // Mod. gravity fn. parameters (simplified Horndeski with z-dependent factor)
  double mg_lambda;
  double mg_mu_lambda;
  double mg_gam_lambda;
  double mg_mu0;
  double mg_mu1;
  double mg_gam0;
  double mg_gam1;

  /// Modified gravity function parameters (Planck parametrization ansatz)
  double mg_E11;
  double mg_E22;
  ...
  // Modified gravity function parameters (BZ parametrization ansatz)
  double mg_B1;
  double mg_B2;
  double mg_lambda1_2;
  double mg_lambda2_2;
  ...
  // Modified gravity function parameters (Geff parametrization ansatz)
  double mg_ga;
  double mg_gn;

  // Modified gravity function parameters (z_expans parametrization ansatz)
  double mg_muz;
  double mg_gamz;
  double mg_zzn;

  // Modified gravity function parameters (z_expans_2 parametrization ansatz)
  double mg_T1;
  double mg_T2;
  double mg_T3;
  double mg_T4;
  
  // Modified gravity function parameters (QR parametrization ansatz)
  double mg_Q;
  double mg_R;
  double mg_QRss;

  // Modified gravity function parameters (FR ansatz)
  double F_R0;
  double FRn;

  // Modified gravity function parameters (diL ansatz)
  double beta0;
  double xi0;
  double DilS;
  double DilR;

  // Modified gravity function parameters (syM ansatz)
  double beta_star;
  double a_star;
  double xi_star;

  // Modified gravity function parameters (JBD ansatz)
  double w_JBD;
  // Modified gravity function parameters (nDGP ansatz)
  double rc;
  double Omega0_lambda_nDGP;

  // Modified gravity function parameters (kmoufl ansatz)
   double beta_kmfl;
   double k0_kmfl; 

  // Modified gravity function parameters (growth index ansatz)
  double gamGI;
