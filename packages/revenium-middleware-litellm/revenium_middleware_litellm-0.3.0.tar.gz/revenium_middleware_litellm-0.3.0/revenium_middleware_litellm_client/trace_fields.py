...
def get_transaction_name(usage_metadata: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    # Try usage_metadata if provided
    if usage_metadata:
        # Try transactionName (camelCase)
        transaction_name = usage_metadata.get("transactionName") or usage_metadata.get("transaction_name")
        if transaction_name:
            return transaction_name
...
def get_retry_number() -> int:
    ...
    """
    retry_str = os.getenv(ENV_REVENIUM_RETRY_NUMBER)
    if retry_str:
        try:
            return int(retry_str)
        except ValueError:
            logger.warning(f"Invalid REVENIUM_RETRY_NUMBER value: {retry_str}, using 0")
            return 0
    return 0
...
def validate_trace_type(trace_type: str) -> Optional[str]:
    """
    Validate trace type according to specification.
    ...
    Args:
        trace_type: The trace type to validate

    Returns:
        Validated trace type or None if invalid
    """
    if not trace_type:
        return None
    ...
    if len(trace_type) > TRACE_TYPE_MAX_LENGTH:
        logger.warning(
            f"trace_type exceeds max length of {TRACE_TYPE_MAX_LENGTH} characters: {trace_type[:50]}..."
        )
        return None
    ...
    if not TRACE_TYPE_PATTERN.match(trace_type):
        logger.warning(
            f"trace_type contains invalid characters (only alphanumeric, hyphens, and underscores allowed): {trace_type}"
        )
        return None
...
def validate_trace_name(trace_name: str) -> Optional[str]:
    """
    Validate and truncate trace name if needed.

    Rules:
    - Max 256 characters (auto-truncates with warning)

    Args:
        trace_name: The trace name to validate

    Returns:
        Validated (possibly truncated) trace name or None if empty
    """
    if not trace_name:
        return None
    ...
    if len(trace_name) > TRACE_NAME_MAX_LENGTH:
        logger.warning(
            f"trace_name exceeds max length of {TRACE_NAME_MAX_LENGTH} characters, truncating: {trace_name[:50]}..."
        )
