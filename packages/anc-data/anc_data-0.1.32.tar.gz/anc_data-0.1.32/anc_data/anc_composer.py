...
class SeqSplitConfig:
    ...
    def __init__(self, allow_split=False, split_threshold=None, freeze_batch_interval=None):
...
class AncComposerBuffer:
    ...
    def __init__(
        ...
        self.sample_allow_split_fn = sample_allow_split_fn
    ...
    def get_with_split(self, target_length, ignore_split_freezing=False):
        ...
        for i, item in enumerate(self.buffer):
            if i in self.delete_idx:
                continue
            if self.sample_allow_split_fn is not None and not self.sample_allow_split_fn(item[1]):
                continue
        ...
        if not self.enable_split_freeze or ignore_split_freezing:
            ...
            self.total_token_length = self.total_token_length - max_length + remain_length
    ...
    def unfrozen_splits(self):
        ...
        """
        to_freeze = [
            (i[0], i[1]) for i in self.freeze_splits_buffer if self.batch_count - i[2] > self.freeze_batch_interval
        ]
        self.unfrozen_splits_buffer += to_freeze
        self.freeze_splits_buffer = [
            i for i in self.freeze_splits_buffer if self.batch_count - i[2] <= self.freeze_batch_interval
        ]
        if self.enable_logging:
            print(f"[Composer] item count in unfrozen_splits_buffer: " f"{len(self.unfrozen_splits_buffer)}")
            print(
...
class AncComposer:
    ...
    def get_items(self, is_last_call=False, ignore_split_freezing=False):
        ...
        Args:
            ...
            ignore_split_freezing: Whether to bypass freeze mechanism for splits
        Returns:
            list: List of items in the micro batch, the sum of the token length of the items
            should be equal to or less than max_seq_len
        """
        items = []
        if is_last_call:
    ...
    def apply(self, sample, is_last_call=False):
        ...
        """
        sample_allow_split = self.allow_split and (
            self.buffer.sample_allow_split_fn is None or self.buffer.sample_allow_split_fn(sample)
        )
        if sample_allow_split or sample is None or self.buffer._get_token_length(sample) <= self.max_seq_len:
