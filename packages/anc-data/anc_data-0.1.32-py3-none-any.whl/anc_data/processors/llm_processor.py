...
class PretrainProcessor(Processor):
    ...
    def __init__(
        ...
        # data_to_skip_path is a file that contains the data to skip
        # each line contains two columns: filepath and row_idx
        ...
        try:
            ...
            logging.error(f"Error getting too long sample threshold, setting to None: {e}")
    ...
    def transform(self, item, is_last_sample=False):
        ...
        if self.too_long_sample_threshold is not None and len(tokens) > self.too_long_sample_threshold:
            logging.warning(
                f"Too long sample threshold is set, and sample length {len(tokens)} "
                f"longer than threshold {self.too_long_sample_threshold}. Will drop"
            )
            return None
        if len(tokens) > self.max_seq_len and not self.seq_split_config.allow_split:
            logging.warning(
                f"pretrain sample encounter with token length {len(tokens)} larger than "
                f"{self.max_seq_len}, will ignore and continue"
            )
            return None
    ...
    def create_batch(self, items):
        ...
        for k in items[0]:
            if k == "tokens":
                res[k] = torch.stack(
                    [self._collate_item(item[k], max_token_count, self.pad_token_id) for item in items]
                )
            elif k == "labels":
                res[k] = torch.stack(
                    [self._collate_item(item[k], max_token_count, self.pad_label_id) for item in items]
                )
    ...
    def transform(self, item, is_last_sample=False):
        ...
        if isinstance(item["messages"], str):
            try:
                ...
                logging.warn(f"failed to decode messages from item: {item['messages']}, error: {e}")
                raise e
        ...
        if not all(
            turn["role"] and turn["content"] and type(turn["role"]) is str and type(turn["content"]) is str
            for turn in item["messages"]
        ):
            logging.warn("Found an illegal sample and will skip it...")
            return
        ...
        if len(context_vec) > self.max_seq_len and not self.seq_split_config.allow_split:
            logging.warn(
                f"encounter sample with token length {len(context_vec)} larger than "
                f"{self.max_seq_len}, will ignore and continue"
            )
            return None
