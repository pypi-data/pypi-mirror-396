...
class SimpleLRUCache[T = "LazyAttrKey"]:
    ...
    def get(self, key: T, d: Any = NOT_FOUND) -> Any | NotFoundType:
    ...
    def set(self, key: T, value: Any) -> None:
...
class LazyAttrMeta(type):
    ...
    @classmethod
    def check_cache(cls, name: str, module_name: str) -> Any:
        key: LazyAttrKey = LazyAttrKey(module_name, attr_name=name)
        return cls._cached.get(key)

    @classmethod
    def get_cache(cls, name: str, module_name: str) -> CacheCheckResult:
        key: LazyAttrKey = LazyAttrKey(module_name, attr_name=name)
...
class LazyAttr(metaclass=LazyAttrMeta):
    ...
    def __reduce__(self) -> tuple[Callable[[str, str], Any], tuple[str, str]]:
        ...
        self._ensure_loaded()
        if self._cached_attr is NOT_FOUND:
            raise TypeError("Cannot pickle LazyAttr with NOT_FOUND value")
    ...
    def __len__(self) -> int:  # pragma: no cover
        self._ensure_loaded()
        if not hasattr(self._cached_attr, "__len__"):
