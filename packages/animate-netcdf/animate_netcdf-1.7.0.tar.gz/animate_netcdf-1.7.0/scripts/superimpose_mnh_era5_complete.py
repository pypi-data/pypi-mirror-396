#!/usr/bin/env python3
"""
Complete MNH vs ERA5 NetCDF Superimposition Tool (Corrected)
...
4. Align time series accounting for different temporal resolutions

Creates comprehensive 2D time series plots and analysis.

Usage:
    python superimpose_mnh_era5_complete.py mnh_file.nc era5_file.nc --output output.nc
"""
...
class CompleteMNHERA5Superimposer:
    """Complete class to handle superimposition of MNH vs ERA5 with proper timestep handling."""
    ...
    def convert_mnh_time_to_datetime(self, mnh_time: np.ndarray, mnh_timestep: float) -> np.ndarray:
        """Convert MNH timestep data to datetime objects."""
        try:
            # MNH time is in seconds since a reference date
            # We need to convert this to actual datetime
            ...
            if 'seconds since' in time_units:
                ...
                try:
                    ...
                    try:
                        ...
                        # Fallback: assume reference date
                        ref_date = datetime(2025, 8, 15, 0, 0, 0)
                        logger.warning(f"Could not parse reference date '{ref_date_str}', using {ref_date}")
                
                # Convert seconds to datetime
                datetime_data = [ref_date + timedelta(seconds=float(t)) for t in mnh_time]
                return np.array(datetime_data)
            else:
                # Fallback: create relative datetime from timesteps
                logger.warning(f"Unknown MNH time units: {time_units}, creating relative timestamps")
                start_time = datetime(2025, 8, 15, 0, 0, 0)  # Arbitrary start
                datetime_data = [start_time + timedelta(seconds=float(t)) for t in mnh_time]
                return np.array(datetime_data)
    ...
    def create_comprehensive_netcdf(self, common_time: np.ndarray, mnh_interp: np.ndarray, 
        ...
        output_file.title = "Complete MNH vs ERA5 Solar Radiation Data (Corrected Units & Timesteps)"
        output_file.history = f"Created by Complete MNH-ERA5 Superimposer on {datetime.now().isoformat()}"
    ...
    def create_comprehensive_visualization(self, common_time: np.ndarray, mnh_interp: np.ndarray, 
        ...
        ax1.set_title(f'Complete MNH vs ERA5 Solar Radiation Comparison\nBoth in W m⁻² - Different Temporal Resolutions', 
        ...
        # Save plot
        output_path = os.path.join(output_dir, 'complete_mnh_vs_era5_analysis.png')
    ...
    def superimpose_complete(self, output_path: str, create_viz: bool = True) -> str:
        ...
        logger.info("=== STARTING COMPLETE MNH vs ERA5 SUPERIMPOSITION ===")
...
def main():
    """Main function to run the complete MNH vs ERA5 superimposition."""
    parser = argparse.ArgumentParser(description='Complete MNH vs ERA5 NetCDF superimposition with proper timestep handling')
    ...
    parser.add_argument('--output', '-o', default='complete_superimposed_mnh_era5.nc', 
    ...
    try:
        with CompleteMNHERA5Superimposer(args.mnh_file, args.era5_file) as superimposer:
            output_path = superimposer.superimpose_complete(
                args.output,
                create_viz=not args.no_viz
            )
            logger.info(f"=== COMPLETE SUPERIMPOSITION SUCCESSFUL ===")
            logger.info(f"Output: {output_path}")
            
    except Exception as e:
        logger.error(f"Complete superimposition failed: {e}")
        sys.exit(1)
