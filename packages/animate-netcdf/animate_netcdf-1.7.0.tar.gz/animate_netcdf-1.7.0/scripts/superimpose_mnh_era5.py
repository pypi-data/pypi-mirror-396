#!/usr/bin/env python3
"""
Specialized NetCDF Superimposition Tool for MNH vs ERA5

This script superimposes MNH and ERA5 NetCDF files with different data structures:
- ERA5: ssrd variable (48 time steps, single point)
- MNH: SWD variable in Stations.Mahambo group (289 time steps, single point)

Creates a 2D time series plot comparing the two datasets.

Usage:
    python superimpose_mnh_era5.py mnh_file.nc era5_file.nc --output output.nc
"""
...
class MNHERA5Superimposer:
    """Specialized class to handle superimposition of MNH vs ERA5 NetCDF files."""
    ...
    def extract_era5_ssrd(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            # ERA5 data is (time, lat, lon) - extract single point
            ssrd_data = ssrd_data[:, 0, 0]  # Remove spatial dimensions
    ...
    def convert_time_to_datetime(self, time_data: np.ndarray, time_units: str) -> np.ndarray:
        ...
        try:
            if time_units != 'unknown':
                ...
                # Try to infer time units or use as-is
    ...
    def create_superimposed_netcdf(self, common_time: np.ndarray, mnh_interp: np.ndarray, 
        ...
        era5_var.long_name = 'ERA5 SSRD (Surface Solar Radiation Downward)'
        era5_var.units = 'J m-2'
        ...
        output_file.title = "Superimposed MNH vs ERA5 Solar Radiation Data"
        output_file.history = f"Created by MNH-ERA5 Superimposer on {datetime.now().isoformat()}"
        output_file.source_mnh = self.mnh_file_path
        output_file.source_era5 = self.era5_file_path
        output_file.description = "Comparison of MNH SWD and ERA5 SSRD variables"
    ...
    def create_2d_timeseries_plot(self, common_time: np.ndarray, mnh_interp: np.ndarray, 
        ...
        fig.suptitle('MNH vs ERA5 Solar Radiation Comparison', fontsize=16, fontweight='bold')
        ...
        if valid_mask.sum() > 0:
            ...
            # Add 1:1 line
            min_val = min(np.nanmin(mnh_interp), np.nanmin(era5_interp))
            max_val = max(np.nanmax(mnh_interp), np.nanmax(era5_interp))
        ...
        # Save plot
        output_path = os.path.join(output_dir, 'mnh_vs_era5_solar_radiation_2d_timeseries.png')
    ...
    def superimpose(self, output_path: str, create_viz: bool = True) -> str:
        ...
        logger.info("Starting MNH vs ERA5 superimposition...")
...
def main():
    """Main function to run the MNH vs ERA5 superimposition."""
    parser = argparse.ArgumentParser(description='Superimpose MNH vs ERA5 NetCDF files')
    ...
    parser.add_argument('--output', '-o', default='superimposed_mnh_era5.nc', 
    ...
    try:
        with MNHERA5Superimposer(args.mnh_file, args.era5_file) as superimposer:
