#!/usr/bin/env python3
"""
Final MNH vs ERA5 NetCDF Superimposition Tool
...
IMPORTANT: The script converts ERA5 data from accumulated energy (J m⁻²) to hourly average power (W m⁻²)
by dividing by the time period in seconds. This assumes ERA5 data represents energy accumulated over
each time step, which is typical for reanalysis products.

Creates a 2D time series plot comparing the two datasets with proper units.

Usage:
    python superimpose_mnh_era5_final.py mnh_file.nc era5_file.nc --output output.nc
"""
...
class FinalMNHERA5Superimposer:
    """Final, robust class to handle superimposition of MNH vs ERA5 NetCDF files with proper unit conversion."""
    ...
    def create_superimposed_netcdf(self, common_time: np.ndarray, mnh_interp: np.ndarray, 
                                  era5_interp: np.ndarray, output_path: str, time_resolution: float) -> str:
        ...
        mnh_var.units = 'W m-2'
        mnh_var.description = 'Instantaneous shortwave downward radiation from MNH model'
        ...
        era5_var.note = f'Converted from J m-2 to W m-2 by dividing by {time_resolution} seconds'
        era5_var.description = f'Time-averaged shortwave downward radiation from ERA5 reanalysis (averaged over {time_resolution/3600:.2f} hour periods)'
        ...
        output_file.history = f"Created by Final MNH-ERA5 Superimposer on {datetime.now().isoformat()}"
        ...
        output_file.note = f"ERA5 data converted from accumulated energy (J m-2) to time-averaged power (W m-2) using {time_resolution} second time resolution"
        output_file.unit_conversion = f"ERA5: J m-2 / {time_resolution} s = W m-2"
        output_file.comparison_note = "MNH: instantaneous power (W m-2), ERA5: time-averaged power (W m-2)"
    ...
    def create_2d_timeseries_plot(self, common_time: np.ndarray, mnh_interp: np.ndarray, 
                                 era5_interp: np.ndarray, time_resolution: float, output_dir: str = ".") -> str:
        ...
        if isinstance(common_time[0], datetime):
            ax1.plot(common_time, mnh_interp, 'b-', linewidth=2, label='MNH SWD (Mahambo) - Instantaneous', alpha=0.8)
            ax1.plot(common_time, era5_interp, 'r-', linewidth=2, 
                    label=f'ERA5 SSRD - {time_resolution/3600:.2f}h Average', alpha=0.8)
        ...
        ax1.set_title(f'Time Series Comparison (Both in W m⁻²)\nMNH: Instantaneous, ERA5: {time_resolution/3600:.2f}h Average')
        ax1.legend()
        ...
        if valid_mask.sum() > 0:
            ...
            ax2.set_xlabel('MNH SWD (W m⁻²) - Instantaneous')
            ax2.set_ylabel(f'ERA5 SSRD (W m⁻²) - {time_resolution/3600:.2f}h Average')
        ...
        # Save plot
        output_path = os.path.join(output_dir, 'mnh_vs_era5_solar_radiation_2d_timeseries_final.png')
    ...
    def superimpose(self, output_path: str, create_viz: bool = True) -> str:
        ...
        logger.info("Starting final MNH vs ERA5 superimposition...")
        
        try:
            # Extract data from both files
            mnh_swd, mnh_time = self.extract_mnh_swd()
            era5_ssrd, era5_time, time_resolution = self.extract_era5_ssrd()
            ...
            logger.info(f"Time resolution used for conversion: {time_resolution} seconds ({time_resolution/3600:.2f} hours)")
            
            # Create superimposed NetCDF
            output_file_path = self.create_superimposed_netcdf(
                common_time, mnh_interp, era5_interp, output_path, time_resolution
            )
            
            # Create visualization
            if create_viz:
                viz_path = self.create_2d_timeseries_plot(
                    common_time, mnh_interp, era5_interp, time_resolution,
...
def main():
    """Main function to run the final MNH vs ERA5 superimposition."""
    ...
    parser.add_argument('--output', '-o', default='superimposed_mnh_era5_final.nc', 
    ...
    try:
        with FinalMNHERA5Superimposer(args.mnh_file, args.era5_file) as superimposer:
