...
class GenerativeUIAgent:
    ...
    async def _call_rest_api(self, model: str, system_prompt: str, user_prompt: str) -> str:
        ...
        # Gemini 3 模型必须启用 thinking mode
        # 通过环境变量 GENUI_THINKING_BUDGET 设置 thinking tokens 预算
        # 默认 8192，设置为 0 会报错（Gemini 3 不支持禁用 thinking）
        thinking_budget = int(os.getenv("GENUI_THINKING_BUDGET", "8192"))
        if thinking_budget > 0:
            payload["generationConfig"]["thinkingConfig"] = {
                "thinkingBudget": thinking_budget
            }
            print(f"🧠 Thinking 模式已启用，budget: {thinking_budget}")
