...
class ElasticSearchDataSource(SearchIndexDataSource):
    ...
    def connect(self) -> Elasticsearch:
        ...
        try:
            ...
            self.client = Elasticsearch(hosts=[{"host": host, "port": port}], http_auth=auth)
            if not self.client.ping():
                raise Exception("Failed to connect to ElasticSearch data source")
            return self.client
        except Exception as e:
            raise DataChecksDataSourcesConnectionError(f"Failed to connect to ElasticSearch data source: [{str(e)}]")
