...
from mcp_audit.display.theme_detect import (
    detect_terminal_mode,
    get_active_theme,
    is_ascii_mode,
    _detect_macos_mode,
)
from mcp_audit.display.themes import CatppuccinMocha, CatppuccinLatte
...
class TestDetectTerminalMode:
    ...
    def test_explicit_dark_env(self, monkeypatch):
        """MCP_AUDIT_THEME=dark should return dark."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "dark")
        assert detect_terminal_mode() == "dark"

    def test_explicit_light_env(self, monkeypatch):
        """MCP_AUDIT_THEME=light should return light."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "light")
        assert detect_terminal_mode() == "light"

    def test_explicit_mocha_env(self, monkeypatch):
        """MCP_AUDIT_THEME=mocha should return dark."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "mocha")
        assert detect_terminal_mode() == "dark"

    def test_explicit_latte_env(self, monkeypatch):
        """MCP_AUDIT_THEME=latte should return light."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "latte")
        assert detect_terminal_mode() == "light"

    def test_explicit_catppuccin_mocha_env(self, monkeypatch):
        """MCP_AUDIT_THEME=catppuccin-mocha should return dark."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "catppuccin-mocha")
        assert detect_terminal_mode() == "dark"

    def test_explicit_catppuccin_latte_env(self, monkeypatch):
        """MCP_AUDIT_THEME=catppuccin-latte should return light."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "catppuccin-latte")
        assert detect_terminal_mode() == "light"

    def test_explicit_hc_dark_env(self, monkeypatch):
        """MCP_AUDIT_THEME=hc-dark should return dark."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "hc-dark")
        assert detect_terminal_mode() == "dark"

    def test_explicit_hc_light_env(self, monkeypatch):
        """MCP_AUDIT_THEME=hc-light should return light."""
        monkeypatch.setenv("MCP_AUDIT_THEME", "hc-light")
        assert detect_terminal_mode() == "light"

    def test_colorfgbg_dark(self, monkeypatch):
        """COLORFGBG with dark background should return dark."""
        monkeypatch.delenv("MCP_AUDIT_THEME", raising=False)
...
class TestGetActiveTheme:
    ...
    def test_defaults_to_dark(self, monkeypatch):
        ...
        with patch("mcp_audit.display.theme_detect._is_macos", return_value=False):
...
class TestIsAsciiMode:
    ...
    def test_empty_string(self, monkeypatch):
        ...
        monkeypatch.setenv("MCP_AUDIT_ASCII", "")
        assert is_ascii_mode() is False
