...
class TestProductionAPIEndpoints(unittest.TestCase):
    ...
    def test_recall_endpoint(self):
        """Test recall() calls correct endpoint"""
        with patch.object(self.client, '_request') as mock_request:
...
class TestProductionAPIRequestPayloads(unittest.TestCase):
    ...
    def test_recall_request_params(self):
        """Test recall() sends correct query parameters"""
        with patch.object(self.client, '_request') as mock_request:
            mock_request.return_value = {'memories': [], 'count': 0}
            ...
            self.assertEqual(params['query'], "search query")
            self.assertEqual(params['limit'], 20)
            self.assertEqual(params['min_helpfulness_score'], 0.8)
            self.assertEqual(params['organized'], 'true')
            self.assertEqual(params['project_id'], "my-project")
