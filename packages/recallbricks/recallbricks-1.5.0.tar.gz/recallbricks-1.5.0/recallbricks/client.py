...
class RecallBricks:
    ...
    def recall(
        ...
        if min_helpfulness_score is not None:
            params["min_helpfulness_score"] = min_helpfulness_score

        if organized:
            params["organized"] = "true"

        if user_id:
            params["user_id"] = self._sanitize_input(user_id, max_length=256)

        if project_id:
            params["project_id"] = project_id

        return self._request("GET", "/memories/recall", params=params)
