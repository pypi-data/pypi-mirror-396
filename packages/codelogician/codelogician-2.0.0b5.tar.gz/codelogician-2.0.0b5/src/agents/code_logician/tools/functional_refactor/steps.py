...
complex_function_refactoring_example = RefactoringExample(
    before="""def process_order(order_data, inventory, customer_info):
    # 45 lines of intertwined validation, pricing, inventory checks,
    # discount calculations, tax computation, and order finalization
    # Hard to follow which variables affect what logic
    # Multiple nested if-else blocks mixing business rules
    # Difficult to test individual business rules in isolation
    ...""",
    after="""def calculate_order_pricing(order_data, customer_info):
    # Handle complex pricing logic: base prices, customer tiers,
    # bulk discounts, promotional codes, tax calculations
    ...

def validate_and_reserve_inventory(order_data, inventory):
    # Check availability, handle backorders, reserve items,
    # update inventory counts with proper error handling
    ...

def process_order(order_data, inventory, customer_info):
    pricing = calculate_order_pricing(order_data, customer_info)
    reservation = validate_and_reserve_inventory(order_data, inventory)
    return finalize_order(order_data, pricing, reservation)""",
)
...
complex_function_refactoring = FunctionalRefactoringData(
    ...
    description=(
        "For functions with intertwined logic or unclear reasoning flow, extract "
        "complex sub-processes into focused functions. The original function "
        "name and the order and default values of parameters must be preserved "
        "(type hints are allowed to be changed). Large line count doesn't necessarily "
        "mean a function needs refactoring - we care about logical clarity and "
        "ease of reasoning, not arbitrary size limits. Prioritize clarity over brevity."
    ),
    non_functional_patterns=[
        "Functions with intertwined logic that's hard to follow",
        "Functions where multiple concerns are mixed together",
        "Functions with unclear control flow or data dependencies",
        "Functions with deeply nested conditional logic",
        "Functions where the main logic is obscured by implementation details",
        "Functions that are difficult to test due to tightly coupled operations",
        "Functions where variable state is modified in non-obvious ways",
    ],
    desired_patterns=[
        "Clear separation of distinct concerns or complex sub-processes",
        "Each extracted function should represent a cohesive operation",
        "Function names should clearly communicate intent and operation",
        "Keep original function as a clear composition of logical steps",
        "Maintain linear, easy-to-follow control flow",
        "Extract only when it improves readability and reasoning",
        "Prefer meaningful chunks over arbitrary line count limits",
        "Maintain the same overall behavior and error handling",
    ],
    reference_transformation_steps=[
        "Identify substantial sub-processes that are currently intertwined",
        "Look for repeated patterns or complex sub-operations that obscure main logic",
        "Extract operations that can be understood and tested independently",
        "Create descriptive names that capture the business logic or intent",
