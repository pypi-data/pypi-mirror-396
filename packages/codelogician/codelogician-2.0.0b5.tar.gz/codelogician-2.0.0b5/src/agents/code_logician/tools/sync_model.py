...
# TODO: we probably need few-shot examples or RAG for the following two
...
async def update_src_code(
    ...
    res = await llm.with_structured_output(UpdatedSrcCode).ainvoke(
        [
...
async def update_iml_code(
    ...
    res = await llm.with_structured_output(UpdatedIMLCode).ainvoke(
        [
