...
def test_decode_contact() -> None:
    expected_data = ContactObj(
        ...
        ContactDates(
            _assert_utc(datetime(2015, 2, 16, 14, 46, 24, tzinfo=UTC)),
            _assert_utc(datetime(2015, 3, 2, 14, 49, 32, tzinfo=UTC)),
        ),
        AccountId(NaturalOperations.absolute(123456)),
        UserId(NaturalOperations.absolute(3263000000057001)),
        CrmId(NaturalOperations.absolute(5000000014010)),
        Maybe.empty(),
        Maybe.empty(),
        Maybe.empty(),
        Maybe.empty(),
