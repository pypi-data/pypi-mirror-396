...
class Operation:
    ...
    def run(self, data: Context, args: list, display="steps", 
        ...
        data_dict = dict(data)
        self.validate_input(data_dict)
        # Update the Context with any changes (like defaults)
...
class SubOperationStep:
    ...
    def __init__(self, dyngleverse, declarations: Context, definition: dict):
        ...
        self.outbound_payload_key = definition.get("send")
        self.inbound_payload_key = definition.get("receive")
    ...
    def run(self, data: Context, args: list, display="steps", 
        ...
        if self.outbound_payload_key:
            # Resolve the outbound payload - must be a dict
            outbound = full_context.resolve(
                self.outbound_payload_key, str_only=False
            )
        ...
        if self.inbound_payload_key:
            data[self.inbound_payload_key] = result
