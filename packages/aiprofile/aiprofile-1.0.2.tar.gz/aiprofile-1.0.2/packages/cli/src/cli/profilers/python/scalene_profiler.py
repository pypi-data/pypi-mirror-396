...
class ScaleneProfiler(Profiler):
        ...
        try:
            ...
            try:
                ...
                if not result.get("success"):
                    # Build error message with script error details if available
                    error_msg = result.get("error_message", "Unknown error")
                    script_error = result.get("script_error", "")

                    if script_error:
                        # Include the actual script error output
    ...
    def _run_scalene(
        ...
        Returns:
            Success dict with error details if script fails
        """
        try:
            # Run with Scalene
            ...
            try:
                ...
                # Check if the script itself failed (exit code != 0)
                if result.returncode != 0:
                    # Script failed - propagate the error
                    error_output = result.stderr if result.stderr else result.stdout
                    exit_code_msg = f"Script execution failed with exit code {result.returncode}"
                    return {
                        "success": False,
                        "error_message": exit_code_msg,
                        "script_error": error_output,
                        "exit_code": result.returncode,
                    }

                # Process completed successfully, check if output was created
                ...
                # Check if there was a script error in stderr before timeout
                script_error = e.stderr if e.stderr else ""
                if script_error and "Error" in script_error:
                    return {
                        "success": False,
                        "error_message": "Script execution failed during profiling",
