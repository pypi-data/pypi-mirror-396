...
BioSAK batch_itol -api API_key -project batch_access_tmp -a annotation_files.txt -i input.tree -o out.pdf
BioSAK batch_itol -api API_key -project batch_access_tmp -a annotation_files.txt -i tree_dir -x tree -o out_pdf
...
def itol_single_tree(tree_file, annotation_files_txt, project_name, APIkey, op_plot):
    ...
    itol_exporter.set_export_param_value('internal_scale', '0')
    ...
    itol_exporter.set_export_param_value('vertical_shift_factor', '1')
    itol_exporter.set_export_param_value('horizontal_scale_factor', '0.9')

    # range
    itol_exporter.set_export_param_value('range_mode', '2') # Possible values: 0,1 or 2 (0=off, 1=cover labels only, 2=cover full clades)
    itol_exporter.set_export_param_value('include_ranges_legend', '0')
    itol_exporter.set_export_param_value('ranges_legend_title', 'to be added')

    # label
    # itol_exporter.set_export_param_value('current_font_size', '12') # the default looks good
    itol_exporter.set_export_param_value('current_font_name', 'Courier')
    itol_exporter.set_export_param_value('default_label_color', '#000000')

    # branch
    itol_exporter.set_export_param_value('line_width', '2')
    itol_exporter.set_export_param_value('dashed_lines', '1')
    itol_exporter.set_export_param_value('default_branch_color', '#000000')

    # bootstrap
    itol_exporter.set_export_param_value('metadata_source', 'bootstrap')         # Which metadata source to use for bootstrap display options
    itol_exporter.set_export_param_value('bootstrap_display', '1')               # possible values: 0 or 1
    itol_exporter.set_export_param_value('bootstrap_type', '2')                  # Possible values: 1, 2, 3 or 4 (1=Symbol, 2=Text label, 3=Branch color and 4=Branch width)
    itol_exporter.set_export_param_value('bootstrap_label_size', '15')           # in pixels, integer >= 9
    itol_exporter.set_export_param_value('bootstrap_label_percent_factor', '10') # in pixels, integer >= 9
...
def batch_itol(args):
    ...
    if os.path.isfile(tree_file_dir) is True:
        itol_single_tree(tree_file_dir, annotation_files_txt, project_name, API_key, op_file_dir)
        ...
        for each_file in file_list:
            ...
            itol_single_tree(each_file, annotation_files_txt, project_name, API_key, op_pdf)
    else:
...
if __name__ == '__main__':
    ...
    batch_itol_parser.add_argument('-project',  required=False, default='batch_access_tmp', help='iTOL project name, default: batch_access_tmp')
