...
class MidiMapper:
    ...
    def _setup_reference_pitch(self) -> None:
        ...
        if self.data.reference_pitch:
            ...
            is_loader_default = (
                ...
                and abs(self.reference_frequency - DEFAULT_REFERENCE_FREQUENCY) < 1.0
            )
            ...
            if self.notes_per_octave == TET_12_NOTES_PER_OCTAVE:
                if is_loader_default:
                    ...
                    self.reference_midi_note = (
                        self.reference_octave - MIDI_OCTAVE_OFFSET
                    ) * 12 + self.reference_index
                else:
            ...
            if self.notes_per_octave == TET_12_NOTES_PER_OCTAVE:
                ...
                self.reference_midi_note = (
                    DEFAULT_REFERENCE_OCTAVE - MIDI_OCTAVE_OFFSET
                ) * 12 + DEFAULT_REFERENCE_INDEX_12TET
            else:
    ...
    def _midi_range_by_index(self, note_range: NoteRange) -> Tuple[int, int]:
        ...
        if self.notes_per_octave == TET_12_NOTES_PER_OCTAVE:
            ...
            min_midi = self._octave_index_to_midi(
                note_range.min_octave, note_range.min_index
            )

            if note_range.max_octave is not None and note_range.max_index is not None:
                max_midi = self._octave_index_to_midi(
                    note_range.max_octave, note_range.max_index
                )
            else:
                max_midi = min(MIDI_MAX, min_midi + MIDI_NOTE_COUNT - 1)
    ...
    def _octave_index_to_midi(self, octave: int, index: int) -> int:
        ...
        total_pitch_classes = octave_offset * self.notes_per_octave + (
    ...
    def _calculate_frequency_for_midi(self, midi_num: int) -> float:
        ...
        if self.notes_per_octave == TET_12_NOTES_PER_OCTAVE:
            ...
            cents_from_ref = (
                octaves_from_ref * self.octave_cents
            ) + cents_from_ref_note
