...
#pragma pack(pop)
        ...
        inline uint64_t extract_zip64_compressed_size_within(std::istream& stream, std::streamsize nbytes) {
            for (extensible_data_field desc; nbytes > sizeof(desc);) {
                ...
                if (desc.header_id == 0x0001) {
                    ...
                    if (desc.data_size > sizeof(zip64) || desc.data_size < sizeof(uint64_t) * 2)
                        throw std::runtime_error(
                            "load_npz: zip64 extended "
                            "information is malformed.");
                    if (!stream.read(reinterpret_cast<char*>(&zip64), desc.data_size))
                        throw std::runtime_error(
                            "load_npz: unexpected end-of-file in zip64 "
                            "extended information.");
                    if (!stream.ignore(nbytes)) throw std::runtime_error("load_npz: failed reading extra field.");
                    return zip64.compressed_size;
                } else {
                    if (!stream.ignore(desc.data_size))
                        throw std::runtime_error("load_npz: failed reading extensible data field.");
                }
            }

            throw std::runtime_error("load_npz: missing zip64 extended information.");
        }

        inline uint64_t extract_zip64_compressed_size(std::istream& stream, zip_local_header const& entry) {
            if (entry.compressed_size == 0xffffffff)
                return extract_zip64_compressed_size_within(stream, entry.extra_field_len);
            else {
        ...
        for (;;) {
            ...
            switch (entry.compression_method) {
                case 0:
                    ...
                    arrays.insert(result_type::value_type(varname, detail::load_npy_file(stream)));
                    break;
                case 8: {
                    ...
                    arrays.insert(result_type::value_type(varname, detail::load_npy_file(zstream)));
                    if (!stream.seekg(pos + std::streamoff(compressed_size))) {
                        throw std::runtime_error("load_npz: unable to read the next variable.");
                    }
    ...
    xarray<T> load_npz(std::string filename, std::string search_varname) {
        ...
        for (;;) {
            ...
            if (varname == search_varname) {
                ...
                stream.seekg(static_cast<std::streamsize>(entry.compressed_size), std::ios_base::cur);
            }
        }
    ...
    namespace detail {
        ...
        inline void parse_zip_footer(std::istream& stream, uint16_t& nrecs, std::streamsize& global_header_size,
    ...
    namespace detail {
        ...
        inline std::pair<uint16_t, uint16_t> time_pair() {
            ...
            return {msdos_time(static_cast<uint16_t>(tm->tm_hour), static_cast<uint16_t>(tm->tm_min),
                               static_cast<uint16_t>(tm->tm_sec)),
                    msdos_date(static_cast<uint16_t>(tm->tm_year + 1900), static_cast<uint16_t>(tm->tm_mon + 1),
     ...
     */
    template <class E>
    void dump_npz(std::string filename, std::string varname, const xt::xexpression<E>& e, bool compression = false,
        ...
        uint32_t crc = static_cast<uint32_t>(crc32(0L, reinterpret_cast<uint8_t*>(&array_contents[0]),
                                                   static_cast<unsigned int>(array_contents.size())));
