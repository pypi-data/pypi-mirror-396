    ...
    def fit(self):
        ...
        for wrapper in self.base_wrappers:
            ...
            if wrapper.best_model is None:
                ...
                print(f"Using pre-trained base model: {wrapper.descriptor}")
                num_classes = wrapper.num_classes
                preds = [b.predict() for b in self.base_wrappers]
                stacked_values = np.stack(preds, axis=1)
        ...
        # self.meta_learner.val_idx_arr = None
        preds = [b.predict() for b in self.base_wrappers]
        self.meta_learner.test_data = np.stack(preds, axis=1)
...
class GenericEnsembleWrapper(ModelWrapper):
    ...
    def __init__(
        ...
        for s in ensemble_strategies.split(","):
            s = s.strip().lower()
            if s not in self.STRATEGY_MAP:
                raise ValueError(f"Unknown strategy '{s}'")
            if s == "stacking":
                self.selected_strategies.append(StackingStrategy(meta_learner= meta_learner, base_wrappers= base_wrappers))
            else:
    ...
    def get_metrics_summary(self) -> Dict:
        return {
            ...
            "strategies": [s.get_strategy_name() for s in self.selected_strategies]
        }
