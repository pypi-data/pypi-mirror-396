...
class LambdaTokenValidator:
    ...
    def __init__(self, lambda_arn: str):
        ...
        try:
            ...
            log.debug(
                "Lambda client initialized for validator: %s", lambda_arn
            )
        except ImportError:
            log.error("boto3 not available for Lambda token validation")
            raise ImportError(
                "boto3 is required for Lambda token validation"
            )
    ...
    def validate(
        ...
        try:
            ...
            log.debug(
                "Token validated successfully, claims extracted from context"
            )
            return context
    ...
    def decorator(handler: Callable) -> Callable:
        ...
        def wrapper(event: dict, context: Any) -> dict:
            ...
            if event.get("requestContext", {}).get("authorizer"):
                log.debug(
                    "Authorizer already exists, skipping JWT processing"
                )
            ...
            config_jwks_url = (
                jwks_url
                or f"{os.getenv('JWKS_HOST')}/.well-known/jwks.json"
            ...
            config_audience = (
                audience
                or os.getenv("JWT_ALLOWED_AUDIENCES", "").split(",")
                if os.getenv("JWT_ALLOWED_AUDIENCES")
                else []
            )
            config_algorithms = algorithms or ["RS256"]
            config_anonymous_role = anonymous_role or os.getenv(
                "ANONYMOUS_ROLE"
            )
            log.debug("anonymous_role parameter: %s", anonymous_role)
            log.debug(
                "ANONYMOUS_ROLE env: %s", os.getenv("ANONYMOUS_ROLE", "")
            )
            ...
            if not config_lambda_arn and not config_jwks_url:
                ...
                if config_anonymous_role:
                    log.debug(
                        "Using anonymous role: %s", config_anonymous_role
                    )
            ...
            try:
                ...
                if token and wrapper._lambda_validator:
                    try:
                        ...
                        decoded_token = wrapper._lambda_validator.validate(
                            token
                        )
                ...
                if token and not decoded_token and wrapper._jwt_decoder:
                    try:
                        ...
                        decoded_token = wrapper._jwt_decoder.decode_token(
                            token
                        )
