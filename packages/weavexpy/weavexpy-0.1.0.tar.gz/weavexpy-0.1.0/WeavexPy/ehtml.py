...
class select:
    def __init__(self, cls = None, id = None, name = None, disabled:bool=False, param = None):
        self.cls = f' class="{cls}"' if not cls == None else ''
        self.id = f' id="{id}"' if not id == None else ''
        self.name = f' name="{name}"' if not name == None else ''
        self.s = style('line')
    ...
    def option(self, value = '', content = '', param = None) :
        param = ''
        if not param == None:
            if isinstance(param, dict):
                param_names = list(param.keys())
                ...
                for i, n in enumerate(param_names):
                    value = param_values[i] if not isinstance(param_values[i], str) else f'"{param_values[i]}"'
                    param += f' {n} = {value}'
        self.optios += f'  <option value="{value}" {param}>{content}</option>\n'
    def __str__(self) : return f'<select {self.param}{self.cls}{self.id}{self.name}{self.disabled}>\n{self.optios}\n</select>'
...
class section:
    ...
    def __str__(self) : 
        Format = ''
        for e in self.content : Format += f'{e}\n'
        return f'<section {self.param}{self.cls}{self.id}{self.name} style={self.s.set()}>\n   {Format}\n</section>\n'
