...
class RAGPipeline:
    ...
    def execute_rag(
        ...
        if "query_reformulation" in prompts and prompts["query_reformulation"].strip():
            reformulated_query = self._reformulate_query(
                query, prompts["query_reformulation"]
            )
        ...
        if "reranking_criteria" in prompts and prompts["reranking_criteria"].strip():
            retrieved_docs = self._rerank_documents(
                retrieved_docs, query, prompts["reranking_criteria"], config
            )

        # Step 4: Context synthesis
        context = self._synthesize_context(
            retrieved_docs, query, prompts.get("context_synthesis", "")
        )

        # Step 5: Answer generation
        answer = self._generate_answer(
            query, context, prompts.get("answer_generation", "")
        )
        ...
        return {
            ...
            "metadata": {
                ...
                "vector_store_type": self.vector_store.get_collection_info().get(
    ...
    def _reformulate_query(self, query: str, reformulation_prompt: str) -> str:
        ...
        try:
            if callable(self.llm_client):
                ...
                response = (
                    self.llm_client.completion(messages=messages)
                    .choices[0]
                    .message.content
                )
        ...
        if retrieval_strategy == "similarity":
            ...
            if self.vector_store.supports_hybrid_search():
                ...
                return self.vector_store.hybrid_search(
                    query, k=k, alpha=alpha, filters=filters
                )
            else:
    ...
    def _synthesize_context(
        self, documents: list[dict[str, Any]], query: str, synthesis_prompt: str
    ) -> str:
        ...
        doc_context = "\n\n".join(
            f"Document {i + 1}: {text}" for i, text in enumerate(doc_texts)
        )
