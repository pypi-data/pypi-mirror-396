...
class RAGEvaluationMetrics:
    ...
    def evaluate_retrieval(
        self, retrieved_docs: list[dict[str, Any]], relevant_doc_ids: list[str]
    ) -> dict[str, float]:
        """
        ...
        for doc in retrieved_docs:
            doc_id = doc.get("metadata", {}).get("doc_id") or doc.get(
                "metadata", {}
            ).get("id")
            if doc_id:
        ...
        if len(retrieved_set) == 0:
            ...
            precision = len(relevant_set.intersection(retrieved_set)) / len(
                retrieved_set
            )
    ...
    def evaluate_generation(
        self, generated_answer: str, ground_truth: str, context: str
    ) -> dict[str, float]:
        """
        ...
        return {
            ...
            "answer_confidence": (f1_score + relevance_score + faithfulness_score)
            / 3.0,
        }
    ...
    def _simple_bleu(self, prediction: str, ground_truth: str, n: int = 2) -> float:
        ...
        pred_ngrams = {
            tuple(pred_words[i : i + n]) for i in range(len(pred_words) - n + 1)
        }
        truth_ngrams = {
            tuple(truth_words[i : i + n]) for i in range(len(truth_words) - n + 1)
        }
