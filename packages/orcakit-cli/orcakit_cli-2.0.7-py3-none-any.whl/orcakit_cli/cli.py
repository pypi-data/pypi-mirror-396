...
from orcakit_cli.templates import _get_templates_list, create_new
from orcakit_cli.util import _load_env_vars, warn_non_wolfi_distro
...
def _prepare_chat_ui_context(config_json: dict | None = None) -> str | None:
    ...
    if config_json:
        ...
        if configured_path:
            ...
            if path.exists():
                click.secho(
                    f"✅ 使用配置文件中的 UI 路径: {configured_path}", fg="green"
                )
                return str(path)
            else:
                # 用户配置的路径不存在，直接报错停止
    ...
    try:
        ...
        zip_url = (
            "https://github.com/OrcaAgent-AI/agent-chat-ui/archive/refs/heads/main.zip"
        )
...
@click.option("--chat-ui-port", type=int, default=3000, help="agent-chat-ui 服务端口。")
@click.option(
    "--debug-compose",
    is_flag=True,
    default=False,
        ...
        if debug_compose or save_compose:
            ...
            if save_compose:
                ...
                if not debug_compose:
                    click.secho(
                        "✅ 文件已保存，退出（使用 --debug-compose 同时运行服务）",
                        fg="yellow",
                    )
                    return
        ...
        def on_stdout(line: str):
            ...
            # Also check for orcakit-specific startup messages.
                ...
                if enable_chat_ui and chat_ui_context:
                    ...
                    output_lines.append(
                        f"- Chat UI: https://agentchat.vercel.app/?apiUrl=http://localhost:{port}&assistantId={assistant_id}"
                    )
            ...
            with open(str(save_path.parent / ".env"), "w", encoding="utf-8") as f:
                f.writelines(
                    [
                        ...
                        "\n",
                        "# 或如果您拥有LangGraph平台许可证密钥,请取消注释以下行：",
                        "\n",
...
@click.option("--chat-ui-port", type=int, default=3000, help="agent-chat-ui 服务端口。")
@cli.command(help="🐳 为OrcaAgent API服务器生成一个docker-compose.yml文件。")
@OPT_CONFIG
def dockercompose(
    config: pathlib.Path,
    save_path: str,
...
def prepare_args_and_stdin(
    *,
    ...
    print("prepare args and stdin " + assistant_id)
...
def _resolve_graph_entry(config: Config, graph_id: str | None) -> tuple[str, str]:
...
def _collect_env_overrides(
    config: Config, project_root: pathlib.Path
) -> dict[str, str]:
...
def _run_backend(host, port, selected_graph_id, config_json, server_log_level):
    ...
    try:
        ...
        raise RuntimeError("orcakit-runner包未安装，无法启动后端服务") from e
...
def chat(
    ...
    if _wait_for_backend(agent_url, timeout=wait_timeout):
        ...
        click.echo("⚠️ 未能在预期时间内确认后端可用。请手动检查，继续尝试连接 UI。")
