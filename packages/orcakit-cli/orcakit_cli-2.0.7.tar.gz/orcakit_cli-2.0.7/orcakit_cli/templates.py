...
def _get_templates_list():
    ...
    try:
        ...
        if not template_repos:
            click.secho("⚠️ 未找到模板仓库，使用固定模板列表", fg="yellow")
            return [
        ...
        click.secho(
            f"🎉 成功获取 {len(template_repos)} 个模板仓库", fg="green", bold=True
        )
        ...
        if e.response is not None:
            ...
            if status_code == 401:
                click.secho(
                    f"⚠️  GitHub API 鉴权失败 ({status_code})，使用固定模板列表",
                    fg="yellow",
                )
            elif status_code == 403:
                click.secho(
                    "⚠️  GitHub API 限流 (rate limit)，使用固定模板列表", fg="yellow"
                )
                click.secho("💡 提示: 设置 GITHUB_TOKEN 可提高限流额度", fg="blue")
            else:
                click.secho(
                    f"⚠️  GitHub API 错误 ({status_code})，使用固定模板列表", fg="yellow"
                )
            # 回退到固定模板
            fixed_repo_list = [
                {"name": name, "description": detail["description"]}
    ...
    try:
        with request.urlopen(zip_url) as response:
            if response.status == 200:
                ...
                click.secho(f"🎉 成功下载并解压模板 {template} 到 {path}", fg="green")
...
def create_new(path: str | None, template: str | None) -> None:
    ...
    if not path:
        path = click.prompt("📂 请指定应用程序的创建路径。", default=".")
