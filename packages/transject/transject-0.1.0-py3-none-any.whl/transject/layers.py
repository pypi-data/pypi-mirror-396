...
class IsoEncoder(nn.Module):
    def __init__(self, vocab_size, hidden_size, max_seq_length, num_layers, n_experts, use_eigen=False, random_features=True, use_bert=False, use_embedder=True):
        super().__init__()
        ...
        if use_embedder == True:
            self.embed = Embedder(vocab_size, hidden_size//2)
            self.pe = ConcatenatedPositionalEncoder(hidden_size//2, max_seq_length)
        self.layers = nn.ModuleList([IsoEncoderLayer(hidden_size, n_experts, random_features=random_features) for i in range(num_layers)])
        #self.resweight = nn.Parameter(torch.Tensor([0.1]*num_layers), requires_grad=True)
        # initialized as 0 as per ReZero paper
        self.resweight = nn.Parameter(torch.Tensor([0]*num_layers), requires_grad=True)
        self.U = torch.nn.utils.parametrizations.orthogonal(nn.Linear(hidden_size, hidden_size))
    ...
    def forward(self, src, output_expert_weights=False, return_all_hidden_states=False):
        ...
        if self.use_eigen == True:
            ...
            eigs = []
            for bs in range(x.size(0)):
                eig = nn.Parameter(torch.rand(self.hidden_size), requires_grad=True).to(x.device)
        ...
        for i in range(self.num_layers):
            #resweight = F.relu(torch.sigmoid(self.resweight[i]))/self.num_layers #torch.sigmoid(self.resweight[i]/((i+1)**2))
            resweight = torch.tanh(self.resweight[i])/self.num_layers
...
class IsoDecoder(nn.Module):
    def __init__(self, vocab_size, hidden_size, max_seq_length, num_layers, n_experts, use_eigen=False, random_features=True):
        super().__init__()
        self.num_layers = num_layers
        self.hidden_size = hidden_size
        self.use_eigen = use_eigen
        self.embed = Embedder(vocab_size, hidden_size//2)
        self.pe = ConcatenatedPositionalEncoder(hidden_size//2, max_seq_length)
        self.decoder_attn_layers = nn.ModuleList([IsoEncoderLayer(hidden_size, n_experts,random_features=random_features) for i in range(num_layers)])
        self.encoder_decoder_cross_attn_layers = nn.ModuleList([IsoEncoderLayer(hidden_size, n_experts, random_features=random_features) for i in range(num_layers)])
        self.resweight = nn.Parameter(torch.Tensor([0]*num_layers), requires_grad=True)
        self.U1 = torch.nn.utils.parametrizations.orthogonal(nn.Linear(hidden_size, hidden_size))
        self.U2 = torch.nn.utils.parametrizations.orthogonal(nn.Linear(hidden_size, hidden_size))
