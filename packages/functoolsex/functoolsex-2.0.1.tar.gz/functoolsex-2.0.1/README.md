...
F 真正的价值在于长逻辑调度，而非表达式求值计算逻辑，后者使用表达式就好，慢了还可以上 numby numba。
...
# 但如果只有三个函数调用，不如直接写，也是非常清晰。事不过三，三则重构。
return HttpResponse(
    json.dumps(
        conv_a_data_to_dict(data),
        strict_mode=True,
        foo=1,
    ),
    status_code=200,
...
def get_conf_item(filename: str, key: str):
    ...
    def conv_value_to_dict(file_value) -> dict | None:
        # 其实这里也应返回 OP_EMPTY 而不是 None，这里只为演示 op_filter
        return (
            F(lambda x: x.encode())
...
3. 还一种可选方案是先在 CPython 上的 fastapi 用函数式，需要提能时硬切到 PyPy 上，以解决 CPython 函数式风格慢的问题。

如果是普通的小脚本，没那么高的性能要求，建议体验下各种函数式风格再选自己喜欢的，如 fn funcy toolz。

不过，性能问题在未实测之前都不能确定是问题，只要不是那些生产上必须要极限压榨性能的项目，都可以试了再说，最后只改痛点即可。
(就算是 fastapi CPython 某个接口的函数式风格造成了瓶颈，也可个是1000个请求才调1次的业务，优化也对整体帮助不大)
1. 很多项目的接口痛点可能是 uuid4() 这种微秒级的，或是 PIL 这种大动作，而不是 F lambda 这种百纳秒级的。
...
比如 get get_in 这些，很多时候用表达式造个轮子会更快也更明确，而且 它 和 pyrsistent 一起用 get 也有 bug。
...
还有，很多时候，让 AI 生成 utils 的代码要方便很多，所以 toolz 和 funcy 都不是必选。
但强烈推荐 pyrsistent。

request.json() 返回的是字典对象，但可以转成 pmap 后向再后面函数中传递。
view 返回的也可以是 PRecord 这种定以好的响应类，再由中间件打包成 json 返回。
慢吗？也许，但请返回看性能章节。
