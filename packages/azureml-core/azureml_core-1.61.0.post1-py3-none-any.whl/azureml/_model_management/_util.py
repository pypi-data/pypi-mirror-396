...
def upload_dependency(workspace, dependency, create_tar=False, arcname=None, show_output=False):
    ...
    dependency = os.path.normpath(os.path.abspath(dependency))
    # CodeQL [SM01305] untrusted data is validated before being used in the path
    if dependency.startswith(('http', 'wasb', 'aml://', 'azureml://')):
        return dependency, urlparse(dependency).path.split('/')[-1]
    if not os.path.exists(dependency):  # CodeQL [SM01305] untrusted data is validated before being used in the path
        module_logger.error(f"Dependency path does not exist: {dependency}")
        raise WebserviceException(
            f'Error resolving dependency: no such file or directory {dependency}',
