            ...
            try:
                ...
                if status_code in status_codes_to_retry:
                    if cur_attempt <= retries:
                        # Determine appropriate delay. If server specifies "Retry-After" header and
                        # the value (in seconds) is larger than the current back-off delay, honour it.
                        retry_after_header = None
                        try:
                            retry_after_header = float(error.response.headers.get('Retry-After'))
                        except Exception:
                            retry_after_header = None

                        sleep_delay = _delay
                        if retry_after_header is not None and retry_after_header > sleep_delay:
                            sleep_delay = retry_after_header

                        jitter = random.uniform(0, sleep_delay * 0.1)
                        sleep_delay += jitter

                        _get_logger().warning(
                            'Request `{}` failed at attempt {} with status code: {}. Retrying in {} seconds'.format(
                                request_fn.__name__,
                                cur_attempt,
                                status_code,
                                sleep_delay))
                        ...
                        sleep(sleep_delay)
                        # Prepare _delay for the *next* iteration based on exponential back-off baseline
...
def _fetch_rslex_dataflow_yaml(workspace, fetch_id):
    ...
    success, response = _make_request(fetch_dataflow,
                                      # Unless Retry-After is returned we double delay
                                      # on every attempt and first retry is immediate.
                                      # Should come to ~1 minute delay on last retry
                                      # and sum to ~100 seconds for all retries
                                      backoff=0.1,
                                      retries=10)
    if success:
