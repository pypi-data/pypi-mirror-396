...
def get_run_config_dir_name(project_path):
    ...
    if not os.path.exists(run_config_dir_path):     # CodeQL [SM01305] untrusted data is validated before being used in the path
...
def get_config_file_name(project_config_path):
    ...
    """
    # CodeQL [SM01305] untrusted data is validated before being used in the path
    project_config_path = os.path.abspath(os.path.normpath(project_config_path))
    legacy_config_file_path = os.path.join(project_config_path, LEGACY_PROJECT_FILENAME)
    if os.path.exists(legacy_config_file_path): # CodeQL [SM01305] untrusted data is validated before being used in the path
