...
def _get_formatted_index_url():
    index_url = _current_index()
    if index_url:
        splits = index_url.split('/')
        if splits[-1] != 'index.html':
            splits.append('index.html')
        index_url = '/'.join(splits)
    return index_url
...
def _update_requirements_binding(repo_path, config_dir_to_use):
    ...
    default_index = "https://mlsdkv1fdnprivate.azureedge.net/sdk-release/Preview/E7501C02541B433786111FE8E140CAA1"
...
def attach_project(project_id, project_path, scope, compute_target_dict):
    ...
    cwd = os.getcwd()
    project_path = os.path.normpath(os.path.join(cwd, project_path))
    project_path = os.path.abspath(os.path.normpath(project_path))
    if not os.path.exists(project_path):    # CodeQL [SM01305] untrusted data is validated before being used in the path
    ...
    if not is_existing_dir:
        ...
        os.makedirs(os.path.abspath(project_path)) # CodeQL [SM01305] untrusted data is validated before being used in the path
