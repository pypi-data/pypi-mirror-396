...
class Model(object):
    ...
    def _validate_model_path(model_path, child_paths=None):
        ...
        """
        cwd = os.getcwd()
        model_path = os.path.normpath(os.path.join(cwd, model_path))
        # Check if path exists
        if not os.path.exists(model_path):  # CodeQL [SM01305] untrusted data is validated before being used in the path
            #raise FileNotFoundError("Path does not exist")
            raise FileNotFoundError(f"Path does not exist: {model_path}")
        # Check if model_path is within cwd to prevent path traversal
        #if not os.path.abspath(model_path).startswith(os.path.abspath(cwd)):
        if not model_path.startswith(os.path.abspath(cwd) + os.sep):
            #raise PermissionError("Unauthorized access: Path outside current working directory")
            raise PermissionError(f"Unauthorized access: Path outside current working directory: {model_path}")
        # CodeQL [SM01305] untrusted data is validated before being used in the path
        ...
        if child_paths:
            ...
            for child in child_paths:
                #model_path = os.path.normpath(model_path)
                child_full = os.path.normpath(os.path.join(model_path, child))
                # CodeQL [SM01305] untrusted data is validated before being used in the path
                ...
                if not os.path.exists(child_full):
                    ...
                    #                           logger=module_logger)
                    raise WebserviceException(f'Child path not found: "{child}"', logger=module_logger)
    ...
    def _collect_model_artifact_paths(model_path, child_paths):
        ...
        :param model_path: Path to the model root folder or file
        :type model_path: str
        :param child_paths: List of specific files or folders inside model_path
        :type child_paths: list[str]
        :return: (list of full file paths, list of relative artifact names)
        :rtype: tuple[list[str], list[str]]
        """
        # CodeQL [SM01305] untrusted data is validated before being used in the path
        model_path = os.path.abspath(os.path.normpath(model_path))
        # Define a safe base (restrict models under current working dir, or a known root)
        allowed_base = os.getcwd()
        if os.path.commonpath([model_path, allowed_base]) != allowed_base:
            raise UserErrorException(
                f"Invalid model path: {model_path}. Must be under {allowed_base}"
            )
        model_parent_path = os.path.dirname(model_path)
        ...
        if child_paths:
            ...
            for child_path in child_paths:
                ...
                if os.path.isdir(child_full_path):
                    ...
                    for dir_path, _, filenames in os.walk(child_full_path):      # CodeQL [SM01305] untrusted data is validated before being used in the path
                        for file_name in filenames:
                            full_file_path = os.path.abspath(os.path.join(dir_path, file_name))
                            all_files.add(full_file_path)
                else:
                    #file_names.add(child_full_path)
                    all_files.add(child_full_path)
            ...
            artifact_names = [os.path.relpath(f, start=model_parent_path) for f in file_names]
        elif os.path.isdir(model_path):  # CodeQL [SM01305] untrusted data is validated before being used in the path
            file_names = [os.path.abspath(os.path.join(dir_path, file_name))
                          for dir_path, _, filenames in os.walk(model_path)      # CodeQL [SM01305] untrusted data is validated before being used in the path
                          for file_name in filenames]
            #artifact_names = [os.path.relpath(file_name, start=model_parent_path) for file_name in file_names]
            artifact_names = [os.path.relpath(f, start=model_parent_path) for f in file_names]
        else:
