...
class RunConfiguration(_AbstractRunConfigElement):
    ...
    def load(path, name=None):
        ...
        if os.path.isfile(full_runconfig_path):     # CodeQL [SM01305]:full_runconfig_path is built in internal and validated earlier.
            full_runconfig_path = os.path.normpath(full_runconfig_path)
    ...
    def delete(path, name):
        ...
        legacy_full_file_path = os.path.normpath(legacy_full_file_path)
         # CodeQL [SM01305]:legacy_full_file_path is built in internal and validated earlier.
        if os.path.isfile(legacy_full_file_path):  
        # CodeQL [SM01305]: True if the specified path is a project directory. False if the path specified is a file.    
            file_found = True
            legacy_full_file_path = os.path.normpath(legacy_full_file_path)
            os.remove(legacy_full_file_path)   # CodeQL [SM01305]:full_runconfig_path is built in internal and validated earlier.     
            # CodeQL [SM01305]:legacy_full_file_path is built in internal and validated earlier.
            full_file_path = os.path.normpath(full_file_path)  # CodeQL [SM01305]:full_runconfig_path is built in internal and validated earlier.
        if os.path.isfile(full_file_path):  # CodeQL [SM01305]:full_file_path is built in internal and validated earlier.
            file_found = True
    ...
    def _modify_runconfig_using_spark_config(spark_dependencies_file, run_config_object, path,
        ...
        if spark_dependencies_file:
            ...
            # CodeQL [SM01305] untrusted data is validated before being used in the path
            # CodeQL [SM01305] :spark_dependencies_path is validated internally
    ...
    def _save_with_default_comments(self, commented_map_dict, path, name, separate_environment_yaml=False,
        ...
        if separate_environment_yaml:
            ...
            full_runconfig_path = os.path.normpath(full_runconfig_path)
            # CodeQL [SM01305]:full_runconfig_path is built from internal and validated earlier.
            ...
            # CodeQL [SM01305]:full_env_path is constructed internally and its safe.
