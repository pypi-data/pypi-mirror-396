...
def makedirs_for_file_path(file_path):
    ...
    if not os.path.exists(parent_path):
        os.makedirs(parent_path, exist_ok=True) # CodeQL [SM01305] untrusted data is validated before being used in the path 
    return True
...
def download_file(source_uri, path=None, max_retries=5, stream=True, protocol="https", session=None,
    ...
    if is_source_uri_matches_storage_blob(source_uri):
        ...
        def exec_func():
            ...
            with open(path, "wb") as my_blob:   # CodeQL [SM01305] untrusted data is validated before being used in the path
            ...
            file_size = os.stat(path).st_size   # CodeQL [SM01305] untrusted data is validated before being used in the path 
