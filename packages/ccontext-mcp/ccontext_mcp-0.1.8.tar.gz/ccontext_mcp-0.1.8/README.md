...
|----------|------|---------|
| **Context** | `get_context()` | Your project memory - **call this FIRST** (returns `version`, `now`, and `diagnostics`) |
| | `commit_updates()` | Batch multiple updates in one call (presence + task progress + notes/refs) |
| **Presence** | `get_presence()` | See what other agents are doing |
| | `update_my_status()` | Update your status for other agents |
| | `set_idle()` | Clear your status |
...
## Diagnostics (Context Hygiene)

`get_context()` also returns `diagnostics` to keep context clear and actionable:

- `debt_score`: small number = healthy context, larger = needs cleanup
- `issues`: structured issues with severity/info
- `top_issues`: quick summary for the agent to act on

For compatibility, `warnings`/`hints` may also appear (derived from `diagnostics`).

## commit_updates (Batch Updates)

Agents often need to update multiple things at once (status + task step + note). Instead of many tool calls, use `commit_updates()`:

```python
commit_updates(
  ops=[
    {"op": "presence.set", "agent_id": "peer-a", "status": "Implementing parser fix"},
    {"op": "task.step", "task_id": "T001", "step_id": "S2", "step_status": "done"},
    {"op": "note.add", "content": "Found edge case in parser: empty header triggers fallback path", "score": 50},
  ]
)
```
