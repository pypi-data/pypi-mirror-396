...
class TestContextOperations:
    ...
    def test_save_and_load_context(self, storage):
        ...
        context = Context(
            ...
            notes=[
                Note(id="N001", content="Important note", score=50),
            ],
            references=[
                Reference(id="R001", url="src/main.py", note="Main file", score=30),
            ],
        )
...
class TestNoteOperations:
    ...
    def test_generate_note_id(self, storage):
        ...
        context.notes.append(Note(id="N001", content="Test", score=15))
    ...
    def test_get_note_by_id(self, storage):
        ...
        context = Context(
            notes=[
                Note(id="N001", content="First", score=50),
                Note(id="N002", content="Second", score=30),
            ]
        )
...
class TestReferenceOperations:
    ...
    def test_generate_reference_id(self, storage):
        ...
        context.references.append(Reference(id="R001", url="test.py", note="Test", score=15))
    ...
    def test_get_reference_by_id(self, storage):
        ...
        context = Context(
            references=[
                Reference(id="R001", url="a.py", note="File A", score=50),
                Reference(id="R002", url="b.py", note="File B", score=30),
            ]
        )
...
class TestScoreDecay:
    """Tests for score decay and archiving."""

    def test_decay_scores(self, storage):
        """Decay scores for notes and references."""
        context = Context(
            milestones=[],
            notes=[
                Note(id="N001", content="High", score=50),
                Note(id="N002", content="Low", score=0),
                Note(id="N003", content="Archive", score=-4),
            ],
            references=[
                Reference(id="R001", url="a.py", note="High", score=30),
                Reference(id="R002", url="b.py", note="Archive", score=-4),
            ],
        )
        ...
        # Scores decremented
        assert context.notes[0].score == 49
        assert context.notes[1].score == -1

        # Archived items removed from context
        assert len(context.notes) == 2
        assert len(context.references) == 1

        # Archived items returned
        assert len(archived_notes) == 1
        assert archived_notes[0].id == "N003"
        assert len(archived_refs) == 1
        assert archived_refs[0].id == "R002"

    def test_archive_entries(self, storage):
        """Archive notes and references to files."""
        notes = [Note(id="N001", content="Archived note", score=-5)]
        refs = [Reference(id="R001", url="old.py", note="Old file", score=-5)]
