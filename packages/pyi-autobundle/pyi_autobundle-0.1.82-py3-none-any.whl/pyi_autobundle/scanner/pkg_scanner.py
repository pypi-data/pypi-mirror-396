...
DATA_EXT = {".txt", ".json", ".png", ".jpg", ".jpeg", ".html", ".xml", ".csv", ".npy", ".npz", ".css", ".ico", ".svg", ".woff", ".woff2", ".ttf", ".otf", ".scss", ".less"}
BINARY_EXT = {".so", ".pyd", ".dll"}

# Per-package handler registry. A handler may inspect the distribution member and
# return a tuple (kind, dest) where kind is 'data' or 'binary'. Returning None
# falls back to default heuristics.
from typing import Callable, Dict

PackageHandler = Callable[[md.Distribution, Path, Path], Optional[tuple]]
_HANDLERS: Dict[str, PackageHandler] = {}

def register_handler(pkg_name: str):
    def _decor(fn: PackageHandler):
        _HANDLERS[pkg_name] = fn
        return fn
    return _decor
    ...
    for m in sorted(set(modules)):
        ...
        for dist_name in dist_names:
            ...
            for member in dist.files or []:
                ...
                # Allow per-package handlers to decide inclusion and destination
                handler = _HANDLERS.get(m)
                if handler:
                    try:
                        hres = handler(dist, mp, full)
                    except Exception:
                        hres = None
                    if hres:
                        kind, dest = hres
                        if kind == 'data':
                            if not _match_ignore(str(full), ignore_patterns):
                                ...
                                added_for_module = True
                        elif kind == 'binary':
                            if not _match_ignore(str(full), ignore_patterns):
                                binaries.append((str(full), dest))
                                added_for_module = True
                        # handler decides; move to next member
                        continue
        ...
        # If distribution inspection didn't yield results for this module,
        # attempt a filesystem fallback: locate the installed module path
        # and look for common repr/html/css locations (e.g., sklearn/utils/_repr_html)
        if not added_for_module:
            try:
                spec = util.find_spec(m)
                if spec and spec.origin:
                    mod_path = Path(spec.origin)
                    if mod_path.name == "__init__.py":
                        base = mod_path.parent
                    else:
                        base = mod_path.parent
                    candidate = base / "utils" / "_repr_html"
                    if candidate.exists() and candidate.is_dir():
                        for p in candidate.rglob("*.css"):
                            # dest under package prefix
                            dest = f"{m.replace('.', '/')}/utils/_repr_html"
                            if not _match_ignore(str(p), ignore_patterns):
                                datas.append((str(p), dest))
            except Exception:
                pass


# Built-in handler for sklearn to ensure CSS and repr HTML assets are included
@register_handler('sklearn')
def _sklearn_handler(dist: md.Distribution, member: Path, full_path: Path):
    # Include files under sklearn/utils/_repr_html and other UI resources.
    parts = list(member.parts)
    # Normalize candidate path string
    s = str(member).replace('\\', '/')
    if '_repr_html' in s or s.endswith('.css'):
        # map to package-prefixed destination
        pkg = 'sklearn'
        # drop the leading top-level package name if present
        if parts and parts[0] == pkg:
            rel = Path(*parts[1:])
        else:
            rel = Path(*parts)
        dest_parent = str(rel.parent) if str(rel.parent) != '.' else ''
        dest = f"{pkg}/{dest_parent}" if dest_parent else f"{pkg}"
        return ('data', dest)
    return None
