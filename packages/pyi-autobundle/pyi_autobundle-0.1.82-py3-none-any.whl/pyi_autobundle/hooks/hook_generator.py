...
def generate_hooks(out_dir: Path, datas: List[Tuple[str, str]], binaries: List[Tuple[str, str]], modules: List[str], hiddenimports: Optional[List[str]] = None) -> List[Path]:
    ...
    for mod in sorted(set(modules)):
        ...
        mod_datas = _sanitize(mod_datas)
        mod_bins = _sanitize(mod_bins)
    ...
    # sanitize leftovers too
    def _sanitize_all(items):
        out = []
        for s, d in items:
            if not d:
                d = "."
    ...
    remaining_datas = _sanitize_all(remaining_datas)
    remaining_bins = _sanitize_all(remaining_bins)
