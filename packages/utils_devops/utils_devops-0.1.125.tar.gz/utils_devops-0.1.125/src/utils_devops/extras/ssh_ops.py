...
def ssh_test_connection_with_retry(
    ...
    **kwargs
    ...
    """
    auth_method = None
    temp_key_files = []  # Track all temporary files for cleanup
    
    try:
        for attempt in range(1, max_retries + 1):
            try:
                _logger.debug(f"Connection attempt {attempt}/{max_retries} to {username}@{host}:{port}")
                    ...
                    try:
                        with ssh_connect(
                            ...
                            **kwargs
                            ...
                            if transport:
                                # Get the actual auth method used
                                auth_methods = transport.auth_methods(username)
                                if auth_methods:
                                    auth_method = auth_methods[0]
                            
                            auth_method = auth_method or "ssh_key"
                            _logger.info(f"✅ Connected to {username}@{host}:{port} via {auth_method} (attempt {attempt})")
                            return True, auth_method
                    except SSHAuthError:
                        _logger.debug(f"SSH key authentication failed for {host}")
                        # Don't retry with other methods in this attempt
                        # We'll try password or agent in next attempts
                        pass
                    ...
                    try:
                        with ssh_connect(
                            ...
                            **kwargs
                            ...
                            _logger.info(f"✅ Connected to {username}@{host}:{port} via password (attempt {attempt})")
                            return True, auth_method
                    except SSHAuthError:
                        _logger.debug(f"Password authentication failed for {host}")
                ...
                _logger.debug(f"Connection attempt {attempt} failed with error: {e}")
                if attempt < max_retries:
                    time.sleep(retry_delay)
        ...
        for temp_file in temp_key_files:
