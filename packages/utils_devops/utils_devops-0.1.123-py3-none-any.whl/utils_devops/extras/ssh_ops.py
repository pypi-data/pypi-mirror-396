...
# Core imports
from ..core.logs import task, get_logger, debug, info, warning, error , get_library_logger
    ...
    try:
        ...
        if auto_add_host:
            ...
            try:
                ...
                warning("Could not load system host keys")
        ...
        # Add key file if provided and exists
        if key_file:
            key_path = Path(key_file).expanduser()
            if key_path.exists():
                connect_kwargs['key_filename'] = str(key_path)
                debug(f"Using key file: {key_path}")
            else:
                warning(f"Key file not found: {key_file}")
        ...
        info(f"✅ Connected to {username}@{host}:{port} in {connect_time:.2f}s")
        
        yield client
        
    except AuthenticationException as e:
        error(f"Authentication failed for {username}@{host}:{port}")
    ...
    **kwargs: Any,
    ...
    with task(task_name, hide_on_success=hide_on_success):
        try:
            ...
            error(f"Command timeout on {host}: {command_str}")
            raise SSHTimeoutError(f"Command timeout: {e}") from e
        except Exception as e:
            error(f"SSH command failed on {host}: {e}")
            if check:
                raise
    ...
    **kwargs
    ...
    try:
        ...
        if clean and remote_path:
            info(f"🧹 Cleaning remote directory: {remote_path}")
            execute_on_machine(
                machine=machine,
        ...
        info(f"✅ Deployed to {machine.name}")
        return True
        
    except Exception as e:
        error(f"❌ Deployment failed to {machine.name}: {e}")
        return False
    ...
    try:
        ...
        for machine_data in config_data['machines']:
            try:
                ...
                error(f"Failed to parse machine config: {e}")
                continue
        
        info(f"Loaded {len(machines)} machines from {config_file}")
        return machines
...
def save_machines_config(
    ...
    try:
        ...
        info(f"Saved {len(machines)} machines to {config_file}")
    ...
    try:
        ...
        if machine.name in existing_names:
            if overwrite:
                ...
                warning(f"Machine '{machine.name}' already exists. Use overwrite=True to replace.")
                return False
        ...
        info(f"Added machine '{machine.name}' to {config_file}")
        return True
        
    except Exception as e:
        error(f"Failed to add machine to config: {e}")
        return False
...
class SSHConnectionPool:
        ...
        with self._lock:
            ...
            if pool_key in self._pool:
                ...
                if (client.get_transport() and 
                    ...
                    debug(f"Reusing connection: {pool_key}")
                    return client
                else:
            ...
            debug(f"Creating new connection: {pool_key}")
            try:
                client = paramiko.SSHClient()
                client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ...
                info(f"Created new connection to {username}@{host}")
                return client
    ...
    def _clean_idle_connections(self):
        ...
        for key in to_remove:
            if key in self._pool:
                ...
                debug(f"Removed idle connection: {key}")
    ...
    def _remove_lru_connection(self):
        ...
        if lru_key in self._pool:
            ...
            debug(f"Removed LRU connection: {lru_key}")
...
def _create_machines_config(
    ...
    files.write_file(config_file, yaml_content)
    info(f"Created new config file: {config_file}")
...
def remove_machine_from_config(
    ...
    try:
        ...
        if machine_name not in existing_names:
            warning(f"Machine '{machine_name}' not found in config")
            return False
        ...
        info(f"Removed machine '{machine_name}' from {config_file}")
        return True
        
    except Exception as e:
        error(f"Failed to remove machine from config: {e}")
        return False
...
def update_machine_in_config(
    ...
    try:
        ...
        if machine.name not in existing_names:
            warning(f"Machine '{machine.name}' not found in config")
        ...
        info(f"Updated machine '{machine.name}' in {config_file}")
        return True
        
    except Exception as e:
        error(f"Failed to update machine in config: {e}")
        return False
...
def test_machine_connection(
    ...
    try:
        ...
        error(f"Machine connection test failed for {machine.name}: {e}")
...
def gather_machine_info(machine: MachineConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        error(f"Failed to gather info for {machine.name}: {e}")
...
def ssh_register_key_with_verification(
    ...
    **kwargs
    ...
    try:
        ...
        if test_after:
            info(f"Verifying key registration for {username}@{host}")
            ...
            if success and auth_method and 'key' in auth_method.lower():
                info(f"✅ Key registration verified for {username}@{host} via {auth_method}")
                return True
            else:
                warning(f"⚠️  Key registration may not have worked for {username}@{host}")
                return False
        
        return True
        
    except Exception as e:
        error(f"SSH key registration with verification failed: {e}")
        return False
...
def ssh_generate_key(
    ...
    **kwargs
    ...
    try:
        ...
        info(f"✅ Generated {key_type.upper()} key: {private_key_path}")
        info(f"   Fingerprint: {fingerprint_formatted}")
        info(f"   Public key: {public_key_path}")
        
        return key_info
        
    except Exception as e:
        error(f"Failed to generate SSH key: {e}")
...
def ssh_test_connection_with_retry(
    ...
    **kwargs
    ...
    for attempt in range(1, max_retries + 1):
        try:
            debug(f"Connection attempt {attempt}/{max_retries} to {username}@{host}:{port}")
                    ...
                    try:
                        with ssh_connect(
                            ...
                            **kwargs
                            ...
                            auth_method = "ssh_key"
                            info(f"✅ Connected to {username}@{host}:{port} via SSH key (attempt {attempt})")
                            return True, auth_method
                    except SSHAuthError:
                        debug(f"SSH key authentication failed for {host}")
                ...
                try:
                    with ssh_connect(
                        ...
                        **kwargs
                        ...
                        info(f"✅ Connected to {username}@{host}:{port} via password (attempt {attempt})")
                        return True, auth_method
                except SSHAuthError:
                    debug(f"Password authentication failed for {host}")
            
            # Try without password/key (might use SSH agent or other methods)
            try:
                with ssh_connect(
                    host=host,
                    username=username,
                    ...
                    **kwargs
                    ...
                    # Determine auth method from transport
                    transport = client.get_transport()
                    if transport:
                        auth_methods = transport.auth_methods(username)
                        if auth_methods:
                            auth_method = auth_methods[0]
                    
                    auth_method = auth_method or "agent_or_none"
                    info(f"✅ Connected to {username}@{host}:{port} via {auth_method} (attempt {attempt})")
                    return True, auth_method
            except Exception as e:
                debug(f"Connection attempt {attempt} failed: {e}")
            ...
            debug(f"Connection attempt {attempt} failed with error: {e}")
            if attempt < max_retries:
                time.sleep(retry_delay)
    
    error(f"Failed to connect to {username}@{host}:{port} after {max_retries} attempts")
    return False, auth_method
...
def get_ssh_agent_keys() -> List[SSHKeyInfo]:
    ...
    try:
        ...
        debug(f"Found {len(key_infos)} keys in SSH agent")
        return key_infos
        
    except Exception as e:
        debug(f"SSH agent not available or error: {e}")
        return []
