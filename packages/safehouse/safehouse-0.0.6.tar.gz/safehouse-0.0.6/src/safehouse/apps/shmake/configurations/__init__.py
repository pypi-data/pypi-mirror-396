...
class Configuration:
    ...
    def build_directory(self) -> str:
        '''
        Returns the root directory for the source code being built.
        '''
        if self.compiler.aggregates_targets:
            return os.path.join(
                self.trunk_directory,
    ...
    def save(self):
        ...
        try:
            with open(config.CONFIGURATION_FILEPATH, 'w') as f:
    ...
    def source_directory(self) -> str:
        return os.path.join(self.trunk_directory, self.project.relative_source_path)
...
def from_file() -> Configuration:
    filepath = config.CONFIGURATION_FILEPATH
    try:
        ...
        return configuration_for(compiler_name=compiler, platform_name=platform, target_name=target)
