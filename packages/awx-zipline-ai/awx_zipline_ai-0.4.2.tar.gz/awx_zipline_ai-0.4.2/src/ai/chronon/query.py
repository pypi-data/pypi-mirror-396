...
def Query(
    ...
    :param clustering_column:
        Column to use for clustering-based pruning on non-partitioned tables. Mutually exclusive with partition_column.
    :type clustering_column: str, optional
    :param clustering_format:
        Date/time format string (Java DateTimeFormatter) if clustering_column is stored as string; omit for native timestamps.
    :type clustering_format: str, optional
    ...
    return api.Query(
        ...
        clusteringColumn=clustering_column,
        clusteringFormat=clustering_format,
    )
