...
class A8:
    ...
    def de_en_data(self, s: int, is_en: bool) -> int:
        # 数据加值
        if is_en:
            return s + 3652
        return s - 3652

    def unpad_pro(self, s: str) -> int:
        # 处理数据
        ss = int(s[:-ord(s[-1])])
        return self.de_en_data(ss, False)

    def pad_pro(self, s: int) -> str:
        # 填充数据
        ss = str(self.de_en_data(s, True))
        ss_i = 16 - len(ss) % 16
        return ss + (ss_i * chr(ss_i))

    def yi_jie_data(self, raw: str) -> int:
        _id_new = self.data_de_pro(raw)
        val = self.unpad_pro(_id_new)
        return val

    def yi_jia_data(self, val: int) -> str:
        raw = self.pad_pro(int(val))
        _id_new = self.data_en_pro(raw)
        return _id_new
