...
class ConnectionConfig:
    ...
    @staticmethod
    def _http_ssl():
        return os.getenv("E2B_HTTP_SSL", "false").lower() == "true"
    @staticmethod
    def _sandbox_domain():
        return os.getenv("E2B_SANDBOX_DOMAIN") 
    @staticmethod
    def _dns_domain_style():
        return os.getenv("E2B_DNS_DOMAIN_STYLE", "true").lower() == "true"
    @staticmethod
    def _dns_path_style():
        return os.getenv("E2B_DNS_PATH_STYLE", "false").lower() == "true"
    ...
    def __init__(
        ...
        self.http_ssl = ConnectionConfig._http_ssl()
        self.sandbox_domain = ConnectionConfig._sandbox_domain()
        self.dns_domain_style = ConnectionConfig._dns_domain_style()
        self.dns_path_style = ConnectionConfig._dns_path_style()
    ...
    def get_sandbox_url(self, sandbox_id: str, sandbox_domain: str) -> str:
        ...
        return f"{'http' if self.debug else 'https' if ConnectionConfig._http_ssl() else 'http'}://{self.get_host(sandbox_id, sandbox_domain, self.envd_port)}"
    ...
    def get_host(self, sandbox_id: str, sandbox_domain: str, port: int) -> str:
        ...
        if self.dns_path_style:
            return f"{sandbox_domain}/sandbox-{sandbox_id}-{port}"
        if self.dns_domain_style:
            return f"{port}-{sandbox_id}.{sandbox_domain}"
        return f"{sandbox_domain}"
