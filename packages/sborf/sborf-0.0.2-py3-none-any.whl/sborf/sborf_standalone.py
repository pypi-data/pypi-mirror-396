...
def run_optimization(sequence, organism, outfile,verbose=True,iterations=100,mating_parents=200,pop_size=1500,target_sol=1.0,num_optimized_seqs=1):
    ...
    for sname in sequences.keys():
        ...
        s = optimize(target_seq = sequences[sname], organism=organism, verbose=verbose,iterations=iterations,num_parents_mating=mating_parents,pop_size=pop_size,TARGET_SOL=target_sol,num_optimized_seqs=num_optimized_seqs)
...
def prediction(sequence,organism,outfile=None):
    ...
    pred = run_prediction(sequences,organism,printPreds=False)
    if outfile is None:
        print("###########")
        print("# CHAPLIN RESULTS #")
        for k in pred.keys():
...
def explaination(sequence,outfolder=None):
    ...
    pred = run_explaination(sequences)
...
def main():
    ...
    if args.command == "predict":
        prediction(args.sequence,outfile=args.outfile,organism=args.organism)
    elif args.command == "optimize":
        run_optimization(args.sequence,outfile=args.outfile,verbose = not args.silent,iterations=args.iterations,mating_parents=args.mating_parents,pop_size=args.pop_size,target_sol=args.target_optimization,num_optimized_seqs=args.num_optimized_seqs,organism=args.organism)
    elif args.command == "explain":
        explaination(args.sequence,outfolder=args.outfile,organism=args.organism)
