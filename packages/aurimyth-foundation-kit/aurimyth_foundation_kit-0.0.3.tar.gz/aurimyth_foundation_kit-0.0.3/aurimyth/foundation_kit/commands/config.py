...
读取和写入 pyproject.toml 中的 [tool.aurimyth] 配置。
"""
...
def get_project_config(base_path: Path | None = None) -> ProjectConfig:
    ...
    从 pyproject.toml 的 [tool.aurimyth] 读取配置。
    ...
    # 简单解析 [tool.aurimyth] 部分
    package = _parse_toml_value(content, "tool.aurimyth", "package")
    app = _parse_toml_value(content, "tool.aurimyth", "app")
...
def save_project_config(config: ProjectConfig, base_path: Path | None = None) -> bool:
    ...
    # 检查是否已有 [tool.aurimyth] 部分
    if "[tool.aurimyth]" in content:
        # 更新现有配置
        lines = content.split("\n")
        new_lines = []
        in_aurimyth_section = False

        for line in lines:
            if line.strip() == "[tool.aurimyth]":
                in_aurimyth_section = True
        ...
        # 添加新的 [tool.aurimyth] 部分
        aurimyth_section = "\n[tool.aurimyth]\n"
        if config.has_package:
            aurimyth_section += f'package = "{config.package}"\n'
        if config.app:
            aurimyth_section += f'app = "{config.app}"\n'

        content += aurimyth_section
