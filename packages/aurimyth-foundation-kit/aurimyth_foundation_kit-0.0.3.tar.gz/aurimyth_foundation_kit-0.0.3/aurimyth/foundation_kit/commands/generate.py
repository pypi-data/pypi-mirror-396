...
2. 交互式（人类友好）：
   aurimyth generate model user -i
"""
...
def generate_model(
    ...
    示例：
        ...
        aurimyth generate model user -b VersionedUUIDModel
        aurimyth generate model user email:str:unique age:int?
        aurimyth generate model article title:str(200) content:text status:str=draft
    """
    base_path = Path.cwd()
...
def generate_repo(
    ...
    示例：
        ...
        aurimyth generate repo user email:str:unique  # 生成 get_by_email 方法
    """
    base_path = Path.cwd()
...
def generate_service(
    ...
    示例：
        ...
        aurimyth generate service user email:str:unique  # 创建时检查 email 重复
    """
    base_path = Path.cwd()
...
def generate_api(
    ...
    示例：
        ...
        aurimyth generate api user --no-register  # 不自动注册到 api/__init__.py
    """
    base_path = Path.cwd()
...
def generate_schema(
    ...
    示例：
        ...
        aurimyth generate schema user email:str:unique age:int?
    """
    base_path = Path.cwd()
...
def generate_crud(
    ...
    示例：
        ...
        aurimyth generate crud user --base Model  # 使用 int 主键
        aurimyth generate crud user --base UUIDAuditableStateModel  # 使用 UUID 主键（推荐）
        aurimyth generate crud user email:str:unique age:int? --force
    ...
    console.print(f'     [cyan]aum migrate make -m "add {model.file_name} table"[/cyan]')
    console.print("  2. 执行迁移：")
    console.print("     [cyan]aum migrate up[/cyan]")
