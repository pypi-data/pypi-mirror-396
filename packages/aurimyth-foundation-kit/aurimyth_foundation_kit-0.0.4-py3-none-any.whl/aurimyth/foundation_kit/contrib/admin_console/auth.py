...
def _require_sqladmin():
    try:
        ...
        raise ImportError(
            "未安装 sqladmin。请先安装: uv add \"aurimyth-foundation-kit[admin]\" 或 uv add sqladmin"
        ) from exc
...
class BasicAdminAuthBackend(_SQLAdminAuthenticationBackend or object):
    ...
    def __init__(self, *, username: str, password: str, secret_key: str, session_key: str = "aurimyth_admin") -> None:
        _require_sqladmin()
...
class BearerWhitelistAdminAuthBackend(_SQLAdminAuthenticationBackend or object):
    ...
    def __init__(
        ...
        *,
        tokens: list[str],
        secret_key: str,
        session_key: str = "aurimyth_admin_token",
    ) -> None:
        _require_sqladmin()
        super().__init__(secret_key=secret_key)  # type: ignore[misc]
...
def wrap_authenticate(
    *,
    ...
    session_key: str = "aurimyth_admin",
):
