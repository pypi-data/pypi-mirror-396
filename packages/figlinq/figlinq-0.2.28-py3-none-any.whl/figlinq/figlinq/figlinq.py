...
def _format_analysis_result(result):
    ...
    # If there's a 'results' key, focus on that
    data = result.get("results", result)

    # If data is still not a dict, just return string representation
    if not isinstance(data, dict):
        return str(data)

    lines = []
    # Prioritize certain keys
    priority_keys = ["test", "statistic", "p_value", "p_adjust", "effect_size", "dof"]
    ...
    for key, val in data.items():
        if key in priority_keys or key in ignored_keys:
            continue
        if isinstance(val, (dict, list)):
            # Skip complex nested structures for summary
            continue
        if isinstance(val, float):
            lines.append(f"{key}: {val:.4g}")
        else:
