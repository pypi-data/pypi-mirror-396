...
class LightLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model = LightGBM.LGBMRegressor(random_state=42, n_estimators=100, max_depth=10, learning_rate=0.1, subsample=0.8, colsample_bytree=0.8, reg_alpha=0, reg_lambda=1)
        else:
...
class LightDecisionRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Initialize base model and meta model if not provided
        if self.base_model is None:
            self.base_model = CatBoost.CatBoostRegressor(random_state=42, iterations=100, depth=10, learning_rate=0.1, subsample=0.8, colsample_bylevel=0.8, reg_alpha=0, reg_lambda=1)
        else:
...
class LightSupportClassifier(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
...
class LightXGBRRegressor(BaseEstimator, RegressorMixin): 
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            ...
            self.base_model_ = clone(self.base_model)
        if self.meta_model is None:
            self.meta_model_ = XGBoost.XGBRegressor(objective='regression', random_state=42, n_estimators=100, max_depth=10, learning_rate=0.1, subsample=0.8, colsample_bytree=0.8, reg_alpha=0, reg_lambda=1)
        else:
            self.meta_model_ = clone(self.meta_model)
        # 2. Generate "out-of-sample" predictions for the meta model
        # We use cross_val_predict ensures that for every point in X,
