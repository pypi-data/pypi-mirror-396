...
class LinearForestRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.base_model is None:
            self.base_model_ = LinearRegression(n_jobs=-1, random_state=42)
        else:
            self.base_model_ = clone(self.base_model)
            
        if self.meta_model is None:
            self.meta_model_ = RandomForestRegressor(random_state=42, n_jobs=-1, n_estimators=100, max_depth=10, min_samples_split=5)
        else:
...
class LinearSupportRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = SVR(shrinking=True, kernel='rbf', gamma='scale', C=1.0, epsilon=0.1, max_iter=-1, tol=1e-3, cache_size=200, verbose=False, random_state=42)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class LinearDecisionRegressor(BaseEstimator, RegressorMixin):
...
class LinearKNNRegressor(BaseEstimator, RegressorMixin):
...
class LinearXGBRRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = XGBoost.XGBRegressor(objective ='reg:squarederror', random_state=42, n_jobs=-1, n_estimators=100, max_depth=10, learning_rate=0.1, subsample=0.8, colsample_bytree=0.8, gamma=0, reg_alpha=0, reg_lambda=1)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class LinearCatRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = CatBoost.CatBoostRegressor(verbose=0, random_state=42, iterations=100, depth=10, learning_rate=0.1, loss_function='RMSE', thread_count=-1, l2_leaf_reg=3, border_count=32, bagging_temperature=1.0, random_strength=1)
        else:
            self.meta_model_ = clone(self.meta_model)
...
class LinearGradientRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model_ = GradientBoostingRegressor(n_estimators=100, random_state=42, max_depth=10, learning_rate=0.1, min_samples_split=5, min_samples_leaf=2, subsample=0.8, max_features='sqrt')
        else:
            self.meta_model_ = clone(self.meta_model)
...
class LinearAdaRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
...
class LinearLightRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_model=None, meta_model=None, cv=5):
        self.base_model = base_model
        self.meta_model = meta_model
        self.cv = cv # Number of cross-validation folds
