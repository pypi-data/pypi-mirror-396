...
class LightLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model = LinearRegression( n_jobs=-1, positive=True, fit_intercept=True )
        else:
...
class LightSupportRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.meta_model is None:
            self.meta_model = SVR(kernel='rbf', C=1.0, epsilon=0.1, max_iter=1000, tol=1e-3, cache_size=200)
        else:
            self.meta_model = clone(self.meta_model)
