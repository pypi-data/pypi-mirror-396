...
def generate_html_report_with_screenshots(probe_results: List, api_keys_dict: dict, output_dir: Path) -> str:
    ...
    if api_keys_dict:
        ...
        for host, keys in api_keys_dict.items():
            for key_type, key_value, source in sorted(list(set(keys))):
                # Mask the key value by default (show first 8 and last 4 chars)
                if len(key_value) > 16:
                    masked_value = key_value[:8] + "•" * (len(key_value) - 12) + key_value[-4:]
                else:
                    masked_value = "•" * len(key_value)
                
                import html
                escaped_value = html.escape(key_value)
                escaped_masked = html.escape(masked_value)
                
                card = f"""
                <div class="api-key-card">
                    <div class="api-key-type">🔥 {key_type}</div>
                    <div class="api-key-value-container">
                        <div class="api-key-value masked" data-full="{escaped_value}" data-masked="{escaped_masked}">{escaped_masked}</div>
                        <button class="api-key-toggle" onclick="toggleApiKey(this)">👁️ Show</button>
                        <button class="api-key-copy" onclick="copyApiKey(this, '{escaped_value}')">📋 Copy</button>
                    </div>
        ...
        <div class="api-keys-section">
            <div class="api-keys-header" onclick="toggleApiKeysSection(this)">
                <span class="api-keys-arrow">▶</span>
                <h2>🔥 Found {total_keys} Potential API Keys</h2>
                <span class="api-keys-hint">(click to expand)</span>
            </div>
    ...
    def generate_card_with_duplicates(group: Dict) -> str:
        ...
        try:
            parsed_url = urlparse(primary.url)
        ...
        # Generate body preview HTML for small responses
        body_preview_html = ""
        response_body = getattr(primary, 'response_body', '')
        if response_body and len(response_body.strip()) > 0:
            # Escape HTML entities for safe display
            import html
            escaped_body = html.escape(response_body)
            
            # Check if it looks like JSON and try to format it
            body_display = escaped_body
            if response_body.strip().startswith('{') or response_body.strip().startswith('['):
                try:
                    import json
                    parsed = json.loads(response_body)
                    body_display = html.escape(json.dumps(parsed, indent=2, ensure_ascii=False))
                except:
                    pass
            
            needs_toggle = len(response_body) > 100
            body_preview_html = f'''
            <div class="body-preview{'' if needs_toggle else ' expanded'}">
                <code>{body_display}</code>
                {'<div class="body-preview-fade"></div>' if needs_toggle else ''}
            </div>
            {'<button class="body-toggle" onclick="toggleBody(this)">▼ Show more</button>' if needs_toggle else ''}
            '''
