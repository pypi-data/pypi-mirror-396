...
async def analyzer_worker_streaming(
    ...
    while True:
        ...
        try:
            ...
            # First scan (old_hash is None) - analyze endpoints but skip AI diff
            is_first_scan = old_hash is None
            
            if not has_changes and not is_first_scan:
                if args.debug: print(f"[DEBUG] Content unchanged for {js_url}, skipping all analysis.")
                continue
            
            if is_first_scan:
                if args.debug: print(f"[NEW] First scan for {js_url}, analyzing endpoints...")
                # Analyze endpoints for new URLs
                ...
                # Remember fingerprint for new content
                await r.set(fingerprint_key, "1", ex=86400 * 30)
                continue
            
            print(f"\n[+] CHANGE DETECTED for: {js_url}")
            
            # AI diff analysis - only if AI provider is configured
            ai_provider = getattr(args, 'ai_provider', None)
            
            if ai_provider and diff.strip() and old_hash:
                # === TRIVIA FILTER: Check if change is significant (only when AI is enabled) ===
                from jsmon.analysis.trivia_filter import should_analyze_with_ai
                
                # Pass content for content-based filtering (check if file has interesting patterns)
                should_analyze, skip_reason = await should_analyze_with_ai(diff, js_url, args.debug, content=content)
                
                if not should_analyze:
                    print(f"[TRIVIA SKIP] {js_url}: {skip_reason}")
                    # Log trivia skip to diff log file
                    if args.log_diffs:
                        try:
                            with open(args.log_diffs, "a", encoding="utf-8") as f:
                                f.write(f"\n{'='*60}\n")
                                f.write(f"[TRIVIA SKIP] {js_url}\n")
                                f.write(f"Reason: {skip_reason}\n")
                                f.write(f"{'='*60}\n")
                                f.write(f"--- DIFF (skipped by trivia filter) ---\n{diff}\n--- END DIFF ---\n\n")
                        except Exception as log_e:
                            print(f"\n[!] Failed to write trivia skip to log: {log_e}", file=sys.stderr)
...
async def analyze_orchestrate(args):
    ...
    while True:
        ...
        try:
            from jsmon.analysis.ai import get_cache_stats
            cache_stats = get_cache_stats()
            if cache_stats["hits"] > 0 or cache_stats["misses"] > 0:
                print(f"\n[AI CACHE] Hits: {cache_stats['hits']}, Misses: {cache_stats['misses']}, Size: {cache_stats['size']}")
        except ImportError:
            pass
        ...
        print(f"\n[zzz] Sleeping for {args.delay} seconds...")
