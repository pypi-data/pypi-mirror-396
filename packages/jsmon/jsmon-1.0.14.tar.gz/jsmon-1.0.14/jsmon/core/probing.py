...
class ProbeResultWithScreenshot:
    ...
    response_body: str = ""  # Body preview for small responses
...
async def _probe_with_aiohttp(session, full_url, display_name, header_manager, bearer_token=None):
    ...
    try:
        ...
        async with session.get(full_url, headers=headers, timeout=15, ssl=False, allow_redirects=True) as resp:
            ...
            # Save body preview for small responses (< 2KB)
            if length < 2048 and content_str:
                response_body = content_str[:1500]  # Limit to 1.5KB
            
            return ProbeResultWithScreenshot(
                url=full_url, display_name=display_name, status=status, title=title,
                content_length=length, response_time=response_time, screenshot_path="", 
                fetch_method="aiohttp", response_body=response_body
            )
    except Exception as e:
...
async def _probe_with_playwright(full_url, display_name, playwright_context, domain_session, screenshots_dir, session=None, header_manager=None):
    ...
    try:
        ...
        # Save body preview for small responses (< 2KB)
        if length < 2048 and body_bytes:
            try:
                response_body = body_bytes.decode('utf-8', errors='ignore')[:1500]
            except:
                pass
        ...
        return ProbeResultWithScreenshot(
            ...
            content_length=length, response_time=response_time, screenshot_path=screenshot_path, 
            fetch_method="browser", response_body=response_body
        )
