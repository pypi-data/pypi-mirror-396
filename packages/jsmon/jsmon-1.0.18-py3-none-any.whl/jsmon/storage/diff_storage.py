...
from jsmon.utils.url import get_canonical_url
...
class HybridDiffStorage:
    ...
    async def get_and_compare(self, url: str, new_content: str) -> tuple:
        ...
        Uses canonical URL for comparison, so files like:
        - main-app-abc123.js and main-app-def456.js are compared as main-app.js
        - Hash-only files (2209b2ca-3e859df398a3c95b.js) keep their full URL
        """
        ...
        # Use canonical URL so main-app-abc123.js compares with main-app-def456.js
        canonical_url = get_canonical_url(url)
        url_key = f"url_hash:{canonical_url}"
