...
def build_parser():
    ...
    p.add_argument("--gofile-api-key", help="Gofile.io API key for automatic report upload.")
    
    return p

async def _run_with_watchdog(args):
    """Run analyze_orchestrate with a watchdog that dumps stuck tasks"""
    import time
    ...
    async def watchdog():
        ...
        while True:
            ...
            if args.debug:
                print(f"\n[WATCHDOG] {len(tasks)} active tasks at {time.strftime('%H:%M:%S')}")
    ...
    try:
        await analyze_orchestrate(args)
    finally:
...
def main():
    ...
    # Enable asyncio debug mode if --debug
    if args.debug:
        import logging
        logging.basicConfig(level=logging.WARNING)
        # Warn about slow callbacks (>0.5s)
        # asyncio.get_event_loop().slow_callback_duration = 0.5
    
    try: 
        asyncio.run(_run_with_watchdog(args))
