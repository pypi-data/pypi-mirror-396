...
def get_canonical_url(url: str) -> str:
    ...
    - Does NOT canonicalize files where name is only a hash (e.g., 2209b2ca-3e859df398a3c95b.js)
    """
    try:
        ...
        # Check if filename is ONLY a hash (no base name)
        # Pattern: /path/to/abc123-def456.js or /path/to/abc123def456.js
        filename = clean_path.split('/')[-1] if '/' in clean_path else clean_path
        hash_only_pattern = re.compile(r'^[a-f0-9]{6,64}(-[a-f0-9]{6,64})?\.js$', re.IGNORECASE)
        if hash_only_pattern.match(filename):
            # Don't canonicalize - return original URL without query/fragment
