...
async def generate_report_and_notify(findings_dict, api_keys_dict, args, session, timing, header_manager, session_manager=None):
    ...
    gofile_api_key = getattr(args, 'gofile_api_key', None)
    download_link = await create_and_upload_archive(report_dir, timestamp, gofile_api_key)
    ...
    if download_link:
        ...
        message = (
            ...
            f"   curl -X POST 'https://upload.gofile.io/uploadfile' -H 'Authorization: Bearer YOUR_KEY' -F 'file=@report_{timestamp}.rar'"
        )
