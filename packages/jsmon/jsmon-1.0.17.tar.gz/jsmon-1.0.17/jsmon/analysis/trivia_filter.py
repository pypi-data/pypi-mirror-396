...
- Asset URL changes
- Anti-bot/fingerprinting script token rotations
- CSP nonce changes
- Request/Session ID rotations
"""
...
# Anti-bot/fingerprinting script patterns (Akamai, PerimeterX, Incapsula)
ANTIBOT_PATTERNS = [
    # e.detail.init() with base64 arguments (Capital One pattern)
    re.compile(r'\.detail\.init\s*\(\s*["\'][A-Za-z0-9+/=]{20,}["\']'),
    # seed= parameter with base64-like value
    re.compile(r'seed=[A-Za-z0-9+/=_-]{20,}'),
    # Arrays of large numbers (fingerprint data)
    re.compile(r'\[\s*\d{8,}\s*,\s*\d{8,}'),
    # Short random event names like v = "IhBSZZXCC"
    re.compile(r'\bv\s*=\s*["\'][A-Za-z]{8,12}["\']'),
    # _Incapsula_Resource URL pattern
    re.compile(r'_Incapsula_Resource'),
    # Incapsula cb= cache buster parameter
    re.compile(r'[&?]cb=\d{6,}'),
    # Cloudflare challenge params
    re.compile(r'__CF\$cv\$params'),
]

# CSP Nonce patterns
NONCE_PATTERNS = [
    # HTML nonce attribute
    re.compile(r'nonce\s*=\s*["\'][A-Za-z0-9+/=]{16,}["\']'),
    # window.cspNonce assignment
    re.compile(r'cspNonce\s*=\s*["\'][A-Za-z0-9+/=]{16,}["\']'),
]

# Session/Request ID patterns (Amazon, Dropbox, etc.)
SESSION_REQUEST_PATTERNS = [
    # Generic request/session IDs
    re.compile(r'(requestId|hostPageRID|rid|containerRequestId|clientRequestId)\s*[:=]\s*["\'][^"\']+["\']', re.I),
    # Amazon-specific tokens (HTML meta tags and JSON)
    re.compile(r'(encrypted-slate-token|flow-closure-id|offerToken)', re.I),
    # Amazon CSA widget IDs
    re.compile(r'(cel_widget_id|data-csa-c-)[a-z-]+\s*=', re.I),
    # Dropbox base64InitData
    re.compile(r'base64InitData\s*[:=]\s*["\'][A-Za-z0-9+/=]{50,}["\']'),
    # Dynamic day/date values (JSON format with quotes)
    re.compile(r'["\']dayOfTheWeek["\']\s*:\s*\d'),
    re.compile(r'["\']userDateTime["\']\s*:\s*["\'][\d-]+["\']'),
    # tmxId (ThreatMetrix) - JSON format
    re.compile(r'["\']tmxId["\']\s*:\s*["\'][a-f0-9-]{20,}["\']', re.I),
    # Instance IDs (numeric) - JSON format
    re.compile(r'["\']instanceId["\']\s*:\s*["\']?\d{5,}["\']?'),
    # Instance IDs (numeric)
    re.compile(r'instanceId\s*[:=]\s*["\']?\d{5,}["\']?'),
]

# CSRF token patterns
CSRF_PATTERNS = [
    re.compile(r'anti-csrftoken-[a-z0-9]+', re.I),
    re.compile(r'glow-validation-token', re.I),
]

# Known noise URL patterns (for pre-filtering)
NOISE_URL_PATTERNS = [
    re.compile(r'_Incapsula_Resource'),
    re.compile(r'cof_common\.js'),  # Capital One anti-bot
    re.compile(r'/fp/tags\.js'),  # Fingerprint tags
    # Next.js / Webpack runtime files (always noise for AI analysis)
    re.compile(r'/webpack-[a-f0-9]+\.js'),  # Webpack runtime
    re.compile(r'/framework-[a-f0-9]+\.js'),  # React/Next.js framework
    re.compile(r'/polyfills-[a-f0-9]+\.js'),  # Polyfills
    re.compile(r'/main-[a-f0-9]+\.js'),  # Next.js main runtime
]

# Maximum diff size to send to AI (200KB)
...
def is_known_noise_url(url: str) -> bool:
    """Pre-filter known noise URLs before diff analysis."""
    return any(p.search(url) for p in NOISE_URL_PATTERNS)


def is_antibot_script_change(diff: str) -> bool:
    """Detects anti-bot/fingerprinting script token rotations (Akamai, PerimeterX, Incapsula)."""
    lines = diff.split('\n')
    antibot_lines = 0
    total_changes = 0
    ...
    for line in lines:
        if line.startswith('+') or line.startswith('-'):
            ...
            if any(p.search(line) for p in ANTIBOT_PATTERNS):
                antibot_lines += 1
    
    if total_changes == 0:
        return False
    
    # If >50% of changes are anti-bot related, it's noise
    return (antibot_lines / total_changes) > 0.5


def is_nonce_only_change(diff: str) -> bool:
    """Detects CSP nonce-only changes."""
    lines = diff.split('\n')
    nonce_lines = 0
    total_changes = 0
    ...
    for line in lines:
        if line.startswith('+') or line.startswith('-'):
            ...
            if any(p.search(line) for p in NONCE_PATTERNS):
                nonce_lines += 1
    
    if total_changes == 0:
        return False
    
    # If >70% of changes are nonce-related, it's noise
    return (nonce_lines / total_changes) > 0.7


def is_session_request_id_change(diff: str) -> bool:
    """Detects session/request ID rotations (Amazon, Dropbox, etc.)."""
    lines = diff.split('\n')
    session_lines = 0
    total_changes = 0
    ...
    for line in lines:
        if line.startswith('+') or line.startswith('-'):
            ...
            # Check for session/request ID patterns
            if any(p.search(line) for p in SESSION_REQUEST_PATTERNS):
                session_lines += 1
            # Also check CSRF patterns
            elif any(p.search(line) for p in CSRF_PATTERNS):
                session_lines += 1
    
    if total_changes == 0:
        return False
    
    # If >60% of changes are session/request IDs, it's noise
    return (session_lines / total_changes) > 0.6
...
def calculate_trivia_score(diff: str) -> float:
    ...
    # Check new trivia types first (highest priority)
    if is_antibot_script_change(diff):
        scores.append(1.0)
    
    if is_nonce_only_change(diff):
        scores.append(1.0)
    
    if is_session_request_id_change(diff):
        scores.append(1.0)
...
# Patterns that indicate important/interesting code
API_INDICATOR_PATTERNS = [
    ...
    re.compile(r'(adminPass|adminUser|debugPass|secretKey|password|apiKey|api_key)\s*[:=]', re.I),  # Hardcoded creds
    re.compile(r'(XMLHttpRequest|\.ajax\(|axios\.|\.post\(|\.get\()'),  # HTTP calls
    re.compile(r'(localStorage|sessionStorage)\.(get|set)Item'),  # Storage access
    re.compile(r'(Bearer|Authorization)\s*[:=]', re.I),  # Auth headers
]
...
def has_api_indicators(text: str, check_diff_lines: bool = True) -> bool:
    """
    Check if text contains API-related patterns that should always be analyzed.
    
    Args:
        text: The text to check (can be diff or full content)
        check_diff_lines: If True, only check lines starting with '+' (for diffs)
                         If False, check all lines (for full content)
    """
    for line in text.split('\n'):
        ...
        if any(p.search(line) for p in API_EXCLUDE_PATTERNS):
            continue
        
        # Check for API indicators
        for pattern in API_INDICATOR_PATTERNS:
            if pattern.search(line):
                return True
    return False
...
def has_interesting_content(content: str) -> bool:
    """
    Check if file content contains interesting patterns worth monitoring.
    This checks the FULL file content, not just diff.
    
    Returns True if file contains API calls, auth logic, etc.
    """
    return has_api_indicators(content, check_diff_lines=False)


async def should_analyze_with_ai(diff: str, js_url: str, debug: bool = False, content: str = None) -> Tuple[bool, str]:
    """
    ...
    Args:
        ...
        content: Full file content (optional, for content-based filtering)
    ...
    # PRE-FILTER 1: Check URL for known noise patterns
    if is_known_noise_url(js_url):
        if debug:
            print(f"[TRIVIA] {js_url}: known noise URL, skipping")
            _log_trivia_decision(js_url, diff, {"noise_url": True}, 1.0, False, False, "noise_url")
        return False, "noise_url"
    
    # PRE-FILTER 2: Check diff size limit
    diff_size = len(diff)
    if diff_size > MAX_DIFF_SIZE:
        if debug:
            print(f"[TRIVIA] {js_url}: diff too large ({diff_size} > {MAX_DIFF_SIZE}), skipping")
            _log_trivia_decision(js_url, diff[:1000], {"diff_too_large": True}, 1.0, False, False, "diff_too_large")
        return False, f"diff_too_large_{diff_size}"
    
    # PRE-FILTER 3: If content provided, check if file has interesting patterns
    if content is not None:
        if not has_interesting_content(content):
            if debug:
                print(f"[TRIVIA] {js_url}: no interesting content in file, skipping")
                _log_trivia_decision(js_url, diff, {"no_interesting_content": True}, 1.0, False, False, "no_interesting_content")
            return False, "no_interesting_content"
    
    # FIRST: Check for API indicators in diff - these always get analyzed
    api_indicators = has_api_indicators(diff, check_diff_lines=True)
    if api_indicators:
        if debug:
    ...
    # NEW: Check for anti-bot/fingerprinting script changes
    checks["antibot_script"] = is_antibot_script_change(diff)
    if checks["antibot_script"]:
        if debug:
            _log_trivia_decision(js_url, diff, checks, 1.0, False, False, "antibot_script")
        return False, "antibot_script"
    
    # NEW: Check for CSP nonce-only changes
    checks["nonce_only"] = is_nonce_only_change(diff)
    if checks["nonce_only"]:
        if debug:
            _log_trivia_decision(js_url, diff, checks, 1.0, False, False, "nonce_only")
        return False, "nonce_only"
    
    # NEW: Check for session/request ID rotations
    checks["session_request_id"] = is_session_request_id_change(diff)
    if checks["session_request_id"]:
        if debug:
            _log_trivia_decision(js_url, diff, checks, 1.0, False, False, "session_request_id")
        return False, "session_request_id"
