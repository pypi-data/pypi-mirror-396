...
# Cache for AI analysis results by diff content hash
# Key: MD5 hash of diff content, Value: AI result dict
_diff_analysis_cache: Dict[str, Dict[str, Any]] = {}
_cache_hits = 0
_cache_misses = 0


def _get_diff_hash(diff: str) -> str:
    """Generate MD5 hash of diff content for caching."""
...
async def send_diff_to_ai(
    ...
    # Check cache first - same diff content = same result
    global _cache_hits, _cache_misses
    diff_hash = _get_diff_hash(diff)
    
    if diff_hash in _diff_analysis_cache:
        _cache_hits += 1
        cached_result = _diff_analysis_cache[diff_hash]
        if debug:
            print(f"[AI CACHE HIT] {js_url} - using cached result (hash: {diff_hash[:8]}...)")
    ...
    if debug:
        print(f"[AI] Analyzing {js_url} with {provider_name} ({len(diff)} chars, hash: {diff_hash[:8]}...)")
    ...
    if result is not None:
        _diff_analysis_cache[diff_hash] = result
        if debug:
            print(f"[AI CACHE STORE] Cached result for hash {diff_hash[:8]}... (cache size: {len(_diff_analysis_cache)})")
