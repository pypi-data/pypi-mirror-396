...
class wafencode:
    ...
    version = "0.1"
    summary = "replace one character with its hex encoding to bypass waf"
    category = ["url_safe", "url"]
    priority = 99
    ...
    def encode(self, string):
        ...
        for i in range(len(string)-1,-1,-1):
            if string[i] in list("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"):
                index = i
                break
        if index == -1:
            return string
        else:
            return string.replace(string[index],"%{0:0>2x}".format(ord(string[index])),1)
