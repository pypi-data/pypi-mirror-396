...
case_id = "Optimizacion_de_superclases/8_superclases_continua_Hito3/Modelo_1_BRF_Muslo/"                                         # Identificador del caso
...
for i in range(1, N_RUNS + 1):                                      # Bucle principal: repite N_RUNS veces
    ...
    # --- TRAIN ---
    train_args_with_i = train_args + ["--run-index", str(i)]
    print(f"\n--- TRAIN (ejecución {i}) ---")                       # Mensaje de inicio de entrenamiento
    subprocess.run([python_exe] + train_args_with_i, check=True)           # Ejecuta trainer.py con los argumentos definidos
