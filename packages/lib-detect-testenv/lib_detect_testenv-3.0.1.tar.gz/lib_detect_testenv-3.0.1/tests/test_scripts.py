...
def test_get_project_metadata_fields():
    ...
    assert meta.name == "lib_detect_testenv"
    assert meta.slug == "lib-detect-testenv"
    assert meta.import_package == "lib_detect_testenv"
    assert meta.coverage_source == "src/lib_detect_testenv"
    assert meta.github_tarball_url("1.2.3").endswith("/bitranox/lib_detect_testenv/archive/refs/tags/v1.2.3.tar.gz")
    assert meta.version
    assert meta.summary
    assert meta.author_name
    assert meta.metadata_module.as_posix().endswith("src/lib_detect_testenv/__init__conf__.py")
