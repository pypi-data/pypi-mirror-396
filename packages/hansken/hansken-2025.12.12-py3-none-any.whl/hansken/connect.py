...
def connect(
    ...
    **kwargs,
    ...
    if kwargs:
        ...
        log.warning(
            'ignoring {} keyword arguments to {}.connect: {}', len(kwargs), __name__, ', '.join(sorted(kwargs.keys()))
        )

    # take either a preconfigured HanskenAuthBase or resolve one against the remote
    auth = auth or resolve(
        base_url=endpoint, idp_id=idp, username=username, password=password, interactive=interactive, verify=verify
    )
    return Connection(
        base_url=endpoint,
