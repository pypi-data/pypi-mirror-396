...
class data_stream:  # noqa: N801
    ...
    def get_encoding(self, trace):
        mime_type = trace.get(f'data.{self.data_type}.mimeType')
        ...
        if encoding:
            ...
            log.debug(
                'falling back to encoding {} for stream {} of trace {}',
            ...
            log.debug(
                'unable to determine encoding of stream {} for trace {} (mime type: {})',
                self.data_type,
                trace.uid,
                mime_type,
            )
    ...
    def to_text(self, trace, default=None):
        ...
        if not encoding:
            log.warning(
                'omitting data for stream {} for trace {}, '
                'unable to determine encoding and no fallback encoding provided',
                self.data_type,
                trace.uid,
            )
            return default
...
def to_csv(
    traces, output, fields, to_dict=get_fields, delimiter='\t', lineterminator='\n', encoding='utf-8', **fmtparams
):
    """
    ...
    def do_write(writable):
        writer = csv.DictWriter(
            writable, fieldnames=names, delimiter=delimiter, lineterminator=lineterminator, **fmtparams
        )
        writer.writeheader()
        for trace in traces:
...
def safe_name(trace, num=None, split=1000, stream='raw', template='{trace.image_id}_{trace.id}_{stream}_{trace.name}'):
    ...
    """
    file_name = template.format(trace=trace, num=num, split=split, stream=stream)
    ...
    if len(name) > MAX_FILENAME_LEN:
        ...
        log.warning('truncating file name to {} characters: {} -> {}', MAX_FILENAME_LEN, name, short_name)
        return short_name
...
def bulk(
    traces, dest, split=1000, stream='raw', fname=safe_name, write=to_file, on_error=None, side_effect=None, jobs=16
):
    """
    ...
    with executor:
        ...
        for num, trace in enumerate(traces):
            ...
            if not selected_stream:
                ...
                log.warning('omitting trace {} from bulk export, no stream to export was selected', trace.get('uid'))
                continue
    ...
    *(Intended for internal use with `.bulk`)*
    ...
    if error:
        if callable(on_error):
            on_error(
                num=task['num'], trace=task['trace'], stream=task['stream'], output=task['output'], exception=error
            )
        else:
