...
try:
    ...
    raise ImportError(
        'jinja2 is not available; install hansken.py with the "report" or "all" '
        'extras (e.g. pip install hansken[report]), or install jinja2 manually'
    ) from e
...
try:
    ...
    log.warning('failed to import weasyprint, PDF rendering will not be available', e)

    def missing_lib(*args, **kwargs):
        raise ImportError(
            'weasyprint is not available; install hansken.py with the "report" or "all" '
            'extras (e.g. pip install hansken[report]), or install weasyprint manually'
        )
