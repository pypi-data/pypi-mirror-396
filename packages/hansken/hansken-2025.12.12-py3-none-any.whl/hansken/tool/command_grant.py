...
def run_grant(*args, **kwargs):
    warnings.warn(f'{__name__}.run_grant has been renamed to {__name__}.run_grant_command', DeprecationWarning)
    run_grant_command(*args, **kwargs)


def grant(base_url, keystore_url, project=None, image=None, identities=None, auth=None, verify=True):
...
grant_parser = add_command(
    'grant',
    run_grant_command,
    optional_project=False,
    help='grant keys for specific or project images to target identity or identities',
)
grant_parser.add_argument('identities', metavar='IDENTITY', nargs='+', help='grant keys to IDENTITY')
...
images_group.add_argument('-p', '--project', metavar='PROJECT', help='grant keys for all images in PROJECT')
images_group.add_argument('-i', '--image', metavar='IMAGE', help='grant keys for a specific IMAGE')
