...
def run_versions_command(args):
    ...
    with Connection(args.endpoint, keystore_url=args.keystore, auth=args.auth, verify=args.verify) as connection:
        ...
        print(f'hansken.py:    {__version__}')
        ...
        if args.project:
            ...
            with ProjectContext(connection, args.project) as context:
                ...
                for root in roots:
                    ...
                    image.update(version=version, date=date, size=format_byte_size(root.data.raw.size))
...
def run_versions(*args, **kwargs):
    warnings.warn(f'{__name__}.run_versions has been renamed to {__name__}.run_versions_command', DeprecationWarning)
    run_versions_command(*args, **kwargs)


# define command versions
versions_parser = add_command(
    'versions', run_versions_command, optional_project=False, help='print client, server and extraction versions'
)
versions_parser.add_argument(
    'project', metavar='PROJECT', nargs='?', help='print versions at extraction time for images in PROJECT'
)
versions_parser.add_argument(
    '-s', '--size', dest='size', action='store_true', default=False, help='display the raw data size of images'
)
versions_parser.add_argument(
    '-S',
    '--no-size',
...
versions_parser.add_argument(
    '-t', '--traces', dest='traces', action='store_true', default=False, help='display the number of traces per image'
)
versions_parser.add_argument(
    '-T',
    '--no-traces',
