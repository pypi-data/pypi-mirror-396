...
def run_stats_command(args):
    ...
    stats(
        ...
        args.project,
        args.images,
        show_image_props=args.show_image_props,
        show_type_counters=args.show_type_counters,
        auth=args.auth,
        verify=args.verify,
    )
    return 0
...
def run_stats(*args, **kwargs):
    warnings.warn(f'{__name__}.run_stats has been renamed to {__name__}.run_stats_command', DeprecationWarning)
    run_stats_command(*args, **kwargs)


def stats(base_url, project, images=None, show_image_props=False, show_type_counters=True, auth=None, verify=True):
    """
    ...
    with ProjectContext(base_url, project, auth=auth, verify=verify) as context:
        ...
        for image in images:
            ...
            results = context.search(query=Term('image', image.get('id')), count=0, facets=facet)
...
stats_parser.add_argument('project', metavar='PROJECT', help='generate stats for PROJECT')
stats_parser.add_argument(
    'images', metavar='IMAGE', nargs='*', help='only generate stats for IMAGE within PROJECT (defaults to all images)'
)
stats_parser.add_argument(
    '--image-properties',
    '-i',
    dest='show_image_props',
    action='store_true',
    default=False,
