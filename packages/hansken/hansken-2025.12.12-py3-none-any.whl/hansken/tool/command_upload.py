...
from hansken.util import Namespace, b64decode, b64encode, format_byte_size, parse_byte_size


log = Logger(__name__)


upload_info = Namespace(
    marker='\f',  # marker used by the image tool to indicate line of machine-readable output
    names={
        ...
        try:
            ...
            for name, value in upload(
                ...
                if progress and input_size and name == 'num_read':
                    ...
                    print(
                        '\033[2K\r[{:<40}] ({})'.format('»' * int(40 * value / input_size), format_byte_size(value)),
                        end='',
                    )
...
def run_upload(*args, **kwargs):
    warnings.warn(f'{__name__}.run_upload has been renamed to {__name__}.run_upload_command', DeprecationWarning)
    run_upload_command(*args, **kwargs)


def upload(
    connection_or_endpoint,
    image_id,
    image_file,
    key,
    destination=local_proxy,
    bufsize=8 << 20,
    max_retries=8,
    retry_wait=10.0,
    auth=None,
    ...
    store_key=False,
    verify=True,
    image_tool_path='hansken-image-tool',
    streaming=False,
    **image_meta,
):
    """
    ...
    if not upload_info.marker.isspace():
        raise ValueError(f'image tool communication marker ({upload_info.marker}) needs to be whitespace')
    ...
    def run_image_tool(destination, uploaded_image):
        ...
        process = Popen(command_line, universal_newlines=True, stdout=PIPE, stderr=STDOUT)  # noqa: S603
        ...
        message = (
            'conversion process (image id {image_id}) terminated with return code {return_code}, '
            'output follows {tail}\n---\n{output}'
        )
        ...
        if process.wait() != 0:
            raise ValueError(
                message.format(
                    image_id=image_id if image_id is not None else 'unknown',
    ...
    def do_upload(connection):
        ...
        try:
            if destination is local_proxy:
                ...
                with WebHDFSServer(
                    force_auth_upload_callback,
                    bufsize=bufsize,
                    max_retries=max_retries,
                    retry_wait=retry_wait,
                    streaming=streaming,
                ) as server:
                    yield from run_image_tool(f'http://localhost:{server.server_port}/', uploaded_image)
            else:
            ...
            log.warning('upload was interrupted, removing partially uploaded image {}', image_id)
            if project:
                connection.unlink_image(project, image_id)
            connection.delete_image(image_id)
            log.warning('partial upload removed')
            raise
...
upload_parser = add_command('upload', run_upload_command, help='upload an image to Hansken image store')
...
upload_parser.add_argument('-o', '--output', dest='hdfs_url', metavar='ENDPOINT', default=local_proxy, help=SUPPRESS)
...
upload_parser.add_argument(
    ...
    help='prompt for a key (base64-encoded)',
)
upload_parser.add_argument(
    '--streaming',
    action='store_true',
...
upload_parser.add_argument('-x', '--extract', action='store_true', help='start extraction of traces after image upload')
upload_parser.add_argument(
    '-s',
    '--store-key',
    dest='store_key',
...
upload_parser.add_argument(
    '-S', '--no-store-key', dest='store_key', action='store_false', help='do not store key in keystore'
)
upload_parser.add_argument(
    '--hansken-image-tool',
    dest='image_tool_path',
    default='hansken-image-tool',
    help="(Path to) script calling the Hansken image tool. Should include script'sextension in Windows",
)
