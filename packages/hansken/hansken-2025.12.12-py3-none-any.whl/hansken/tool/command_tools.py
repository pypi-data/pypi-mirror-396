...
def run_tools_command(args):
    ...
    with Connection(args.endpoint, auth=args.auth, verify=args.verify) as connection:
        ...
        table = [
            ('[x]' if tool.get('defaultEnabled') else '[ ]', name, tool.get('version'), tool.get('description'))
        ...
        print(
            tabulate(
                sorted(table, key=lambda tool: tool[1]),
...
def run_tools(*args, **kwargs):
    warnings.warn(f'{__name__}.run_tools has been renamed to {__name__}.run_tools_command', DeprecationWarning)
    run_tools_command(*args, **kwargs)


tools_parser = add_command(
    'tools', run_tools_command, optional_project=False, help='print a table of available extraction tools'
)
