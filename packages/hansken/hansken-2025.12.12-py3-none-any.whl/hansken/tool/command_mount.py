...
try:
    ...
    def run_mount_command(args):
        ...
        with ProjectContext(
            args.endpoint, args.project, keystore_url=args.keystore, auth=args.auth, verify=args.verify
        ) as context:
    ...
    class HanskenFS(Operations):
        ...
        def __init__(self, context, *images):
            ...
            self.roots = {f'/{root.image_id}': root for root in context.roots() if root.image_id in images}

            log.debug('got {} roots', len(self.roots), len(images))
            # store relevant image metadata to create 'symlinks' to the image data
            self.links = {
                '/{}'.format(image['description']): image['id']
            ...
            self.keys = {f'/{image}': context.connection.key(image, raise_on_not_found=False) for image in images}
        ...
        def getattr(self, path, fh=None):
            ...
            if path in self.roots:
                ...
                return {'st_mode': S_IFREG | 0o444, 'st_size': size, 'st_mtime': mtime}

            if path in self.links:
                # getattr for a 'link', return it as a link with access rights u+rx,g+rx,o+rx
                size, mtime = self._stat(f'/{self.links.get(path)}')
                return {'st_mode': S_IFLNK | 0o555, 'st_size': size, 'st_mtime': mtime, 'st_nlinks': 1}
        ...
        def read(self, path, size, offset, fh=None):
            ...
            super().read(path, size, offset, fh)
    ...
    def run_mount_command(*args, **kwargs):
        mount_parser.error(
            "command requires FUSE's python bindings, install hansken.py with the 'mount' extra or "
            "install package 'fusepy' manually"
        )


def run_mount(*args, **kwargs):
    warnings.warn(f'{__name__}.run_mount has been renamed to {__name__}.run_mount_command', DeprecationWarning)
...
mount_parser = add_command(
    'mount',
    run_mount_command,  # either the real deal or an error-wrapper
    optional_project=False,
    help=help_text,
)
mount_parser.add_argument('project', metavar='PROJECT', help='the project to mount images from')
mount_parser.add_argument('mount_point', metavar='MOUNT_POINT', help='an empty directory to use as the mount point')
