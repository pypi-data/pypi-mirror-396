...
def render_tasks(tasks, tablefmt='simple', **filters):
    ...
    # (provide type-consistent default values to keep the sort working)
    tasks = sorted(tasks, key=lambda row: (row[0] if row[0] is not None else -1, row[-1] or 'z'))
