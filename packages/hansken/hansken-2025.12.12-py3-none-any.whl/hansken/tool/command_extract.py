...
def run_extract_command(args, progress=sys.stdout.isatty()):
    ...
    def images_with_user_content():
        ...
        return set(
            chain.from_iterable(
                ...
                user_content_images(batch)
                for batch in chunked(images, DEFAULT_MAX_CLAUSE_COUNT - 4)
            )
        )

    def what_to_extract():
        # provide information on the images that have user content
        print(f'the following images have user content within project {args.project}:')
    ...
    with ProjectContext(args.endpoint, args.project, args.keystore, auth=args.auth, verify=args.verify) as context:
        ...
        if args.images:
            ...
            if not_linked:
                raise ValueError('images not linked to project {}: {}'.format(args.project, ', '.join(not_linked)))
        ...
        if args.include_tools or args.exclude_tools:
            ...
            if include & exclude:
                raise ValueError(
                    'cannot both include and exclude tools [{}]'.format(', '.join(sorted(include & exclude)))
                )
            ...
            tools -= exclude
            log.info(
                'using {} tools after including [{}] and excluding [{}]',
                len(tools),
                ', '.join(sorted(include)),
                ', '.join(sorted(exclude)),
            )
                        ...
                        if progress:
                            ...
                            print(
                                '\033[2K\r[{:<40}] ({}/{}){}'.format(
                                    ...
                                    f' ({num_errors} errors)' if num_errors > 0 else '',
                                ),
                                end='',
                            )
...
def run_extract(*args, **kwargs):
    warnings.warn(f'{__name__}.run_extract has been renamed to {__name__}.run_extract_command', DeprecationWarning)
    return run_extract_command(*args, **kwargs)


extract_parser = add_command(
    'extract', run_extract_command, optional_project=False, help='start extraction of images linked to a project'
)
extract_parser.add_argument('project', metavar='PROJECT', help='the project to extract images for')
extract_parser.add_argument(
    'images', metavar='IMAGE', nargs='*', help='the images to be extracted (defaults to all images linked to PROJECT)'
)
...
extract_parser.add_argument(
    '-c',
    '--check-user-content',
    dest='check_user_content',
    action='store_true',
    default=True,
    help='check if any selected images have user content and prompt for action to take (this is the default)',
)
extract_parser.add_argument(
    '-C',
    '--no-check-user-content',
    dest='check_user_content',
    action='store_false',
...
extract_parser.add_argument(
    '--exclude-tools', nargs='+', help='exclude named TOOL from the tools to be used to extract IMAGE(S)'
)
