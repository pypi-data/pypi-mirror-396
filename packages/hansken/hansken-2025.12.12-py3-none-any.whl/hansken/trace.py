...
def image_from_trace(trace):
    ...
    raise ValueError(f'unable to get image id from trace {trace}')
...
class Trace(AbstractTrace):
    ...
    def context(self):
        ...
        if self._context:
            ...
            raise ValueError(f'project context for trace {self.uid} not set')
        ...
        if self._scope is Trace._UNINITIALIZED:
            ...
            if self._scope:
                ...
                try:
                    ...
                    raise ValueError('unknown scope state: {}'.format(self.get('user.annotated.scope'))) from e
    ...
    def snippets(self, query, num=100, before=200, after=200):
        ...
        with self.context.search(query=Term('uid', self.uid) & to_query(query), count=1, snippets=num) as result:
            ...
            if hasattr(self, 'data'):
                ...
                data_sizes = {data_type: self.get(f'data.{data_type}.size') for data_type in self.data_types}
            else:
                # plain trace, assume data property path to be in origin system
                data_sizes = {
                    data_type: self.get(f'system.extracted.data.{data_type}.size') for data_type in self.data_types
                }
    ...
    def update(self, key_or_updates=None, value=None, data=None, overwrite=False):
        ...
        if self._update_count > 1:
            log.warning(
                'This trace ({}) is being updated more than once. This could result in lost updates, please '
                'supply a mapping with all batched updates instead of single keys and values.',
                self.id,
            )
    ...
    def __repr__(self):
        return f'<{self.__class__.__module__}.{self.__class__.__name__} {self.uid} ({self.name})>'
...
class TraceModel(DictView):
    ...
    def __init__(self, source):
        ...
        self._mapped_types = {
            type_name: set(type_details.get('keys'))
            for category_name, category in self.get('origins.categories').items()
        ...
        try:
            ...
            if data_types:
                ...
                if data_type in data_types:
                    ...
                    raise KeyError(f'unknown data type "{data_type}" for trace type "{type_name}"')

            # verify the property name is valid for the type
            type_properties = self.get(f'origins.categories.{category}.types.{type_name}.properties', default={})
        ...
        try:
            if '.' not in name:
                ...
                if name in self.intrinsics:
                    if self.get(f'properties.{name}.isMap', default=False):
            ...
            if step in self.categories:
                ...
                if step in self._categorized_properties:
                    if self._categorized_properties[step] != category:
                        raise ValueError(f'unknown trace property: "{name}"')

                    # mapped categorized property (e.g. category.misc.something)
                    if self.get(f'origins.categories.{category}.properties.{step}.isMap', default=False):
                        # glue all remaining steps for the map property together (but require at least one)
            ...
            raise IncompleteTracePropertyError(f'incomplete trace property: "{name}"')
        except KeyError:
            raise ValueError(f'unknown trace property: "{name}"')
        else:
            raise ValueError(f'unknown trace property: "{name}"')
    ...
    def get_serializer(self, steps):
        try:
            if self.is_intrinsic(steps):
                converter = CONVERTERS.get(self.get(f'properties.{steps[0]}.type'))
...
class TraceBuilder(DictView):
    ...
    def __init__(self, model, source=None, target=None, context=None, debug=False):
    ...
    def add_data(self, stream, data):
        ...
        if stream not in self._model.data_types:
            log.warning(
                'data type stream name "{}" not defined by model, trace being constructed might be invalid', stream
            )
    ...
    def build(self):
        ...
        if self.debug:
            ...
            log.warning(
                ...
                builder=self,
                parent=self.target[1],
                updates=', '.join(self.updates),
                data=', '.join(f'{name}: {len(stream)} bytes' for name, stream in sorted(self._data.items()))
                or '(no data)',
            )
            ...
            if self._uid:
                ...
                raise ValueError(f'builder was already built (resulting uid: {self._uid})')
    ...
    for category, types in model['origins']['categories'].items():
        ...
        if types:
            ...
            for type_name, trace_type in types['types'].items():
                ...
                properties = {prop: value_converter_for(details) for prop, details in trace_type['properties'].items()}
...
def expand_types(model, *type_names):
    ...
    for type_name in type_names:
        ...
        names.extend(sorted(f'{type_name}.{prop}' for prop in model_types[type_name]['properties'].keys()))
...
class TraceTypeView(DictView):
    ...
    def get(self, key, default=None):
        for origin in self._origins:
            ...
            value = super().get(
                self._separator.join((origin, self._category, self._name, key)), default=self._no_default
            )
    ...
    def __str__(self):
        ...
        return str({origin: parent.get('.'.join((origin, self._category, self._name))) for origin in self._origins})

    def __repr__(self):
        return f'<{self.__class__.__module__}.{self.__class__.__name__} ({self._name})>'
...
def trace_class_from_model(model):
    ...
    intrinsics = {prop: value_converter_for(details) for prop, details in model['properties'].items()}
    ...
    type_classes = {
        (category, type_name): type(
            '.'.join(map(to_attr_name, (category, type_name))),
        ...
        def __init__(self, source, context=None):
            ...
            for category_name, type_name in types:
                ...
                if (category_name, type_name) in mapped_types:
                    ...
                    keys = set(
                        chain.from_iterable(
                            (self.get('.'.join((origin, category_name, type_name))) or {}).keys() for origin in origins
                        )
                    )
...
trace type. Check for this with `'type_name' in trace.types`.""".format(
    ...
    TraceView.__doc__ = f'{Trace.__doc__}\n\n{type_doc}'
