...
from hansken.util import GeographicLocation, Vector, format_datetime, omit_empty
...
class Nested(Query):
    ...
    def as_dict(self):
        return {'nested': {'field': self.field, 'query': self.query.as_dict()}}
...
class Trace(Query):
    ...
    def as_dict(self):
        return {'hasTrace': {'query': self.query.as_dict()}}
...
class Term(Query):
    ...
    def __init__(self, field_or_value, value=None, full=False):
        ...
        if isinstance(value, float):
            raise TypeError(f'float value for Term not supported: {value}')
    ...
    def __init__(self, field_or_pattern, pattern=None, full=False):
        ...
        if isinstance(pattern, re.Pattern):
            if pattern.flags & ~re.UNICODE:
                log.warning(
                    'regular expression flags in Query objects are ignored ({} used by pattern)',
...
class Phrase(Query):
    ...
    def __init__(self, field_or_value, value=None, distance=0):
        ...
        if len(value) > 1000:
            raise ValueError(f'value too long for Phrase: {len(value)}')
        if '?' in value or '*' in value:
            raise ValueError(f'wildcard in value not allowed for Phrase: {value}')
...
class HQLHuman(Query):
    ...
    def __init__(self, query):
        if not isinstance(query, str):
            raise TypeError(f'HQL-Human query not a string: {query}')
...
def to_query(query):
    ...
    raise TypeError(f'query should be either str or hansken.query.Query, not {type(query)}')
...
class Sort:
    def __init__(self, field, direction=None, filter=None, mode=None, value=None):  # noqa: A002
        """
    ...
    def from_str(cls, sort):
        ...
        if len(parts) == 2:
            filter = HQLHuman(parts[1].strip())  # noqa: A001
...
def to_sort(sort):
    ...
    raise TypeError(f'sort should be either str or hansken.query.Sort, not {type(sort)}')


class Facet:
    def __init__(self, field, size=100, include_total=None, scale=None, filter=None):  # noqa: A002
        self.field = field
        self.size = size
...
class TermFacet(Facet):
    def __init__(self, field, size=100, include_total=None, filter=None):  # noqa: A002
        """
...
class RangeFacet(Facet):
    def __init__(self, field, scale, base=None, interval=None, min=None, max=None, include_total=None, filter=None):  # noqa: A002
        """
...
class GeohashFacet(Facet):
    def __init__(self, field, size=100, include_total=None, precision=1, southwest=None, northeast=None, filter=None):  # noqa: A002
        """
...
def to_facet(facet):
    ...
    raise TypeError(f'facet should be either str or hansken.query.Facet, not {type(facet)}')
