...
# 'magic' value that indicates explicit success in a SAML response
SAML_SOAP_SUCCES = 'urn:oasis:names:tc:SAML:2.0:status:Success'
# content type of PAOS requests/responses
PAOS_CONTENT_TYPE = 'application/vnd.paos+xml'
# content types that are considered acceptable as response types:
# - application/vnd.paos+xml: reverse soap (paos)
# - application/soap+xml: regular soap (shouldn't happen, really)
# - text/xml: just xml, hoping that the actual content is a saml response in a soap/paos envelope
PAOS_ACCEPTABLE_CONTENT_TYPES = {PAOS_CONTENT_TYPE, 'application/soap+xml', 'text/xml'}
...
def get_idps(base_url, verify=True):
    ...
    if idps.status_code == codes.ok:
        ...
        log.warning('remote does not understand SAML: {}: {}', idps.status_code, idps.reason)
        return {}
    else:
...
def resolve(
    ...
    if idp_url and not username:
        ...
        log.warning('IDP url known, user+pass auth required but no username supplied')
        env_user = getuser()
...
class SAMLAuthBase(HanskenAuthBase):
    ...
    def __call__(self, request):
        ...
        with self._lock:
            ...
            request.headers['Accept'] = ', '.join([accept, PAOS_CONTENT_TYPE] if accept else [PAOS_CONTENT_TYPE])
...
class KeycloakAuth(SAMLAuthBase):
    ...
    def handle_paos_request(self, response):
        ...
        if saml_response.status_code != codes.ok:
            # authentication request failed, raise auth error
            raise HanskenAuthError(
                f'authentication request failed with {saml_response.status_code}: {saml_response.reason}'
                f', expected 200: OK',
                response=saml_response,
            )

        if saml_response.headers.get('Content-Type') in PAOS_ACCEPTABLE_CONTENT_TYPES:
            if SAML_SOAP_SUCCES in saml_response.text:
                # provide raw saml response and any gathered history
                return saml_response.content, response.history + [response, saml_response]
            else:
                # likely a PAOS response, as expected, but no indicator of a successful auth in the response
                # treat this as a signal that auth has failed
                raise HanskenAuthError(
                    f'no success indicator in SAML response from IDP endpoint {self.idp_url}, SAML auth failed',
                    response=saml_response,
                )
        else:
            raise HanskenAuthError(
                f'unexpected content type in SAML response: {content_type}, SAML auth failed',
                response=saml_response,
            )
