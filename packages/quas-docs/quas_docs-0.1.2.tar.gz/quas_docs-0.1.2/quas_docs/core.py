...
class FlaskOpenAPISpec:
    ...
    def _apply_registered_endpoints(self, inner: Any) -> None:
        ...
        # Initialize wrapped schemas and hoisted defs dicts before building responses
        if not hasattr(self, '_wrapped_schemas'):
            self._wrapped_schemas: Dict[str, Dict[str, Any]] = {}
        if not hasattr(self, '_hoisted_defs'):
            self._hoisted_defs: Dict[str, Dict[str, Any]] = {}
        ...
        # Normalize model schemas and collect defs
        normalized_model_schemas: Dict[str, Dict[str, Any]] = {}
        for model in models_to_register:
            raw_schema = self._get_model_schema(model)
            normalized_schema, defs = self._normalize_schema(raw_schema)
            normalized_model_schemas[model.__name__] = normalized_schema
            self._hoisted_defs.update({k: v for k, v in defs.items() if k not in self._hoisted_defs})

        # Register Pydantic model schemas in components/schemas section
        if normalized_model_schemas:
            try:
                comps: Any = getattr(inner, 'components', None)
                if comps is not None and hasattr(comps, 'schema'):
                    # Use flask-pydantic-spec's schema registration method
                    for name, schema in normalized_model_schemas.items():
        ...
        # Register hoisted $defs from models and wrapped schemas
        if hasattr(self, '_hoisted_defs') and self._hoisted_defs:
            try:
                comps: Any = getattr(inner, 'components', None)
                if comps is not None and hasattr(comps, 'schema'):
                    for name, schema in self._hoisted_defs.items():
                        comps.schema(name, schema)
                elif isinstance(inner, dict):
                    schemas = inner.setdefault('components', {}).setdefault('schemas', {})
                    for name, schema in self._hoisted_defs.items():
    ...
    def _get_model_schema(self, model: Type[BaseModel]) -> Dict[str, Any]:
        """
        Retrieve a JSON schema from a Pydantic model.
    ...
    def _rewrite_refs(self, obj: Any, defs_prefix: str = "#/$defs/") -> Any:
        """Rewrite $ref targets from $defs/definitions to components/schemas recursively."""
        if isinstance(obj, dict):
            new_obj = {}
            for k, v in obj.items():
                if k == "$ref" and isinstance(v, str):
                    if v.startswith(defs_prefix):
                        ref_name = v.split(defs_prefix, 1)[1]
                        new_obj[k] = f"#/components/schemas/{ref_name}"
                    elif v.startswith("/$defs/"):
                        ref_name = v.split("/$defs/", 1)[1]
                        new_obj[k] = f"#/components/schemas/{ref_name}"
                    elif v.startswith("#/definitions/"):
                        ref_name = v.split("#/definitions/", 1)[1]
                        new_obj[k] = f"#/components/schemas/{ref_name}"
                    elif v.startswith("/definitions/"):
                        ref_name = v.split("/definitions/", 1)[1]
                        new_obj[k] = f"#/components/schemas/{ref_name}"
                    ...
                    new_obj[k] = self._rewrite_refs(v, defs_prefix)
            return new_obj
        if isinstance(obj, list):
            return [self._rewrite_refs(i, defs_prefix) for i in obj]
        return obj
    
    def _normalize_schema(self, schema: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """
        Hoist $defs and rewrite refs to components/schemas.
        
        Returns (normalized_schema, hoisted_defs)
        """
        ...
        normalized_schema = self._rewrite_refs(schema)
        # Also rewrite refs inside hoisted defs
        normalized_defs = {}
        for name, def_schema in defs.items():
            normalized_defs[name] = self._rewrite_refs(def_schema)
        return normalized_schema, normalized_defs
    ...
    def _build_wrapped_response_schema(
        ...
        base_schema = self._get_model_schema(base_schema_class)
        
        # Normalize base schema and hoist any $defs
        normalized_base, defs = self._normalize_schema(base_schema)
        self._hoisted_defs.update({k: v for k, v in defs.items() if k not in self._hoisted_defs})
        
        wrapped_schema = copy.deepcopy(normalized_base)
