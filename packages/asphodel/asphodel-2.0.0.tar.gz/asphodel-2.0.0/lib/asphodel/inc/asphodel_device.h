...
typedef struct AsphodelDevice_t {
	...
	// Returns max_incoming_param_length, max_outgoing_param_length, and stream_packet_length for the remote device,
	// if applicable. This function will return an error for non-radio devices.
	int (*get_remote_lengths)(struct AsphodelDevice_t * device, size_t *max_incoming_param_length,
		size_t *max_outgoing_param_length, size_t *stream_packet_length);

	// Extra space allocated to allow backwards compatibility in future library versions
	void * reserved[8];
} AsphodelDevice_t;
