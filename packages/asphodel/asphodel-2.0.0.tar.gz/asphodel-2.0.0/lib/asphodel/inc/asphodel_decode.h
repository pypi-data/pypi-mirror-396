...
ASPHODEL_API int asphodel_create_channel_decoder(const AsphodelChannelInfo_t *channel_info,
    uint16_t channel_bit_offset, AsphodelChannelDecoder_t **decoder);

// Same as asphodel_create_channel_decoder, but will return an error if the configuration exceeds stream_packet_length.
ASPHODEL_API int asphodel_create_channel_decoder_checked(const AsphodelChannelInfo_t *channel_info,
    uint16_t channel_bit_offset, size_t stream_packet_length, AsphodelChannelDecoder_t **decoder);
...
ASPHODEL_API int asphodel_create_stream_decoder(const AsphodelStreamAndChannels_t *info,
    uint16_t stream_bit_offset, AsphodelStreamDecoder_t **decoder);

// Same as asphodel_create_stream_decoder, but will return an error if the configuration exceeds stream_packet_length.
ASPHODEL_API int asphodel_create_stream_decoder_checked(const AsphodelStreamAndChannels_t *info,
    uint16_t stream_bit_offset, size_t stream_packet_length, AsphodelStreamDecoder_t **decoder);
...
ASPHODEL_API int asphodel_create_device_decoder(const AsphodelStreamAndChannels_t *info_array, uint8_t info_array_size,
    uint8_t filler_bits, uint8_t id_bits, AsphodelDeviceDecoder_t **decoder);

// Same as asphodel_create_device_decoder, but will return an error if the configuration exceeds stream_packet_length.
ASPHODEL_API int asphodel_create_device_decoder_checked(const AsphodelStreamAndChannels_t *info_array,
    uint8_t info_array_size, uint8_t filler_bits, uint8_t id_bits, size_t stream_packet_length,
    AsphodelDeviceDecoder_t **decoder);

// Create a device decoder from the device info. The active streams array is filled with the stream ids of the streams
// that will be used to create the decoder. The decoder can be freed via its free_decoder function.
...
// The same as asphodel_get_streaming_counts, but takes an AsphodelDeviceInfo_t instead of an array of streams.
