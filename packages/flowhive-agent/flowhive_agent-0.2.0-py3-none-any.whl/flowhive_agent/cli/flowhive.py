...
Authentication:
  ...
  flowhive config api_key
  
  # Set a value
  flowhive config api_key your-api-key-here
  
  # Set value in specific config file
  flowhive config --config /path/to/config.toml control_base_url https://control.flowhive.io
        """,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    if args.key is None:
        ...
        try:
            ...
            if not loaded_path:
                ...
                print("  flowhive config control_base_url <url>", file=sys.stderr)
                return 1
            else:
...
def main() -> int:
    ...
    if len(sys.argv) == 1:
        ...
        print("     flowhive config api_key <your-api-key>")
