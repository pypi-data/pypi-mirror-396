...
from .config import get_or_create_agent_key_pair, _get_host_unique_id
from .crypto import sign_agent_id
...
class ControlPlaneClient:
        ...
        if system == "Windows":
            ...
            try:
                ...
                if mac_ver:
                    ...
                    if major_version >= 13:
                        return (system, f"macOS {mac_ver} (Ventura or later)")
    ...
    def _build_headers(self) -> Dict[str, str]:
        ...
        headers = {
            ...
            "x-flowhive-api-key": self._config.api_key,
        }
        ...
        headers["x-flowhive-host-fingerprint"] = _get_host_unique_id()
        ...
        try:
            ...
            headers["x-flowhive-signature"] = signature
            headers["x-flowhive-timestamp"] = str(timestamp)
            headers["x-flowhive-public-key-b64"] = base64.urlsafe_b64encode( self._public_key_pem.encode("utf-8") ).decode("ascii")
            LOG.debug("Including cryptographic signature for identity verification")
        except Exception as e:
            LOG.warning("Failed to generate signature, identity verification disabled", error=str(e))
