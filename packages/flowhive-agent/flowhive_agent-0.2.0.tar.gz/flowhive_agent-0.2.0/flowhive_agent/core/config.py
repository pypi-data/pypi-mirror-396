...
DEFAULT_CONFIG_DIR = Path.home() / ".config" / "flowhive-agent"
SYSTEM_CONFIG_DIR = Path("/etc/flowhive-agent")
...
def get_config_dir(explicit: Optional[str] = None) -> Path:
    ...
    2. Environment variable path (FLOWHIVE_AGENT_CONFIG)
    3. User config directory (~/.config/flowhive-agent)
    4. System config directory (/etc/flowhive-agent)
