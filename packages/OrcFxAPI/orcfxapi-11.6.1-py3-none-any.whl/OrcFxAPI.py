...
def _supportsAntiAliasingMaxSampleCount() -> bool:
    return _checkVersion("11.6b")


def _supportsShadedProjectionMode() -> bool:
    return _checkVersion("11.6b")
...
@enum.unique
class ProjectionMode(IntEnumAllowsUndefined):
    Perspective = 0
    Parallel = 1
...
class ViewParameters(PackedStructureWithObjectHandles):
    ...
    if _supportsAntiAliasingMaxSampleCount():
        _fields_.extend(
            [("AntiAliasingMaxSampleCount", ctypes.c_int)]
        )
    if _supportsShadedProjectionMode():
        _fields_.extend(
            [("ShadedProjectionMode", ctypes.c_int)]
        )
