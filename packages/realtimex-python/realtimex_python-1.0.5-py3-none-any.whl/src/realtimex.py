...
class RealtimeX:
    ...
    def _get_auth_data(self, channel_name: str) -> Dict:
        """Get auth data from callback or HTTP endpoint."""
        socket_id = self.connection.socket_id or ''
        
        # Use custom auth_callback if provided
        if self.auth_callback:
            auth_result = self.auth_callback(channel_name, socket_id)
            return self._process_auth_result(auth_result, channel_name)
        
        # Use auth_endpoint if provided
        auth_endpoint = self.options.get('auth_endpoint')
        if auth_endpoint:
            auth_result = self._request_auth(auth_endpoint, channel_name, socket_id)
            return self._process_auth_result(auth_result, channel_name)
        
        # No auth method provided
        raise ValueError(f"Cannot subscribe to {channel_name}: auth_callback or auth_endpoint is required for private/presence channels")

    def _request_auth(self, endpoint: str, channel_name: str, socket_id: str) -> Dict:
    ...
    def _process_auth_result(self, auth_result, channel_name: str) -> Dict:
        """Process auth result from callback or HTTP."""
        auth_data = {}
    ...
    def _merge_options(self, options: RealtimeXOptions) -> RealtimeXOptions:
        defaults = {
            ...
            'ws_port': 443,
            'encrypted': True,
            'auth_endpoint': None  # Optional HTTP endpoint for auth
        }
