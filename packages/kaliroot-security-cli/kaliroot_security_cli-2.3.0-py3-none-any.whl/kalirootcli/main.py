...
def run_chat_session(chat_manager, session):
    ...
    while True:
        ...
        if session.messages:
            console.print("[dim]─── Historial ───[/dim]\n")
            for msg in session.messages[-6:]:  # Show last 6 messages
                role_style = "bold cyan" if msg["role"] == "user" else "bold magenta"
                role_label = "Tú" if msg["role"] == "user" else "KR-AI"
                console.print(f"[{role_style}]{role_label}:[/{role_style}] {msg['content'][:200]}")
...
Responde al último mensaje del usuario de forma natural y coherente con el contexto.
        ...
        if result["success"]:
            ...
            # Display response (no "Press Enter" pause)
            console.print(f"\n[bold magenta]KR-AI:[/bold magenta] {ai_response}\n")
