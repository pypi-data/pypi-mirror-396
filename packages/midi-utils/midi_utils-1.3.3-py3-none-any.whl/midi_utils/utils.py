...
def midi_to_freq(midi_note_num: int, tuning: Optional[str] = None) -> float:
    """Convert a MIDI note number to its corresponding frequency in Hz.

    Supports alternative tuning systems via asclpy. Defaults to 12-TET (standard tuning)
    for backwards compatibility.

    Args:
        midi_note_num: A MIDI note number between 0 and 127 (e.g., 69 for A4).
        tuning: Optional tuning system name from asclpy (e.g., '19_edo', 'just_c').
                If None, uses standard 12-TET tuning. See asclpy.list_tunings() for
                available tunings.

    Returns:
        float: Frequency in Hertz (e.g., 440.0 for MIDI note 69 in 12-TET).

    Raises:
        ValueError: If MIDI note number is not between 0 and 127, or if tuning
                    system is not supported.

    Examples:
        >>> midi_to_freq(69)  # Standard tuning (A4)
        440.0
        >>> midi_to_freq(69, tuning='19_edo')  # 19-tone equal temperament
        440.0
    """
    if tuning is None:
        # Use standard 12-TET tuning for backwards compatibility
        try:
            return MIDI_TO_FREQ[midi_note_num]
        except KeyError:
            raise ValueError(f"Invalid midi note num: {midi_note_num}")
    
    # Use asclpy for alternative tunings
    try:
        tuning_obj = asclpy.load_tuning(tuning)
        return tuning_obj.midi_to_freq(midi_note_num)
    except (KeyError, FileNotFoundError) as e:
        available = asclpy.list_tunings()
        raise ValueError(
            f"Unsupported tuning: '{tuning}'. "
            f"Available tunings: {', '.join(available[:5])}... "
            f"(see https://github.com/gltd/asclpy for full list)"
        ) from e
