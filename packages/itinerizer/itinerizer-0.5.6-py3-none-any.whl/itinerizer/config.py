...
from .constants import (
    Directories, FileExtensions, ConfigKeys, ConfigDefaults, Messages
)
...
class ItinerarizerConfig:
    ...
    storage_path: str
    backup_path: str
    web_ui_port: int = ConfigDefaults.WEB_UI_PORT
    api_port: int = ConfigDefaults.API_PORT
    log_level: str = ConfigDefaults.LOG_LEVEL
    auto_backup: bool = ConfigDefaults.AUTO_BACKUP
    backup_retention_days: int = ConfigDefaults.BACKUP_RETENTION_DAYS
...
class ConfigManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        """
        self.config_dir = config_dir or Path.cwd() / Directories.CONFIG_DIR
        self.config_file = self.config_dir / f"config{FileExtensions.JSON}"
    ...
    def _load_or_create_config(self) -> ItinerarizerConfig:
        ...
        if self.config_file.exists():
            try:
                ...
                print(Messages.Error.INVALID_CONFIG_FILE.format(e))

        # Create default configuration
        config = ItinerarizerConfig(
            storage_path=str(self.config_dir / Directories.ITINERARIES_DIR),
            backup_path=str(self.config_dir / Directories.BACKUPS_DIR)
        )
    ...
    def update_config(self, **kwargs) -> ItinerarizerConfig:
        ...
        if invalid_keys:
            raise ValueError(Messages.Error.INVALID_CONFIG_KEYS.format(', '.join(invalid_keys)))
...
def setup_local_storage(base_dir: Optional[Path] = None) -> tuple[str, str]:
    ...
    """
    config_manager = get_config_manager(base_dir / Directories.CONFIG_DIR if base_dir else None)
