...
def define_run(profilefile: io.TextIOWrapper, bash_options: list = [''], works: list = None,
    ...
    create_custom_group()
    profiling_call = f'likwid-perfctr -g ./CUSTOM_GROUP.txt -t 300s -O -f '
...
def create_custom_group():
    ...
    ls_results = subprocess.run(["ls", perfgroups_location], capture_output=True, text=True).stdout.strip().split('\n')
    memory_variables = {}
    memory_long_line = ""
    flop_variables = {}
    flop_metric_lines = []
                ...
                if "LONG" in line:
                    ...
                    if passed_long:
                        ...
                        if 'read' in line:
                            ...
                            prefix = ""
                        counter_variables += [prefix + v for v in output_of_interest.split("+")]
    ...
    long_memory_sum = "+".join(
        [v[:-1] + '*' for k, v in memory_variables.items() if k[-1] == '0' and not k[-2].isdigit()])
    group_doc_lines += [
        flop_long_line + "\n",
        memory_long_line + '\n',
        "-\n"
    ...
    with open(f"{os.getcwd()}/CUSTOM_GROUP.txt", 'w') as fp:
