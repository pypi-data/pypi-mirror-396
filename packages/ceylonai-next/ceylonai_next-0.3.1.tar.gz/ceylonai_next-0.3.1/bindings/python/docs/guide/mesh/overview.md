...
request_id = mesh.submit("assistant", "What is the capital of France?")

# Wait for the result (async result retrieval)
# This polls the mesh efficiently
result = mesh.wait_for(request_id)
...
req1 = mesh.submit("assistant", "Topic 1")
req2 = mesh.submit("assistant", "Topic 2")

# Wait for results
res1 = mesh.wait_for(req1)
res2 = mesh.wait_for(req2)
```
...
- `mesh.submit(receiver, message)`: Returns `MeshRequest` with `request_id`.
- `mesh.wait_for(request_object, timeout=30)`: Blocks until result available.
- `mesh.get_results()`: Returns list of completed `MeshResult`s.
- `mesh.collect_results(requests)`: Helper to wait for multiple requests.
