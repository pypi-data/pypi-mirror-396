...
class AcadDatabase(AppObject):
    ...
    def CopyObjects(self, Objects: In[vObjectArray], Owner: In[vObject] = None) -> vObjectArray: #AcadIDPair
        IDPairs = vObjectEmpty
        return vObjectArray(self._obj.CopyObjects(Objects, Owner(), IDPairs()))

    def HandleToObject(self, Handle: In[str]) -> AppObject: 
        return AppObject(self._obj.HandleToObject(Handle))

    def ObjectIdToObject(self, ID: In[int]) -> AppObject: 
