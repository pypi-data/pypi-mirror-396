...
def sanity_check_atmo(atmo):
    ...
    if any(np.gradient(P, z, edge_order=2) >= 0):
        raise Exception("Pressure derivative is not decreasing with height.")

    if any(np.gradient(z, edge_order=2) < 0):
        raise Exception("Height must be always increasing")
        
    # 3) Lapse rate
    Gamma = -np.gradient(T.loc[1000:30000], z.loc[1000:30000], edge_order=2) * 1000  # K/km

    if (Gamma > 25).any():
        raise Exception("⚠ Lapse rate > 25 K/km.")
    if (Gamma < -12).any():
        raise Exception("⚠ Strong inversion (< -12 K/km).")
...
def radiosonde(finput_rs, delimiter, skip_header, skip_footer, 
    ...
    paths = glob.glob(os.path.join(finput_rs,'*_*.txt')) + \
        glob.glob(os.path.join(finput_rs,'*_*.csv'))
    
    if len(paths) == 0:
        
        raise Exception(f"-- Error: No txt or csv file was found in the radiosonde folder: {finput_rs} Please make sure that the radiosonde files are in txt format")
