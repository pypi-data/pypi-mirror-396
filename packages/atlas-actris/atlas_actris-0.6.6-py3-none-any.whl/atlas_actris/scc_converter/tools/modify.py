...
def map_channels(a, b):
    """
    Map each channel in list a to exactly one channel in list b.

    Matching rule:
        b_entry matches a_entry if:
            - b_entry == a_entry
            OR
            - b_entry.startswith(a_entry + "_")

    Rules:
      1) a can be a subset or superset of b.
      2) Unmatched entries in b are allowed (no warning).
      3) Unmatched entries in a trigger a warning.
      4) If an entry of a has more than one possible match in b,
    ...
    for ch in a:
        # Find all possible matches in b
        candidates = [
            x for x in b
            if x == ch or x.startswith(ch + "_")
        ]

        if len(candidates) == 0:
            # No match for this a entry
            unmatched_a.append(ch)
        elif len(candidates) == 1:
            # Exactly one match → OK
            mapping[ch] = candidates[0]
        else:
            # More than one potential match → ERROR
            raise ValueError(
                f"More than one possible matches for recorder channel ID '{ch}': {candidates}"
            )

    # Rule 3: warn if some a entries could not be mapped
    if unmatched_a:
        print(
            f"--Warning: The following recorder channel IDs defined in the configuration file could not be mapped to the respective IDs in the raw files. These channels will be skipped: {unmatched_a}",
            UserWarning,
        )
...
def trim_channels(cfg, sig, shots, channel_info, meas_type):
    ...
    channel_map = map_channels(channel_ind_cfg, channel_ind_raw)
    
    channel_ind_com_cfg = [key for key in channel_map.keys()]
    channel_ind_com_raw = [key for key in channel_map.values()]
                            
    sig = sig.loc[dict(channel = channel_ind_com_raw)]
    
    shots = shots.loc[dict(channel = channel_ind_com_raw)]
    
    channel_info = channel_info.loc[channel_ind_com_raw,:]
    
    cfg.channels = cfg.channels.loc[channel_ind_com_cfg,:]
    
    sig = sig.assign_coords(channel=channel_ind_com_cfg)
    shots = shots.assign_coords(channel=channel_ind_com_cfg)
    channel_info.index = channel_ind_com_cfg
            ...
            if not np.isnan(shots_ch).all():
                ...
                if len(time[mask_t]) > 0:                
                    ...
                    for t in time[mask_t]:
                        ...
                        print(f'    Filename: {time_info.filename[ind_t]} | Channel: {ch} | Shots: {int(shots_ch[ind_t])} | Max shots: {int(np.nanmax(shots_ch))}')
...
def slice_in_time(sig_raw, shots, time_info, slice_reg):
    ...
    if s_slice != None and e_slice != None:
        ...
        if e_hour + e_mins / 60.  > s_hour + s_mins / 60.:
            ...
            e_dt = e_dt + dt.timedelta(days=1)
    
        time_info = time_info.loc[s_dt:e_dt].copy()
        sig_raw = sig_raw.loc[s_dt:e_dt,:,:].copy()
        shots = shots.loc[s_dt:e_dt,:].copy()
        
        if len(sig_raw.time) == 0:
            raise Exception(f"--Error: The slice_rayleigh option is out of the measurement range. No data to process\nslice_rayleigh = {slice_reg}")
