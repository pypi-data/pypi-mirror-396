...
def find_lower_indices(H, heights):
    ...
    # Ensure H is sorted (required for searchsorted)
    # Remove these two lines if you KNOW H is sorted ascending.
    # s = np.argsort(H)
    # H = H[s]
...
# def hypsometric_pressure_interpolation(H, P, T, heights):
    
#     g = 9.80665      # m/s^2
#     R_d = 287.0      # J/(kg*K) for dry air

#     ind_H = find_lower_indices(H, heights)
    
#     T_der = np.diff(T, append = heights[-1]) / np.diff(H, append = H[1] - H[0])
...
#     P_log = np.log(P_i) - T_der_i * (g / R_d) * np.log(T_der_i * (heights - H_i) / T_i + 1.)
    
#     P_interp = np.exp(P_log)
