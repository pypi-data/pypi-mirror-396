...
class LoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Gather client info and dynamic fields before creating task
        ...
        # Calculate response time before creating task
        response_time = datetime.now().timestamp() - request.state.start_time

        # Fire-and-forget response logging (non-blocking)
        asyncio.create_task(
            self._log_response(
                correlation_id=correlation_id,
    ...
    async def _log_request(
        ...
        """Log request in background without blocking."""
        try:
            logger.info(
            ...
            pass  # Silently ignore logging errors to not affect request flow
    ...
    async def _log_response(
        ...
        """Log response in background without blocking."""
        try:
            dynamic_fields = await get_dynamic_fields()
            logger.info(
                msg="response_sent",
