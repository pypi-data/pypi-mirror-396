...
def is_date(value: Any) -> bool:
    """
    Returns True if value is a date or a date-like string that can be parsed.
    """
    ...
    if isinstance(value, str):
        value = value.strip()
        try:
            pd.to_datetime(value, errors="raise", dayfirst=False)
            return True
        except Exception:
            return False
...
def extract_date_string(value: Any) -> str:
    """
    Convert Timestamp / datetime / date / date-like string to 'YYYY-MM-DD'.
    """
    ...
    raise RuntimeError(f"The provided value is not date-like: {value!r} (type {type(value)})")
