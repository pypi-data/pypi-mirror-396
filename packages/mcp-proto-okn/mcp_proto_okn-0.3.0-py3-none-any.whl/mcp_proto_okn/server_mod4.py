...
class SPARQLServer:
    ...
    def execute(self, query_string: str, format: str = 'compact') -> Union[Dict[str, Any], str]:
        ...
        try:
            ...
            if format == 'simplified':
                ...
                # Return columns array + data arrays (no repeated keys)
    ...
    def query_schema(self, compact: bool = False) -> Dict[str, Any]:
        ...
        # Always query for classes and predicates to know what exists
#         class_query = """
...
WHERE {{
  ?s ?predicate ?o .
        ...
        # Full format: columns + data arrays with descriptions
        if len(metadata) > 0:
            class_data = []
            for uri in class_uris:
                label = metadata.get(uri, {}).get('label', uri.split('/')[-1].split('#')[-1])
                description = metadata.get(uri, {}).get('description', '')
                class_data.append([uri, label, description])
            
            predicate_data = []
            for uri in predicate_uris:
                label = metadata.get(uri, {}).get('label', uri.split('/')[-1].split('#')[-1])
                description = metadata.get(uri, {}).get('description', '')
                predicate_data.append([uri, label, description])
            
            return {
                'classes': {
                    'columns': ['uri', 'label', 'description'],
                    'data': class_data,
            ...
            # No metadata available, just URIs
