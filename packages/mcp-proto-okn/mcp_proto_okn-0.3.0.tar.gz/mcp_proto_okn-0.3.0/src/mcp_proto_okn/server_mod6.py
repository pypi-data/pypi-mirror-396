...
"""
    ...
    # Add prompt for class diagram creation
    @mcp.prompt()
    def create_class_diagram() -> str:
        """Guidelines for creating class diagrams from knowledge graph schemas.
        
        Returns instructions for generating clean, well-formatted class diagrams.
        """
        return """Create a class diagram from the knowledge graph schema following these guidelines:

1. **NEVER use note boxes**: ABSOLUTELY DO NOT include any `note for` or `note` statements in the Mermaid diagram. Yellow note boxes should NEVER be rendered under any circumstances. This is a critical requirement.
2. **Clean syntax**: Remove curly braces from classes without properties
3. **Separate relationship properties**: Do not append relationship properties to relationship names. Instead, render them separately using dotted lines (e.g., `Assay ..> Gene : log2fc`)
4. **No styles**: Do not style the entity boxes in different colors
5. **Validation**: Check the Mermaid syntax for correctness before rendering
6. **Mermaid file**: Generate the output in .mermaid format, not in .svg or .png!

CRITICAL: The diagram must NOT contain any of the following:
- `note for ClassName "text"`
- `note "text"`
- `note left of`
- `note right of`
- Any other note syntax

If you need to show additional properties or metadata, use dotted relationship lines instead (e.g., `Class1 ..> Class2 : property1, property2`).

Steps:
1. Call get_schema() to retrieve the knowledge graph schema
2. Identify classes and their properties
3. Identify relationships (predicates) between classes
4. Create a Mermaid class diagram following the guidelines above
5. Remove any lines that start with `note` from the .mermaid file
6. Save the diagram to a .mermaid file
"""
