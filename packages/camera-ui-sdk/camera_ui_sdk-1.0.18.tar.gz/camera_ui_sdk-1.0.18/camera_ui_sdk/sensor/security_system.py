...
class SecuritySystem(Sensor[SecuritySystemProperties, TStorage, str], Generic[TStorage]):
    ...
    @abstractmethod
    async def setTargetState(self, value: int | SecuritySystemState) -> None:
        """
        ...
        Example:
            async def setTargetState(self, value: int | SecuritySystemState) -> None:
                state = SecuritySystemState(value) if isinstance(value, int) else value
                await self.alarm_panel.arm(state)
                self.targetState = state
                self.currentState = state
        """
        ...

    async def setCurrentState(self, value: int | SecuritySystemState) -> None:
        """
        Set current state (for plugin use - e.g., when alarm is triggered).
        ...
        """
        state = SecuritySystemState(value) if isinstance(value, int) else value  # pyright: ignore[reportUnnecessaryIsInstance]
        self.currentState = state
