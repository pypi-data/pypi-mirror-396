...
class ModelSpec(TypedDict):
    """
    Model specification for video detector sensors.

    Defines input format, output labels, and optional trigger labels for cascade detection.

    Example:
        # Object Detection (no triggerLabels - runs after motion)
        object_model_spec: ModelSpec = {
            "input": {"width": 640, "height": 640, "format": "rgb"},
            "outputLabels": ["person", "car", "dog", ...],
        }
    ...
    triggerLabels: NotRequired[list[str]]  # Labels from Object Detection that trigger this detector
...
class StoredSensorData(TypedDict):
    ...
    modelSpec: NotRequired[ModelSpec]  # Model specification for detector sensors
