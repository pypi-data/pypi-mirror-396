...
class NodeJSMCPModelClass(MCPModelClass):
    ...
    def _find_config_file(self) -> Optional[str]:
        ...
        Looks for config.yaml relative to where the subclass is defined,
        not the current working directory. This allows the base class to
        find config.yaml when used as a base class.
        """
        ...
        try:
            ...
            logger.info(f"Class file location: {class_file}")
            logger.info(f"Class directory: {class_dir}")
            
            # Look for config.yaml in the same directory as the class file
            config_path = os.path.join(os.path.dirname(class_dir), "config.yaml")
            logger.info(f"finding config.yaml at {config_path}")
            if os.path.exists(config_path):
                logger.info(f"config.yaml found at {config_path}")
                return config_path
        ...
        logger.info(f"Falling back to current directory: {current_dir}")
