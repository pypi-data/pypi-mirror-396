...
class RegistryPackage(Package[ArchiveDownloadAndExtractInstaller]):
	...
	def _get_installer(A,version:str)->ArchiveDownloadAndExtractInstaller:return RegistryPackageInstaller('docker-registry',version)
class RegistryPackageInstaller(ArchiveDownloadAndExtractInstaller):
	def _get_install_marker_path(B,install_dir:str)->str:A=f"{platform.system().lower()}-{get_arch()}";return os.path.join(install_dir,f"registry.{A}")
	def _get_download_url(C)->str:
	...
	def _install(A,target:InstallTarget)->None:
		B=target
