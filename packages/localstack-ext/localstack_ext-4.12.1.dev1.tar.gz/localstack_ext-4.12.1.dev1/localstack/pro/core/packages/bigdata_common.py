...
def download_and_cache_jar_file(jar_url:str,cache_dir:str,target_dir:str)->str:
	...
	cp_r(A,B);return A
def bigdata_jar_cache_dir(target:InstallTarget)->str:return os.path.join(target.value,'bigdata-jars')
def replace_in_zip_file(zip_file:str,file_path:str,search_replace:dict[str,str],raise_if_missing:bool=False):
...
def replace_in_file(file_path:str,search_replace:dict[str,str]):
