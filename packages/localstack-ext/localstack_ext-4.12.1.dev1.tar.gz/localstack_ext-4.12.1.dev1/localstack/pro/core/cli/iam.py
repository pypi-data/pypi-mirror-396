_E='Invalid response from the LocalStack instance.\nPlease update your LocalStack instance!'
_D='Unable to connect to the LocalStack Pro instance.\nPlease make sure you have an instance up and running!'
_C='Please perform request against LocalStack to start seeing policies here. Waiting for policies...'
_B='plain'
_A=None
...
def _print_plain(generated_policy:dict[str,Any]):A=generated_policy;console.print(f'Attached to {A["policy_type"]}: "{A["resource"]}"');console.line();console.print('Policy: ');console.print_json(data=A['policy_document']);console.line();console.rule();console.line()
def print_generated_policy_plain(generated_policy:bytes):A=generated_policy;A=json.loads(to_str(A));_print_plain(A)
def print_generated_policy_json(generated_policy:bytes):console.print(to_str(generated_policy),highlight=False)
...
@click.option('-f','--format','format_',type=click.Choice([_B,'json']),default=_B,help='The formatting style for the command output. Use plain if it should be human readable, and json to get a newline-separated list of json documents.')
@publish_invocation
def cmd_iam_stream(format_:str)->_A:
	A=format_
	try:
		with requests.get(f"{get_iam_endpoint()}/policies/stream",stream=True)as C:
			D=_C;console.print(D)
			for B in C.iter_lines():
				if A==_B:print_generated_policy_plain(B)
				elif A=='json':print_generated_policy_json(B)
	except requests.ConnectionError:raise ClickException(_D)
	except json.JSONDecodeError:raise ClickException(_E)
...
@publish_invocation
def cmd_iam_summary(output:str|_A,follow:bool)->_A:
	D=output;A=_A
	try:
		if follow:
