...
def get_application(package_name: str | None = None,
    ...
    async def _encryption_key(app: web.Application) -> AsyncIterator[None]:
        encryption_key = config.parsed_config['DEFAULT'].get('EncryptionKey', None) if config else None
        app[appproperty.HEA_ENCRYPTION_KEY] = encryption_key
        yield

    app.cleanup_ctx.append(_encryption_key)
