    ...
    if cached_value is not None:
        ...
        async def fetch_from_mongo(mongo: Mongo) -> DesktopObjectDict | None:
            ...
            if result is not None:
                obj = _desktop_object_from_dict(request.app, result)
...
async def get_desktop_object(request: Request, collection: str, *,
    ...
    return _dict_to_desktop_object(request.app, obj_dict, type_=type_, obj=obj)
    ...
    async with MongoContext(request, volume_id) as mongo:
        try:
            result = await mongo.post(request, obj, collection, default_content,
                                      encryption=_get_encryption(request.app))
            to_delete = []
...
async def post_dict_return_id(request: Request, obj_dict: DesktopObjectDict, collection: str,
    ...
    try:
        obj = _desktop_object_from_dict(request.app, obj_dict, type_=type_)
    except (DeserializeException, ValueError, TypeError) as e:
        raise response.status_bad_request(str(e))
    async with MongoContext(request, volume_id) as mongo:
        try:
            return await mongo.post(request, obj, collection, default_content, encryption=_get_encryption(request.app))
        except DuplicateKeyError as e:
    ...
    async with MongoContext(request, volume_id) as mongo:
        ...
        try:
            ...
            result = await mongo.put(request, obj_, collection, context=context,
                                     encryption=_get_encryption(request.app))  # if lacks permissions or object is not in database, then updates no records.
...
async def aggregate(request: Request, collection: str,
    ...
    async with MongoContext(request, volume_id) as mongo:
        ...
        try:
            context = mongo.get_default_permission_context(request)
            async for r in agg:
                obj = _desktop_object_from_dict(request.app, r)
                permitted = await RESTPermissionGroup.GETTER_PERMS.has_any(obj, context)
                if permitted:
    ...
    async def fetch_objects(mongo: Mongo) -> AsyncGenerator[DesktopObject, None]:
        ...
        try:
            async for desktop_object_dict in gen:
                obj = _desktop_object_from_dict(request.app, desktop_object_dict, type_=type_)
...
@overload
def _dict_to_desktop_object(app: Application, obj_dict: None, *, type_: type[DesktopObject],
                            obj: DesktopObject | None = None) -> None:
    ...


@overload
def _dict_to_desktop_object(app: Application, obj_dict: DesktopObjectDict, *,
                            obj: DesktopObject | None = None) -> DesktopObject:
    ...


@overload
def _dict_to_desktop_object(app: Application, obj_dict: DesktopObjectDict, *, type_: type[DesktopObjectTypeVar],
                            obj: DesktopObjectTypeVar | None = None) -> DesktopObjectTypeVar:
    ...


def _dict_to_desktop_object(app: Application, obj_dict: DesktopObjectDict | None, *, type_ = DesktopObject, obj:
    ...
    if obj_dict is None:
        ...
        return _desktop_object_from_dict(app, obj_dict, type_=type_)


def _get_encryption(app: Application) -> Optional[Encryption]:
    """
    Returns the Encryption instance for the app, if it exists.

    :param app: the Application.
    :return: the Encryption instance, or None if no encryption key is set.
    """
    encryption_key = app[appproperty.HEA_ENCRYPTION_KEY]
    if encryption_key:
        return Encryption(key=encryption_key)
    else:
        return None


def _desktop_object_from_dict(app: Application, d: DesktopObjectDict | DesktopObjectDictIncludesEncrypted,
    ...
    """
    return desktop_object_from_dict(d, type_=type_, encryption=_get_encryption(app))
