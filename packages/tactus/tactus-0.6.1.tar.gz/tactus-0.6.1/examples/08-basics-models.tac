...
-- Meta Llama Models via Bedrock (don't support tools in streaming mode)
agent("llama_8b", {
    provider = "bedrock",
    ...
    initial_message = common_prompt,
    tools = {"done"},
    disable_streaming = true,  -- Llama doesn't support tools in streaming mode
    model_settings = {
        temperature = 0.7,
    }
})
...
agent("llama_3b", {
    ...
    system_prompt = "You are a physics expert. Be concise and accurate.",
    initial_message = common_prompt,
    -- No tools specified - this model doesn't support tool calling
    model_settings = {
        temperature = 0.7,
    }
})
...
agent("nova_micro", {
    ...
    system_prompt = "You are a physics expert. Be concise and accurate.",
    initial_message = common_prompt,
    -- No tools - using direct response mode
    model_settings = {
        temperature = 0.7,
    }
})
...
procedure(function()
    ...
    results.claude_haiku = run_agent_with_tools(Claude_haiku, "Claude 4.5 Haiku")
    results.llama_8b = run_agent_with_tools(Llama_8b, "Llama 3.1 8B")
    
    -- Run models without tool support (direct response)
