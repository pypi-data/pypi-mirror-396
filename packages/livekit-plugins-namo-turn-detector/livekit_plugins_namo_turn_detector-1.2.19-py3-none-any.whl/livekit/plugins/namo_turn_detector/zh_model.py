...
HF_REPO = "videosdk-live/Namo-Turn-Detector-v1-Chinese"
LANGUAGE_CODE = "zh"


class _NamoTurnRunnerChinese(_InferenceRunner):
    """Inference runner for Chinese turn detection model."""

    INFERENCE_METHOD = "namo_turn_detection_zh"
    ...
    def initialize(self) -> None:
        ...
        logger.info(f"Loading Chinese turn detection model from {HF_REPO}")
    ...
    def run(self, data: bytes) -> bytes | None:
        """Execute inference on Chinese text."""
        try:
            ...
            logger.debug(
                f"Chinese turn detection completed",
                extra={
            ...
            logger.error(f"Error during Chinese turn detection: {e}")
            return json.dumps({"probability": 0.0, "error": str(e)}).encode()


_InferenceRunner.register_runner(_NamoTurnRunnerChinese)


class ChineseModel(NamoModelBase):
    """
    Chinese-specific Namo Turn Detection model.

    This model loads only the Chinese model file for better memory efficiency.
    """
    ...
    def _inference_method(self) -> str:
        ...
        return _NamoTurnRunnerChinese.INFERENCE_METHOD
