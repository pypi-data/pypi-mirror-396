...
raw_scope_config = CONFIG.get("MS.GRAPH_SCOPES", "https://graph.microsoft.com/.default")
...
def _is_graph_access_token(token: str) -> bool:
    """Check if the provided token is already a Microsoft Graph access token."""
    try:
        payload = _decode_jwt_payload(token)
        aud = str(payload.get("aud") or "").lower()
        scopes = str(payload.get("scp") or payload.get("scope") or "")
        ...
        if "graph.microsoft.com" in scopes.lower():
            return True
    except Exception:
        return False
    return False
...
def get_on_behalf_of_token(user_assertion: str) -> str:
    ...
    if _is_graph_access_token(normalized_assertion):
