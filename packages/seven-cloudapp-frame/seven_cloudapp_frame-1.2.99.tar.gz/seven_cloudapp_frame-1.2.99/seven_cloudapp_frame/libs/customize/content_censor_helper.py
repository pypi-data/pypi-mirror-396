    ...
    def image_censor(self, img_list, labels=None, result=None, score=None, biz_type=None, is_async=None, callback=None, freeze=None, is_in=False):
        ...
        def should_add_to_fail_list(item, is_in, labels=None, result=None, score=None):
            ...
            if is_in:
                ...
                result_cond = (result and result_val in result) and (score is not None and item_score is not None and item_score <= score)
            else:
                # 排除模式
                label_cond = labels and label not in labels
                result_cond = (result and result_val not in result) and (score is not None and item_score is not None and item_score > score)
            return any([label_cond, result_cond])
