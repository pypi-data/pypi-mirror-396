...
class TrialHook:
        ...
        for trial in all_trials:
            ...
            try:
                ...
                with mlflow.start_run(run_name=run_name, nested=True) as trial_run:
                    ...
                    user_uin = os.environ.get("QCLOUD_UIN") or os.environ.get("USER_UIN", "")
                    if user_uin:
                ...
                # 🆕 为子 run 设置 mlflow.source.name tag（必须设置，否则 WeData 服务器可能会清理掉）
                # 注意：不再删除这个 tag，而是确保所有子 runs 都有这个 tag
                try:
                    client = mlflow.tracking.MlflowClient()
                    client.set_tag(trial_run_id, "mlflow.source.name", "wedata-automl-trial")
                except Exception:
                    pass  # 忽略设置失败的错误
    ...
    def cleanup_child_runs_source_name(self, experiment_id: str, preserve_best: bool = True) -> int:
        ...
        try:
            ...
            # 🆕 不再删除 mlflow.source.name tag，因为 WeData 服务器可能会清理没有这个 tag 的 runs
            # 改为：确保最佳子 run 有正确的 source.name，其他子 runs 保持 "wedata-automl-trial"
            for run_id in child_run_ids:
                # 如果是最佳子 run，设置为 "wedata-automl"
                if run_id == self.best_trial_run_id:
                    try:
                        client.set_tag(run_id, "mlflow.source.name", "wedata-automl")
                        cleaned_count += 1
                    except Exception:
                        pass
                # 其他子 runs 保持 "wedata-automl-trial"（已在 _create_trial_runs 中设置）

            if cleaned_count > 0:
                safe_print(f"✅ Updated mlflow.source.name for best trial run")
        except Exception as e:
