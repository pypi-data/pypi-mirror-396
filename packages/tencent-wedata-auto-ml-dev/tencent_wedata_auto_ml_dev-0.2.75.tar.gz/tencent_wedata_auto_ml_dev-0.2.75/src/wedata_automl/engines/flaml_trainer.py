...
def _get_wedata_tags(task: str = "classification") -> Dict[str, str]:
    ...
    user_uin = os.environ.get("QCLOUD_UIN", "")
...
class FLAMLTrainer:
    ...
    def _log_forecast_model(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            model_info = {
                "task": self.task,
                "estimator": self.automl.best_estimator if hasattr(self.automl, 'best_estimator') else "unknown",
                "horizon": self.kwargs.get("horizon"),
                "frequency": self.kwargs.get("frequency"),
                "time_col": self.kwargs.get("time_col"),
            }
            mlflow.log_dict(model_info, "model/model_info.json")

        model_uri = f"runs:/{parent_run_id}/model"
        safe_print(f"✅ Forecast model logged to MLflow: {model_uri}")
        ...
        if registered_model_name:
            try:
                ...
                safe_print(f"⚠️  Model registration failed: {e}")
                mlflow.set_tag("wedata.has_registered_model", "false")
        else:
            safe_print(f"ℹ️  Model not registered (register_model={self.register_model}, model_name={self.model_name})")
            mlflow.set_tag("wedata.has_registered_model", "false")
    ...
    def _save_forecast_predictions(
        ...
        try:
            ...
            if predictions is None:
                predictions = self.automl.predict(pdf)
                predictions = predictions[:horizon] if len(predictions) >= horizon else predictions
    ...
    def _prepare_forecast_data(self, settings: dict) -> dict:
        ...
        # 按时间排序
        train_df_for_flaml = train_df_for_flaml.sort_values(by=time_col).reset_index(drop=True)
        ...
        safe_print(f"  Frequency: {frequency}")
    ...
    def _fill_missing_timestamps(self, df: pd.DataFrame, time_col: str, frequency: str) -> pd.DataFrame:
        """
        填充缺失的时间点（参考 Databricks/Azure AutoML 策略）
        ...
        if missing_dates:
            ...
            safe_print(f"✅ No missing time points detected (frequency: {frequency})")
