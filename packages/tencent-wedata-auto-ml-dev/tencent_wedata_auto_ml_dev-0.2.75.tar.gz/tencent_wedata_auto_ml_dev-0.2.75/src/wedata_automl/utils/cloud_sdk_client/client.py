...
class FeatureCloudSDK:
    ...
    def CreateExploreScript(self, request: models.CreateExploreScriptRequest) -> 'models.CreateExploreScriptResponse':
        """
        创建数据探索脚本
        Args:
            request: 创建请求参数
        ...
        logger = get_logger()
        if is_mock():
            logger.debug("Mock CreateExploreScript API")
            return models.CreateExploreScriptResponse()
        try:
            params = request._serialize()
            headers = set_request_header(request.headers)
            print(f"CreateExploreScript headers: {headers}")
            print(f"CreateExploreScript params: {params}")
            logger.debug(f"CreateExploreScript params: {params}")
            logger.debug(f"CreateExploreScript headers: {headers}")
            self._client._apiVersion = "2021-08-20"
            body = self._client.call("CreateExploreScript", params, headers=headers)
            response = json.loads(body)
            model = models.CreateExploreScriptResponse()
            model._deserialize(response["Response"])
            logger.debug(f"CreateExploreScript Response: {response}")
            return model
        except Exception as e:
            if isinstance(e, TencentCloudSDKException):
                raise
