...
def generate_env(env_name, config_path="project.yaml", output_path=".env"):
    ...
    def resolve(key, required_in_prod=True, default=""):
        ...
        - env_overrides (project.yaml)
        - lokalen defaults (local/.env-Defaults)
        - optional OS-Umgebung
        - oder (falls nötig) get_secret()
        """
        # 1) Override aus project.yaml
        if key in env_overrides:
            return env_overrides[key]

        # 2) Lokale Defaults (für ENV=local)
        if is_local:
            return local_defaults.get(key, default)

        # 3) Direktes OS-Environment (z.B. aus CI)
        env_val = os.environ.get(key)
        if env_val:
            return env_val

        # 4) Fallback auf default, falls angegeben
        if default != "":
            return default

        # 5) Secret aus Umgebung holen (kann required sein)
    ...
    # OPTIONALE SECRETS: Nur schreiben, wenn ein Wert gefunden wurde!
    # Das verhindert, dass eine leere Zeile in .env einen Wert aus .env.local überschreibt.
    
    mui_key = resolve('VITE_APP_MUI_LICENSE_KEY', required_in_prod=False)
    if mui_key:
        env_content.append(f"VITE_APP_MUI_LICENSE_KEY={mui_key}")

    shared_secret = resolve('SYNC_SHARED_SECRET', required_in_prod=False)
    if shared_secret:
        env_content.append(f"SYNC_SHARED_SECRET={shared_secret}")

    deepl_key = resolve('DEEPL_API_KEY', required_in_prod=False)
    if deepl_key:
        env_content.append(f"DEEPL_API_KEY={deepl_key}")

    origin_id = resolve('ORIGIN_SERVER_ID', required_in_prod=False)
    if origin_id:
        env_content.append(f"ORIGIN_SERVER_ID={origin_id}")
