...
def generate_env(env_name, config_path="project.yaml", output_path=".env"):
    ...
    def resolve(key, required_in_prod=True):
        # 1) env_overrides aus project.yaml hat immer Vorrang
        if key in env_overrides:
            return env_overrides[key]

        # 2) lokales Environment: zuerst defaults (falls gewollt), dann echte Env
        if is_local:
            # Defaults aus project.yaml (für unkritische Sachen wie Ports etc.)
            val = local_defaults.get(key)
            if val not in (None, ""):
                return val

            # Fallback: echte Environment-Variable (z. B. aus Secret-File per `source`)
            return os.environ.get(key, "")

        # 3) nicht-local (staging/production): Secrets aus CI / Server
