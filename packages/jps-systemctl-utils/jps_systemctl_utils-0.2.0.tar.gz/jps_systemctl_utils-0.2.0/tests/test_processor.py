...
from jps_systemctl_utils.processor import process_services, append_file, run_cmd
...
def test_process_services_processes_all_services(mock_logger, mock_append, mock_run, temp_report_file):
    """Test that process_services processes all services in the list."""
...
@patch('jps_systemctl_utils.processor.run_cmd')
@patch('jps_systemctl_utils.processor.append_file')
def test_process_services_skips_empty_service_names(mock_append, mock_run, temp_report_file):
    """Test that process_services skips empty service names."""
    services = ["nginx", "", "  ", "apache2"]
    mock_run.return_value = (0, "output", "")
    
    process_services(services, temp_report_file, dryrun=False)
    
    # Should only process 2 services (nginx and apache2)
    # Each service runs 4 commands
