"""Tests for restart_services module."""
...
from jps_systemctl_utils.restart_services import app, build_default_paths
...
@pytest.fixture
def temp_yaml_infile(tmp_path):
    """Create a temporary YAML input file with safety checks."""
    infile = tmp_path / "services.yaml"
    yaml_content = """---
nginx.service:
  - pgrep nginx
apache2.service:
...
mysql.service:
  - echo test
"""
    infile.write_text(yaml_content)
    return infile


@pytest.fixture
def temp_yaml_infile_no_checks(tmp_path):
    """Create a temporary YAML input file without safety checks."""
    infile = tmp_path / "services_simple.yaml"
    yaml_content = """---
nginx.service:
apache2.service:
mysql.service:
...
def test_build_default_paths_creates_timestamped_directory():
    ...
    script_path = Path("/usr/local/bin/restart_services.py")
    user = "testuser"
    
    outdir, report, logfile = build_default_paths(script_path, user)
    
    assert "/tmp/testuser/restart_services/" in str(outdir)
...
@patch('jps_systemctl_utils.restart_services.datetime')
...
@patch('jps_systemctl_utils.restart_services.process_services')
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_infile_only(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
    """Test main with only infile parameter (uses defaults)."""
    result = runner.invoke(app, ["--infile", str(temp_yaml_infile_no_checks)])
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_custom_outdir(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks, temp_outdir):
    """Test main with custom output directory."""
    result = runner.invoke(app, ["--infile", str(temp_yaml_infile_no_checks), "--outdir", str(temp_outdir)])
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_custom_report_file(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks, tmp_path):
    """Test main with custom report file."""
    custom_report = tmp_path / "my_report.txt"
    
    result = runner.invoke(app, [
        "--infile", str(temp_yaml_infile_no_checks),
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_custom_logfile(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks, tmp_path):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_dryrun_flag(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
    """Test main with dryrun flag."""
    result = runner.invoke(app, ["--infile", str(temp_yaml_infile_no_checks), "--dryrun"])
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_without_dryrun_flag(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_reads_yaml_file(mock_setup, mock_write, mock_process, temp_yaml_infile):
    """Test that main reads and parses YAML files."""
    result = runner.invoke(app, ["--infile", str(temp_yaml_infile)])
    
    assert result.exit_code == 0
    # Check that process_services was called with dict
    call_args = mock_process.call_args[0]
    services = call_args[0]
    assert isinstance(services, dict)
    assert "nginx.service" in services
    assert isinstance(services["nginx.service"], list)


@patch('jps_systemctl_utils.restart_services.process_services')
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_reads_yaml_file_without_safety_checks(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
    """Test that main handles YAML files without safety checks."""
    result = runner.invoke(app, ["--infile", str(temp_yaml_infile_no_checks)])
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_calls_setup_logging(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_calls_write_header(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_calls_process_services(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_outdir_creates_report_in_outdir(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks, temp_outdir):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_with_outdir_creates_logfile_in_outdir(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks, temp_outdir):
...
@patch('jps_systemctl_utils.restart_services.write_header')
@patch('jps_systemctl_utils.restart_services.setup_logging')
def test_main_preserves_explicit_report_file_with_outdir(mock_setup, mock_write, mock_process, temp_yaml_infile_no_checks, temp_outdir, tmp_path):
...
@patch('jps_systemctl_utils.restart_services.getpass.getuser')
def test_main_uses_current_user(mock_getuser, temp_yaml_infile_no_checks):
    """Test that main uses current user for default paths."""
    mock_getuser.return_value = "testuser"
    
    with patch('jps_systemctl_utils.restart_services.setup_logging'), \
         patch('jps_systemctl_utils.restart_services.write_header'), \
         patch('jps_systemctl_utils.restart_services.process_services'):
