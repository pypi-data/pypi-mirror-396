...
class AgentBillCallback(BaseCallbackHandler):
    ...
    def __init__(
        ...
        if self.debug:
            ...
            if enable_cost_guard and customer_id:
                print(f"[AgentBill] Cost Guard enabled - will validate before LLM calls")
            elif not customer_id:
                print(f"[AgentBill] ⚠️ No customer_id - Cost Guard validation disabled")
    ...
    def _validate_request(self, model: str, prompts: List[str]) -> Dict:
        ...
        """
        if not self.enable_cost_guard or not self.customer_id:
            return {"allowed": True}
    ...
    def on_llm_start(
        ...
        **kwargs: Any
        ...
        # CRITICAL: Validate BEFORE storing run info (pre-flight check)
        if self.enable_cost_guard and self.customer_id:
