...
`stockdata77` provides `Stocks` class which facilitates interfacing with real-time information providers of securities trading data. Currently it supports APIs of Financial Modeling Prep, Alpha Vantage and MOEX. It is designed primarily to be used with stock and ETF symbols, though FMP will also provide FOREX data.

Please note that FMP and AV providers will require an API key. Also note that Yahoo Finance API support has been discontinued since version 0.0.5 as the provider has closed the access to its API.
...
> Note: As of October 2023 Yahoo Finance API was not operational. The examples below use "AV" simply to avoid using a specific `api_key` paramater as AV API works fine with just something like "KEY" for demo purposes.
