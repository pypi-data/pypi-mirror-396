...
try:
    ...
    import mysql.errors as dberror
except ImportError as err:
    logging.info("Failed from mysql import connector")
...
class TestMysqlHandlerInit:
    ...
    def test__init__cnx(self, mysql_options):
        cnx = dbconnector.connect(**mysql_options)
    ...
    def test_connection_error(self, k, v, mysql_options):
        ...
        with pytest.raises(dberror.Error):
...
class TestMysqlHandlerPopulated:
    ...
    def test_fetchone_param_table_name(self, fixture, mysql_handler, populate):
        ...
        with pytest.raises(dberror.ProgrammingError):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # dbconnector.errors.Error,
            dberror.IntegrityError,
            dberror.InterfaceError,
            dberror.InternalError,
            dberror.NotSupportedError,
            dberror.OperationalError,
            dberror.ProgrammingError,
            dberror.Warning,
        ],
    )
...
class TestMysqlExceptionHandling:
    ...
    def test_context_manager(self, caplog, mysql_options):
        ...
        msg = "hi"
        errno = 123
        error = dbconnector.Error(msg, 123)
        with pytest.raises(dbconnector.Error, match=msg) as cm_ex:
            with MysqlHandler(mysql_options):
                raise dbconnector.Error(msg=msg, errno=errno)
            # Exception
            assert cm_ex.type == dbconnector.Error
    ...
    def test_execute_exception(self, caplog, mysql_options):
        caplog.set_level(logging.INFO)
        with pytest.raises(dbconnector.Error) as cm_ex:
    ...
    def test_insert_select_on_duplicate_key_update_exception(self, caplog, mysql_options):
        ...
        with pytest.raises(dbconnector.Error) as cm_ex, MysqlHandler(mysql_options) as mh:
            mh.insert_on_duplicate_key_update(table_from, table_into, colmap, keys)
        # Exception
        assert cm_ex.type == dberror.InterfaceError
        # Logging
        for record in caplog.records:
            assert record.levelname == "CRITICAL"
