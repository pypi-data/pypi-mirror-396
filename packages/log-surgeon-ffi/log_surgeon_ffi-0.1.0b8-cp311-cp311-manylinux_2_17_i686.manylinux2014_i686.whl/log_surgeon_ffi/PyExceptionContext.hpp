#ifndef LOG_SURGEON_FFI_PYEXCEPTIONCONTEXT_HPP
#define LOG_SURGEON_FFI_PYEXCEPTIONCONTEXT_HPP

#include <wrapped_facade_headers/Python.hpp>

namespace log_surgeon_ffi {
/**
 * Class to get/set Python exception context, designed to capture the current
 * exception state upon instantiation and provide a restore API that allows
 * users to reinstate the exception context when needed. Doc:
...
public:
     ...
     * - If there's no exception in the stored context, the error indicator will
     * be cleared.
     * - This method should be called strictly once, otherwise the error indicator
     * will be cleared.
...
#endif  // LOG_SURGEON_FFI_PYEXCEPTIONCONTEXT_HPP
