#ifndef LOG_SURGEON_FFI_PY_OBJECT_CAST_HPP
#define LOG_SURGEON_FFI_PY_OBJECT_CAST_HPP
...
/**
 * Casts a given function pointer to a `getbufferproc` CPython function. The
 * main purpose of this function is to silence clang-tidy checks on using
 * `reinterpret_cast`. It does not perform any further function type checking.
 * @tparam Src The source function pointer type.
 * @param src The source function pointer.
...
/**
 * Casts a given function pointer to a `releasebufferproc` CPython function. The
 * main purpose of this function is to silence clang-tidy checks on using
 * `reinterpret_cast`. It does not perform any further function type checking.
 * @tparam Src The source function pointer type.
 * @param src The source function pointer.
 ...
 * `IsPythonObject`, which is used to determine whether a type T is a valid
 * Python object type.
 * @tparam T
 */
...
/**
 * The macro to create a specialization of `IsPythonObject` for a given type T.
 * Only types that are marked with this macro will be considered as a valid
 * Python object type.
 */
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define LOG_SURGEON_FFI_MARK_AS_PYOBJECT(T) \
    template <> \
...
LOG_SURGEON_FFI_MARK_AS_PYOBJECT(PyLogEvent);
LOG_SURGEON_FFI_MARK_AS_PYOBJECT(PyReaderParser);
LOG_SURGEON_FFI_MARK_AS_PYOBJECT(PyBytesObject);
LOG_SURGEON_FFI_MARK_AS_PYOBJECT(PyDictObject);
LOG_SURGEON_FFI_MARK_AS_PYOBJECT(PyTypeObject);
LOG_SURGEON_FFI_MARK_AS_PYOBJECT(PyUnicodeObject);
}  // namespace log_surgeon_ffi

#endif  // LOG_SURGEON_FFI_PY_OBJECT_CAST_HPP
