...
from dataset_up.config.path_config import get_config_dir,get_config_path
...
def get_config(ak=None, sk=None, auth=False):
    ...
    if not os.path.exists(get_config_path()):
        ...
        if auth is True and not ak_env_value:
            raise Exception(
                'Please login with ak/sk, try "jndataset-up login"'
            )
        return None
...
class LoginConfig(object):
    ...
    def get_login_config(self,ak = None,sk = None):
        if ak is not None and sk is not None:
            return LoginConfig(ak, sk)
