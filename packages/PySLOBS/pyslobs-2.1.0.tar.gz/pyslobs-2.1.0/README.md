...
* PySLOBS is mainly built to the published API documentation. That documentation
  is out-of-date. 
  * Some features that [can be seen in API source code](https://github.com/streamlabs/desktop/tree/master/app/services/api/external-api)
  are NOT yet implemented in PySLOBS. (Contact me if you would like to see them
  added.)
  ...
  * Some features that can be seen in the source code (only) *have* been
    implemented:
    * NotificationService supports undocumented `notificationPushed` and
...
* `Sources` have a method `refresh`. It raises an Internal Server Error;
this was reported to Streamlabs with no response.
...
* When a source is added, the `source_updated` callback may also be triggered
  an arbitrary number of times.
* Commands may raise an asyncio.TimeoutError after 5 seconds if a websocket 
  fails (e.g. a network error) between the time the command was sent and a
  reply was received.
