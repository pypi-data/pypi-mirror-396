...
You can see the content of the token with the `show` command. This command will always try to show a 'valid' token.
This means that if the access token is not valid anymore, then this will be refreshed in the background.
If you don't want your token to be refreshed, you can use the `--no-refresh` parameter. ::
...
This package provides 2 ways of reading and using a *valid token* from other programs.
...
Other option to show your access token is with the command `show` and the option `--no-decode` to show it ready
to be used. ::
