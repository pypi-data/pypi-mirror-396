...
# Temporarily commented to avoid circular import during testing
...
# ---------------------------------------------------------------- request
# @json_rpc_method("request/secretsCrud")  # Commented to avoid circular import
@dataclass  # (slots #=True)
class SecretCrudRequest:  # Removed RequestMethod inheritance temporarily
    session: str
    fabricId: str
    userId: str
    providerId: str
...
@dataclass  # (slots #=True)
class LSecretsResponse:  # Removed JsonRpcResult inheritance temporarily
    session: str
    fabricId: str
    userId: str
    providerId: str
...
class SecretManager:
    ...
    def get(
        ...
        if provider == SecretsProvider.PROPHECY:
            return SecretManager._get_prophecy_secret(scope, key)
    ...
    # ── Prophecy helper ──────────────────────────────────────────────────
    @staticmethod
    def _get_prophecy_secret(scope: str, key: str) -> str:
        """Fetch secret from Prophecy's Go secret vault via py4go gateway"""
        try:
            from prophecy.lib_wrapper import load_lib
            lib = load_lib()
            return lib.get_prophecy_secret(scope, key)
        except Exception as exc:
            raise RuntimeError(f"Failed to fetch Prophecy secret (scope={scope}, key={key}): {str(exc)}") from exc
...
# Import hvac only when needed (for HashiCorp Vault)
try:
