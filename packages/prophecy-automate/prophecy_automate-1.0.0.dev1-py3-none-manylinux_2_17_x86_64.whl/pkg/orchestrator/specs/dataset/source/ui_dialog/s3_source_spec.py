...
def dialog() -> Dialog:
    stack = StackLayout(gap="32px") \
        ...
        .addElement(StackLayout(gap="16px") \
                    ...
                    .addElement(Switch("Delete files after successfully processed").withToolTip("Deletes files from the S3 server after all its rows have been successfully processed.").bindProperty("properties.fileOperationProperties.deleteFilesAfterProcessing")) \
                    .addElement(StackLayout(gap="8px")
                                .addElement(Switch("Move files after successfully processed").withToolTip("Move files from the S3 server after all its rows have been successfully processed.").bindProperty("properties.fileOperationProperties.moveFilesToDestinationFolder")) \
