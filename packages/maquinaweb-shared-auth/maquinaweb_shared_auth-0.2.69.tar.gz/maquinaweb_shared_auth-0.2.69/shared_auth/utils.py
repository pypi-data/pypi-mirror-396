...
def get_subscription_model():
    ...
    """
    model_string = get_setting(
        "SHARED_AUTH_SUBSCRIPTION_MODEL", "shared_auth.Subscription"
    )
...
def get_group_permissions_model():
    ...
    """
    model_string = get_setting(
        "SHARED_AUTH_GROUP_PERMISSIONS_MODEL", "shared_auth.GroupPermissions"
    )
...
def get_member_system_group_model():
    ...
    """
    model_string = get_setting(
        "SHARED_AUTH_MEMBER_SYSTEM_GROUP_MODEL", "shared_auth.MemberSystemGroup"
    )
...
def get_contact_model():
    """
    Retorna o model de Contact configurado ou o padrão.
    ...
    """
    model_string = get_setting("SHARED_AUTH_CONTACT_MODEL", "shared_auth.Contact")

    try:
        return apps.get_model(model_string, require_ready=False)
    except ValueError:
        raise ImproperlyConfigured(
            f"SHARED_AUTH_CONTACT_MODEL deve estar no formato 'app_label.model_name'. "
            f"Recebido: '{model_string}'"
        )
    except LookupError:
        raise ImproperlyConfigured(
            f"SHARED_AUTH_CONTACT_MODEL refere-se ao model '{model_string}' "
            f"que não foi instalado ou é inválido."
        )
...
def get_message_model():
    """
    Retorna o model de Message configurado ou o padrão.

    Usage:
        from shared_auth.utils import get_message_model

        Message = get_message_model()
        message = Message.objects.get(id=1)
    """
    model_string = get_setting("SHARED_AUTH_MESSAGE_MODEL", "shared_auth.Message")

    try:
        return apps.get_model(model_string, require_ready=False)
    except ValueError:
        raise ImproperlyConfigured(
            f"SHARED_AUTH_MESSAGE_MODEL deve estar no formato 'app_label.model_name'. "
            f"Recebido: '{model_string}'"
        )
    except LookupError:
        raise ImproperlyConfigured(
            f"SHARED_AUTH_MESSAGE_MODEL refere-se ao model '{model_string}' "
            f"que não foi instalado ou é inválido."
        )
