...
def load_config(base_path: Path | None = None) -> ArtificerConfig:
    ...
    if artificer_config is None:
        raise RuntimeError(
            ...
            "Add a [tool.artificer] section with 'features'."
        )
    ...
    if not isinstance(features, list):
        raise RuntimeError(f"'features' must be a list, got {type(features).__name__}.")

    return ArtificerConfig(features=features)
