...
from gnuhealth.config import CONFIG, PIXMAPS_DIR, GNUHEALTH_ICON, \
    BANNER, get_config_dir
...
class DBListEditor(object):
    ...
    def __init__(self, parent, profile_store, profiles, callback):
        ...
        self.profile_tree.insert_column_with_attributes(
            -1, _('Profile'), self.cell, text=0)
        ...
        self.database_entry.connect(
            'changed', self.update_profiles, 'database')
        ...
        self.username_entry.connect(
            'changed', self.update_profiles, 'username')
    ...
    def profile_create(self, button):
        ...
        self.profile_tree.set_cursor(
            len(model) - 1, column, start_editing=True)
        self.db_cache = None
    ...
    def profile_selected(self, treeview):
        ...
        for field in fields:
            ...
            try:
                entry_value = self.profiles.get(
                    self.current_profile['name'], field)
    ...
    def edit_started(self, renderer, editable, path):
        if isinstance(editable, Gtk.Entry):
            editable.connect(
                'focus-out-event', self.edit_profilename, renderer, path)
    ...
    def refresh_databases(self, host, port):
        ...
        if CONFIG['thread']:
            threading.Thread(
                target=self.refresh_databases_start, args=(host, port)).start()
...
class DBLogin(object):
    def __init__(self):
        ...
        self.dialog = Gtk.Dialog(
            title="GNU Health - " + _('Login'), modal=True)
        ...
        tooltips.set_tip(
            self.button_connect, _('Connect the GNU Health server'))
        ...
        overlay.add(image)
        label = Gtk.Label(
            '<span color="white" background="#0090ad">'
            '<b>%s</b></span>' % __version__,
            use_markup=True)
        ...
        self.entry_database.connect_after(
            'focus-out-event',
            self.clear_profile_combo)
    ...
    def profile_manage(self, widget):
        ...
        dia = DBListEditor(
            self.dialog, self.profile_store, self.profiles, callback)
