...
from gnuhealth.config import CONFIG, PIXMAPS_DIR, SOUNDS_DIR, GNUHEALTH_ICON
...
from gnuhealth.exceptions import GNUHealthError, GNUHealthServerError
...
class IconFactory:
    ...
    def load_icons(cls, refresh=False):
        ...
        try:
            icons = rpc.execute(
                'model', 'ir.ui.icon', 'list_icons', rpc.CONTEXT)
    ...
    def register_icon(cls, iconname):
        ...
        to_load = slice(
            max(0, idx - cls.batchnum // 2), idx + cls.batchnum // 2)
        ids = [e[0] for e in cls._gnuhealth_icons[to_load]]
        try:
            icons = rpc.execute(
                'model', 'ir.ui.icon',
                'read', ids, ['name', 'icon'], rpc.CONTEXT)
        except GNUHealthServerError:
            icons = []
        for icon in icons:
            name = icon['name']
    ...
    def load_models(self, refresh=False):
        ...
        try:
            self._models = rpc.execute(
                'model', 'ir.model', 'list_models', rpc.CONTEXT)
        except GNUHealthServerError:
...
class ModelHistory(object):
    ...
    def load_history(self):
        self._models.clear()
        try:
            self._models.update(rpc.execute(
                'model', 'ir.model', 'list_history', rpc.CONTEXT))
        except GNUHealthServerError:
            pass
...
def open_donation():
    ghdonation = "https://www.gnuhealth.org/donate/"
    webbrowser_open(ghdonation)
...
<html>
    ...
    <body>
        ...
        if not server.session_set:
            raise GNUHealthError('SessionFailed')
