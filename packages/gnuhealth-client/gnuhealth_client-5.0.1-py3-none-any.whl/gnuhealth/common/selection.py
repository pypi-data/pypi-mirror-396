...
from gnuhealth.common import RPCException, RPCExecute, eval_domain
...
class SelectionMixin(object):
    ...
    def get_inactive_selection(self, value):
        ...
        for val, text in self.inactive_selection:
            ...
            try:
                result, = RPCExecute(
                    'model', self.attrs['relation'], 'read',
                    [value], ['rec_name'])
...
def freeze_value(value):
    if isinstance(value, dict):
        return tuple(
            sorted((k, freeze_value(v)) for k, v in value.items()))
...
class PopdownMixin(object):
    ...
    def set_popdown(self, selection, entry):
        ...
        if lengths:
            ...
            deviation = int(
                math.sqrt(sum((x - average) ** 2 for x in pop) / len(pop)))
            width = max(next(
