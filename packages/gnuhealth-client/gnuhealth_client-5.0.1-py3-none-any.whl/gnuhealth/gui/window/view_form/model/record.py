...
from gnuhealth.common import RPCException, RPCExecute
...
class Record:
    ...
    def default_get(self, defaults=None):
        if len(self.group.fields):
            ...
            try:
                vals = RPCExecute(
                    'model', self.model_name, 'default_get',
    ...
    def rec_name(self):
        try:
            return RPCExecute(
                'model', self.model_name, 'read', [self.id],
    ...
    def set_default(self, val, modified=True, validate=True):
        ...
        for fieldname, value in list(val.items()):
            ...
            if fieldname == self.group.exclude_field:
                continue
            if isinstance(
                self.group.fields[fieldname],
                    (fields.M2OField, fields.ReferenceField)):
                related = fieldname + '.'
