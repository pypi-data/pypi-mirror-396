...
from gnuhealth import __version__, common, gui, translate
from gnuhealth.config import CONFIG, copy_previous_configuration, \
        get_config_dir
...
def main():
    ...
    copy_previous_configuration('gnuhealth.cfg')
    ...
    if CONFIG.arguments or DBLogin().run():
        ...
        application_id = 'org.gnuhealth.GNUHealth-%(version)s._%(server)s' % {
            'version': version,
            'server': server,
            }
        app = gui.Main(
