...
class RefreshToken(AuthBase):
    ...
    def set_bearer_token(self):
        ...
        try:
            ...
            error_msg = f"Error while making request to the auth server in RefreshToken: {e}"
            logging.error(error_msg)
            raise RuntimeError(error_msg) from e

        if not response or response.status_code != 200:
            response_text = getattr(response, "text", "No response text")
            error_msg = f"Error response from the auth server in RefreshToken (status: {getattr(response, 'status_code', 'unknown')}): {response_text}"
            logging.error(error_msg)
        ...
        try:
            ...
            error_msg = f"Invalid JSON in RefreshToken response: {e}"
            logging.error(error_msg)
            raise RuntimeError(error_msg) from e
...
class AuthToken(AuthBase):
    ...
    def set_bearer_token(self):
        ...
        try:
            ...
            error_msg = f"Error while making request to the auth server in AuthToken: {e}"
            logging.error(error_msg)
            raise RuntimeError(error_msg) from e

        if not response or response.status_code != 200:
            response_text = getattr(response, "text", "No response text")
            error_msg = f"Error response from the auth server in AuthToken (status: {getattr(response, 'status_code', 'unknown')}): {response_text}"
            logging.error(error_msg)
            raise RuntimeError(error_msg)
        ...
        try:
            ...
            error_msg = f"Invalid JSON in AuthToken response: {e}"
            logging.error(error_msg)
            raise RuntimeError(error_msg) from e
