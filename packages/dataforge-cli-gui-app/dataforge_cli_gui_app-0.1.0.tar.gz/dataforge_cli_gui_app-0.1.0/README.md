...
```bash
dataforge_cli-gui_app
```

### 2. CLI Mode: Convert (DataFy)
Convert a structured sentence into a data format and output it to the console or a file.
**Syntax:** `dataforge_cli-gui_app convert "SENTENCE" --format [json|yaml|xml] [-o OUTPUT_FILE]`
```bash

# Example: Convert to JSON and print to console
dataforge_cli-gui_app convert "User is Rudra, Location is Home, Age is 10" -f json

# Example: Convert to XML and save to a file
dataforge_cli-gui_app convert "Project is DataForge, Status is Completed" --format xml -o project.xml
```

### 3. CLI Mode: Query (QueriFy)
Query data from a file using simple questions.
**Syntax:** `dataforge_cli-gui_app query -i INPUT_FILE -t [json|yaml|xml] -q "QUERY" [-o OUTPUT_FILE]`
...
dataforge_cli-gui_app query -i config.yaml -t yaml -q "version = ?"

# Example: Query a JSON file for a condition check
dataforge_cli-gui_app query -i data.json -t json -q "score > 90"
```
---
