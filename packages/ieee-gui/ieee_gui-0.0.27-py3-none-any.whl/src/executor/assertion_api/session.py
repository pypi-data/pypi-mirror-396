...
class Session:
    ...
    def __init__(
        ...
        self._page_transition_in_progress = False  # Track page transitions
    ...
    async def create(
        ...
        async def attach_popup_handler(current_page: Page):
            async def on_popup(popup: Page):
                session._page_transition_in_progress = True
                ...
                session._page_transition_in_progress = False
    ...
    async def wait_for_page_transition(self, timeout: int = 30000) -> None:
        """Wait for any ongoing page transitions to complete."""
        start_time = time.time()
        while self._page_transition_in_progress:
            if time.time() - start_time > timeout / 1000:
                logger.warning("Timeout waiting for page transition to complete")
                break
