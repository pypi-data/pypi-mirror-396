...
def _client(client: Optional[Any]) -> Any:
    ...
    if _default_client is None:
        raise RuntimeError("No client provided and no default client set. Call set_default_client(client).")
...
def _run(coro):
    ...
    # If loop is running, we need to use asyncio.run (creates new loop)
    # Otherwise, use run_until_complete on existing loop
    if loop.is_running():
        # This shouldn't happen in sync context, but create new loop
        return asyncio.run(coro)
    else:
        # Use the existing loop - this reuses connections
        try:
...
def login(client: Optional[Any] = None, **kwargs) -> dict[str, Any]:
    c = _client(client)
    return _run(c.auth.login(**kwargs))
