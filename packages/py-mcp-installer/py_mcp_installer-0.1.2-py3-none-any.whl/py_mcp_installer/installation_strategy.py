...
class NativeCLIStrategy(InstallationStrategy):
    ...
    def _build_cli_command(
        ...
        Note:
            Claude Code CLI uses positional arguments, not flags:
            claude mcp add <name> <command> [args...] -e KEY=val --scope project
        """
        # Platform-specific command building
        if self.platform in (Platform.CLAUDE_CODE, Platform.CLAUDE_DESKTOP):
            # claude mcp add <name> <command> [args...] --env KEY=val --scope project
            cmd = [
                self.cli_command,
                "mcp",
                "add",
            ...
            # Add env vars BEFORE the positional args (using -e flag)
            if server.env:
                for key, value in server.env.items():
                    cmd.extend(["-e", f"{key}={value}"])

            # Add scope BEFORE positional args
            scope_str = "project" if scope == Scope.PROJECT else "user"
            cmd.extend(["--scope", scope_str])

            # Add separator to indicate end of options, start of positional args
            cmd.append("--")

            # Add positional args: <name> <command> [args...]
            cmd.append(server.name)
            cmd.append(server.command)

            # Add command args as positional arguments
            if server.args:
