...
class CugaLiteNode(BaseNode):
    ...
    async def node(self, state: AgentState) -> Command[Literal['FinalAnswerAgent']]:
        ...
        # Sync new and modified variables from CodeAct execution to var_manager
        ...
        if updated_variables:
            for var_name, var_value in updated_variables.items():
                if var_name.startswith('_'):
                    continue
                if var_name not in initial_var_set:
                    new_var_names.append(var_name)
                else:
                    modified_var_names.append(var_name)

            logger.info(f"Adding {len(new_var_names)} new variables to variables_manager")
            logger.info(f"Updating {len(modified_var_names)} modified variables in variables_manager")
            ...
            for var_name in modified_var_names:
                var_value = updated_variables[var_name]
                state.variables_manager.add_variable(
                    var_value, name=var_name, description="Updated by CugaLite"
                )
                logger.info(f"Updated modified variable '{var_name}' in variables_manager")
