...
class TestConnectionTypeMapping:
    ...
    def test_redshift_default_catalog(self):
        """Test that Redshift connections include default_catalog for 2-part naming"""
        ...
        # Redshift should have default_catalog set to database name
        # This tells SQLMesh to use 2-part naming (schema.table) not 3-part (catalog.schema.table)
        assert result["type"] == "redshift"
        assert result["database"] == "stg_carrier_quality_redshift_db"
        assert result["default_catalog"] == "stg_carrier_quality_redshift_db"
