...
def test_generator_with_explicit_environment(tmp_path):
    """Test that explicitly passing environment='some_env' works"""
    # Create minimal SQLMesh project
    project_path = tmp_path / "sqlmesh_project"
    project_path.mkdir()
    (project_path / "config.yaml").write_text("""
...
""")
    ...
    # Create generator with explicit environment
    with pytest.warns(DeprecationWarning, match="SQLMesh virtual environment"):
        generator = SQLMeshDAGGenerator(
            sqlmesh_project_path=str(project_path),
            dag_id="test_dag",
