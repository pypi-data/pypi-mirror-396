...
class SQLMeshConfig:
    ...
    def __post_init__(self):
        ...
        # Warn if environment is set to something other than empty string and no gateway is set
        # Empty string is the correct default for production (no virtual environment)
        if self.environment and self.environment != "" and not self.gateway:
            warnings.warn(
                f"The 'environment' parameter is set to '{self.environment}' but no gateway is specified. "
                f"This will create a SQLMesh virtual environment. "
                f"For production, use environment='' (empty string) to use main schemas directly, "
                f"or use 'gateway' to switch between environments. "
                f"See docs/MULTI_ENVIRONMENT.md for details.",
                DeprecationWarning,
                stacklevel=2
            )
