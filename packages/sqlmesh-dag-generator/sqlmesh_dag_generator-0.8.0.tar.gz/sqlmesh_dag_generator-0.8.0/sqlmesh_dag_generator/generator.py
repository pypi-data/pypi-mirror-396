    ...
    def load_sqlmesh_context(self) -> Context:
        ...
        try:
            ...
            if self.config.sqlmesh.connection_config or self.config.sqlmesh.state_connection_config or self.config.sqlmesh.config_overrides or cache_dir:
                ...
                # Inject default_catalog if provided
                if self.config.sqlmesh.default_catalog:
                    config_dict["default_catalog"] = self.config.sqlmesh.default_catalog
                    logger.info(f"Setting default_catalog: {self.config.sqlmesh.default_catalog}")
    ...
    def create_tasks_in_dag(self, dag, models: Optional[List[str]] = None):
        ...
        if self.config.generation.auto_replan_on_change:
            def run_replan(**context):
                ...
                logger.info("Running SQLMesh plan with auto-apply...")
                run_ctx.plan(
            ...
            logger.info("Created auto-replan task: sqlmesh_plan_apply")
                ...
                def execute_model(**context):
                    ...
                    try:
                        result = run_ctx.run(
                            ...
                            select_models=[m_fqn],
                            skip_audits=self.config.generation.skip_audits,
                        )
