...
class TestTokens(unittest.TestCase):
    ...
    def test_generic_candidate_function(self):
        ...
        self.assertEqual((['23.04.2055'], ['I was born at the ', '.'], [(18, 28)]), my_function('I was born at the 23.04.2055.'))
    ...
    def test_iban_regex_from_offline_project(self):
        ...
        assert '[A-ZÄÖÜ]+\\d\\d[ ]{1,2}\\d\\d\\d\\d[ ]{1,2}\\d\\d\\d\\d[ ]{1,2}' '\\d\\d\\d\\d[ ]{1,2}\\d\\d\\d\\d[ ]{1,2}\\d\\d' in tokens
...
class TestTokensMultipleCategories(unittest.TestCase):
    ...
    def test_tokens_evaluations_multiple_categories(self):
        ...
        assert '(?P<Label_3_F_11_0>[A-ZÄÖÜ][a-zäöüß]+)' in [e['regex'] for e in self.label._evaluations[self.category_2.id_]]
...
class TestRegexGenerator(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        assert len(cls.prj.get_label_by_name('Austellungsdatum').annotations(categories=[cls.category])) == cls.correct_annotations
    ...
    def test_annotation_to_regex(self):
        ...
        label = Label(id_=22, text='Label Name', text_clean='LabelName', project=project, label_sets=[label_set], threshold=0.5)
    ...
    def test_two_annotation_of_one_label_to_regex(self):
        ...
        regex = label.find_regex(category=category)
        expected = r'[ ]{1,2}(?:(?P<Label_22_N_None_5>\d\d\.\d\d\.\d\d\d\d)|(?P<Label_22_N_None_19>\d\.\d\.\d\d\d\d))[ ]{1,2}'
...
class Test_named_group_multi_match:
    ...
    def test_keys_in_first_match(self):
        ...
        for i, result_dict in enumerate(self.results):
            assert (
                list(result_dict.keys()).sort() == ['regex_used', 'name', 'regex_group', 'value', 'start_offset', 'end_offset', 'start_text'].sort()
            )
...
def test_regex_spans_runtime():
    ...
    assert runtime < 0.16  # multiply by 4 as Azure VM is 4 times slower than local machine
