...
from konfuzio_sdk.utils import is_file, get_ascii_string, select_text, clean_text_preserve_layout, get_spans_from_bbox
...
def restored_project_id():
    ...
    if _restored_project_id is None:
        ...
        _restored_project_id = restore_snapshot(snapshot_id=116)
...
class TestOnlineProject(unittest.TestCase):
    ...
    def test_find_keywords_per_document(self):
        # We need a multipage document to test image merging.
        document = next(x for x in self.project._documents if x.number_of_pages == 8)
        keywords = {'Introduction': ['NTRODUCTION'], 'Conclusion': ['ONCLUSION']}
        ...
        tokenizer.tokenize(tokenized_document)

        areas = tokenized_document.find_keyword_spans_per_document(keywords)

        assert len(areas) == len(list(keywords.keys()))
        for area in areas:
            # Ensure required keys are present
            assert {'category', 'page_index', 'text', 'x0', 'x1', 'y0', 'y1'} == set(area.keys())

        # Round all float values to 6 decimal places for comparison
        def rounded(area):
            return {
                **area,
                'x0': round(area['x0'], 6),
        ...
        rounded_areas = [rounded(a) for a in areas]
        ...
        assert rounded_areas == expected, f"\nExpected:\n{expected}\n\nGot:\n{rounded_areas}"
    ...
    def test_find_keywords(self):
        ...
        result = tokenized_document.find_keyword_spans_for_per_page(keywords, return_all=True)

        expected = {
            0: {'Conclusion': [], 'Introduction': []},
            1: {'Conclusion': [], 'Introduction': ['Span (4096, 4107): "NTRODUCTION"']},
            2: {'Conclusion': [], 'Introduction': []},
            3: {'Conclusion': [], 'Introduction': []},
            4: {'Conclusion': [], 'Introduction': []},
            5: {'Conclusion': [], 'Introduction': []},
            6: {'Conclusion': ['Span (36096, 36105): "ONCLUSION"'], 'Introduction': []},
            7: {'Conclusion': [], 'Introduction': []},
        }

        def span_to_str(span):
            return f'Span ({span.start_offset}, {span.end_offset}): "{span.offset_string}"'

        actual = {
            page: {
                section: [span_to_str(s) for s in spans]
                for section, spans in page_dict.items()
            }
            for page, page_dict in result.items()
        }

        assert actual == expected

    def test_get_text_for_area(self):
        document = next(x for x in self.project._documents if x.number_of_pages == 8)
        ...
        area_spans: List[Dict] = tokenized_document.find_keyword_spans_per_document(keywords)
        areas: List[Dict] = tokenized_document.define_areas(area_spans)

        area_texts = {}
        for area, _ in keywords.items():
            spans_in_area = [
                span for span in tokenized_document.spans(use_correct=False)
                if tokenized_document.query_area(areas, span) == area
            ]
            raw_text = select_text(spans_in_area, tokenized_document)
            area_texts[area] = clean_text_preserve_layout(raw_text)

        assert len(area_texts['Introduction']) == 38598
        assert len(area_texts['Conclusion']), 1920

    def test_create_merged_image(self):
        """Test creating a merged image from all pages of the document."""
        # We need a multipage document to test image merging.
        test_document = next(x for x in self.project._documents if x.number_of_pages == 8)
        image = test_document.draw_merged_document()

        # Assert that the image was created and is a PIL Image instance
        assert image is not None, "Merged image is None"
        from PIL import Image
        assert isinstance(image, Image.Image), "Merged image is not a PIL Image"

        # Get page dimensions to calculate expected merged image size
        pages = test_document.pages()
        total_height = int(sum(page.height for page in pages))  # total height of all pages
        max_width = int(max(page.width for page in pages))  # width of the widest page

        # Assert the size of the merged image matches expected dimensions
        assert image.width == max_width, f"Expected width {max_width}, got {image.width}"
        assert image.height == total_height, f"Expected height {total_height}, got {image.height}"
