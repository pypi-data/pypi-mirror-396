...
def compare(
    ...
    if strict:  # many to many inner join to keep all Spans of both Documents
        ...
        spans = spans.groupby('id_local', dropna=False).apply(lambda group: grouped(group, 'id_')).reset_index(drop=True)
        # add check to evaluate Annotation Sets
        spans = spans.groupby('annotation_set_id_predicted', dropna=False).apply(lambda group: grouped(group, 'annotation_set_id')).reset_index(drop=True)
    else:
    ...
    if not strict:
        ...
        spans_not_multiple = spans_not_multiple.groupby(['annotation_set_id_predicted', 'label_id_predicted']).apply(prioritize_rows).reset_index(drop=True)
    ...
    spans['frequency'] = spans['frequency'].fillna(0)
...
class ExtractionConfusionMatrix:
    ...
    def calculate(self, data: pd.DataFrame):
        ...
        data['id_'] = data['id_'].fillna('no_match')
