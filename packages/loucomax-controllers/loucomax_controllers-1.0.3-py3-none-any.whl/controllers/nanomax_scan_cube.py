...
def cube_scan_meshgrid(hdf5_filepath, bounds:tuple, dwell_time, roi_energy, step_size_um:int):
    ...
    # if not nanomax_controller.is_all_axes_homed():
    nanomax_controller.home_all_axes()
    ...
    c = int((bounds[2][1] - bounds[2][0])*1000 // step_size_um)
    ...
    count = 0
    best_sum_spectrum = 0
    real_best_coord = (0.0,0.0,0.0)
    index_best_coord = (0,0,0)
    for point in points:
        count += 1
        x, y, z = point
        x = round(x, 3)
        ...
        if sum_spectrum >= best_sum_spectrum :
            best_sum_spectrum = sum_spectrum
            real_best_coord = (x, y, z)
            index_best_coord = (x_index, y_index, z_index)
        ...
        metadata_scan = {"x_range": bounds[0]
                         ,"y_range": bounds[1]
                         ,"z_range": bounds[2]
                         ,"dwell_time (ms)": dwell_time
                         ,"step (µm)": step_size_um
                         ,"best sum spectrum": best_sum_spectrum
                         ,"nanomax best coord (cm)": real_best_coord
                         ,"index cube best coord": index_best_coord
                         ,"energy roi": roi_energy
                         }
        Hdf5Handler.save_data_to_hdf5(hdf5_filepath, data_cube, np.array([0,1,0]), project_name="Test confocal alignment",metadata=metadata_scan)
...
if __name__ == "__main__":
    ...
    hdf5_filepath=r"C:\Users\CXRF\Desktop\test_confocal_cube_scan_12.hdf5"
    
    # UNCOMMENT TO RUN CUBE SCAN
    cube_scan_meshgrid(hdf5_filepath=hdf5_filepath,
                        bounds=((3.62, 3.66), (5.80, 6.20), (3.10, 3.50)),
                        dwell_time=100,
                        roi_energy=(7.9, 9.0),
                        step_size_um=10)
