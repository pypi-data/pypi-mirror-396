    ...
    def get_tensor(param: KernelParam) -> torch.Tensor:
        ...
        if supply_type == TensorSupplyType.Auto:
            ...
            if is_unsigned:
                ...
                return torch.randint(
                    low=-128, high=128, shape=shape, dtype=torch.int32).to(dtype).to(device)
            elif is_boolean:
        ...
        if supply_type == TensorSupplyType.Integer:
            ...
            if is_unsigned:
                ...
                return torch.randint(
                    low=-128, high=128, shape=shape, dtype=torch.int8).to(dtype).to(device)
            elif is_boolean:
                return torch.randint(low=0, high=2, shape=shape, dtype=dtype).to(device)
            else:
                return torch.randint(low=-2, high=3, shape=shape, dtype=dtype).to(device)
            ...
            return torch.empty(
                *shape, dtype=torch.float32).uniform_(-1.0, 1.0).to(dtype).to(device)
        elif supply_type == TensorSupplyType.Normal:
            return torch.empty(
                *shape, dtype=torch.float32).normal_(-1.0, 1.0).to(dtype).to(device)
