...
class InMemorySimpleCarRepository:
    ...
    def add_range(self, cars: List[SimpleCar]) -> None:
        """
        Add a range of simple cars to the repository.

        Args:
            cars: List of cars to add.

        Raises:
            ValueError: If cars is None.
        """
        if cars is None:
            raise ValueError('cars cannot be None')
