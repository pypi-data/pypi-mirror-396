...
impl TosClient {
    pub fn new(
        ...
        Self::new_with_redirect(region, endpoint, ak, sk, part_size, max_retry_count, max_prefetch_tasks, 
                               shared_prefetch_tasks, enable_crc, max_upload_part_tasks, 
                               shared_upload_part_tasks, max_worker_threads, None)
    }
    ...
    pub fn new_with_redirect(
        ...
        shared_upload_part_tasks: isize,
        max_worker_threads: isize,
        max_redirects: Option<usize>, // 最大重定向次数，None表示使用默认值
    ) -> Result<Self, TosError> {
