...
Revision ID: 5c32ec2d4beb
Revises:
Create Date: 2024-08-16 16:52:38.207687
...
def upgrade() -> None:
    ...
    from basejump.core.database.setup_db.db_connect import (
        ConnectDB,
        basejump_conn_params,
    )
    from sqlalchemy import text

    db_conn = ConnectDB(conn_params=basejump_conn_params)
    basejump_engine_noasync = db_conn.connect_db()
    for suffix in ["", "0"]:
        for schema in ["connect", "transaction", "ai_model", "account"]:
        ...
        op.create_table(
            ...
            sa.Column("etl_date", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()"), nullable=False),
        ...
        op.create_table(
            ...
            sa.Column(
                "etl_date", sa.TIMESTAMP(timezone=True), server_default=sa.text("CURRENT_TIMESTAMP"), nullable=False
            ),
        ...
        op.create_table(
            ...
            sa.Column("etl_date", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()"), nullable=False),
            sa.ForeignKeyConstraint(["conn_id"], [f"connect{suffix}.database.conn_id"], ondelete="CASCADE"),
        ...
        op.create_table(
            "sql_vector",
            sa.Column("sql_conn_id", sa.Integer(), nullable=False),
            sa.Column("vector_conn_id", sa.Integer(), nullable=False),
            sa.ForeignKeyConstraint(["sql_conn_id"], [f"connect{suffix}.database.conn_id"], ondelete="CASCADE"),
            sa.ForeignKeyConstraint(
                ["vector_conn_id"], [f"connect{suffix}.vector_db.vector_conn_id"], ondelete="CASCADE"
            ),
            sa.PrimaryKeyConstraint("sql_conn_id", "vector_conn_id"),
            schema=f"connect{suffix}",
        )
        op.create_table(
            "team_tables",
            sa.Column("sql_tbl_id", sa.Integer(), nullable=False),
            sa.Column("team_id", sa.Integer(), nullable=False),
            sa.ForeignKeyConstraint(
                ["sql_tbl_id"], [f"connect{suffix}.database_tables.sql_tbl_id"], ondelete="CASCADE"
            ),
            sa.ForeignKeyConstraint(["team_id"], ["account.team.team_id"], ondelete="CASCADE"),
            sa.PrimaryKeyConstraint("sql_tbl_id", "team_id"),
            schema=f"connect{suffix}",
        )
...
def downgrade() -> None:
    # NOTE: Create any new schemas
    from basejump.core.database.setup_db.db_connect import ConnectDB, db_params_noasync
    from sqlalchemy import text

    db_conn = ConnectDB(db_params=db_params_noasync)
    basejump_engine_noasync = db_conn.connect_db()
    for suffix in ["0", ""]:
        for schema in ["connect", "transaction", "ai_model", "account"]:
            with basejump_engine_noasync.begin() as conn:
                conn.execute(text(f"CREATE SCHEMA IF NOT EXISTS {schema}{suffix}"))
        # ### commands auto generated by Alembic - please adjust! ###
        op.drop_table("team_tables", schema=f"connect{suffix}")
