...
def synthesize_response(llm, prompt, text_node, prompt_tmpl, **kwargs):
    """
    Parameters
    ---------
    text_node
        This eventually becomes context_str in the prompt template
    """
    formatted_prompt = prompt_tmpl.partial_format(query_str=prompt, **kwargs)

    response_synthesizer = get_response_synthesizer(
        llm=llm,
        text_qa_template=formatted_prompt,
        verbose=True,
    )

    node = TextNode(text=text_node)
    retrieved_nodes = [NodeWithScore(node=node)]

    feedback = response_synthesizer.synthesize(
        query=prompt,
...
async def setup_connection(
    ...
    task: Task = asyncio.create_task(
        crud_connection.setup_connection_assoc_table(
            client_id=client_id, conn_id=copy.copy(db_login.conn_id), conn_params=conn_params, sql_engine=sql_engine
        )
    )
    ...
    return sch.GetSQLConn(
        ...
        database_name_alias=database_name_alias,
    )
...
async def update_agent_tokens(agent: BaseAgent, max_tokens: int = 500):
    ...
    if not isinstance(agent, SimpleAgent):
        agent.agent.memory.token_limit = agent.memory.get_llm_token_limit(llm=agent.agent_llm)
        agent.agent.agent_worker._llm.max_tokens = max_tokens
