...
from basejump.core.database.db_utils import process_db_tables
...
class DBTableIndexer:
    ...
    async def _update_index(
        ...
        try:
            ...
            await vector_store.async_add(final_nodes)
...
async def reindex_db(
    ...
    try:
        ...
        try:
            ...
            tables = await process_db_tables(tables=tables_base)
            index_db_tables = DBTableIndexer(
                client_id=client_user.client_id,
...
async def index_new_schemas(
    ...
    """Index new database schemas

    Notes
    -----
    This needs to be kept separate from so it can be submitted to a background server queue.
    A class instance with an active database connection can't be submitted to the background due to
    threading concerns.
    """
