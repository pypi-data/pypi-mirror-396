...
async def get_connection_types(db: AsyncSession, team_id: int, user_id: int) -> list[models.Connection]:
...
async def setup_connection_assoc_table(
    client_id: int, conn_id: int, conn_params: sch.SQLDBSchema, sql_engine: AsyncEngine
):
    ...
    table_names = [table.full_table_name for table in permitted_tables]
    session = LocalSession(client_id=client_id, engine=sql_engine)
    db = await session.open()
    try:
        tables = await crud_table.get_conn_tables_from_nms(db=db, table_names=table_names)
        for table in tables:
