...
async def get_conn_tables_from_nms(db: AsyncSession, table_names: list[str]) -> list[DBTables]:
    stmt = select(DBTables).filter(DBTables.table_name.in_(table_names))
    result = await db.execute(stmt)
...
async def upload_table_names(
    ...
    try:
        ...
        logger.error(constants.NO_PERMITTED_TABLES)
        raise AssertionError(constants.NO_PERMITTED_TABLES)
    return tables
