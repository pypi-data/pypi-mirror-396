...
from llama_index.core.readers.file.base import SimpleDirectoryReader  # type: ignore
from llama_index.core.vector_stores import MetadataInfo

# TODO: Needs to be update when switching to new version
from llama_index.readers.file import UnstructuredReader

logger = set_logging(handler_option="stream", name=__name__)


def html_reader(file_list: list) -> list[Document]:
    loader = UnstructuredReader()
    documents = []
    for file_path in file_list:
        document_list = loader.load_data(file=file_path, split_documents=False)

        document = document_list[0]
        document.metadata = {"file_path": file_path.as_posix()}
...
# Copied from here:
# https://github.com/run-llama/llama-hub/blob/main/llama_hub/google_drive/base.py
# I have a PR I made to make the fixes to this class
# to avoid copying the class myself

# Scope for reading and downloading google drive files
SCOPES = ["https://www.googleapis.com/auth/drive.readonly"]


class BaseGoogleDriveReader(BaseReader):
    """Google drive reader."""
    ...
    def __init__(
        ...
        self._mimetypes = {
            "application/vnd.google-apps.document": {
                "mimetype": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",  # noqa
                "extension": ".docx",
            },
    ...
    def _get_credentials(self) -> Any:
        ...
        https://developers.google.com/drive/api/v3/quickstart/python.
            Copy credentials.json file and rename it to client_secrets.json
            file which will be used by pydrive for downloading files.
            So, we need two files:
                1. credentials.json
                2. client_secrets.json
            Both 1, 2 are essentially same but needed with two different names
            according to google-api-python-client, google-auth-httplib2,
            google-auth-oauthlib and pydrive libraries.
        Returns:
            credentials, pydrive object
        """
        from google.api_auth.transport.requests import Request
        ...
        if not creds or not creds.valid:
            print("invalid creds possibly")
            if creds and creds.expired and creds.refresh_token:
                print("creds are getting refreshed")
                creds.refresh(Request())
            else:
                print("from client secrets")
    ...
    def _get_fileids_meta(
        ...
        Args:
            ...
            mime_types: the mimeTypes you want to allow e.g.:
            "application/vnd.google-apps.document"
        Returns:
        ...
        try:
            ...
            if folder_id:
                ...
                for item in items:
                    if item["mimeType"] == folder_mime_type:
                        ...
                        # "owners" doesn't exist in a Shared Drive
            ...
            logger.error("An error occurred while getting fileids metadata: {}".format(e))
    ...
    def _load_data_fileids_meta(self, fileids_meta: List[List[str]]) -> List[Document]:
        ...
        try:
            ...
            logger.error("An error occurred while loading " "data from fileids meta: {}".format(e))
    ...
    def _load_from_folder(self, folder_id: str, mime_types: list) -> List[Document]:
        ...
        Args:
            folder_id: folder id of the folder in google drive.
            mime_types: the mimeTypes you want to allow
    ...
    def load_data(
        ...
        Args:
            ...
            mime_types:
                the mimeTypes you want to allow
                e.g.: "application/vnd.google-apps.document"
        Returns:
...
# Wrap the google drive reader to modify the behavior with metadata
# TODO: Set up abstract class to establish format for these readers for
# metadata
class GoogleDriveReader(BaseGoogleDriveReader):
    def __init__(
        self,
        credentials_path: str = "credentials.json",
        token_path: str = "token.json",
        ...
        """Initialize with parameters."""
        self.llm_metadata_keys: list = ["file name"]
        self._metadata: list[MetadataInfo] = [
            MetadataInfo(
                name="file id",
                type="str",
                description="The file id of the file in google drive",
            ),
            MetadataInfo(name="author", type="str", description="The author of the file"),
            MetadataInfo(
                name="file name",
                type="str",
    ...
    def metadata_keys(self) -> list[str]:
        ...
        for info in self._metadata:
            metadata_keys.append(info.name)
        return metadata_keys

    @property
    def excluded_llm_metadata_keys(self) -> list[str]:
        """Return a list of all excluded metadata keys"""
        excluded_keys = list(set(self.metadata_keys) - set(self.llm_metadata_keys))
        return excluded_keys

    @property
    def doc_metadata(self) -> list[dict]:
        """Process the metadata that is available to the LLM"""
        metadata = []
        for info in self._metadata:
            if info.name in self.excluded_llm_metadata_keys:
                continue
            else:
                metadata.append(info.dict())
        return metadata
    ...
    def load_data(
        ...
        """Set exclusions for metadata keys"""
        documents = super().load_data(folder_id=folder_id, file_ids=file_ids, mime_types=mime_types)
        for document in documents:
            document.excluded_llm_metadata_keys = self.excluded_llm_metadata_keys
            document.excluded_embed_metadata_keys = self.excluded_llm_metadata_keys
        return documents
