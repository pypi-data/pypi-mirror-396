...
async def get_table_info_from_vector_db(
    ...
    index = await redis_client_async.ft(index_name).search(Query(search_str).return_field("_node_content").paging(start, offset))  # type: ignore # noqa
    db_table_info = ""
...
async def delete_nodes(client_id: int, node_uuids: list[uuid.UUID], redis_client_async: RedisAsync):
    ...
    try:
        ...
        logger.warning("Error deleting vector docs")
...
async def delete_semcache_result(
    ...
    try:
        idx_result = await redis_client_async.ft(semcache_idx_nm).search(Query(search_str))  # type: ignore # noqa
...
def get_redis_index(index_name: str, settings: Settings, redis_client_async: RedisAsync) -> BaseIndex:
    schema = IndexSchema.from_dict(
        {
            "index": {"name": index_name, "prefix": index_name + "/vector"},
            "fields": [
...
def get_vector_idx(
    client_id: int, vector_schema: sch.VectorDBSchema, settings: Settings, redis_client_async: RedisAsync
) -> VectorStoreIndex:
