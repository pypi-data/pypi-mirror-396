...
- **numeric**: `True` for int, float, Decimal, datetime, date, time (excluding bool)
- **categorical**: `True` for categorical fields (False for float, datetime, timedelta, composite, or NonCategorical)
...
# The parent field (author) has suggested_human_sorting_field set to "name"
assert metadata["author"].extra["suggested_human_sorting_field"] == "name"
```

This pattern helps identify which nested field should be used for human-readable sorting or display purposes when dealing with composite types.
