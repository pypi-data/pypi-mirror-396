...
def id_update(study):
    ...
    id_with_lib = study.id_df.join(
        study.lib_df.select(["lib_uid", "name", "shortname", "class", "adduct"]),
        on="lib_uid",
        how="left"
    )
    ...
    id_columns = {
        "id_top_name": best_ids.select("consensus_uid", pl.coalesce([pl.col("shortname"), pl.col("name")]).alias("name")),
...
def lib_filter(study, entries):
    ...
    if isinstance(entries, str):
        if entries == 'delete_identified':
            ...
            ms2_conditions = []
            if 'matcher' in study.id_df.columns:
