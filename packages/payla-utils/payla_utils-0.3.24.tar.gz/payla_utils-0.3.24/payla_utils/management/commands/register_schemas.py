...
class Command(BaseCommand):
    ...
    @property
    def custom_dynamic_topics_and_schemas_callback(self) -> Callable[[], dict[str, dict]]:
    ...
    def register_schema(
        ...
        if not message_schema:
            if not schema_name:
                raise ValueError("Either schema_name or message_schema must be provided")
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        for topic, message_schema in self.custom_dynamic_topics_and_schemas_callback().items():
            self.create_topic(topic)
            self.register_schema(
                topic, schema_name=None, schema_registry_client=schema_registry_client, message_schema=message_schema
            )
