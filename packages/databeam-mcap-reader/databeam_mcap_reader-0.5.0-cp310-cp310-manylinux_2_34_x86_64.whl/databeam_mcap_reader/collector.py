...
class Collector:
    ...
    def get_mcap_paths_for_module(self, module_name: str, timestamp_ns_min: Optional[int] = None,
        ...
        for measurement_name, measurement in self._structure.items():
            if module_name in self._structure[measurement_name].modules:
                if timestamp_ns_min is not None and measurement.stop_timestamp_ns < timestamp_ns_min:
                    continue
                if timestamp_ns_max is not None and measurement.start_timestamp_ns > timestamp_ns_max:
                    continue
                if is_finished is not None and measurement.is_finished != is_finished:
                    continue
...
if __name__ == "__main__":
    ...
    # collector.parse_directory(path="/home/sammeradm/Downloads/E-Flex Wessely")
    collector.parse_directory(path="/opt/databeam/data/latest")
    structure = collector.get_structure()
    print(collector.get_structure_as_json())

    paths = collector.get_mcap_paths_for_module('ShellyPro3EM')

    print(paths)


    # TODO I want all data of channel "Lackierkammer_total_act_power" from all measurements


    # li = collector.get_module_names_list()
    # print(li)

    # ms = collector.get_measurements_with_module('MyModule')
    # print(ms)

    # for m in collector.get_measurements_with_module('MyModule'):
    #     print(m.name)

    # fetch data from single module/MCAP
    # reader_ = collector.get_mcap_reader(module=structure['2025-03-17_14-30-29.499_75_xyz'].modules['MyModule'])
    # reader_.get_info_string()
    # module_data = reader_.get_all_data()

    # fetch data from all modules in a measurement
    # for module_ in collector.get_measurement_by_name('2025-03-17_14-30-29.499_75_xyz').modules.values():
    #     reader_ = collector.get_mcap_reader(module_)
    #     reader_.get_all_data()
    #     # reader_.get_info_string()
