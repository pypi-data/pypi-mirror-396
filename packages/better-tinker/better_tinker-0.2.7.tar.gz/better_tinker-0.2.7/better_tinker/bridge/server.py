...
class TinkerClientManager:
    ...
    def __init__(self):
        ...
        self._current_api_key: Optional[str] = None  # Track the current API key
    ...
    def get_client(self, api_key: str):
        ...
        with self._lock:
            if api_key in self._clients:
                # Ensure env var is set for this key (SDK may read it lazily)
            ...
            try:
                ...
                # IMPORTANT: Keep it set! The SDK may read it lazily during API calls
                os.environ["TINKER_API_KEY"] = api_key
                self._current_api_key = api_key
                ...
                # DO NOT delete the env var - SDK needs it for API calls
