...
class Binomial(Distribution):
    ...
    def pdf(self, k):
        ...
        choose = math.factorial(self.n) / (math.factorial(k) * (math.factorial(self.n - k)))
        bin_dist = self.p ** k * (1 - self.p) ** (self.n - k)
        return choose * bin_dist
    ...
    def plot_bar_pdf(self):
        ...
        k_data = [self.pdf(k) for k in range(0, 101)]
        plt.hist(k_data, bins=len(k_data))
        plt.title('Binomial Distribution PDF')
        plt.xlabel('Number of Positive Outcomes')
        plt.ylabel('Odds of Positive Outcomes')
        return range(0, 101), k_data
    ...
    def __add__(self, other):
        ...
        add_bin_dist = Binomial(self.p, self.n + other.n)
        return add_bin_dist
