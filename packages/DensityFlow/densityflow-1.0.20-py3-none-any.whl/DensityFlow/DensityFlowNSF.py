...
class DensityFlowNSF(nn.Module):
    ...
    >>> ln = lb.labels_
    >>> model = DensityFlowNSF(input_size = xs.shape[1],
            ...
            for i in np.arange(self.perturb_size):
                if self.use_bias[i]:
                    if self.specific_mode=='none':
                        self.perturb_effect.append(MLP(
    ...
    def get_complete_embedding(self, xs, us, batch_size:int=1024, show_progress=True):
        ...
        with tqdm(total=len(dataloader), disable=not show_progress, desc='', unit='batch') as pbar:
            for X_batch, U_batch, _ in dataloader:
                X_batch = X_batch.to(self.get_device())
                U_batch = U_batch.to(self.get_device())
    ...
    def predict(self, xs, perturbs_predict:list, perturbs_reference:list, library_sizes=None, show_progress=True):
        ...
        for pert in perturbs_predict:
            if pert in perturbs_reference:
                ...
                dzs = self.get_cell_shift(xs, perturb_idx=pert_idx, perturb_us=ps, show_progress=show_progress)          
    ...
    def _cell_shift(self, zs, perturb_idx, perturb):
        ...
        if perturb.ndim==2:
            if self.specific_mode=='none':
                ms = self.perturb_effect[perturb_idx](perturb)
            else:
                ms = self.perturb_effect[perturb_idx]([zs, perturb])
        else:
            if self.specific_mode=='none':
                ms = self.perturb_effect[perturb_idx](perturb.reshape(-1,1))
            else:
                ms = self.perturb_effect[perturb_idx]([zs, perturb.reshape(-1,1)])
    ...
    def get_counts(self, zs, library_sizes, batch_size: int = 1024, show_progress=True):
        ...
        with tqdm(total=len(dataloader), disable=not show_progress, desc='', unit='batch') as pbar:
            for Z_batch, L_batch, _ in dataloader:
                Z_batch = Z_batch.to(self.get_device())
                L_batch = L_batch.to(self.get_device())
            ...
            for epoch in range(num_epochs):
                ...
                for batch_x, batch_u, batch_c, _ in dataloader:
                    ...
                    if us is None:
                        ...
                        batch_u = batch_u.to(self.get_device())
                    if cs is None:
                        batch_c = None 
                    else:
