...
from cafga.explainers.cafga_captum._utils.models.linear_model import SkLearnLasso
from cafga.explainers.cafga_captum._utils.models.model import Model
from cafga.explainers.cafga_captum._utils.progress import progress
from cafga.explainers.cafga_captum._utils.typing import BaselineType, TargetType, TensorOrTupleOfTensorsGeneric
from cafga.explainers.cafga_captum.attr._utils.attribution import PerturbationAttribution
from cafga.explainers.cafga_captum.attr._utils.batching import _batch_example_iterator
...
class LimeBase(PerturbationAttribution):
    ...
    def attribute(
        ...
        **kwargs: object,
        ...
        # The dataset up to this point is actually the train + test set (this is of course not in the original implementation)
        n_features = combined_interp_inps.shape[1]
        n_train = n_samples - n_test
        n_pos_perturbations = 2 ** n_features 
        if n_pos_perturbations < n_samples:
            if n_pos_perturbations > n_train:
                n_test = n_pos_perturbations - n_train
            else:
                n_train = n_pos_perturbations - 100
                n_test = 100
        local_ds = (combined_interp_inps[:n_train], combined_outputs[:n_train])
        local_test_set = (combined_interp_inps[n_train:], combined_outputs[n_train:])
        actual_train_size = local_ds[0].shape[0]
        actual_test_size = local_test_set[0].shape[0]
        dataset = TensorDataset(combined_interp_inps[:n_train], combined_outputs[:n_train], combined_sim[:n_train])
        self.interpretable_model.fit(DataLoader(dataset, batch_size=batch_count))
        return self.interpretable_model.representation(), self.interpretable_model.bias(), local_ds, local_test_set
...
class Lime(LimeBase):
    ...
    def _attribute_kwargs(  # type: ignore
        ...
        **kwargs: object,
        ...
        coefs, bias, ds, test_ds = None, None, None, None
        coefs, bias, ds, test_ds = super().attribute.__wrapped__(
            self,
            inputs=inputs,
            target=target,
            additional_forward_args=additional_forward_args,
        ...
        if return_input_shape:
            ...
            #  `Tuple[Tensor, ...]`.
            coefs = self._convert_output_shape(
                formatted_inputs,
                feature_mask,
                coefs,
                num_interp_features,
