...
class KernelExplainer(Explainer):
    ...
    def __init__(self, model, feature_names=None, link="identity", **kwargs):
        # Since we are explaining text in Cafga we do not have a background dataset 
        # and we only know the number of features once we start explaining. Thus we rerun the init procedure at the start of explain.
        self.model = model
        

    def reinit(self, empty_vector, link="identity", **kwargs):
        model = self.model
        data = empty_vector
    ...
    def explain(self, X, **kwargs): # This was the "shap_values" method, renamed to explain 
        ...
        n_features = len(X)
        empty_vector = pd.Series([False] * n_features)
        self.reinit(empty_vector)
        ...
        if len(X.shape) == 1:
            ...
            explanation = self._explain(data, **kwargs)
            return explanation
        ...
        # explain the whole dataset
            ...
            for i in tqdm(range(X.shape[0]), disable=kwargs.get("silent", False)):
                ...
                explanations.append(self._explain(data, **kwargs))
    ...
    def _explain(self, incoming_instance, **kwargs):
        # convert incoming input to a standardized iml object
        
        instance = convert_to_instance(incoming_instance)
        n_features = len(instance.x[0])
        n_train_samples = kwargs.get("n_samples")
        n_test_samples = kwargs.get("n_test_samples", 0)
        n_possible = 2**n_features
        if n_train_samples + n_test_samples >= n_possible:
            if n_train_samples > n_possible:
                n_test_samples = min(n_test_samples, n_possible//5)
                n_train_samples = n_possible - n_test_samples
            else:
                n_test_samples = n_possible - n_train_samples 
        self.nsamples = n_train_samples + n_test_samples
        kwargs["nsamples"] = self.nsamples 
        ...
        return {
            "explanation": phi,
            "bias": 0, # Bias is already accounted for in the y values
            "local_dataset": self.local_ds,
    ...
    def solve(self, fraction_evaluated, dim):
        ...
        try:
            n_train_samples = self.nsamples - self.n_test
            total_X, total_y = X.T @ WX, WX.T @ y
            train_X, train_y = total_X[:n_train_samples, :], total_y[:n_train_samples]
            test_X, test_y = total_X[n_train_samples:, :], total_y[n_train_samples:]
            self.local_ds = train_X, train_y
            self.test_ds = test_X, test_y
            w = np.linalg.solve(train_X, train_y)
            ...
            n_train_samples = self.nsamples - self.n_test
            total_X, total_y = sqrt_W[:, None] * X, sqrt_W * y
            train_X, train_y = total_X[:n_train_samples, :], total_y[:n_train_samples]
            test_X, test_y = total_X[n_train_samples:, :], total_y[n_train_samples:]
            self.local_ds = train_X, train_y
            self.test_ds = test_X, test_y
