...
def run_hook_test(command: str, should_block: bool) -> bool:
    """Run a single command against the security hook."""
...
def _test_validate_init_script():
...
def main():
    ...
    ext_passed, ext_failed = _test_extract_commands()
    passed += ext_passed
    failed += ext_failed

    # Test chmod validation
    chmod_passed, chmod_failed = _test_validate_chmod()
    passed += chmod_passed
    failed += chmod_failed

    # Test init.sh validation
    init_passed, init_failed = _test_validate_init_script()
    passed += init_passed
    failed += init_failed
    ...
    for cmd in dangerous:
        if run_hook_test(cmd, should_block=True):
            passed += 1
        else:
            failed += 1
    ...
    for cmd in safe:
        if run_hook_test(cmd, should_block=False):
            passed += 1
        else:
            failed += 1
