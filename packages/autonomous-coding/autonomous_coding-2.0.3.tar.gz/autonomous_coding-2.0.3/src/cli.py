...
Examples:
    ...
    # Use custom spec file (path to your own spec)
    autonomous-coding --project-dir ./my_project --spec ~/my-specs/my_app_spec.txt

    # Force update spec file from template/path
...
Custom specs:
    You can also provide a path to your own spec file:
    - Relative: --spec ./my_spec.txt
    - Absolute: --spec /path/to/my_spec.txt
    - Home: --spec ~/specs/my_app.txt
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Template name OR path to spec file. Templates: 'ecommerce', 'task_manager'. "
        "Paths: './my_spec.txt', '~/specs/app.txt', '/path/to/spec.txt'",
    )
    parser.add_argument(
        "--force-spec",
        action="store_true",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="autonomous-coding 2.0.3",
    )
    ...
    from core.prompts import is_spec_path

    # Determine if --spec is a path or template name
    spec_path = None
    template_name = None

    if args.spec:
        if is_spec_path(args.spec):
            spec_path = args.spec
        else:
