...
def filter_overrepresented_families(
   ...
   **Filtering Strategy:**
   ...
   - Families with few targets (≤ min_targets_threshold) are never filtered (not enough redundancy)
...
def compute(mesh_headings: list, chembl_version: str = '36', trials_only_last_n_years: int = None, filter_families: bool = True) -> dict:
   ...
   **Workflow:**
   ...
   4. **Per-Disease Processing**: For each disease indication:
      ...
      a. Calculates phase scores (sum of trial phases 0-3)
      b. Identifies approved targets (phase 4 trials present)
      c. Expands to include MeSH descendants (e.g., "Lung Neoplasms" includes "Adenocarcinoma of Lung")
      d. Optionally filters over-represented gene families per drug-disease combination
      e. Aggregates scores across all drugs and sub-diseases per target
      f. Computes final clinical score = phase_scores + (20 if approved else 0)
